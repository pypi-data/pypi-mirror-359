...
class Experiment():
    def __init__(self, params, acousticType = AOT_biomaps.AOT_Acoustic.WaveType.StructuredWave, fieldDataPath = None, AOsignalDataPath = None, formatSave = AOT_biomaps.AOT_Acoustic.FormatSave.HDR_IMG, isMultiCPU = True):
        self.params = params
        ...
        if isMultiCPU:
            self.params.general['multiCPU'] = True
        else:
            self.params.general['multiCPU'] = False
    ...
    def generateAOsignal(self,withTumor = True, loadAOfile = False):
        ...
        if self.AOfile is not None and loadAOfile:
            if not os.path.exists(self.AOfile):
                raise FileNotFoundError(f"AO file {self.AOfile} not found.")
            AOmatrix = self._load_AOSignal(self.AOfile)
...
class Tomography(Experiment):
    ...
    def _generateAcousticFields_STRUCT_CPU(self, fieldDataPath, fieldParamPath):
        ...
        progress_bar = trange(1,len(patternList), desc="Generating system matrix -- CPU")
