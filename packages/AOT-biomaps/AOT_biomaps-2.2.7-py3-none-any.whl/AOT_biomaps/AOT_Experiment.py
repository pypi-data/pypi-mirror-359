...
class Experiment():
    def __init__(self, params, acousticType = AOT_biomaps.AOT_Acoustic.WaveType.StructuredWave, fieldDataPath = None, AOsignalDataPath = None, formatSave = AOT_biomaps.AOT_Acoustic.FormatSave.HDR_IMG):
        self.params = params
        ...
        self.AOsignalDataPath = AOsignalDataPath
    ...
    @abstractmethod
    def generateAcousticFields(self, fieldDataPath, fieldParamPath = None):
        pass
    
    def cuteAcousticFields(self,max_t,min_t=0,saveFields=False):
        ...
        self.AOsignal_withTumor = self._generateAOsignal(withTumor= True)
        self.AOsignal_withoutTumor = self._generateAOsignal(withTumor= False)

        if saveFields:
            self.saveAcousticFields(self.fieldDataPath)

    def saveAcousticFields(self, save_directory):
        for i in trange(len(self.AcousticFields), desc="Saving Acoustic Fields"):
            field = self.AcousticFields[i]
            # Update the description with the current field's name
            trange.set_postfix_str(f"-- {field.getName_field()}")
    ...
    def generateAOsignal(self,withTumor = True, loadAOfile = False):

        if self.AcousticFields is None:
            raise ValueError("AcousticFields is not initialized. Please generate the system matrix first.")

        if self.AOsignalDataPath is not None and loadAOfile:
            if not os.path.exists(self.AOsignalDataPath):
                raise FileNotFoundError(f"AO file {self.AOsignalDataPath} not found.")
            AOmatrix = self._load_AOSignal(self.AOsignalDataPath)
            if AOmatrix.shape[0] != self.AcousticFields[0].enveloppe.shape[0]:
                print(f"AO signal shape {AOmatrix.shape} does not match the expected shape {self.AcousticFields[0].enveloppe.shape}. Generating corrected AO signal to match...")
            else:
                return AOmatrix
        ...
        for i in trange(len(self.AcousticFields), desc="Generating AO Signal"):
            for t in range(self.AcousticFields[i].enveloppe.shape[0]):
                if withTumor:
                    ...
                    interaction = self.OpticImage.laser * self.AcousticFields[i].enveloppe[t, :, :]
    ...
    def show_AOsignal(self, save_dir=None, wave_name=None):
        """
...
class Focus(Experiment):
    def __init__(self, **kwargs):
        ...
        self.AcousticFields = self.generateAcousticFields(self.fieldDataPath, None)
        self.AOsignal_withTumor = self.generateAOsignal(withTumor= True)
        self.AOsignal_withoutTumor = self.generateAOsignal(withTumor= False)
    ...
    def check(self):
        ...
        for field in self.AcousticFields:
            if field.enveloppe.shape[0] != self.AOsignal_withTumor.shape[0]:
                return False, f"Field {field.getName_field()} has an invalid Time shape: {field.enveloppe.shape[0]}. Expected time shape to be {self.AOsignal_withTumor.shape[0]}."
        if not all(field.enveloppe.shape == self.AcousticFields[0].enveloppe.shape for field in self.AcousticFields):
        ...
        if self.OpticImage.phantom.shape[0] != self.AcousticFields[0].enveloppe.shape[1] or self.OpticImage.phantom.shape[1] != self.AcousticFields[0].enveloppe.shape[2]:
            return False, f"OpticImage phantom shape {self.OpticImage.phantom.shape} does not match AcousticFields shape {self.AcousticFields[0].enveloppe.shape[1:]}."
        
        return True, "Experiment is correctly initialized."
    
class Tomography(Experiment):

    def __init__(self, fieldParamPath, **kwargs):
        super().__init__(**kwargs)
        self.AcousticFields = self.generateAcousticFields(self.fieldDataPath, fieldParamPath)
        self.AOsignal_withTumor = self.generateAOsignal(withTumor= True)
        self.AOsignal_withoutTumor = self.generateAOsignal(withTumor= False)
    ...
    def generateAcousticFields(self, fieldDataPath, fieldParamPath):
        """
        ...
        if self.TypeAcoustic.value == AOT_biomaps.AOT_Acoustic.WaveType.StructuredWave.value:
            if self.params.general['multiCPU']:
                try:
                    return self._generateAcousticFields_STRUCT_multiCPU(fieldDataPath, fieldParamPath)
                except Exception as e:
                    print(f"Error generating system matrix with multithreading: {e}, falling back to single CPU generation.")
                    return self._generateAcousticFields_STRUCT_CPU(fieldDataPath, fieldParamPath)
            else:
                return self._generateAcousticFields_STRUCT_CPU(fieldDataPath, fieldParamPath)
        else:
    ...
    def _generateAcousticFields_STRUCT_CPU(self, fieldDataPath, fieldParamPath):
        ...
        progress_bar = trange(1,len(patternList), desc="Generating system matrix -- working CPU")
            ...
            if not pathField is None and os.path.exists(pathField):
                if progress_bar is not None:
                    ...
                    try:
                        ...
                        progress_bar.set_postfix_str(f"Error loading field -> Generating field - {AcousticField.getName_field()} -- Memory used :{memory.percent}%")
                        AcousticField.generate_field()
                        progress_bar.set_postfix_str(f"Calculating squared envelope of the field - {AcousticField.getName_field()} -- Memory used :{memory.percent}%")
                        AcousticField.calculate_envelope_squared()
                        del AcousticField.field # Free memory
                ...
                progress_bar.set_postfix_str(f"Generating field - {AcousticField.getName_field()} -- Memory used :{memory.percent}%")
                AcousticField.generate_field()
    ...
    def _generateAcousticFields_STRUCT_multiCPU(self, fieldDataPath, fieldParamPath):
        @staticmethod
        def _process_batch(batch, fieldDataPath, FormatSave, params, progress_bar=None):
            batch_fields = []
            for pattern in batch:
                ...
                if fieldDataPath is not None:
                    pathField = os.path.join(fieldDataPath, os.path.basename(AcousticField.getName_field() + FormatSave.value))
                ...
                if pathField is not None and os.path.exists(pathField):
                    ...
                    AcousticField.load_field(fieldDataPath, FormatSave)
                else:
                    if progress_bar is not None:
                ...
                batch_fields.append(AcousticField)
                del AcousticField
                gc.collect()
        ...
        listAcousticFields = []
        with trange(len(patternList), desc="Generating system matrix -- MultiCPU") as progress_bar:
            for i in range(0, len(patternList), config.batchSize):
                batch = patternList[i:i + config.batchSize]
                listAcousticFields.extend(
                    _process_batch(batch, fieldDataPath, self.FormatSave, self.params, progress_bar)
                )
