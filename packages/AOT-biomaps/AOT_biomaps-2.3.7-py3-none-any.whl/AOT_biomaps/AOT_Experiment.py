...
class Experiment():
    ...
    def generatePhantom(self):
        ...
        if self.OpticImage is None:
            ...
            print("Phantom already generated.")
    ...
    def generateAcousticFields(self, fieldDataPath, fieldParamPath):
        ...
        if self.AcousticFields is not None:
            print("AcousticFields already generated.")
            return
        if self.TypeAcoustic.value == AOT_biomaps.AOT_Acoustic.WaveType.StructuredWave.value:

            if self.params.general['multiCPU']:
                try:
                    self.AcousticFields = self._generateAcousticFields_STRUCT_multiCPU(fieldDataPath, fieldParamPath)
                except Exception as e:
                    print(f"Error generating system matrix with multithreading: {e}, falling back to single CPU generation.")
                    self.AcousticFields = self._generateAcousticFields_STRUCT_CPU(fieldDataPath, fieldParamPath)
            else:
                self.AcousticFields = self._generateAcousticFields_STRUCT_CPU(fieldDataPath, fieldParamPath)
        else:
            raise ValueError("Unsupported wave type.")
        
    def cuteAcousticFields(self,max_t,min_t=0):
        # max_t and min_t are in seconds format example "40e-3" for 40 ms
        # convert to samples
        ...
        for i in trange(len(self.AcousticFields), desc="Cutting Acoustic Fields"):
            ...
            if field.enveloppe.shape[0] < max_sample:
    ...
    def generateAOsignal(self,withTumor = True, AOsignalDataPath = None):
        ...
        if withTumor and self.AOsignal_withTumor is not None:
            print("AO signal with tumor already generated.")
            return
        if not withTumor and self.AOsignal_withoutTumor is not None:
            print("AO signal without tumor already generated.")
            return
        ...
        for i in trange(len(self.AcousticFields), desc=description):
            for t in range(self.AcousticFields[i].enveloppe.shape[0]):
                if withTumor:
                    ...
                    interaction = self.OpticImage.laser.intensity * self.AcousticFields[i].enveloppe[t, :, :]
    ...
    def show_AOsignal(self, withTumor = True, save_dir=None, wave_name=None):
        ...
        time_axis = np.arange(AOsignal.shape[0]) * float(self.params.acoustic['f_AQ']) * 1000
            ...
            if not pathField is None and os.path.exists(pathField):
                if progress_bar is not None:
                    ...
                    try:
                        ...
                        progress_bar.set_postfix_str(f"Error loading field -> Generating field - {AcousticField.getName_field()} -- Memory used :{memory.percent}% -- working {config.get_process().upper()}")
                        AcousticField.generate_field()
                        progress_bar.set_postfix_str(f"Calculating squared envelope of the field - {AcousticField.getName_field()} -- Memory used :{memory.percent}% -- working {config.get_process().upper()}")
                ...
                progress_bar.set_postfix_str(f"Generating field - {AcousticField.getName_field()} -- Memory used :{memory.percent}% -- working {config.get_process().upper()}")
                AcousticField.generate_field()
