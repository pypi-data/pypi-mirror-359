...
class IterativeRecon(Recon):
    ...
    def _iterativeReconPython(self,withTumor):
        SMatrix = np.zeros((self.experiment.AcousticFields.enveloppe.shape[0], self.experiment.AcousticFields.shape[1], self.experiment.AcousticFields.shape[2],len(self.experiment.AcousticFields)))
        for i in range(len(self.experiment.AcousticFields)):
            SMatrix[:,:,:,i] = self.experiment.AcousticFields[i].enveloppe
        if withTumor:
