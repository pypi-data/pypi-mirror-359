...
class AcousticField(ABC):
        ...
        try:
            if self.params['typeSim'] == TypeSim.FIELD2.value:
                ...
                if self.params["dim"] == Dim.D2.value:
                    field = self._generate_2Dacoustic_field_KWAVE(isGpu)
                    self.field = AcousticField.calculate_envelope_squared(field, isGpu)
                elif self.params["dim"] == Dim.D3.value:
                    field = self._generate_3Dacoustic_field_KWAVE(isGpu)
                    self.field = AcousticField.calculate_envelope_squared(field, isGpu)
    ...
    @staticmethod
    def calculate_envelope_squared(field, isGPU=True if config.get_process() == 'gpu' else False):
        """
