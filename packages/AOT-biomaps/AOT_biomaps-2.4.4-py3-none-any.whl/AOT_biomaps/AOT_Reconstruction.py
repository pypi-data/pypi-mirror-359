...
class IterativeRecon(Recon):
    ...
    def _iterativeReconPython(self,withTumor):
        ...
        if withTumor:
            if self.opti.value == IterativeType.MLEM.value:
                self.reconPhantom = self._MLEM(SMatrix=self.experiment.AcousticFields, y=self.experiment.AOsignal_withTumor)
        else:
            if self.opti.value == IterativeType.MLEM.value:
                self.reconLaser = self._MLEM(SMatrix=self.experiment.AcousticFields, y=self.experiment.AOsignal_withoutTumor)
    ...
    def _MLEM(self, SMatrix, y):
        ...
        if config.get_process() == 'gpu':
            return IterativeRecon._MLEM_GPU(SMatrix, y, self.numIterations* self.numSubsets)
        else:
            return IterativeRecon._MLEM_CPU(SMatrix, y, self.numIterations* self.numSubsets)
    
    ### ALGORITHMS IMPLEMENTATION ###

    @staticmethod
    def _MLEM_CPU(SMatrix, y, numIterations):
        if os.cpu_count() > 1:
            print(f"Using {os.cpu_count()} CPU cores for MLEM.")
            try:
                recons = IterativeRecon._MLEM_CPU_multi(SMatrix, y, numIterations)
                ...
                try:
                    recons = IterativeRecon._MLEM_CPU_opti(SMatrix, y, numIterations)
                    if recons is None or len(recons) == 0:
                        raise ValueError("Optimized MLEM returned empty result.")
                except Exception as e:
                    print(f"Error using optimized MLEM: {e}. Falling back to basic MLEM.")
                    try:
                        recons = IterativeRecon._MLEM_CPU_basic(SMatrix, y, numIterations)
                        if recons is None or len(recons) == 0:
    ...
    @staticmethod
    def _MLEM_GPU(SMatrix, y, numIterations):
        if not torch.cuda.is_available():
            print("CUDA not available, falling back to CPU.")
            return IterativeRecon._MLEM_CPU(SMatrix, y, numIterations)

        try:
            if torch.cuda.device_count() > 1:
                print(f"Using {torch.cuda.device_count()} GPUs for MLEM.")
                recons = IterativeRecon._MLEM_GPU_multi(SMatrix, y, numIterations)
                if recons is None or len(recons) == 0:
                    raise ValueError("Multi-GPU MLEM returned empty result.")
            else:
                print("Using single GPU for MLEM.")
                recons = IterativeRecon._MLEM_GPU_basic(SMatrix, y, numIterations)
                if recons is None or len(recons) == 0:
            ...
            try:
                recons = IterativeRecon._MLEM_CPU(SMatrix, y, numIterations)
                if recons is None or len(recons) == 0:
...
class BayesianRecon(IterativeRecon):
    ...
    def _bayesianReconPython(self, withTumor):
            if withTumor:
                if self.isPenalizedLogLikelyHood:
                    self.reconPhantom = BayesianRecon.MAPEM_STOP(SMatrix=self.experiment.AcousticFields, y=self.experiment.AOsignal_withTumor)
                else:
                    self.reconPhantom = BayesianRecon.MAPEM(SMatrix=self.experiment.AcousticFields, y=self.experiment.AOsignal_withTumor)
            else:
                if self.isPenalizedLogLikelyHood:
                    self.reconLaser = BayesianRecon.MAPEM_STOP(SMatrix=self.experiment.AcousticFields, y=self.experiment.AOsignal_withTumor)
                else:
                    self.reconLaser = BayesianRecon.MAPEM(SMatrix=self.experiment.AcousticFields, y=self.experiment.AOsignal_withTumor)
    ...
    def MAPEM_STOP(self, SMatrix, y):
        if config.get_process() == 'cpu':
            return BayesianRecon.MAPEM_CPU_STOP(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        elif config.get_process() == 'gpu':
            return BayesianRecon.MAPEM_GPU_STOP(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
    ...
    def MAPEM(self, SMatrix, y):
        if config.get_process() == 'cpu':
            return BayesianRecon.MAPEM_CPU(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        elif config.get_process() == 'gpu':
            return BayesianRecon.MAPEM_GPU(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
