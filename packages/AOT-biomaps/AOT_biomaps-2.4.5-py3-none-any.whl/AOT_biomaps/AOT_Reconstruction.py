...
class Recon:
    ...
    def _calculateCRC(self,ROI_mask = None):
        """
...
class IterativeRecon(Recon):
    ...
    """
    def __init__(self, opti = IterativeType.MLEM, numIterations = 10000, numSubsets = 1, **kwargs):
    ...
    def run(self, processType = ProcessType.PYTHON, withTumor= True,  isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):
        """
        ...
        if(processType == ProcessType.CASToR):
            ...
            self._iterativeReconPython(withTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
        else:
            raise ValueError(f"Unknown iterative reconstruction type: {processType}")
    ...
    def _iterativeReconPython(self,withTumor,  isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):
        SMatrix = np.zeros((self.experiment.AcousticFields.field.shape[0], self.experiment.AcousticFields.field.shape[1], self.experiment.AcousticFields.field.shape[2],len(self.experiment.AcousticFields)))
        for i in range(len(self.experiment.AcousticFields)):
            SMatrix[:,:,:,i] = self.experiment.AcousticFields[i].field
        ...
        if withTumor:
            ...
            if self.opti.value == IterativeType.MLEM.value:
                self.reconPhantom = self._MLEM(SMatrix=SMatrix, y=self.experiment.AOsignal_withTumor,  isGPU= isGPU, isMultiGPU =  isMultiGPU, isMultiCPU = isMultiCPU)
        else:
            ...
            if self.opti.value == IterativeType.MLEM.value:
                self.reconLaser = self._MLEM(SMatrix=SMatrix, y=self.experiment.AOsignal_withoutTumor,  isGPU= isGPU, isMultiGPU =  isMultiGPU, isMultiCPU = isMultiCPU)

    def _iterativeReconCASToR(self, withTumor):
    ...
    def _MLEM(self, SMatrix, y,  isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):
        """
        ...
        """
        if isGPU and isMultiGPU:
            return IterativeRecon._MLEM_GPU_multi(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if isGPU and not isMultiGPU:
            return IterativeRecon._MLEM_GPU_basic(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and isMultiCPU:
            return IterativeRecon._MLEM_CPU_multi(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and not isMultiGPU and not isMultiCPU:
            try:
                return IterativeRecon._MLEM_CPU_opti(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
            except:
                warnings.warn("Optimized MLEM failed. Falling back to basic MLEM.")
                return IterativeRecon._MLEM_CPU_basic(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        
        
    ### ALGORITHMS IMPLEMENTATION ###

    @staticmethod
    def _MLEM_GPU_basic(SMatrix, y, numIteration):
        """
    ...
    @staticmethod
    def _MLEM_CPU_basic(SMatrix, y, numIteration):
        """
    ...
    @staticmethod
    def _MLEM_CPU_multi(SMatrix, y, numIteration):
        """
        ...
        for _ in trange(numIteration, desc="ML-EM Iteration (CPU Multithread)"):
            ...
            IterativeRecon._forward_projection(SMatrix, theta_p, q_p)

            # Step 2: Current error estimate : compute ratio e = m / q
            e_p = y / (q_p + 1e-8)

            # Step 3: Backward projection of the error estimate : c = A.T * e (acc with njit)
            IterativeRecon._backward_projection(SMatrix, e_p, c_p)
    ...
    @staticmethod
    def _MLEM_CPU_opti(SMatrix, y, numIteration):
        """
    ...
    @staticmethod   
    def _MLEM_GPU_multi(SMatrix, y, numIteration):
        """
        ...
        # EM iterative update
        for _ in trange(numIteration, desc="ML-EM Multi-GPU Iteration"):
        ...
        # Average the results from all GPUs
        final_theta = torch.mean(torch.stack(theta_list), dim=0)
...
class BayesianRecon(IterativeRecon):
    ...
    def run(self, processType=ProcessType.PYTHON, withTumor=True,  isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):
        ...
        if(processType == ProcessType.CASToR):
            ...
            self._bayesianReconPython(withTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
        else:
    ...
    def _bayesianReconPython(self, withTumor,  isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):
        SMatrix = np.zeros((self.experiment.AcousticFields.field.shape[0], self.experiment.AcousticFields.field.shape[1], self.experiment.AcousticFields.field.shape[2],len(self.experiment.AcousticFields)))
        for i in range(len(self.experiment.AcousticFields)):
            SMatrix[:,:,:,i] = self.experiment.AcousticFields[i].field
        ...
        if withTumor:
            ...
            if self.isPenalizedLogLikelyHood:
                self.reconPhantom = BayesianRecon.MAPEM_STOP(SMatrix=SMatrix, y=self.experiment.AOsignal_withTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
            else:
                self.reconPhantom = BayesianRecon.MAPEM(SMatrix=SMatrix, y=self.experiment.AOsignal_withTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
        else:
            if self.experiment.AOsignal_withoutTumor is None:
                raise ValueError("AO signal without tumor is not available. Please generate AO signal without tumor the experiment first in the experiment object.")
            if self.isPenalizedLogLikelyHood:
                self.reconLaser = BayesianRecon.MAPEM_STOP(SMatrix=SMatrix, y=self.experiment.AOsignal_withoutTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
            else:
                self.reconLaser = BayesianRecon.MAPEM(SMatrix=SMatrix, y=self.experiment.AOsignal_withoutTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
    ...
    @staticmethod
    def _build_adjacency_sparse_GPU(Z, X,corner = (0.5-np.sqrt(2)/4)/np.sqrt(2),face = 0.5-np.sqrt(2)/4, device = config.select_best_gpu()):
        rows = []
        cols = []
        weights = []
    ...
    def _MAPEM_STOP(self, SMatrix, y, isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):

        if isGPU and isMultiGPU:
            warnings.warn("Multi-GPU MAPEM_STOP is not implemented yet. Falling back to single GPU.")
            return BayesianRecon._MAPEM_GPU_STOP(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if isGPU and not isMultiGPU:
            return BayesianRecon._MAPEM_GPU_STOP(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and isMultiCPU:
            warnings.warn("Multi-CPU MAPEM_STOP is not implemented yet. Falling back to single CPU.")
            return BayesianRecon._MAPEM_CPU_STOP(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and not isMultiGPU and not isMultiCPU:
            return BayesianRecon._MAPEM_CPU_STOP(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        
    @staticmethod
    def _MAPEM_CPU_STOP(SMatrix, y, Omega, iteration, beta, delta, gamma, sigma, previous=-np.inf):
        """
    ...
    @staticmethod  
    def _MAPEM_GPU_STOP(SMatrix, y, Omega, iteration, beta, delta, gamma, sigma, device_str=f"cuda:{config.select_best_gpu()}", previous = -np.inf):
        """
    ...
    def _MAPEM(self, SMatrix, y, isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):      
        
        if isGPU and isMultiGPU:
            warnings.warn("Multi-GPU MAPEM_STOP is not implemented yet. Falling back to single GPU.")
            return BayesianRecon._MAPEM_GPU(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if isGPU and not isMultiGPU:
            return BayesianRecon._MAPEM_GPU(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and isMultiCPU:
            warnings.warn("Multi-CPU MAPEM_STOP is not implemented yet. Falling back to single CPU.")
            return BayesianRecon._MAPEM_CPU(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and not isMultiGPU and not isMultiCPU:
            return BayesianRecon._MAPEM_CPU(SMatrix= SMatrix, y = y, Omega=self.potentialFunction, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)

    @staticmethod
    def _MAPEM_GPU(SMatrix, y, Omega, iteration, beta, delta, gamma, sigma, isSavingEachIteration=False,device_str=f"cuda:{config.select_best_gpu()}"):
        '''
    ...
    @staticmethod
    def _MAPEM_CPU(SMatrix, y, Omega, iteration, beta, delta, gamma, sigma, isSavingEachIteration=False):
