...
class IterativeRecon(Recon):
    ...
    def _MLEM(self, SMatrix, y):
        ...
        if self.isGPU and self.isMultiGPU:
            return IterativeRecon._MLEM_GPU_multi(SMatrix= SMatrix, y = y, iteration=self.numIterations, isSavingEachIteration = self.isSavingEachIteration)
        if self.isGPU and not self.isMultiGPU:
            return IterativeRecon._MLEM_GPU_basic(SMatrix= SMatrix, y = y, iteration=self.numIterations, isSavingEachIteration = self.isSavingEachIteration)
        if not self.isGPU and self.isMultiCPU:
            return IterativeRecon._MLEM_CPU_multi(SMatrix= SMatrix, y = y, iteration=self.numIterations, isSavingEachIteration = self.isSavingEachIteration)
        if not self.isGPU and not self.isMultiGPU and not self.isMultiCPU:
            try:
                return IterativeRecon._MLEM_CPU_opti(SMatrix= SMatrix, y = y, iteration=self.numIterations, isSavingEachIteration = self.isSavingEachIteration)
            except:
                warnings.warn("Optimized MLEM failed. Falling back to basic MLEM.")
                return IterativeRecon._MLEM_CPU_basic(SMatrix= SMatrix, y = y, iteration=self.numIterations, isSavingEachIteration = self.isSavingEachIteration)
    ...
    def _MLEM_CPU_basic(SMatrix, y, numIteration, isSavingEachIteration):
        ...
        Parameters:
            ...
            numIteration: number of iterations for the MLEM algorithm
        """
        #TODO: Only return the last theta if isSavingEachIteration is False
...
class BayesianRecon(IterativeRecon):
    ...
    @staticmethod
    def _MAPEM_CPU_STOP(SMatrix, y, Omega, iteration, beta, delta, gamma, sigma, previous=-np.inf, isSavingEachIteration=False):
        """
    ...
    @staticmethod  
    def _MAPEM_GPU_STOP(SMatrix, y, Omega, iteration, beta, delta, gamma, sigma, previous = -np.inf, isSavingEachIteration=False):
        """
    ...
    @staticmethod
    def _MAPEM_GPU(SMatrix, y, Omega, iteration, beta, delta, gamma, sigma, isSavingEachIteration=False):
        '''
