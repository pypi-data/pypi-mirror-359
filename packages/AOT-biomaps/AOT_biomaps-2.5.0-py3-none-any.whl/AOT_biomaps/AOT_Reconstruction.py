...
class AlgebraicRecon(Recon):
    ...
    def show_recon(self, vmin=None, vmax=None, isSaving=True, iteration=-1):
        ...
        plt.imshow(self.reconPhantom[iteration], extent=(self.experiment.params['Xrange'][0], self.experiment.params['Xrange'][1], self.experiment.params['Zrange'][1], self.experiment.params['Zrange'][0]),cmap='hot',vmin=vmin, vmax=vmax, aspect='equal', cmap='hot')
    ...
    def _MLEM_GPU_basic(SMatrix, y, numIteration, isSavingEachIteration):
        ...
        # EM Algebraic update
        for _ in trange(numIteration, desc=f"AOT-BioMaps -- Algebraic Reconstruction Tomography: ML-EM  ---- processing on single GPU n°{torch.cuda.current_device()} ----"):
...
class BayesianRecon(AlgebraicRecon):
    ...
    def _MAPEM_GPU_STOP(SMatrix, y, Omega, numIteration, beta, delta, gamma, sigma, previous = -np.inf, isSavingEachIteration=False):
        ...
        if Omega == PotentialType.RELATIVE_DIFFERENCE:
                description = f"AOT-BioMaps -- Bayesian Recontruction Tomography : MAP-EM (Sparse RD β:{beta:.4f}, δ:{delta:4f})+ STOP condtion (penalized log-likelihood) ---- processing on single GPU n°{torch.cuda.current_device()} ----"
        elif Omega == PotentialType.HUBER_PIECEWISE:
                description = f"AOT-BioMaps -- Bayesian Recontruction Tomography : MAP-EM (Sparse HUBER β:{beta:.4f}, γ:{gamma:4f})+ STOP condtion (penalized log-likelihood) ---- processing on single GPU n°{torch.cuda.current_device()} ----"
        elif Omega == PotentialType.QUADRATIC:
                description = f"AOT-BioMaps -- Bayesian Recontruction Tomography : MAP-EM (Sparse QUADRATIC σ:{sigma:.4f})+ STOP condtion (penalized log-likelihood) ---- processing on single GPU n°{torch.cuda.current_device()} ----"
    ...
    def _MAPEM_GPU(SMatrix, y, Omega, numIteration, beta, delta, gamma, sigma, isSavingEachIteration=False):
        ...
        if Omega == PotentialType.RELATIVE_DIFFERENCE:
            description = f"AOT-BioMaps -- Bayesian Recontruction Tomography : MAP-EM (Sparse RD β:{beta:.4f}, δ:{delta:4f}) ---- processing on single GPU n°{torch.cuda.current_device()} ----"
        elif Omega == PotentialType.HUBER_PIECEWISE:
            description = f"AOT-BioMaps -- Bayesian Recontruction Tomography : MAP-EM (Sparse HUBER β:{beta:.4f}, γ:{gamma:4f}) ---- processing on single GPU n°{torch.cuda.current_device()} ----"
        elif Omega == PotentialType.QUADRATIC:
            description = f"AOT-BioMaps -- Bayesian Recontruction Tomography : MAP-EM (Sparse QUADRATIC σ:{sigma:.4f}) ---- processing on single GPU n°{torch.cuda.current_device()} ----"
