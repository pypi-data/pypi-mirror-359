...
class StructuredWave(AcousticField):
    ...
    def _generate_2Dacoustic_field_KWAVE(self, isGPU=True if config.get_process() == 'gpu' else False, show_log=True):
        ...
        try:
            ...
            # Add padding to the source mask and sensor mask for simulation
            padded_source_mask = np.pad(source.p_mask, ((0, 512 - self.params['Nx']), (0, 512 - self.params['Nz'])), mode='constant')
            padded_sensor_mask = np.pad(sensor.mask, ((0, 512 - self.params['Nx']), (0, 512 - self.params['Nz'])), mode='constant')

            # Update source and sensor masks with padded versions
            source.p_mask = padded_source_mask
            sensor.mask = padded_sensor_mask
            ...
            # Remove padding from the result to return to original size
            original_sensor_data = sensor_data['p'][:self.kgrid.Nt, :self.params['Nz'], :self.params['Nx']]

            return original_sensor_data
        except Exception as e:
            print(f"Error generating 2D acoustic field: {e}")
            return None
