...
class Experiment():
    ...
    def _saveAOsignal_Castor(self,save_directory):
        """
...
class Tomography(Experiment):
    ...
    def __init__(self, acousticType = AOT_biomaps.AOT_Acoustic.WaveType.StructuredWave ,fieldParamPath=None, fieldDataPath = None, AOfile = None, formatSave = AOT_biomaps.AOT_Acoustic.FormatSave.HDR_IMG, **kwargs):
        super().__init__(**kwargs)

        if acousticType is None or acousticType.value == AOT_biomaps.AOT_Acoustic.WaveType.FocusedWave.value:
            raise ValueError("acousticType must be provided and cannot be FocusedWave for Tomography experiment")
        ...
        self.AOfile = AOfile
        self.AcousticFields = self._generateSMatrix(fieldDataPath, fieldParamPath)
        self.OpticImage = AOT_biomaps.AOT_Optic.Phantom(params=self.params)
        self.AOsignal = self._generateAOsignal()
    ...
    def cutSMatrix(self,max_t,min_t=0,saveFields=False):
    ...
    def check(self):
        ...
        if self.AOsignal is None:
            return False, "AOsignal is not initialized. Please generate the AO signal first."    
    ...
    def _generateSMatrix(self, fieldDataPath, fieldParamPath):
        """
        ...
        if self.TypeAcoustic.value == AOT_biomaps.AOT_Acoustic.WaveType.StructuredWave.value:
            try:
                return self._generateSMatrix_STRUCT_multiCPU(fieldDataPath, fieldParamPath)
            except Exception as e:
                print(f"Error generating system matrix with multithreading: {e}, falling back to single CPU generation.")
                return self._generateSMatrix_STRUCT_CPU(fieldDataPath, fieldParamPath)
        else:
            raise ValueError("Unsupported wave type.")
        
    def _generateSMatrix_STRUCT_CPU(self, fieldDataPath, fieldParamPath):
    ...
    def _generateSMatrix_STRUCT_multiCPU(self, fieldDataPath, fieldParamPath):
        if not os.path.exists(fieldParamPath):
            raise FileNotFoundError(f"Field parameter file {fieldParamPath} not found.")
        ...
        listAcousticFields = []
        with trange(len(patternList), desc="Generating system matrix") as progress_bar:
            for i in range(0, len(patternList), config.batchSize):
                batch = patternList[i:i + config.batchSize]
                listAcousticFields.extend(
                    self._process_batch(batch, fieldDataPath, self.FormatSave, self.params, progress_bar)
                )

        return listAcousticFields
    
    def _generateAOsignal(self):

        if self.AcousticFields is None:
            raise ValueError("AcousticFields is not initialized. Please generate the system matrix first.")

        if self.AOfile is not None:
            if not os.path.exists(self.AOfile):
                raise FileNotFoundError(f"AO file {self.AOfile} not found.")
            AOmatrix = self.load_AOSignal(self.AOfile)
    ...
    def _saveSMatrix_Castor(self, save_directory):
