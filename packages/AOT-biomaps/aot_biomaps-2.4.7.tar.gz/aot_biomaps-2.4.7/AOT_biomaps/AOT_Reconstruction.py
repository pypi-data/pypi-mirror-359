...
class IterativeRecon(Recon):
    ...
    def _MLEM(self, SMatrix, y,  isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):
        ...
        if isGPU and isMultiGPU:
            return IterativeRecon._MLEM_GPU_multi(SMatrix= SMatrix, y = y, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if isGPU and not isMultiGPU:
            return IterativeRecon._MLEM_GPU_basic(SMatrix= SMatrix, y = y, iteration=self.numIterations, device_str=config.select_best_gpu(), beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and isMultiCPU:
            return IterativeRecon._MLEM_CPU_multi(SMatrix= SMatrix, y = y, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
        if not isGPU and not isMultiGPU and not isMultiCPU:
            try:
                return IterativeRecon._MLEM_CPU_opti(SMatrix= SMatrix, y = y, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
            except:
                warnings.warn("Optimized MLEM failed. Falling back to basic MLEM.")
                return IterativeRecon._MLEM_CPU_basic(SMatrix= SMatrix, y = y, iteration=self.numIterations, beta=self.beta, delta=self.delta, gamma=self.gamma, sigma=self.sigma)
    ...
    def _bayesianReconPython(self, withTumor,  isGPU= config.get_process() == 'gpu', isMultiGPU =  True if config.numGPUs > 1 else False, isMultiCPU = True if config.numCPUs > 1 else False):
        ...
        if withTumor:
            ...
            if self.isPenalizedLogLikelyHood:
                self.reconPhantom = BayesianRecon._MAPEM_STOP(SMatrix=SMatrix, y=self.experiment.AOsignal_withTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
            else:
                self.reconPhantom = BayesianRecon._MAPEM(SMatrix=SMatrix, y=self.experiment.AOsignal_withTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
        else:
            if self.experiment.AOsignal_withoutTumor is None:
                raise ValueError("AO signal without tumor is not available. Please generate AO signal without tumor the experiment first in the experiment object.")
            if self.isPenalizedLogLikelyHood:
                self.reconLaser = BayesianRecon._MAPEM_STOP(SMatrix=SMatrix, y=self.experiment.AOsignal_withoutTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
            else:
                self.reconLaser = BayesianRecon._MAPEM(SMatrix=SMatrix, y=self.experiment.AOsignal_withoutTumor, isGPU=isGPU, isMultiGPU=isMultiGPU, isMultiCPU = isMultiCPU)
