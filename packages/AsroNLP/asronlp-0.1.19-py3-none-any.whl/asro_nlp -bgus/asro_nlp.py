...
class AsroNLP:
    ...
    def clean_dataframe(self, df, text_column):
        ...
        df_cleaned[text_column] = df_cleaned[text_column].apply(lambda x: ' '.join([w for w in x.split() if len(w) > 1]))
        ...
        # Data yang dibuang (duplikat dan bersih kosong/satu huruf)
        df_removed = pd.concat([df, df_cleaned]).drop_duplicates(keep=False)

        # Save hasil pembersihan data yang dibuang
    ...
    def visualize_dashboard_graphics(self, df):
        ...
        fig1.suptitle('Dashboard Sentimen dan Distribusi Sumber', fontsize=14)
        ...
        fig2, axs2 = plt.subplots(3, 2, figsize=(14, 14))
        fig2.suptitle('WordCloud dan Frekuensi Kata per Sentimen', fontsize=14)
        ...
        for i, cat in enumerate(categories):
            ...
            # Wordcloud
            wc = WordCloud(width=400, height=300, background_color='white', colormap='tab10').generate(text)
            axs2[i, 0].imshow(wc, interpolation='bilinear')
            axs2[i, 0].axis('off')
            axs2[i, 0].set_title(f'{cat} Sentiment WordCloud', fontsize=14)
            ...
            if counts:
                ...
                for idx, freq in enumerate(freqs):
                    axs2[i, 1].text(idx, freq + total_words * 0.01, f'{freq}', ha='center', va='bottom', fontsize=10, weight='bold')
