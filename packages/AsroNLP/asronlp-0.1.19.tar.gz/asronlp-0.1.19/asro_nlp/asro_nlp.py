...
class AsroNLP:
    ...
    def clean_dataframe(self, df, text_column):
        ...
        # Filter baris berdasarkan frekuensi karakter 1 huruf
        def filter_row(text):
            words = text.split()
            single_char_counts = Counter([w for w in words if len(w) == 1])
            # Jika ada karakter 1 huruf yang muncul lebih dari 5 kali di baris tersebut, drop baris (return None)
            if any(count > 5 for count in single_char_counts.values()):
                return None
            # Jika tidak, kembalikan kalimat bersih (filter hapus huruf 1 dengan frekuensi <=5)
            filtered_words = []
            for w in words:
                if len(w) == 1:
                    if single_char_counts[w] <= 5:
        ...
        # Setelah stemming nanti akan hapus duplikat juga, jadi di sini belum hapus
        ...
        # Data yang dibuang (duplikat awal + baris yang dihapus karena filter)
    ...
    def process_dataframe(self, df):
        ...
        with Progress(console=console) as progress:
            ...
            # Hapus duplikat setelah stemming
