...
def get_jobs_status():
    ...
    for jobname in jobs:
        try:
            ...
            if (not seen) and status is not None:
...
def get_scheduled_jobs_status():
    ...
    for jobname in jobs:
        try:
            status = jobs[jobname]['scheduled']
            running = jobs[jobname]['running']
            if status == 1 and running:
...
class ChatLogs:
    ...
    def cmdTRIGGER(self, read_timeout=7, get_updates_read_timeout=42):
        ...
        send_job_complete_notification = job_queue.run_repeating(self.send_jobs_status, interval=60, first=10)

        send_job_running_notification = job_queue.run_repeating(self.send_scheduled_jobs_status, interval=60, first=10)
