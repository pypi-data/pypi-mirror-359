"""
    ...
    -------------
       ...
       .self.NAME: Nome (para uso interno) e de refencia do banco de dados. Ex.: MONTEREY
...
class ORACLE:
    ...
    def getConnectCX_ORACLE(self):
        ...
        try:
            ...
            self.DRIVER_MODE = "THICK"
            self.setLibrary()
            self.CONNECTION = cx.connect(self.USERNAME, self.PASSWORD, self.getDnsName(), threaded=True)
            result = self.getTestConnect()
            self.CONNECTION_VALID = result
    ...
    def getConnectORACLEDB(self):
        ...
        try:
            ...
            #self.DRIVER_MODE = "THICK"
            self.setLibrary()
            self.CONNECTION = db.connect(user=self.USERNAME, password=self.PASSWORD, dsn=self.getDnsName())
            result = self.getTestConnect()
            self.CONNECTION_VALID = result
            self.DATABASE_ERROR = False
        except Exception as error:
    ...
    def getConnectSQLAlchemy(self):
        ...
        try:
            ...
            __cnn = f"""{self.DATABASE.lower()}+{__libOra}://{self.USERNAME}:{self.PASSWORD}@{self.getDnsName()}"""
            self.CONNECTION = sqa.create_engine(__cnn).connect().connection
