...
def fuzzy_match_label(raw_label: str,
                      choices: list[str],
                      scorer=fuzz.WRatio,
                      threshold: int = 70) -> str | None:
    raw_clean = normalize_text(raw_label)
...
def normalize_categories(
    ...
    if primary is None:
        primary = fuzzy_match_label(raw_primary, primary_choices, threshold=thresh_primary)
    ...
    secondary = fuzzy_match_label(raw_secondary, sec_choices, threshold=thresh_secondary)
    if not secondary:
        secondary = ""
...
def category_hasher(primary: str, secondary: str) -> float:
    # 对于第k个大类的第m个二级类，返回k * 8 + m
    try:
        k = primary_categories.index(primary)
        m = secondary_categories[primary].index(secondary)
        return k * 8 + m
    except:
        return 170

def category_hasher_reverse(hash: float) -> tuple[str, str]:
    if hash < 0 or hash > 56:
        return None, None
    k = int(hash / 8)
    m = int(hash % 8)
    return primary_categories[k], secondary_categories[primary_categories[k]][m]
...
if __name__ == "__main__":
    tests = [
        ...
        ("4", "4.3"),
        ("4. ", "Topology"),
        ("geometry topology", "4.3"),
        ("analysis & diff eq", "3.3 Differential Eq."),
        ("algebra num theory", "2.4 num theory"),
        ("Applied Comp Math", "Numerical methods"),
        ("Unknown Cat", "Something"),
    ]
    for raw_p, raw_s in tests:
        print(f"RAW=({raw_p!r},{raw_s!r}) →",
              normalize_categories(raw_p, raw_s))


    result = category_hasher_reverse(34.0)
    print(result)
    print(category_hasher(*result))