...
class PACScorer(VideoTextScorer):

    def __init__(self, args_dict: dict, cfg=None):
        super().__init__(args_dict)
        self.scorer_name = "PACScorer"
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model_path = args_dict['model_path']
        if not os.path.exists(args_dict['model_path']):
            gdown_model('https://drive.google.com/uc?id=1F-0Pma-vfJPAiDzeyl-iEdSXZIO1cDae', args_dict['model_path'])
        model, preprocess = clip.load("ViT-B/32", device=self.device)
        model = model.to(self.device)                          #for PAC-Score
        model = model.float()                             #for PAC-Score
        checkpoint = torch.load(args_dict['model_path'])  #for PAC-Score
    ...
    def evaluate_batch(self, sample):
        ...
        print("calculating PACScore scores...")
        ...
        if refs:
            ...
            final_results['PACScore(X,X*)'] = refs_results
        ...
        if vids:
            ...
            final_results['PACScore(X,V)'] = vid_results
        
        if refs and vids:
            vid_refs_result = {}
            vid_refs_result['figr_P'] = (final_results['PACScore(X,V)']['figr_P'] + final_results['PACScore(X,X*)']['figr_P'])/2
            vid_refs_result['figr_R'] = (final_results['PACScore(X,V)']['figr_R'] + final_results['PACScore(X,X*)']['figr_R'])/2
            vid_refs_result['figr_F'] = (final_results['PACScore(X,V)']['figr_F'] + final_results['PACScore(X,X*)']['figr_F'])/2
            vid_refs_result['cogr'] = (final_results['PACScore(X,V)']['cogr'] + final_results['PACScore(X,X*)']['cogr'])/2
            ...
            final_results['PACScore(X,V,X*)'] = vid_refs_result
