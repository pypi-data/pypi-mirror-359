...
class LazyLoader(types.ModuleType):
    ...
    def __init__(self, name, path, import_structure):
        ...
        self._base_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), '')
        self.__path__ = [path]
    ...
    def _load_class_from_file(self, file_path, class_name):
        ...
        """
        # 构建完整的文件路径
        full_path = os.path.join(self._base_path, file_path)
        ...
        logger = get_logger()
        try:
            logger.info(f"LazyLoader {self.__path__} trying to import {class_name} from {full_path}")
        ...
        if not hasattr(module, class_name):
            raise AttributeError(f"Class {class_name} not found in {full_path}")
