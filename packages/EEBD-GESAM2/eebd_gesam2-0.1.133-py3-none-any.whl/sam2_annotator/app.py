...
def setup_and_get_paths():
    """
    检查模型文件是否存在于指定位置。
    如果不存在，则向用户显示详细的说明并退出。
    如果存在，则返回模型和配置文件的路径。
    """
    MODEL_FILENAME = "sam2_hiera_large.pt"

    # 使用 appdirs 找到一个跨平台、用户可写的安全目录
    app_name = "EEBD-GESAM2"
    author = "YourAppName"  # 可以用你的名字或应用名
    # 这个函数会生成类似以下的路径:
    # Windows: C:\\Users\\YourUser\\AppData\\Local\\YourAppName\\EEBD-GESAM2
    # macOS:   /Users/YourUser/Library/Application Support/EEBD-GESAM2
    # Linux:   /home/YourUser/.local/share/EEBD-GESAM2
    model_storage_dir = user_data_dir(app_name, author)

    # 确保这个目录存在
    os.makedirs(model_storage_dir, exist_ok=True)

    # 构造模型的完整预期路径
    expected_model_path = os.path.join(model_storage_dir, MODEL_FILENAME)

    # 核心检查：模型文件是否存在？
    if not os.path.exists(expected_model_path):
        # 如果不存在，显示一个详细的错误和指引窗口
        error_title = "缺少模型文件"
        error_message = f"""
未能启动：缺少必要的模型文件。

请手动下载模型文件 "{MODEL_FILENAME}"，然后将其放置在以下目录中：

{model_storage_dir}

您可以从 Segment Anything Model 2 (SAM2) 的官方渠道（如Hugging Face或GitHub）获取此文件。

程序现在将退出。
"""
        # 在显示任何 tkinter 窗口前，需要先创建一个隐藏的根窗口
        root = tk.Tk()
        root.withdraw()
    ...
    # --- 如果模型存在，则继续获取配置文件的路径 ---
    try:
        # __file__ 指向当前文件 (app.py)
        application_path = os.path.dirname(os.path.abspath(__file__))
    except NameError:
        application_path = os.getcwd()

    config_path = os.path.join(application_path, "configs", "sam2", "sam2_hiera_l.yaml")

    if not os.path.exists(config_path):
        messagebox.showerror("配置错误", f"配置文件未找到: {config_path}\n请确认包已正确安装。")
        sys.exit(1)

    # 所有文件都已就绪，返回路径
    return expected_model_path, config_path


def start():  # 函数名改为 start 以匹配 toml 配置中推荐的入口点名称
    """程序的主入口点，由命令行工具调用。"""

    try:
        # 1. 设置并获取所有必要的路径
        print("正在检查资源路径...")
        model_path_to_use, config_path_to_use = setup_and_get_paths()
        print(f"模型路径: {model_path_to_use}")
        print(f"配置路径: {config_path_to_use}")

        # 2. 设置输出目录
        output_dir_to_use = r"./annotations_sam2_tool"
        ...
        root = tk.Tk()
        root.withdraw()
        messagebox.showerror("应用发生意外错误", f"程序遇到严重错误: {e}")
        sys.exit(1)


# 当你直接运行 `python app.py` 时，会执行这个入口
