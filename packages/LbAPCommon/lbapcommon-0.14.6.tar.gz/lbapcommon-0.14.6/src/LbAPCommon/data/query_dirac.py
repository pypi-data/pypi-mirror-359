...
def find_bk_query_from_sample(input_query):
    ...
    samples = list(
        filter(
            lambda x: x.get("name", None) == input_query_tags.get("name", None)
            and x.get("version", None) == input_query_tags.get("version", None),
            samples,
        )
    )
    ...
    if len(samples) == 1:
        ...
        samples = list(
            filter(
                lambda x: all(
                    [
                        input_query_tags.get(tag, None) == value
                        for tag, value in samples_tags[x["sample_id"]].items()
                    ]
                ),
                samples,
            )
    ...
    print(f"Matched sample: {json.dumps(selected_sample, indent=2)}")
...
def main(input_query, turbo):
    ...
    if "bk_query" in input_query:
        ...
        assert set(input_query).issubset(COMMON_KEYS | {"tags"})
