...
class CLlmGenJson:
    # 调用LLM生成JSON数据
    def __init__(self, sOpenAiUrl, sOpenAiKey, sOpenAiModel):
    ...
    def gen_json_via_llm(self, sPromptSystem, sPromptUser, bDebugPrint=False):
        # 通过LLM将用户提示语转为JSON数据
        # sPromptSystem=系统提示语，人设
        # sPromptUser=用户提示语
        # 返回 json 对象
        iRetryCnt = 0  # 重试次数
        while iRetryCnt < 3:
            ...
            if bDebugPrint:
                PrintTimeMsg(f'gen_json_via_llm({sPromptUser}).sLlmResult={sLlmResult}={iRetryCnt}=')
            sJsonResult = parse_json_str_from_text(sLlmResult)
            if bDebugPrint:
                PrintTimeMsg(f'gen_json_via_llm({sPromptUser}).sJsonResult={sJsonResult}={iRetryCnt}=')
            try:
                oResult = json.loads(sJsonResult)
                PrintTimeMsg(f'gen_json_via_llm({sPromptUser}).oResult={json.dumps(oResult)}=')
                return oResult
            except Exception as e:
                PrintTimeMsg(f'gen_json_via_llm({sPromptUser}).sLlmResult={sLlmResult}={iRetryCnt}={repr(e)}')
            iRetryCnt += 1
        return {}
