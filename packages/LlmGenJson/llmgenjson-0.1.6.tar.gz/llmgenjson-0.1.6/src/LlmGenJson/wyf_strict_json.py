...
def convert_to_dict(field: str, keys: dict, delimiter: str) -> dict:
    ...
    # print(f'convert_to_dict.field={field}=')
    # print(f'convert_to_dict.keys={keys}=')
    # print(f'convert_to_dict.delimiter={delimiter}=')
    output_d = {}
    for key in keys:
...
def check_datatype(field, key: dict, data_type: str, **kwargs):
    ...
    print(f'check_datatype.field={field}=')
    print(f'check_datatype.key={key}=')
    print(f'check_datatype.data_type={data_type}=')
...
def strict_json(system_prompt: str, user_prompt: str, output_format: dict, return_as_json = False, custom_checks: dict = None, check_data = None, delimiter: str = '###', num_tries: int = 3, openai_json_mode: bool = False, **kwargs):
    ...
    # Otherwise, implement JSON parsing using Strict JSON
        ...
        # WeiYF.20250630 对可选项不检查
        lsKeyOptional = []
        for k, v in output_format.items():  # 忽略可选项
            if isinstance(v, str) and v.endswith('Optional'):
                lsKeyOptional.append(k)
        print(f'strict_json.lsKeyOptional={lsKeyOptional}=')
        for k in lsKeyOptional:
            del output_format[k]
