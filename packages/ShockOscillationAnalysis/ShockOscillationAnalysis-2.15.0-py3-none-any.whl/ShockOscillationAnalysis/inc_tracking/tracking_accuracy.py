...
def pop_outlier(indx, xloc, columnY, n_slice_new, log_dirc:str=''):
    newxloc = np.delete(xloc, indx)
    newyloc = np.delete(columnY, indx)
    # new_shock_slope = v_least_squares(newxloc, newyloc, n_slice_new - 1)
    new_shock_slope, new_midxloc = ransac(newxloc, newyloc, 1, log_dirc=log_dirc)
