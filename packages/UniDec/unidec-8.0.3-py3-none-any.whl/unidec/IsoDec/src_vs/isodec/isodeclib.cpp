...
IsoConfig SetupConfig(const int maxz, const int pres)
{
	IsoConfig config;
	config.pres = pres;
	config.maxz = maxz;
...
IsoSettings DefaultSettings()
{
	IsoSettings settings;
	settings.phaseres = 8;
	settings.verbose = 0;
...
Weights SetuptWeights(const IsoConfig config)
{
	Weights weights;
...
Weights load_weights(const char* filename, const IsoConfig config)
{
	...
	Weights parsedweights = SetuptWeights(config);
...
int encode(const double* cmz, const float* cint, const int n, float * emat, const IsoConfig config, const IsoSettings settings)
{
	//printf("Encoding\n");
...
int predict_nn(float* emat, const Weights weights, float* h1, float*h2, const IsoConfig config, const float zscore_threshold, int * z2)
{
	float val = 0;
	//Calculate h1
...
void predict_charge(const double* cmz, const float* cint, const int n, const char* fname, int* charge)
{
	...
	IsoConfig config = SetupConfig(50, settings.phaseres);
	...
	// Load Weights
	Weights weights = load_weights(filename, config);
...
int isotope_dist(const float mass, float* isovals, const float* isoparams, const float normfactor, const IsoSettings settings)
{
...
void iso_mz_vals(const float mass, float* mzvals, const int z, const IsoSettings settings, const int realisolength)
{
	int offset = 0;
	if (settings.minusoneaszero == 1)
	{
...
int bestshift_adjust(const double* mz, const float* inten, const int length, float* isomz, float* isodist, const int maxshift,
	const int charge, MatchedPeak* matchpeaks, const int nmatched, const float peakmz, const float mass,
	const IsoSettings settings, int realisolength)
{
	...
	for (int j = 0; j < realisolength; j++)
	{
		...
		int highestindex = -1;
		int mid_index = int(realisolength / 2);
...
int optimize_shift(const int z, const double* mz, float * inten, const int length, MatchedPeak *matchpeaks, const int nmatched, 
	const float peakmz, const IsoSettings settings)
{
...
void print_matches(MatchedPeak* matchedpeaks, int nmatched)
{
...
extern "C" __declspec(dllexport)
int process_spectrum(const double* cmz, const float* cint, int n, const char* fname, MatchedPeak * matchedpeaks, IsoSettings settings)
{
	// Set some parameters
	IsoConfig config = SetupConfig(50, settings.phaseres);
	config.dlen = n;
...
extern "C" __declspec(dllexport)
int process_spectrum_default(const double* cmz, const float* cint, const int n, const char* fname, MatchedPeak * matchedpeaks)
{
	IsoSettings settings = DefaultSettings();
...
void run()
{
	...
	MatchedPeak* matchedpeaks;
	matchedpeaks = (MatchedPeak*)calloc(length, sizeof(MatchedPeak));
