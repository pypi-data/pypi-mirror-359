...
from .config import AddonConfig, Config
...
class AddonBuilder:
    ...
    def build(self, disttype: DistType = "local") -> Path:
        ...
        return self.package_dist(disttype=disttype)
    ...
    def build_dist(self, disttype: DistType = "local") -> None:
        ...
        ui_builder = UIBuilder(dist=PATH_DIST, config=self._addon_config)
    ...
    def _package(self, disttype: DistType) -> Path:
        ...
        qt_target = self._addon_config.targets[0]

        out_name = "{repo_name}-{version}-{qt_target}{dist}.{ext}".format(
