...
class ProjectInitializer:
    ...
    def init_project(self, interactive: bool = True) -> None:
        ...
        print("   1. Test with Anki: python run_anki.py")
    ...
    def _render_template(self, template_name: str, config: AddonConfig) -> str:
        ...
        try:
            ...
            raise ProjectInitializationError(f"Missing template variable: {e}")
    ...
    def _create_template_files(self, config: AddonConfig) -> None:
        ...
        init_content = self._render_template("__init__.py", config)
        (src_dir / "__init__.py").write_text(init_content, encoding="utf-8")

        main_content = self._render_template("main.py", config)
        (src_dir / "main.py").write_text(main_content, encoding="utf-8")

        # Create project configuration files
        gitignore_content = self._render_template("gitignore", config)
        ...
        readme_zh_content = self._render_template("README-zh.md", config)
        (self.target_dir / "README-zh.md").write_text(readme_zh_content, encoding="utf-8")

        python_version_content = self._render_template("python-version", config)
        (self.target_dir / ".python-version").write_text(
            python_version_content, encoding="utf-8"
        )
        ...
        # Create run_anki.py for quick testing
        run_anki_content = self._render_template("run_anki.py", config)
        run_anki_path = self.target_dir / "run_anki.py"
        run_anki_path.write_text(run_anki_content, encoding="utf-8")

        # Make the script executable on Unix-like systems
        try:
            import stat

            run_anki_path.chmod(run_anki_path.stat().st_mode | stat.S_IEXEC)
