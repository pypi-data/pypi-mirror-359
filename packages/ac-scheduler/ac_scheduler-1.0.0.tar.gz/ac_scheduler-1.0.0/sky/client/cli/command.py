...
@click.argument('cluster_name',
                ...
                type=str,
                **_get_shell_complete_args(_complete_cluster_name))
@click.argument('user_hash', required=True, type=str)
@click.option(
    '--expires-in',
    '-e',
    type=float,
...
@flags.yes_option()
@usage_lib.entrypoint
def grants_add(cluster_name: str, user_hash: str, expires_in: Optional[float],
    ...
    .. code-block:: bash
      ...
      # Grant access to user with hash abc123
      sky grants add mycluster abc123
      
      # Grant access that expires in 24 hours
      sky grants add mycluster abc123 --expires-in 24
      
      # Grant access with notes
      sky grants add mycluster abc123 --notes "Temporary access for debugging"
    """
    if not yes:
        click.confirm(
            f'Grant user {user_hash!r} access to cluster {cluster_name!r}?',
            default=True,
            abort=True,
            show_default=True)

    request_id = sdk.grant_cluster_access(cluster_name=cluster_name,
                                          grantee_user_hash=user_hash,
...
def grants_list(cluster_name: str):
    ...
    from sky.utils import status_utils
    from sky.utils import common_utils
    import prettytable
    ...
    for grant in grants:
        ...
        table.add_row([
            grant['grant_id'], grant['user_email'], grant['granted_by'],
            grant['granted_at'],
            expires_at if expires_at == 'Never' else expires_at, notes
        ])
