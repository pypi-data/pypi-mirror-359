...
def get_my_granted_clusters() -> List[str]:
    """Gets all clusters that the current user has been granted access to.
    
    Returns:
        List of cluster names that the current user has access to.
    """
    current_user = common_utils.get_current_user()
    return global_user_state.get_user_granted_clusters(current_user.id)
