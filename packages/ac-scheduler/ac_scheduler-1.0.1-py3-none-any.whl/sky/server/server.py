    ...
    try:
        ...
        if not cluster_records:
            await websocket.send_text(
                f"ERROR: Cluster {cluster_name!r} not found.")
        ...
        if cluster_record['status'] != status_lib.ClusterStatus.UP:
            await websocket.send_text(
                f"ERROR: Cluster {cluster_name!r} is not running (status: {cluster_record['status'].value})."
            )
        ...
        if not isinstance(handle.launched_resources.cloud, clouds.Kubernetes):
            await websocket.send_text(
                f"ERROR: Cluster {cluster_name!r} is not a Kubernetes cluster. Use ssh to connect to the cluster instead."
            )
