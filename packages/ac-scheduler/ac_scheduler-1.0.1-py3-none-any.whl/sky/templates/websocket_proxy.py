...
def _get_service_account_headers() -> Dict[str, str]:
    """Get service account authentication headers including JWT tokens."""
    try:
        # Import here to avoid import errors if sky module is not available
        # pylint: disable=import-outside-toplevel
        from sky.client import service_account_auth
        return service_account_auth.get_service_account_headers()
    except ImportError:
        # Sky module not available, skip service account auth
        return {}
    except Exception:  # pylint: disable=broad-except
        # Any other error getting service account headers, skip gracefully
        return {}


def _get_auth_headers(url: str) -> Dict[str, str]:
    """Get all authentication headers (cookies + service account tokens)."""
    headers = {}

    # Add cookie headers
    cookie_headers = _get_cookie_header(url)
    headers.update(cookie_headers)

    # Add service account headers (JWT Bearer tokens)
    sa_headers = _get_service_account_headers()
    headers.update(sa_headers)

    return headers


async def main(url: str) -> None:
    auth_headers = _get_auth_headers(url)
    async with connect(url, ping_interval=None,
                       additional_headers=auth_headers) as websocket:
