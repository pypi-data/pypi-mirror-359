...
# Table for cluster access grants
# Allows users to grant access to their clusters to other users
cluster_grants_table = sqlalchemy.Table(
    'cluster_grants',
    Base.metadata,
    sqlalchemy.Column('grant_id', sqlalchemy.Text, primary_key=True),
    sqlalchemy.Column('cluster_name', sqlalchemy.Text, nullable=False),
    sqlalchemy.Column('grantor_user_hash', sqlalchemy.Text, nullable=False),
    sqlalchemy.Column('grantee_user_hash', sqlalchemy.Text, nullable=False),
    sqlalchemy.Column('granted_at', sqlalchemy.Integer, nullable=False),
    sqlalchemy.Column('expires_at',
                      sqlalchemy.Integer),  # NULL for permanent grants
    sqlalchemy.Column('revoked_at', sqlalchemy.Integer),  # NULL if not revoked
    sqlalchemy.Column('notes',
                      sqlalchemy.Text),  # Optional notes about the grant
    # Create indexes for efficient lookup
    sqlalchemy.Index('idx_cluster_grants_cluster_grantee', 'cluster_name',
                     'grantee_user_hash'),
    sqlalchemy.Index('idx_cluster_grants_grantor', 'grantor_user_hash'),
)
...
# ========== Cluster Grant Functions ==========
@_init_db
def add_cluster_grant(cluster_name: str,
                      grantor_user_hash: str,
                      grantee_user_hash: str,
                      expires_at: Optional[int] = None,
                      notes: Optional[str] = None) -> str:
    """Grant access to a cluster to another user.
    
    Args:
        cluster_name: Name of the cluster to grant access to
        grantor_user_hash: User hash of the user granting access (must be owner)
        grantee_user_hash: User hash of the user receiving access
        expires_at: Unix timestamp when grant expires (None for permanent)
    ...
    Raises:
        ValueError: If grantor is not the owner of the cluster
    """
    assert _SQLALCHEMY_ENGINE is not None

    # Verify that the grantor owns the cluster
    cluster = get_cluster_from_name(cluster_name)
    if cluster is None:
        raise ValueError(f'Cluster {cluster_name!r} does not exist')

    if cluster['user_hash'] != grantor_user_hash:
        raise ValueError(
            f'User {grantor_user_hash} is not the owner of cluster '
            f'{cluster_name!r}')
    ...
    with orm.Session(_SQLALCHEMY_ENGINE) as session:
        session.execute(cluster_grants_table.insert().values(
            grant_id=grant_id,
            cluster_name=cluster_name,
            grantor_user_hash=grantor_user_hash,
            grantee_user_hash=grantee_user_hash,
...
@_init_db
def revoke_cluster_grant(grant_id: str, revoker_user_hash: str) -> None:
    """Revoke a cluster grant.
    
    Args:
        grant_id: The grant ID to revoke
        revoker_user_hash: User hash of the user revoking (must be grantor)
        
    Raises:
        ValueError: If grant doesn't exist or user is not the grantor
    """
    assert _SQLALCHEMY_ENGINE is not None

    with orm.Session(_SQLALCHEMY_ENGINE) as session:
        # Find the grant
        grant = session.query(cluster_grants_table).filter(
            cluster_grants_table.c.grant_id == grant_id).first()

        if grant is None:
            raise ValueError(f'Grant {grant_id!r} does not exist')

        if grant.grantor_user_hash != revoker_user_hash:
            raise ValueError(
                f'User {revoker_user_hash} is not the grantor of this grant')

        if grant.revoked_at is not None:
            raise ValueError(f'Grant {grant_id!r} is already revoked')

        # Revoke the grant
        session.query(cluster_grants_table).filter(
            cluster_grants_table.c.grant_id == grant_id).update(
                {cluster_grants_table.c.revoked_at: int(time.time())})
        session.commit()


@_init_db
def has_cluster_access_grant(cluster_name: str, grantee_user_hash: str) -> bool:
    """Check if a user has been granted access to a cluster.
    
    Args:
        cluster_name: Name of the cluster
        grantee_user_hash: User hash to check access for
        
    Returns:
        True if user has valid (non-expired, non-revoked) grant
    """
    assert _SQLALCHEMY_ENGINE is not None
    ...
    with orm.Session(_SQLALCHEMY_ENGINE) as session:
        # Look for valid grants
        grant = session.query(cluster_grants_table).filter(
            cluster_grants_table.c.cluster_name == cluster_name,
            cluster_grants_table.c.grantee_user_hash == grantee_user_hash,
            cluster_grants_table.c.revoked_at.is_(None),
            sqlalchemy.or_(
                cluster_grants_table.c.expires_at.is_(None),
                cluster_grants_table.c.expires_at > current_time)).first()

        return grant is not None


@_init_db
def get_cluster_grants(cluster_name: str,
    ...
    with orm.Session(_SQLALCHEMY_ENGINE) as session:
        query = session.query(cluster_grants_table).filter(
            cluster_grants_table.c.cluster_name == cluster_name)

        if not include_revoked:
            query = query.filter(cluster_grants_table.c.revoked_at.is_(None))
        ...
        if not include_expired:
            ...
            query = query.filter(
                sqlalchemy.or_(
                    cluster_grants_table.c.expires_at.is_(None),
                    cluster_grants_table.c.expires_at > current_time))
        ...
        for result in results:
            grants.append({
                ...
                'grantor_user_hash': result.grantor_user_hash,
                'grantee_user_hash': result.grantee_user_hash,
...
@_init_db
def get_user_granted_clusters(grantee_user_hash: str) -> List[str]:
    """Get all clusters that a user has been granted access to.
    
    Args:
        grantee_user_hash: User hash to check grants for
        
    Returns:
        List of cluster names the user has access to
    """
    assert _SQLALCHEMY_ENGINE is not None

    current_time = int(time.time())

    # log grantee user hash
    logger.info(f'grantee_user_hash: {grantee_user_hash}')
    print(f'grantee_user_hash: {grantee_user_hash}')

    with orm.Session(_SQLALCHEMY_ENGINE) as session:
        # Look for valid grants
        results = session.query(
            cluster_grants_table.c.cluster_name.distinct()).filter(
                cluster_grants_table.c.grantee_user_hash == grantee_user_hash,
                cluster_grants_table.c.revoked_at.is_(None),
                sqlalchemy.or_(
                    cluster_grants_table.c.expires_at.is_(None),
                    cluster_grants_table.c.expires_at > current_time)).all()
