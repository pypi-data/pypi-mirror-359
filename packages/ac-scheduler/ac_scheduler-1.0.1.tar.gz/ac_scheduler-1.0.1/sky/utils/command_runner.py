...
def _ssh_control_path(ssh_control_filename: Optional[str]) -> Optional[str]:
    ...
    # Truncate user hash to avoid Unix domain socket path length issues
    # Unix sockets have a ~108 character path limit, so we ensure the user
    # hash portion doesn't exceed a reasonable length
    if len(user_hash) > 16:
        # For service accounts and long user hashes, use a shorter hash
        user_hash = hashlib.md5(user_hash.encode()).hexdigest()[:16]
