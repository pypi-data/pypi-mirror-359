...
def check_owner_identity(cluster_name: str,
                         override_cur_user_hash: Optional[str] = None) -> None:
    ...
    # Add user hash logging
    cluster_user_hash = record.get('user_hash', 'unknown')
    cluster_user_name = record.get('user_name', 'unknown')
    if override_cur_user_hash:
        current_user = global_user_state.get_user(override_cur_user_hash)
        current_user_hash = override_cur_user_hash
        current_user_name = current_user.name if current_user else ""
    else:
        current_user_hash = common_utils.get_user_hash()
        current_user_name = common_utils.get_current_user_name()

    # Make sure the current user is the same as the cluster creator at the Skypilot user level.
    if cluster_user_hash != current_user_hash:
        # Check if the current user has been granted access to this cluster
        if global_user_state.has_cluster_access_grant(cluster_name,
                                                      current_user_hash):
            logger.info(
                f'User {current_user_name} has been granted access to '
                f'cluster {cluster_name!r} owned by {cluster_user_name}.')
        else:
            with ux_utils.print_exception_no_traceback():
                raise exceptions.ClusterOwnerIdentityMismatchError(
                    f'{colorama.Fore.YELLOW}'
                    f'The cluster {cluster_name!r} is owned by {cluster_user_name}.'
