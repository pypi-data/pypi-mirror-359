...
def _get_cluster_records_and_set_ssh_config(
    ...
    for record in cluster_records:
        ...
        if isinstance(handle.launched_resources.cloud, clouds.Kubernetes):
            ...
            # Generate node-specific proxy commands
            auth_config = credentials.copy()

            # For each node (head + workers), create a specific proxy command
            proxy_commands = []
            for i in range(len(handle.cached_external_ips)):
                # Determine node name based on index
                if i == 0:
                    node_name = handle.cluster_name
                else:
                    node_name = f'{handle.cluster_name}-worker{i}'

                # Create node-specific proxy command
                proxy_command = (
                    f'ssh -tt -i {key_path} '
            ...
            # Set the proxy commands - head node uses the default one
            auth_config['ssh_proxy_command'] = proxy_commands[0]
            auth_config['node_specific_proxy_commands'] = proxy_commands
            credentials = auth_config

        # Apply the SSH config
...
# Cluster grants management
@cli.group(cls=_NaturalOrderGroup)
def grants():
    """Manage cluster access grants."""
    pass


@grants.command('add', cls=_DocumentedCodeCommand)
@flags.config_option(expose_value=False)
...
@click.option(
    ...
    type=float,
    required=False,
    help='Grant expiration time in hours. If not specified, access never expires.'
)
@click.option('--notes',
              '-n',
              type=str,
              required=False,
              help='Notes about this grant.')
@flags.yes_option()
@usage_lib.entrypoint
def grants_add(cluster_name: str, user_name: str, expires_in: Optional[float],
               notes: Optional[str], yes: bool):
    ...
    .. code-block:: bash
      ...
      # Grant access to user john.doe@example.com
      sky grants add mycluster john.doe@example.com
      
      # Grant access that expires in 24 hours
      sky grants add mycluster john.doe@example.com --expires-in 24
      
      # Grant access with notes
      sky grants add mycluster john.doe@example.com --notes "Temporary access for debugging"
    """
    if not yes:
        click.confirm(
            f'Grant user {user_name!r} access to cluster {cluster_name!r}?',
            default=True,
            abort=True,
            show_default=True)

    request_id = sdk.grant_cluster_access(cluster_name=cluster_name,
                                          grantee_user_name=user_name,
                                          expires_in_hours=expires_in,
                                          notes=notes)

    grant_result = sdk.get(request_id)
    if grant_result is None:
        click.secho(f'✓ Successfully granted access to cluster {cluster_name}',
                    fg='green')
    else:
        click.secho(f'Failed to grant access: {grant_result}', fg='red')


@grants.command('revoke', cls=_DocumentedCodeCommand)
@flags.config_option(expose_value=False)
@click.argument('grant_id', required=True, type=str)
@flags.yes_option()
@usage_lib.entrypoint
def grants_revoke(grant_id: str, yes: bool):
    """Revoke a user's access to a cluster.

    The grant ID can be obtained from ``sky grants list``.

    Examples:

    .. code-block:: bash

      # Revoke grant with ID 12345
      sky grants revoke 12345
    """
    if not yes:
        click.confirm(f'Revoke grant {grant_id!r}?',
                      default=True,
                      abort=True,
                      show_default=True)

    request_id = sdk.revoke_cluster_access(grant_id=grant_id)

    revoke_result = sdk.get(request_id)
    if revoke_result is None:
        click.secho(f'✓ Successfully revoked grant {grant_id}', fg='green')
    else:
        click.secho(f'Failed to revoke grant: {revoke_result}', fg='red')


@grants.command('list', cls=_DocumentedCodeCommand)
@flags.config_option(expose_value=False)
@click.argument('cluster_name',
                required=True,
                type=str,
                **_get_shell_complete_args(_complete_cluster_name))
@usage_lib.entrypoint
def grants_list(cluster_name: str):
    """List all grants for a cluster.

    Shows all users who have been granted access to the specified cluster.

    Examples:

    .. code-block:: bash

      # List all grants for mycluster
      sky grants list mycluster
    """
    request_id = sdk.get_cluster_grants(cluster_name=cluster_name)
    grants = sdk.get(request_id)

    if not grants:
        click.echo(f'No grants found for cluster {cluster_name!r}')
        return

    # Create a pretty table for displaying grants
    from sky.utils import status_lib
    from sky.utils import common_utils
    import prettytable
    ...
    table.field_names = [
        'GRANT ID', 'USER', 'GRANTED BY', 'GRANTED AT', 'EXPIRES AT', 'NOTES'
    ]
    table.align = 'l'

    for grant in grants:
        expires_at = grant.get('expires_at', 'Never')
        ...
        table.add_row([
            grant['grant_id'], grant['grantee_user_name'],
            grant['grantor_user_name'], granted_at, expires_at, notes
        ])

    click.echo(f'\n{colorama.Fore.CYAN}{colorama.Style.BRIGHT}'
               f'Cluster grants for {cluster_name!r}'
               f'{colorama.Style.RESET_ALL}')
    click.echo(table)


@grants.command('mine', cls=_DocumentedCodeCommand)
@flags.config_option(expose_value=False)
@usage_lib.entrypoint
def grants_mine():
    """List clusters that you have been granted access to.

    Shows all clusters that other users have granted you access to.
    ...
    .. code-block:: bash
      ...
      sky grants mine
    """
    request_id = sdk.get_my_granted_clusters()
    clusters = sdk.get(request_id)

    if not clusters:
        click.echo('No clusters have been granted to you')
        return

    # Create a pretty table for displaying granted clusters
    import prettytable

    table = prettytable.PrettyTable()
    table.field_names = ['CLUSTER', 'GRANTED BY', 'GRANTED AT', 'EXPIRES AT']
    table.align = 'l'

    for cluster in clusters:
        expires_at = cluster.get('expires_at', 'Never')
        table.add_row([
            cluster['cluster_name'], cluster['granted_by'],
            cluster['granted_at'],
            expires_at if expires_at == 'Never' else expires_at
        ])

    click.echo(f'\n{colorama.Fore.CYAN}{colorama.Style.BRIGHT}'
               f'Clusters granted to you'
               f'{colorama.Style.RESET_ALL}')
    click.echo(table)
