...
def get_my_granted_clusters() -> List[Dict[str, Any]]:
    """Gets all clusters that the current user has been granted access to.
    
    Returns:
        List of grant dictionaries with cluster information.
    """
    current_user = common_utils.get_current_user()
    logger.info(f'current_user: {current_user}')
    print(f'current_user: {current_user}')

    # Get the list of cluster names the user has access to
    cluster_names = global_user_state.get_user_granted_clusters(current_user.id)

    # For each cluster, get the grant details
    grants = []
    for cluster_name in cluster_names:
        # Get all grants for this cluster
        all_grants = global_user_state.get_cluster_grants(
            cluster_name=cluster_name,
        ...
        for grant in all_grants:
            if grant['grantee_user_hash'] == current_user.id:
                # Get user names for display
                grantor = global_user_state.get_user(grant['grantor_user_hash'])

                # Format the grant information for the CLI
                grant_info = {
                    'cluster_name': cluster_name,
                    'granted_by': grantor.name if grantor else 'Unknown',
                ...
                if grant_info['expires_at'] is not None:
                    grant_info['expires_at'] = time.strftime(
                        '%Y-%m-%d %H:%M:%S',
                        time.localtime(grant_info['expires_at']))
                else:
                    grant_info['expires_at'] = 'Never'

                grants.append(grant_info)
                break  # Found the grant for this user, move to next cluster

    return grants


def grant_cluster_access(cluster_name: str,
                         grantee_user_name: str,
                         expires_in_hours: Optional[float] = None,
                         notes: Optional[str] = None) -> None:
    """Grants a user access to a cluster.
    
    Args:
        cluster_name: The name of the cluster to grant access to.
        grantee_user_name: The name of the user to grant access to.
        expires_in_hours: Optional expiration time in hours. If None, access
            never expires.
        notes: Optional notes about the grant.
    """
    current_user = common_utils.get_current_user()
    grantor_user_hash = current_user.id

    # Get grantee user hash from name (should be unique on OAuth2 as it is set to the email)
    grantee_users = global_user_state.get_user_by_name(grantee_user_name)
    if len(grantee_users) == 0:
        with ux_utils.print_exception_no_traceback():
            raise ValueError(f'User {grantee_user_name!r} not found')
    grantee_user_hash = grantee_users[0].id
    ...
    global_user_state.add_cluster_grant(cluster_name=cluster_name,
...
def revoke_cluster_access(grant_id: str) -> None:
    """Revokes a user's access to a cluster.
    
    Args:
        grant_id: The ID of the grant to revoke.
    """
    current_user = common_utils.get_current_user()
    revoker_user_hash = current_user.id

    global_user_state.revoke_cluster_grant(grant_id=grant_id,
                                           revoker_user_hash=revoker_user_hash)


def get_cluster_grants(cluster_name: str,
                       include_expired: bool = False,
                       include_revoked: bool = False) -> List[Dict[str, Any]]:
    """Gets all grants for a cluster.
    
    Args:
        cluster_name: The name of the cluster to get grants for.
        include_expired: Whether to include expired grants.
        include_revoked: Whether to include revoked grants.
        
    Returns:
        List of grant dictionaries with user information.
    """
    current_user = common_utils.get_current_user()
    ...
    if cluster['user_hash'] != current_user.id:
        raise ValueError('Only the cluster owner can view grants')

    grants = global_user_state.get_cluster_grants(
        cluster_name=cluster_name,
        include_expired=include_expired,
        include_revoked=include_revoked)

    # Add user names to the grants for better display
    for grant in grants:
        grantor = global_user_state.get_user(grant['grantor_user_hash'])
        grantee = global_user_state.get_user(grant['grantee_user_hash'])
        grant['grantor_user_name'] = grantor.name if grantor else 'Unknown'
        grant['grantee_user_name'] = grantee.name if grantee else 'Unknown'
