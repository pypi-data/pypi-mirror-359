...
class BasicAuthMiddleware(starlette.middleware.base.BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: fastapi.Request, call_next):
        ...
        # Allow Bearer tokens to pass through to BearerTokenMiddleware
...
@app.post('/grant_cluster_access')
async def grant_cluster_access(
        request: fastapi.Request,
        grant_cluster_access_body: payloads.GrantClusterAccessBody) -> None:
    """Grant access to a cluster to another user."""
    executor.schedule_request(
        request_id=request.state.request_id,
        request_name='grant_cluster_access',
        request_body=grant_cluster_access_body,
        func=core.grant_cluster_access,
        schedule_type=requests_lib.ScheduleType.SHORT,
    )


@app.post('/revoke_cluster_access')
async def revoke_cluster_access(
        request: fastapi.Request,
        revoke_cluster_access_body: payloads.RevokeClusterAccessBody) -> None:
    """Revoke a cluster access grant."""
    executor.schedule_request(
        request_id=request.state.request_id,
        request_name='revoke_cluster_access',
        request_body=revoke_cluster_access_body,
        func=core.revoke_cluster_access,
        schedule_type=requests_lib.ScheduleType.SHORT,
    )


@app.get('/cluster_grants/{cluster_name}')
async def get_cluster_grants(request: fastapi.Request,
                             cluster_name: str,
                             include_expired: bool = False,
                             include_revoked: bool = False) -> None:
    """Get all grants for a cluster."""
    ...
    # Create env_vars with the authenticated user's information
    env_vars = payloads.request_body_env_vars().copy()
    env_vars[constants.USER_ID_ENV_VAR] = auth_user.id
    env_vars[constants.USER_ENV_VAR] = auth_user.name

    # Create a request body with the parameters and authenticated user info
    cluster_grants_body = payloads.ClusterGrantsBody(
        cluster_name=cluster_name,
    ...
    executor.schedule_request(
        request_id=request.state.request_id,
        request_name='get_cluster_grants',
        request_body=cluster_grants_body,
        func=core.get_cluster_grants,
        schedule_type=requests_lib.ScheduleType.SHORT,
    )


@app.get('/my_granted_clusters')
async def get_my_granted_clusters(request: fastapi.Request) -> None:
    """Get all clusters that the current user has been granted access to."""
    # Create request body with authenticated user's information
    auth_user = request.state.auth_user
    if auth_user is None:
        raise fastapi.HTTPException(status_code=401,
                                    detail='Authentication required')
    ...
    # Create request body with the updated env_vars
    request_body = payloads.RequestBody(env_vars=env_vars)

    executor.schedule_request(
        request_id=request.state.request_id,
        request_name='get_my_granted_clusters',
        request_body=request_body,
        func=core.get_my_granted_clusters,
        schedule_type=requests_lib.ScheduleType.SHORT,
    )
...
async def kubernetes_pod_ssh_proxy(websocket: fastapi.WebSocket,
    ...
    # Try to get user from OAuth2 proxy headers first
    parsed_userId = None
    if 'X-Auth-Request-Email' in websocket.headers:
        user_name = websocket.headers['X-Auth-Request-Email']
        user_hash = hashlib.md5(
            user_name.encode()).hexdigest()[:common_utils.USER_HASH_LENGTH]
        auth_user = models.User(id=user_hash, name=user_name)
        parsed_userId = auth_user.id
        logger.info(
            f'OAuth2 proxy authenticated user: {auth_user.name} (ID: {auth_user.id})'
        )
    else:
            ...
            if token.startswith('sky_'):
                try:
                    ...
                    if payload:
                        ...
                        user_id = payload.get('sub')
                        if user_id:
                            # Create User object and set in global context
                            parsed_userId = user_id
                            logger.info(f'JWT authenticated user: {user_id}')
                        else:
                            logger.warning('JWT token missing user ID (sub)')
                    else:
                        logger.warning('JWT token verification failed')
                except Exception as e:
                    logger.warning(f'JWT token decoding failed: {e}')

    # Check authentication and handle errors gracefully
    # Determine which pod to connect to based on cluster_name parameter
    # If it's a worker node name (e.g., cluster-worker1), extract the worker index
    worker_index = 0
    cluster_name_for_status = cluster_name
    if '-worker' in cluster_name:
        try:
            # Extract the base cluster name and worker index
            base_name, worker_suffix = cluster_name.rsplit('-worker', 1)
            worker_index = int(worker_suffix)
            # Get the actual cluster name for status lookup
            cluster_name_for_status = base_name
        except (ValueError, IndexError):
            # If parsing fails, treat as head node
            pass

    try:
        backend_utils.check_owner_identity(cluster_name_for_status,
                                           override_cur_user_hash=parsed_userId)
    except exceptions.ClusterOwnerIdentityMismatchError as e:
        clean_message = common_utils.remove_color(str(e))
        await websocket.send_text(f"{clean_message}")
        await websocket.close(code=1008, reason="Authentication failed")
        return
    except Exception as e:
        await websocket.send_text(f"ERROR: {common_utils.format_exception(e)}")
        await websocket.close(code=1011, reason="Internal server error")
        return

    try:
        cluster_records = core.status(cluster_name_for_status, all_users=True)
        if not cluster_records:
            await websocket.send_text(
                f"ERROR: Cluster {cluster_name_for_status!r} not found.")
            await websocket.close(code=1008, reason="Cluster not found")
            return

        cluster_record = cluster_records[0]
        if cluster_record['status'] != status_lib.ClusterStatus.UP:
            await websocket.send_text(
                f"ERROR: Cluster {cluster_name_for_status!r} is not running (status: {cluster_record['status'].value})."
            )
            await websocket.close(code=1008, reason="Cluster not running")
            return
        ...
        if not isinstance(handle.launched_resources.cloud, clouds.Kubernetes):
            await websocket.send_text(
                f"ERROR: Cluster {cluster_name_for_status!r} is not a Kubernetes cluster. Use ssh to connect to the cluster instead."
            )
            await websocket.close(code=1008, reason="Not a Kubernetes cluster")
            return

        # Get the command runner for the specific node
        runners = handle.get_command_runners()
        if worker_index >= len(runners):
            await websocket.send_text(
                f"ERROR: Worker node index {worker_index} out of range. "
                f"Cluster has {len(runners)} nodes.")
            await websocket.close(code=1008, reason="Invalid worker index")
            return

        runner = runners[worker_index]
        kubectl_cmd = runner.port_forward_command(port_forward=[(None, 22)])
        proc = await asyncio.create_subprocess_exec(
            *kubectl_cmd,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.STDOUT)
        logger.info(f'Started kubectl port-forward for node {worker_index} '
                    f'with command: {kubectl_cmd}')
        ...
        while True:
            ...
            if stdout_line:
                ...
                await websocket.send_text(
                    "ERROR: kubectl port-forward failed to start")
                await websocket.close(code=1011, reason="Port forward failed")
                return
        ...
        logger.error(
            f'Error in kubernetes_pod_ssh_proxy: {common_utils.format_exception(e)}'
        )
        try:
