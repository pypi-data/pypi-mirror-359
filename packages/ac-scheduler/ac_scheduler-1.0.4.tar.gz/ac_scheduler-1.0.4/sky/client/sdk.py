...
@server_common.check_server_healthy_or_start
@annotations.client_api
def grant_cluster_access(
        cluster_name: str,
        grantee_user_name: str,
        expires_in_hours: Optional[float] = None,
        notes: Optional[str] = None) -> server_common.RequestId:
    """Grants a user access to a cluster.

    Args:
        cluster_name: The name of the cluster to grant access to.
        grantee_user_name: The name/email of the user to grant access to.
        expires_in_hours: Optional expiration time in hours. If None, access
            never expires.
        notes: Optional notes about the grant.

    Returns:
        The request ID of the grant cluster access request.

    Request Returns:
        None

    Request Raises:
        ValueError: If the cluster does not exist or the user is not authorized.
    """
    body = payloads.GrantClusterAccessBody(
        cluster_name=cluster_name,
        grantee_user_name=grantee_user_name,
...
@server_common.check_server_healthy_or_start
@annotations.client_api
def revoke_cluster_access(grant_id: str) -> server_common.RequestId:
    ...
    Returns:
        The request ID of the revoke cluster access request.

    Request Returns:
        None

    Request Raises:
        ValueError: If the grant does not exist or the user is not authorized.
    """
    body = payloads.RevokeClusterAccessBody(grant_id=grant_id,)
    response = server_common.make_authenticated_request(
        'POST',
        '/revoke_cluster_access',
        json=json.loads(body.model_dump_json()))
    return server_common.get_request_id(response)
...
@server_common.check_server_healthy_or_start
@annotations.client_api
def get_cluster_grants(cluster_name: str) -> server_common.RequestId:
    ...
    Returns:
        The request ID of the get cluster grants request.

    Request Returns:
        grants (List[Dict[str, Any]]): A list of dicts, with each dict
            containing the grant information.

            .. code-block:: python

                {
                    'user_email': (str) email of granted user,
                    'granted_by': (str) email of user who granted access,
                    'granted_at': (str) ISO timestamp of when access was granted,
                }
    ...
    """
    response = server_common.make_authenticated_request(
        'GET', f'/cluster_grants/{cluster_name}')
    return server_common.get_request_id(response)
...
@server_common.check_server_healthy_or_start
@annotations.client_api
def get_my_granted_clusters() -> server_common.RequestId:
    """Gets all clusters that the current user has been granted access to.

    Returns:
        The request ID of the get granted clusters request.

    Request Returns:
        clusters (List[Dict[str, Any]]): A list of dicts, with each dict
...
def api_login(endpoint: Optional[str] = None,
    ...
    def _show_logged_in_message(
        ...
        click.secho(
            f'Logged into Applied Compute Scheduler at: {endpoint}'
            f'{identity_info}'
