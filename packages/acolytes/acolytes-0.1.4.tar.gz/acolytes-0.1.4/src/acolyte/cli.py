...
def init(path: str, name: Optional[str], force: bool):
    ...
    success = initializer.run(project_name=name, force=force)
...
def start(path: str):
    ...
    # Lazy import for health checker
    ...
    try:
        ...
        if result.returncode != 0:
            click.echo(click.style(f"✗ Failed to start services: {result.stderr}", fg="red"))
            sys.exit(1)
        ...
        if not health_checker.wait_for_weaviate():
            click.echo(click.style("✗ Weaviate failed to start", fg="red"))
            sys.exit(1)
        ...
        click.echo(click.style("✓ All services are ready!", fg="green"))
