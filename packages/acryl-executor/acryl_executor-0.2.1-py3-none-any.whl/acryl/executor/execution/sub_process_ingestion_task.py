...
class SubProcessIngestionTask(Task):
    ...
    def _setup_directories(self, exec_id: str) -> tuple[str, str, str]:
        ...
        return exec_out_dir, artifact_output_dir, f"{artifact_output_dir}/artifacts/ingestion_report.json"

    def _prepare_subprocess_environment(self, validated_args: SubProcessIngestionTaskArgs,
                                        exec_out_dir: str, artifact_output_dir: str) -> dict:
    ...
    async def _create_subprocess(self, validated_args: SubProcessIngestionTaskArgs,
                                 plugin: str, recipe_file_path: str,
                                 report_out_file: str, subprocess_env: dict) -> asyncio.subprocess.Process:
    ...
    async def execute(self, args: dict, ctx: ExecutionContext) -> None:
        ...
        exec_out_dir, artifact_output_dir, report_out_file = self._setup_directories(exec_id)
        recipe_file_path = SubProcessTaskUtil._write_recipe_to_file(
            exec_out_dir, recipe
        )

        # 3. Prepare subprocess environment and create subprocess
        subprocess_env = self._prepare_subprocess_environment(validated_args, exec_out_dir, artifact_output_dir)
        ...
        try:
            await self._monitor_subprocess(ingest_process, exec_id, ctx, stdout_lines, full_log_file)
        finally:
            self._handle_subprocess_completion(
                ingest_process, ctx, report_out_file, artifact_output_dir,
                recipe, exec_out_dir, stdout_lines
            )

    async def _monitor_subprocess(self, ingest_process: asyncio.subprocess.Process,
                                  exec_id: str, ctx: ExecutionContext,
                                  stdout_lines: deque[str], full_log_file: IO[str]) -> None:
    ...
    def _handle_subprocess_completion(self, ingest_process: asyncio.subprocess.Process,
                                       ctx: ExecutionContext, report_out_file: str,
                                       artifact_output_dir: str, recipe: dict,
                                       exec_out_dir: str, stdout_lines: deque[str]) -> None:
