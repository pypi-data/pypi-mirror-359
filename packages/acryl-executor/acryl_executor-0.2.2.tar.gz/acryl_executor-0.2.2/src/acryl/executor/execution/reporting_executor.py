...
class ReportingExecutor(DefaultExecutor):
    ...
    def _build_completion_mcp(
        ...
        end_time_ms = exec_result.end_time_ms if exec_result.end_time_ms else int(time.time()*1000)
    ...
    def _build_execution_request_result_aspect(
        ...
        if structured_report is not None and report is not None and ((len(structured_report) + len(report)) > max_length):
