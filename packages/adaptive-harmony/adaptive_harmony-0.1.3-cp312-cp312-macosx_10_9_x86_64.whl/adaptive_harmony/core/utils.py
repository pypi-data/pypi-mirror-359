...
async def wrap_coroutine_with_progress(coroutine, progress_tracker):
    """
...
# async def async_map_batch[S, T](
#    f: Callable[[S], Awaitable[T]], data: Iterator[S], batch_size: int, num_repetitions=1
# ) -> list[T]:
#
#    tasks: list[Awaitable[T]] = [
#        f(item) for _ in range(num_repetitions) for item in itertools.islice(data, batch_size // num_repetitions)
#    ]
#    batch_results: list[T] = await gather_with_auto_tqdm(*tasks, leave=False, desc="Processing Batch")
#
#    return batch_results


async def async_map[S, T](f: Callable[[S], Awaitable[T]], data: Iterable[S]) -> list[T]:
    """
    ...
    """
    tasks: list[Awaitable[T]] = [f(item) for item in data]
    batch_results: list[T] = await asyncio.gather(*tasks)
