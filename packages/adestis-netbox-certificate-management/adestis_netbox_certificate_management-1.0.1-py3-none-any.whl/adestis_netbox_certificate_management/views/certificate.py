...
import re
from adestis_netbox_certificate_management.forms.certificate import *
from adestis_netbox_certificate_management.models.certificate import Certificate, ClusterGroupAssignment, ClusterAssignment, DeviceAssignment, VirtualMachineAssignment, ContactAssignment, InstalledApplication
...
from adestis_netbox_certificate_management.api.serializers.certificate_serializer import CertificateSerializer
...
class CertificateEditView(generic.ObjectEditView):
    ...
    def alter_object(self, obj, request, url_args, url_kwargs):
        ...
        logger = logging.getLogger('CertificateEditView')
        certificate_data = obj.certificate
        
        parent_cert = None
        
        if certificate_data:    
            data = cert_utils.parse_cert(certificate_data)
            logger.error(data)            
            obj.valid_from = data["startdate"].date()
            obj.valid_to = data["enddate"].date()           
            obj.issuer = data["issuer"].replace("\n",";").strip()#datenbereinigen damit der parent certificate den issuer findet. 
            obj.subject = data["subject"].replace("\n",";").strip()# ; damit bei dokumentierung die Daten leichter zu lesen sind 
            obj.key_technology = data["key_technology"]
            obj.subject_alternative_name = data["SubjectAlternativeName"]
            logger.warning(f"=> Issuer: {obj.issuer}")
            logger.warning(f"=> Subject: {obj.subject}")
            logger.warning(f"=> Key Technology: {obj.key_technology}")
            logger.warning(f"=> Subject Alternative Names : {obj.subject_alternative_name}")
            logger.warning(certificate_data)
            
            
            parent_cert = Certificate.objects.filter(subject=obj.issuer).first()
            if parent_cert:
                obj.issuer_parent_certificate = parent_cert
            else:
                obj.issuer_parent_certificate = None           
        return obj
...
class CertificateAffectedInstalledApplicationView(generic.ObjectChildrenView):
    ...
    def get_children(self, request, parent):
        return InstalledApplication.objects.restrict(request.user, 'view').filter(application_certificate=parent)
...
@register_model_view(InstalledApplication, name='certificates')
