...
from .optimize_client import optimize_service_start, optimize_service_get_progress, optimize_service_get_report
...
class OptimizeJob(BaseModel):
    ...
    def create_optimize_job(
        cls, task_id: str, base_version: str, api_key: Optional[str] = None, api_url: Optional[str] = None
    ) -> "OptimizeJob":
        """
    ...
    def get_report(self, ref_version: Optional[str] = None) -> OptimizeReport:
        ...
        try:
            ...
            elif self.optimized_version is None:
...
def create(
    task_id: str, base_version: str, api_key: Optional[str] = None, api_url: Optional[str] = None
) -> OptimizeJob:
    """
    Starts an optimization job for a given task and version.
    ...
    """
    optimize_job = OptimizeJob.create_optimize_job(task_id, base_version, api_key, api_url)
    return optimize_job


def resume(
    task_id: str, base_version: str, job_id: str, api_key: Optional[str] = None, api_url: Optional[str] = None
) -> OptimizeJob:
    """
    ...
    optimize_job = OptimizeJob(
        task_id=task_id, base_version=base_version, job_id=job_id, api_key=api_key, api_url=api_url
    )
    return optimize_job
