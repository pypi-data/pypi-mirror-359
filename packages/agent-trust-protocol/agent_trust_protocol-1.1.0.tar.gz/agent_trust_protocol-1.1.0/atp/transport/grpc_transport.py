...
class GRPCTransport(BaseTransport):
    """
    Simulated gRPC Transport for ATP. Stores agent handlers and simulates message delivery.
    TODO: Integrate with a real gRPC server for production use.
    """
    ...
    def send_message(self, from_agent: str, to_agent: str, envelope: ATPEnvelope) -> None:
        """Simulate sending a message to a specific agent via gRPC."""
        self.message_history.append((from_agent, to_agent, envelope))
        print(f"[gRPC] Message sent from {from_agent} to {to_agent} via gRPC endpoint {self.endpoint}")
        handler = self.connected_agents.get(to_agent)
        if handler:
            try:
                handler(envelope)
            except Exception as e:
                print(f"[gRPC] Error delivering message to {to_agent}: {e}")
        else:
            print(f"[gRPC] No handler registered for agent {to_agent}")
