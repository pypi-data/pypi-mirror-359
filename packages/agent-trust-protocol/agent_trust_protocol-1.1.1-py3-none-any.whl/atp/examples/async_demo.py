...
# Shared in-memory transport for all agents in the demo
...
async def create_agent(agent_id: str, trust_level: TrustLevel, private_key: str) -> ATPClient:
    ...
    client.transport = shared_transport
...
async def run_comprehensive_demo():
    ...
    for i, msg in enumerate(history[-5:], 1):  # Show last 5 messages
        # msg is likely a tuple: (from_agent, to_agent, envelope)
        if isinstance(msg, tuple) and len(msg) == 3:
            from_agent, to_agent, envelope = msg
            payload_type = envelope.payload.get('type', envelope.payload.get('messageType', 'unknown'))
            print(f"   {i}. {from_agent} -> {to_agent}: {payload_type}")
        else:
            print(f"   {i}. {msg}")
