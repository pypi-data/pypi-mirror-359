...
class WebSocketTransport(BaseTransport):
    """
    Simulated WebSocket Transport for ATP. Stores agent handlers and simulates message delivery.
    TODO: Integrate with a real WebSocket server for production use.
    """
    ...
    def send_message(self, from_agent: str, to_agent: str, envelope: ATPEnvelope) -> None:
        """Simulate sending a message to a specific agent via WebSocket."""
        self.message_history.append((from_agent, to_agent, envelope))
        print(f"[WebSocket] Message sent from {from_agent} to {to_agent} via WebSocket endpoint {self.endpoint}")
        handler = self.connected_agents.get(to_agent)
        if handler:
            try:
                handler(envelope)
            except Exception as e:
                print(f"[WebSocket] Error delivering message to {to_agent}: {e}")
        else:
            print(f"[WebSocket] No handler registered for agent {to_agent}")
