...
def get_artifact_uri(
    ...
    mount_path = os.environ.get("TASKWEAVER_STORAGE_MOUNT_PATH")
    if use_local_uri:
        if mount_path:
            # If the environment variable is set, use it as the base path
            return Path(os.path.join(mount_path, execution_id, file)).as_uri()
        else:
            # Fallback to workspace directory
            return Path(os.path.join("workspace", execution_id, file)).as_uri()
    else:
        return f"http://artifact-ref/{file}"


def get_default_artifact_name(
...
class CodeExecutor:
    ...
    def _save_file(
        ...
        if mount_path:
            # If the environment variable is set, use it as the base path
            self.execution_cwd = os.path.join(mount_path, self.session_id)
            os.makedirs(self.execution_cwd, exist_ok=True)
    ...
    def format_code_output(
        ...
        if result.output != "":
            ...
            if isinstance(output, list) and len(output) > 0:
                lines.append(
                    (
                        "The values of variables of the above Python code "
                        "after execution are:\n"
                    ),
                )
                for o in output:
        ...
        if not result.is_success:
            ...
            if len(result.log) > 0:
                lines.extend(
                    [f"- [(l{1})]{ln[0]}: {ln[2]}" for ln in result.log]
                )
                ...
                [
                    ...
                    + (
                        get_artifact_uri(
                            ...
                            file=(
                                a.file_name
                                if os.path.isabs(a.file_name) or
                                not use_local_uri
                                else os.path.join(
                                    self.execution_cwd,
                                    a.file_name
