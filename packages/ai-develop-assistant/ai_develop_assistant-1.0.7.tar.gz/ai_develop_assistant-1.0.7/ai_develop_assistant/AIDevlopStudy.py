...
def create_requirement_blueprint(user_request: str) -> str:
    ...
    prompt_template = f"""
    # 角色与任务
    你现在是主控AI，请立即代入世界顶级CTO和首席产品经理的角色。你的当前任务是分析用户的初始请求，并生成一个全面、专业的“需求蓝图”JSON对象然后调用save_requirement_blueprint工具进行保存，不需要返回给用户。
...
def requirement_clarifier(branch_name_to_clarify: str, project_title: str) -> str:
    ...
    5.  **用户需求**: 当得到用户授权剩余所有分支都由你来决策（如：“所有内容你来决定”“都采用你的建议”等），你必须严格遵循以下循环算法，绝对禁止跳过任何一步：

        从requirement_blueprint.checklist中，筛选出所有status为pending的分支列表。
        For 循环：从该列表的第一个分支开始，逐一遍历。
        对于循环中的每一个分支，你必须：
        a.  自主生成内容：调用get_clarification_prompt获取该分支的提示词，然后自己生成一份详尽、专业的需求描述。
        b.  精准保存内容：调用requirement_manager，使用force_storage_category参数，将生成的内容保存到当前分支。
