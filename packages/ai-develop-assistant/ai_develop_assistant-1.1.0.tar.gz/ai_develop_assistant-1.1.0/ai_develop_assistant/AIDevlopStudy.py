...
def requirement_clarifier(branch_name_to_clarify: str, project_title: str) -> str:
    ...
    2.  **提供专业建议**: 针对你提出的每一个子问题，都输出一个简洁、专业、符合行业最佳实践的建议答案(`ai_suggestion`)。
    3.  **严格的保存格式**: 你必须且只能保存一个格式完全正确的JSON对象，绝对不能包含任何解释性文字或代码块标记以及未经过用户确认认可的建议。
    4.  **经过用户澄清后的调用要求**:在不确定用户授权范围前永远默认只能更新或者保存经过用户确认的单分支或者当前分支的某些问题。
    4.  **行为准则: “你必须严格遵守最小权限原则。当用户授予你自主决策权时（如‘你决定’、‘常规方案’等），该授权仅限于当前正在讨论的、最具体的一个分支的一个或多个问题。你绝对禁止将此授权泛化到任何其他未开始分支或正在进行中的分支中的其他问题上，除非得到用户的明确指令。必要时候询问用户用户授权的是当前分支还是分支内的此问题还是所有分支”
        当你收到用户明确授权剩余所有分支都由你来决策（如：“所有内容你来决定”“都采用你的建议”等），你必须严格遵循以下循环算法，绝对禁止跳过任何一步：
