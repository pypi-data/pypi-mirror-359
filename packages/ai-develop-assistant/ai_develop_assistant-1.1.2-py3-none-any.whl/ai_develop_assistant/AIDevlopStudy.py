...
def requirement_clarifier(branch_name_to_clarify: str, project_title: str) -> str:
    ...
    4.  **提供专业建议**: 针对你提出的每一个子问题，都返回给用户一个简洁、专业、符合行业最佳实践的建议答案用来引导用户。
    5.  **行为准则: “你必须严格遵守最小权限原则。当用户授予你自主决策权时（如‘你决定’、‘常规方案’等），该授权仅限于当前正在讨论的、最具体的一个分支的一个或多个问题。你绝对禁止将此授权泛化到任何其他未开始分支或正在进行中的分支中的其他未确定问题上，除非得到用户的明确指令。收得到类似指令时必须询问用户用户授权的是当前分支还是分支内的此问题还是所有分支”
...
def get_architecture_design_prompt() -> str:
    ...
    storage_dir = get_storage_dir() # 确保获取正确的目录
    ...
    # 你的输出
    现在，请直接开始撰写这份Markdown文档，保存在{md_file_path}中，不需要输出给用户。不要添加任何额外的解释性文字，符合markdown语法，清晰展示，避免文字堆叠，格式混乱使用户无法阅读。
    """
    return prompt_template
