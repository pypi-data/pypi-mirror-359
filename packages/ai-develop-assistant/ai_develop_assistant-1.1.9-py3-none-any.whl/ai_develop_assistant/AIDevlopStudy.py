...
def get_architecture_design_prompt() -> str:
    ...
    # 你的输出
    现在，请直接开始撰写这份Markdown文档，不需要输出给用户，保存在{current_requirements["design_requirements"]}(current_requirements的design_requirements)中，不要添加任何额外的解释性文字，符合markdown语法，清晰展示，避免文字堆叠，下一步调用架构文档导出工具导出保存的内容，避免格式混乱使用户无法阅读。
    """
    return prompt_template
