...
def test_get_all_links(seeded_links: list[Link], client: Albert, seeded_tasks: list[BaseTask]):
    for l in client.links.get_all():
...
def test_adv_get_all(seeded_links: list[Link], client: Albert, seeded_tasks: list[BaseTask]):
    for t in seeded_tasks[1:]:
        for l in client.links.get_all(id=t.id, type="all"):
