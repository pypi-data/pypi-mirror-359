...
class IBKRImporter(importer.ImporterProtocol):
    ...
    def getFeesAccount(self, currency):
        if self.FeesAccount:
            ...
            return ":".join(
                [
                    self.Mainaccount.replace("Assets", "Expenses"),
                    self.FeesSuffix,
                    currency,
                ]
            )
    ...
    def CashTransactions(self, ct):
        ...
        if len(div) != len(wht):
            ...
            # matching WHT & div
            match = pd.merge(div, wht, on=["symbol", "reportDate", "__divtype__"])
        ...
        for idx, row in match.iterrows():
            ...
            if with_wht:
                ...
                if currency != currency_wht:
                    warnings.warn(
                        (
                            "Warning: Dividend currency {} "
                            + "mismatches WHT currency {}. Skipping this"
                            + "Transaction"
                        ).format(currency, currency_wht)
                    )
                    continue
            ...
            regex = "|".join([r"\(([a-zA-Z]{2}[a-zA-Z0-9]{9}\d)\)", self.roc_str])
            ...
            in_lieu_flag = (
                " in lieu"
                if re.match(".*payment in lieu of dividend", dx, re.IGNORECASE)
                else ""
            )
    ...
    def Stocktrades(self, stocks):
        ...
        sale = stocktrades[
            (stocktrades["buySell"] == BuySell.SELL)  # sales, including cancelled ones
