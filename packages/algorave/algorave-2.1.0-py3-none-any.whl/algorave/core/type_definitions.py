"""Module containing type definitions and constants used throughout Algorave.

This module defines common types, constants, and enumerations that are used across the
Algorave library. It includes type aliases for numeric types, enumerations for
...
class ReferenceImage(TypedDict):
    ...
    Algorave, including optional components like masks, bounding boxes,
    and keypoints.
...
class Targets(Enum):
    """Enumeration of supported target types in Algorave.

    This enum defines the different types of data that can be augmented
    by Algorave transforms, including both 2D and 3D targets.
...
NUM_KEYPOINTS_COLUMNS_IN_ALGORAVE = 5
NUM_BBOXES_COLUMNS_IN_ALGORAVE = 4
