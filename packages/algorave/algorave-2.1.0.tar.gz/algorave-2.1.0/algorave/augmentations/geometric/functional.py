...
from algorave.augmentations.utils import angle_2pi_range, handle_empty_array
...
from algorave.core.type_definitions import (
    NUM_BBOXES_COLUMNS_IN_ALGORAVE,
    NUM_KEYPOINTS_COLUMNS_IN_ALGORAVE,
...
def keypoints_scale(
    ...
    if keypoints.shape[1] > NUM_KEYPOINTS_COLUMNS_IN_ALGORAVE:
        return np.column_stack(
            [scaled_keypoints, keypoints[:, NUM_KEYPOINTS_COLUMNS_IN_ALGORAVE:]],
        )
    ...
    if keypoints.shape[1] > NUM_KEYPOINTS_COLUMNS_IN_ALGORAVE:
        return np.column_stack(
            [
                ...
                keypoints[:, NUM_KEYPOINTS_COLUMNS_IN_ALGORAVE:],
            ],
        )
    ...
    for idx, mask in enumerate(masks):
        ...
        for comp_idx in range(1, num_components):  # Skip background (0)
            ...
            if is_valid_component(
                ...
                if bboxes.shape[1] > NUM_BBOXES_COLUMNS_IN_ALGORAVE:
