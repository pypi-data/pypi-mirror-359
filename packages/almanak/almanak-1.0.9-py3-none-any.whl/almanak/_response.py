    ...
    def _parse(
        ...
        *,
        ...
        if inspect.isclass(origin) and not issubclass(origin, BaseModel) and issubclass(origin, pydantic.BaseModel):
            raise TypeError("Pydantic models must subclass our base model type, e.g. `from almanak import BaseModel`")

        if cast_to is not object and origin is not list and origin is not dict and origin is not Union and not issubclass(origin, BaseModel):
            raise RuntimeError(f"Unsupported type, expected {cast_to} to be a subclass of {BaseModel}, {dict}, {list}, {Union}, {NoneType}, {str} or {httpx.Response}.")
...
class AsyncAPIResponse(BaseAPIResponse[R]):
    ...
    async def parse(self, *, to: type[_T] | None = None, unpack_by_keys: str | list[str] = "data") -> R | _T:
        """Returns the rich python representation of this response's data.
