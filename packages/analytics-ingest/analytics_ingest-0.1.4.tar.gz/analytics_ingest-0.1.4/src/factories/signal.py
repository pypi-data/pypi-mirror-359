...
def signal_factory():
    ...
    return [{
        ...
        "data": [
            {
                "value": fake.pyfloat(min_value=1000, max_value=10000000, right_digits=1),
                "time": (fake.date_time_between(start_date="-1d", end_date="now") + timedelta(seconds=i)).strftime("%Y-%m-%dT%H:%M:%SZ")
            } for i in range(120)
        ]
    }]