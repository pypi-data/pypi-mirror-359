...
class TestAnalyticsIngestClientInit(unittest.TestCase):
    ...
    def test_missing_jwt_token_raises_when_not_in_env(self):
        config_data = init_schema_factory(jwt_token=None)
        if "SEC_AUTH_TOKEN" in os.environ:
            del os.environ["SEC_AUTH_TOKEN"]

        with self.assertRaises(ValueError) as ctx:
            AnalyticsIngestClient(**config_data)

        self.assertIn("jwt_token", str(ctx.exception))
