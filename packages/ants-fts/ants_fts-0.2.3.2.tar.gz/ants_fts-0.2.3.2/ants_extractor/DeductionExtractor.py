...
class DeductionExtractor(BaseExtractor):
    ...
    def extract(df, grouped_columns, discount_amount_column, revenue_column):
        ...
        min_pos = _df[_df['discount_rate'] > 0].groupby(grouped_columns)['discount_rate'].min().reset_index()
        min_pos.rename(columns={'discount_rate': '_min_with_condition'}, inplace=True)
        #print(_df)
        deduction_fts = _df.groupby(by=grouped_columns)\
                            ...
                            .reset_index();
        deduction_fts = deduction_fts.merge(min_pos, on=grouped_columns, how='left')
        ...
        #Return
        deduction_fts = deduction_fts[grouped_columns + ["_deduction_rate", "_deduction_ratio", "_avg_discount", "_var_discount", "_min_discount", "_max_discount", "_discount_cnt", "_min_with_condition"]]
