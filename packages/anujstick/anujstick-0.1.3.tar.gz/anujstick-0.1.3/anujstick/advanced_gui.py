...
def execute_attack(attack, target, interface, log_widget):
    ...
    if attack == "brute_force":
        ...
        cmd = ["python", "-m", "anujstick.main", "--attack", attack, "--target", target, "--interface", interface]
...
")
    ...
    for line in process.stdout:
        if stop_flag.is_set():
            process.terminate()
            break
        log_widget.insert(tk.END, f"[{timestamp()}] {line}")
...
def main():
    ...
    create_attack_tab("DoS / DDoS", ["Target Host:"], "dos", has_interface=False)
