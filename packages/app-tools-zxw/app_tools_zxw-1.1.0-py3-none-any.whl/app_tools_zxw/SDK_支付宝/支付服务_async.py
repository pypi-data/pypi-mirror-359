...
class 支付服务:
    ...
    # _回调路径 = "/callback/"
    app_notify_url = "http://0.0.0.0:8000/alipay_callback"

    def __init__(self, app_id: str,
                 key应用私钥: Union[str, Path],
                 key支付宝公钥: Union[str, Path],
                 回调路径_root: str = "http://0.0.0.0:8000",
                 回调路径_prefix: str = "/alipay_callback"):
        """
        :param app_id:
        :param key应用私钥: 路径 或 密匙
        :param key支付宝公钥: 路径 或 密匙
        :param 回调路径_root: 如果需要回调处理的话，此处必填
        :param 回调路径_prefix: 回调路径
        """
        self.app_notify_url = 回调路径_root + 回调路径_prefix
    ...
    async def 查询订单(self, 商户订单号: str) -> OrderStatus:
        ...
        logger.info("[查询订单] .api_alipay_trade_query =", response)
        if response.get("code") == "10000":
    ...
    async def 系统回调api_预处理(self, postBody: Request) -> PaymentResult:
        """
            支付回调处理
        ...
        try:
            # 使用支付宝公钥和签名类型进行验证
            success = self.alipay_client.verify(dataDict, signature)
        except Exception as e:
            raise HTTPException_AppToolsSZXW(
    ...
    def 注册回调接口_示例(
            self, app: APIRouter,
            async_func_支付成功: Callable[[PaymentResult], Awaitable[None]]
    ):
        """
        注册回调接口
        ...
        @app.get(self.app_notify_url)
        async def 回调_验证地址(request):
            print("支付回调get请求：", request)
            return "ok"

        @app.post(self.app_notify_url)
        async def 回调_支付完成处理(postBody):
            """
            支付回调处理
            :param postBody: 请求体,类型：Request
            """
            result = await self.系统回调api_预处理(postBody)
