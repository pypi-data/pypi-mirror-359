...
def get_3Dgraph(G, atoms, rings, ring_indices, title="Molecular Graph 3D"):
    ...
    # Texto en centroides de los anillos
    ...
    for i, ring in enumerate(rings, start=1):
        ...
        if value is not None:
            centroid_trace.x.append(x_c)
            centroid_trace.y.append(y_c)
            centroid_trace.z.append(z_c)
            centroid_trace.text.append(f"{value:.3f}")
