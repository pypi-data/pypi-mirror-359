...
from .exceptions import (
    DataValidationError, ModelingError,
...
class AutoWaterQualityModeler:
    ...
    def __init__(self, 
        ...
        # 创建建模策略
        self.auto_strategy = AutoModelingStrategy(self.feature_manager, self.model_builder)
    ...
    def fit(self, 
            ...
            origin_merged_data: pd.DataFrame | None = None) -> ModelingResult:
        """
        一键式建模流程
    ...
    def _prepare_metric_data(self, metric_data: pd.DataFrame) -> pd.DataFrame:
        """准备实测数据，过滤不需要的列"""
        return metric_data.drop(['index', 'latitude', 'longitude'], axis=1, errors='ignore')
    
    def _prepare_merged_data(self, 
                            origin_merged_data: pd.DataFrame | None,
                            filter_metric_data: pd.DataFrame,
                            matched_idx: list[int] | None) -> pd.DataFrame | None:
        """准备合并数据"""
    ...
    def _format_result(self, result: dict, model_type: int, merged_data: pd.DataFrame) -> dict:
        """格式化模型结果为标准输出格式"""
