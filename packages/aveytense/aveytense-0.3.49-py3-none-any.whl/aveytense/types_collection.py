...
"@lifetime >= 0.3.26rc1? Whole gamut of declarations written in `aveytense.types_collection` module"
...
"@lifetime >= 0.3.26c3. All constants in `tense.types_collection` module"

__non_constants__ = sorted([n for n in globals() if n[:1] != "_" and not n.isupper()])
"@lifetime >= 0.3.26c3. All non-constants (functions, classes, type aliases) in `aveytense.types_collection` module"
...
"@lifetime >= 0.3.26c3. Utility types from `typing` module in `aveytense.tcs` module"
...
"@lifetime >= 0.3.26c3. ABCs (Abstract Base Classes) in `tense.tcs` module"
