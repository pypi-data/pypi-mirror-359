...
GH_INPUTS_PATTERN = re.compile("\s+inputs\.(\S*)\s*")
GH_SECRETS_PATTERN = re.compile("\s+secrets\.(\S*)\s*")
GH_JOB_OUTPUTS_PATTERN = re.compile("\s+jobs\.(\S*)\.outputs\.(\S*)\s*")
GH_STEP_OUTPUTS_PATTERN = re.compile("\s+steps\.(\S*)\.outputs\.(\S*)\s*")
GH_RUN_OUTPUTS_PATTERN = re.compile('echo "(\S*)=.*"\s*>>\s*\$GITHUB_OUTPUT')
GH_NEEDS_PATTERN = re.compile("\s+needs\.(\S*)\.outputs\.(\S*)\s*")
