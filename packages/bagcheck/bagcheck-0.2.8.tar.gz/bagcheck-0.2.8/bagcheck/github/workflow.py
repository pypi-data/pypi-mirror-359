...
class Job:
        ...
        if self.max_depth == -1 or depth < self.max_depth:
            for child in self.children:
                ...
                if child.ident in check_map:
                    for item in check_map[child.ident]:
                        if not item in child.outputs:
                            check_log(self.log, f'[cyan]{child.ident}[/cyan] does not output `{item}`', local_level, self.indent * depth)
                            issue_count += int(local_level == LEVEL_ERROR)
                issue_count += child.check_outputs(local_level, ignore, warn, depth=depth+1)
        ...
        if self.max_depth == -1 or depth < self.max_depth:
            for child in self.children:
                if child.kind != GH_KIND_WORKFLOW:
                    check_log(self.log, f'Job [cyan]{self.ident}[/cyan] has no `timeout-minutes` field set', level, self.indent * depth)
    ...
    def check_runs_on(self, level: str, ignore: dict, warn: dict, depth: int=0) -> int:
        ...
        if self.max_depth == -1 or depth < self.max_depth:
            for child in self.children:
                if child.kind != GH_KIND_WORKFLOW:
                    check_log(self.log, f'Job [cyan]{self.ident}[/cyan] has no `runs-on` field set', level, self.indent * depth)
                    return int(level == LEVEL_ERROR)
                if check_luggage(self.doc, ignore, GH_CHECK_RUNS_ON):
                    self.log.debug(f'Skipping {child.name}')
                    continue
                ...
                if child.kind == GH_KIND_WORKFLOW:
                    ...
                    for field, args in child.inputs.items():
                        if args.get('required', False) and not field in self.with_args:
                            check_log(self.log, f'Job [cyan]{self.ident}[/cyan] is missing required workflow argument `{field}`', local_level, self.indent * depth)
                            issue_count += int(local_level == LEVEL_ERROR)
                    if self.secret_args == 'inherit':
                        continue
                    for field, args in child.secret_args.items():
                        if self.secret_args != 'inherit' and args.get('required', False) and not field in self.secret_args:
                            check_log(self.log, f'Job [cyan]{self.ident}[/cyan] is missing required workflow secret `{field}`', local_level, self.indent * depth)
                            issue_count += int(local_level == LEVEL_ERROR)
            ...
            for child in self.children:
                ...
                if self.uses and "@" in self.uses:
                    ...
                    if not re.match(ref_regex, ref):
                        check_log(self.log, f'Job [cyan]{self.ident}[/cyan] references an invalid ref `{ref}`', local_level, self.indent * depth)
                        issue_count += int(local_level == LEVEL_ERROR)
...
class Step:
    ...
    def __get_output_refs__(self) -> list:
        ...
        print(f'Getting step output refs for {self.ident}')

        for key, val in re.findall(GH_STEP_OUTPUTS_PATTERN, self.if_statement):
            outputs.append((key, val))
        print(f'Checking run {self.run}')
    ...
    def __load_children__(self) -> list:
        if self.uses:
            return [Action(self.log, path=self.uses, args=self.with_args, name=self.name, ident=self.ident, indent=self.indent, max_depth=self.max_depth)]
        return []
        ...
        if self.max_depth == -1 or depth < self.max_depth:
            for child in self.children:
                if child.kind == GH_KIND_ACTION:
                    ...
                    for field, args in child.inputs.items():
                        if args.get('required', False) and not field in self.with_args:
                            check_log(self.log, f'Step [cyan]{self.ident}[/cyan] is missing required action argument `{field}`', local_level, self.indent * depth)
                            issue_count += int(local_level == LEVEL_ERROR)
                    if self.secret_args == 'inherit':
                        continue
                    for field, args in child.secret_args.items():
                        if self.secret_args != 'inherit' and args.get('required', False) and not field in self.secret_args:
                            check_log(self.log, f'Step [cyan]{self.ident}[/cyan] is missing required action secret `{field}`', local_level, self.indent * depth)
                            issue_count += int(local_level == LEVEL_ERROR)
        ...
        if self.max_depth == -1 or depth < self.max_depth:
            for child in self.children:
                ...
                if self.uses and "@" in self.uses:
                    ref = self.uses.split('@')[1]
                    if not re.match(ref_regex, ref):
                        check_log(self.log, f'Step [cyan]{self.ident}[/cyan] references an invalid ref `{ref}`', local_level, self.indent * depth)
                        issue_count += int(local_level == LEVEL_ERROR)
                issue_count += child.check_refs(local_level, ignore, warn, ref_regex, depth=depth+1)
...
class Action:
    def __init__(self, log: logging.Logger, path: str="", doc: dict=None, args: dict=None, name: str='', ident: str='', if_statement: str='', indent: int=4, max_depth: int=-1):
    ...
    def __load_children__(self) -> list:
        ...
        for idx, step in enumerate(self.doc['runs'].get('steps', [])):
            if 'uses' in step:
                out.append(Action(self.log, path=step['uses'], args=step.get('with', {}), name=step.get('name', ''), ident=step.get('id', ''), indent=self.indent, max_depth=self.max_depth))
                continue
            out.append(Step(self.log, idx, step, indent=self.indent, max_depth=self.max_depth))
        return out
