...
class IndependentVariableSet(VariableSet):
    ...
    def __init__(self, location: str, latitude_deg_north: float, longitude_deg_east: float, csv_measurement_filename: str | None = None,  starting_stringdate: str | None = None, ending_stringdate: str | None = None, parent_ivs: IndependentVariableSet | None = None, albedo: float = .1, pollution: float = 0.1,  deleted_variables: tuple[str] = (),  number_of_levels: int = 4):
        ...
        all_deleted_variables: list[str] = ['dewpoint_2m', 'weathercode', 'cloudcover_low', 'cloudcover_mid', 'cloudcover_high', 'et0_fao_evapotranspiration', 'vapor_pressure_deficit', 'weather_soil_temperature_0cm', 'soil_moisture_0_to_7cm', 'soil_moisture_7_to_28cm', 'soil_moisture_28_to_100cm', 'soil_moisture_100_to_255cm', 'is_day', 'Tyanis', 'zetaW7', 'zetaW9', 'occupancy', 'temp_min', 'temp_max', 'description', 'power_heater', 'temperature_2m', 'relativehumidity_2m', 'dewpoint_2m', 'apparent_temperature', 'weathercode', 'et0_fao_evapotranspiration', 'vapor_pressure_deficit', 'windgusts_10m', 'shortwave_radiation']  # 'pressure', 'pressure_msl', 'surface_pressure',
        ...
        if parent_ivs is None:
            ...
            openweather_file: WeatherJsonReader = WeatherJsonReader(location=location, from_requested_stringdate=self.specified_starting_stringdatetime.split(
                ' ')[0], to_requested_stringdate=self.specified_ending_stringdatetime.split(' ')[0], albedo=albedo, pollution=pollution, latitude_north_deg=latitude_deg_north, longitude_east_deg=longitude_deg_east)
    ...
    def add_external_variable(self, name: str, values: list[float]) -> None:
        ...
        try:
            ...
            raise ValueError('Variable "%s" has a length equal to %i instead of %i' % (name, len(values), len(self)))
...
class IndependentVariableAccessor(VariableAccessor):
    ...
    def signature(self, k: int) -> int:
        ...
        if type(k) is int:
            ...
            raise ValueError(f"IndependentVariableAccessor.signature k: {k} is not an integer")
...
class ParameterizedVariableAccessor(VariableAccessor):
    ...
    def __init__(self, parameterized_variable_set: ParameterizedVariableSet, name: str, required_data: list[VariableAccessor], nominal_value: float, resolution: float) -> None:
        ...
        self.default_signature: int = round(self.nominal_value // self.resolution)
...
class DataProvider:
    ...
    def __call__(self, data_name: str, k: int | None = 0, value: float = None) -> float | list[float]:
        ...
        if k is None:
            if value is None:
                data_accessor: None | type[VariableAccessor] = self.variable_accessor_registry(data_name)
