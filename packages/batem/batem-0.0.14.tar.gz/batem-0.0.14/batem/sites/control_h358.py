...
from batem.core.siggen import WEEKDAYS, ModeSetpointSiggen, HeaterModeSetpointSiggen
...
if not with_air_conditioning:
    print('heating only')
    mode_setpoint_siggen = HeaterModeSetpointSiggen(dp.datetimes, ('15/10', '15/4'), [WEEKDAYS.MONDAY, WEEKDAYS.TUESDAY, WEEKDAYS.WEDNESDAY, WEEKDAYS.THURSDAY, WEEKDAYS.FRIDAY], {0: None, 6: 21, 18: None}, 'mode')
    modes: list[float] = mode_setpoint_siggen.get_modes()
    setpoints = mode_setpoint_siggen.get_setpoints()
else:
    print('heating and cooling')
    mode_setpoint_siggen = ModeSetpointSiggen(dp.datetimes, heating_period=('15/10', '15/4'), cooling_period=('15/6', '15/9'))
