...
def create_server() -> FastMCP:
    ...
    @mcp.tool()
    def find_bonds(name_query: str) -> List[Dict[str, Any]]:
        """根据债券名称查找债券信息（支持模糊匹配）"""
        return find_bonds_by_name(name_query)

    @mcp.tool()
    def find_stocks(name_query: str) -> List[Dict[str, Any]]:
        """根据股票名称查找股票信息（支持模糊匹配）"""
        return find_stocks_by_name(name_query)

    @mcp.tool()
    def get_quotes(codes_or_names: List[str]) -> Dict[str, Dict[str, Any]]:
        """获取指定债券和股票的实时报价（支持代码或名称查询）"""
        return get_realtime_quotes(codes_or_names)
    ...
    @mcp.tool()
    def get_bond_sensitivity(bond_code: str) -> Dict[str, Any]:
        """计算可转债的价格敏感性指标（替代复杂的希腊字母）"""
        return calculate_bond_sensitivity(bond_code)
