...
def find_bonds_by_name(name_query: str) -> List[Dict[str, Any]]:
    """
    根据债券名称查找债券信息。

    Args:
        name_query: 债券名称查询字符串（支持模糊匹配）
    ...
    try:
        ...
        # 模糊匹配债券名称
        matched = all_bonds_df[all_bonds_df['bond_name'].str.contains(name_query, case=False, na=False)]
        ...
        print(f"找到 {len(result)} 只包含 '{name_query}' 的可转债")
        return result

    except Exception as e:
        print(f"[find_bonds_by_name] 错误: {e}")
        return []


def find_stocks_by_name(name_query: str) -> List[Dict[str, Any]]:
    """
    根据股票名称查找股票信息。

    Args:
        name_query: 股票名称查询字符串（支持模糊匹配）
    ...
    try:
        ...
        # 模糊匹配股票名称
        matched = all_bonds_df[all_bonds_df['stock_name'].str.contains(name_query, case=False, na=False)]
        ...
        for _, row in matched.iterrows():
            ...
            if stock_code not in seen_stocks:
                ...
                result.append({
                    ...
                    "stock_name": str(row['stock_name']),
                    "stock_price": _safe_float(row['stock_price']),
                    "related_bond_code": str(row['bond_code']),
                    "related_bond_name": str(row['bond_name'])
                })

        print(f"找到 {len(result)} 只包含 '{name_query}' 的股票")
        return result

    except Exception as e:
        print(f"[find_stocks_by_name] 错误: {e}")
        return []


def get_realtime_quotes(codes_or_names: List[str]) -> Dict[str, Dict[str, Any]]:
    """
    获取指定债券和股票的实时报价，支持代码或名称查询。

    Args:
        codes_or_names: 债券代码、股票代码或名称列表
    ...
    try:
        ...
        for item in codes_or_names:
            # 首先尝试作为代码查找
            if item.startswith(('11', '12', '13')):
                # 债券代码
                bond_data = all_bonds_df[all_bonds_df['bond_code'] == item]
            ...
            elif item.isdigit() or (len(item) == 6 and item[:3] in ['000', '001', '002', '300', '600', '601', '603', '688']):
                # 股票代码
                stock_data = all_bonds_df[all_bonds_df['stock_code'] == item]
                ...
                # 尝试作为名称查找
                # 先查找债券名称
                bond_matches = all_bonds_df[all_bonds_df['bond_name'].str.contains(item, case=False, na=False)]
                if not bond_matches.empty:
                    for _, row in bond_matches.iterrows():
                        key = f"{row['bond_code']}({row['bond_name']})"
                        result[key] = {
                            "code": str(row['bond_code']),
                            "name": str(row['bond_name']),
                            "price": _safe_float(row['bond_price']),
                ...
                # 再查找股票名称
                stock_matches = all_bonds_df[all_bonds_df['stock_name'].str.contains(item, case=False, na=False)]
                seen_stocks = set()
                for _, row in stock_matches.iterrows():
                    stock_code = str(row['stock_code'])
                    if stock_code not in seen_stocks:
                        seen_stocks.add(stock_code)
                        key = f"{stock_code}({row['stock_name']})"
                        result[key] = {
                            "code": stock_code,
                            "name": str(row['stock_name']),
                            "price": _safe_float(row['stock_price']),
...
def calculate_bond_sensitivity(bond_code: str) -> Dict[str, Any]:
    """
    计算可转债的价格敏感性指标（替代复杂的希腊字母）。
    ...
    try:
        ...
        premium_rate = (premium / conversion_value) if conversion_value > 0 else 0

        # 股价敏感性：股价变动1%时债券价格的变动
        stock_sensitivity = conversion_ratio * stock_price * 0.01

        # 转股价值弹性：股价变动1%时转股价值的变动百分比
        conversion_elasticity = 1.0  # 转股价值与股价同比例变动

        # 债券价格弹性：股价变动1%时债券价格的变动百分比（考虑溢价缓冲）
        if bond_price > 0:
            bond_elasticity = (stock_sensitivity / bond_price) * 100
        else:
            bond_elasticity = 0

        # 溢价衰减速度：估算溢价随股价上涨的衰减程度
        if premium > 0:
            premium_decay_rate = min(premium_rate * 0.5, 0.8)  # 溢价衰减率
        else:
            premium_decay_rate = 0

        # 下跌保护程度：债券相对于股票的下跌保护
        downside_protection = max(0, (bond_price - conversion_value) / bond_price) if bond_price > 0 else 0
        ...
        return {
            ...
            "sensitivity_analysis": {
                "stock_price_sensitivity": {
                    "value": round(stock_sensitivity, 2),
                    "description": f"股价变动1%时，债券价格变动约{stock_sensitivity:.2f}元"
                },
                "bond_price_elasticity": {
                    "value": round(bond_elasticity, 2),
                    "description": f"股价变动1%时，债券价格变动约{bond_elasticity:.2f}%"
                },
                "premium_decay_rate": {
                    "value": round(premium_decay_rate * 100, 2),
                    "description": f"股价上涨时，溢价衰减速度约{premium_decay_rate*100:.1f}%"
                },
                "downside_protection": {
                    "value": round(downside_protection * 100, 2),
                    "description": f"相对于股票有{downside_protection*100:.1f}%的下跌保护"
                }
            },
            "risk_assessment": {
                "volatility_risk": "高" if bond_elasticity > 80 else "中" if bond_elasticity > 40 else "低",
                "premium_risk": "高" if premium_rate > 0.3 else "中" if premium_rate > 0.1 else "低",
                "conversion_attractiveness": "高" if premium_rate < 0.05 else "中" if premium_rate < 0.2 else "低"
            },
            "note": "敏感性分析基于当前市场数据的简化计算，实际情况可能因市场波动而有所不同"
        }

    except Exception as e:
        return {"error": f"[calculate_bond_sensitivity] 错误: {e}"}
