...
async def test_get_antibody_information_valid_id():
    ...
    async with Client(core_mcp) as client:
        ...
        search_result_text = await client.call_tool("get_antibody_list", {"search": "TRPC6"})
        search_result = json.loads(search_result_text[0].text)
...
async def test_antibody_workflow():
    ...
    async with Client(core_mcp) as client:
        ...
        search_result_text = await client.call_tool("get_antibody_list", {"search": "TP53"})
        search_result = json.loads(search_result_text[0].text)
        ...
        detail_result_text = await client.call_tool("get_antibody_information", {"ab_id": ab_id})
        detail_result = json.loads(detail_result_text[0].text)
