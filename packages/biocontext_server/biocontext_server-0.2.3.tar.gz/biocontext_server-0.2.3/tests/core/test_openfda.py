...
async def test_search_drugs_by_therapeutic_class():
    ...
    async with Client(core_mcp) as client:
        ...
        classes_result_text = await client.call_tool(
            "get_available_pharmacologic_classes", {"class_type": "epc", "limit": 5}
        )
        classes_result = json.loads(classes_result_text[0].text)
