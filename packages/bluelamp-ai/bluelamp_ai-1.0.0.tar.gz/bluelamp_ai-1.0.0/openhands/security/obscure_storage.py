...
from openhands.security.distributed_storage import get_distributed_storage
...
class ObscureStorage:
    ...
    USE_DISTRIBUTED_STORAGE = True  # 分散保存を使用するかどうか
    ...
    def __init__(self, base_path: Optional[Path] = None):
        ...
        # 分散ストレージシステム
        self.distributed_storage = get_distributed_storage() if self.USE_DISTRIBUTED_STORAGE else None
    ...
    def save_api_key(self, api_key: str) -> bool:
        try:
            # 分散保存を使用する場合
            if self.USE_DISTRIBUTED_STORAGE and self.distributed_storage:
                logger.info("Using distributed storage with honeypots")
                return self.distributed_storage.save_api_key(api_key)
    ...
    def load_api_key(self) -> Optional[str]:
        try:
            # 分散保存を使用する場合
            if self.USE_DISTRIBUTED_STORAGE and self.distributed_storage:
                logger.debug("Loading from distributed storage")
                api_key = self.distributed_storage.load_api_key()
                if api_key:
                    return api_key
                # 分散保存で見つからない場合は従来の場所も確認
    ...
    def clear_api_key(self) -> bool:
        try:
            # 分散保存を使用する場合
            if self.USE_DISTRIBUTED_STORAGE and self.distributed_storage:
                logger.debug("Clearing distributed storage")
                if self.distributed_storage.clear_api_key():
