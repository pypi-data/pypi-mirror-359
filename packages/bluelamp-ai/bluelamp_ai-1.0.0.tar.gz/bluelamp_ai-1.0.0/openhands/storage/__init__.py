...
# Optional storage backends - only available if dependencies are installed
try:
...
def get_file_store(
    ...
    if file_store_type == 'local':
        ...
        if S3FileStore is None:
            raise ValueError('S3 storage not available - boto3 dependency missing')
        store = S3FileStore(file_store_path)
    elif file_store_type == 'google_cloud':
        if GoogleCloudFileStore is None:
            raise ValueError('Google Cloud storage not available - google-cloud-storage dependency missing')
