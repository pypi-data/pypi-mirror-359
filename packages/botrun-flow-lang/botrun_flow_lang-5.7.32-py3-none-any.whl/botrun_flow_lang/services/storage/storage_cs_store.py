    ...
    def create_bucket(self, bucket_name: str) -> Optional[storage.Bucket]:
        ...
        try:
            ...
            if not bucket.exists():
                ...
                logger.info(f"Created bucket {bucket_name} in asia-east1 and set lifecycle rules.")
            else:
                logger.info(f"Bucket {bucket_name} already exists. Checking lifecycle rules.")
                ...
                if current_rules != desired_rules:
                    ...
                    bucket.lifecycle_rules = desired_rules
                    bucket.patch()
                    logger.info(f"Successfully updated lifecycle rules for bucket {bucket_name}")
                else:
                    logger.info(f"Lifecycle rules for bucket {bucket_name} are already up-to-date.")
