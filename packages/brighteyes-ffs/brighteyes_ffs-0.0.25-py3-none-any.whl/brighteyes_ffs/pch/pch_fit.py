...
def fit_pch(hist, fit_info, param, psf, lBounds=[-10,-10,-10,-10], uBounds=[10,10,10,10], weights=1):
    """
    ...
    fitresult = least_squares(fitfun_pch, fitparam_start, args=(fixed_param, fit_info, hist/np.sum(hist), psf, weights), bounds=(lowerBounds, upperBounds)) #, xtol=1e-12
...
def fitfun_pch(fitparam, fixedparam, fit_info, hist, psf, weights=1):
    ...
    pch_theo = simulate_pch_1c(psf, k_max=len(hist), c=concentration, q=brightness, T=T, dV=dV)
