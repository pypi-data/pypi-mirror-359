...
def fit_pch(hist, fit_info, param, psf, lBounds=[-10,-10,-10,-10], uBounds=[10,10,10,10], weights=1, n_bins=1e5):
    """
    ...
    fitresult = least_squares(fitfun_pch, fitparam_start, args=(fixed_param, fit_info, hist/np.sum(hist), psf_compressed/np.max(psf_compressed), psf_weights, weights), bounds=(lowerBounds, upperBounds)) #, xtol=1e-12
