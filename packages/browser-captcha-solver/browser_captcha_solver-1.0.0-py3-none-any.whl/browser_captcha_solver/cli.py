...
def main(args: Optional[list] = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int, 
        default=0,
        help="Port for HTTP server (0 for auto-select)"
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="browser-captcha-solver 1.0.0"
    )
    
    subparsers = parser.add_subparsers(dest="command", help="Available commands")
    
    # Start server command
    start_parser = subparsers.add_parser("start", help="Start the captcha solver server")
...
def cmd_test(args) -> int:
    ...
    with CaptchaSolver(port=args.port, browser_command=args.browser) as solver:
        ...
        print(f"✓ Opening browser at: http://localhost:{solver.port}/captcha/{challenge.challenge_type.lower()}/{challenge.host}/?id={challenge.id}")
