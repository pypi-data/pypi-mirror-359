...
class TelegramBot(MessengerBot):
            ...
            if update.chat_id:
                ...
                try:
                    ...
                    if chat.data['title'] != title:
                        if 'old_title' not in chat.data:
                            chat.data['old_title'] = []
                        if chat.data['title'] not in chat.data['old_title']:
                            chat.data['old_title'].append(chat.data['title'])

                    chat.data['title'] = title
                except KeyNotFound:
                    chat.init_by_data({
                        'bot_id': self.obj_id,
                ...
                if chat:
                    ...
                    if my_chat_member:
                        ...
                        if new_chat_member.get('status') == 'member':
                            ...
                            pass
                        if new_chat_member.get('status') == 'kicked':
                            await getattr(self, 'process_command_kicked')(update, chat=chat)
                        return
                    ...
                    if next_update:
                        if next_update['type'] == 'command_param':
                            ...
                            try:
                                next_update = await getattr(self, command_handler)(update, chat=chat)
                            except Exception as err:
                                err = ExtException(parent=err, action=command_handler)
                                self.log.error(err)
                                await self.send_message_to_admin(
                                    f'Failed process command {command_handler} {json.dumps(update.data, ensure_ascii=False)}')
                                next_update = True
    ...
    async def process_message(self, chat, update):
        try:
            ...
            if 'reply_to_message' in update.message and update.message['reply_to_message']['from']['id'] == self.data['_id']:
                # пользователь запретил пересылку настройками конфиденциальности
                original_chat = Chat(self.storage, bot_id=self.obj_id)
                forward_origin =  update.message['reply_to_message']['forward_origin']
                try:
                    if 'forward_from' not in update.message['reply_to_message']:
                        original_chat_id = None
                        user_name = forward_origin['sender_user_name']
                        message_date = datetime.fromtimestamp(forward_origin['date'], tz=tz_info)
                        await original_chat.find_by_message_date_and_user_name(message_date, user_name)
                    else:
                        original_chat_id = update.message['reply_to_message']['forward_from']['id']
                        await original_chat.find_by_chat_id(original_chat_id)
                    res = await self.api.send_message(original_chat.chat_id, _message['text'])
                    ...
                    await self.send_message_to_admin(
                            f"Сообщение не доставлено до получателя "
    ...
    async def process_command_kicked(self, update: BotUpdate, *, chat: Chat = None):
        await self.send_message_to_admin(f"User kicked {chat.data['title']} {chat.data['chat_id']}")
    ...
    async def process_command_set_phone(self, update: BotUpdate, *, chat: Chat = None):
        ...
        if contact and contact['user_id'] == message['from']['id']:
            ...
            await self.send_message_to_admin(f"User set phone {contact.get('first_name','')} {contact.get('last_name','')} {chat.data['phone']}")
            return True
        return False
