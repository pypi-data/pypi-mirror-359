...
def upload_one_file(name_tuple: (str, str), bucket: str, s3_client=None, reconnect=True, s3_label='s3_web',
    ...
    try:
        ...
        with open(filename, "rb") as f:
            response = requests.put(url, data=f, headers={"Content-Type": content_type})

        if response.status_code != 200:
            raise RuntimeError(f"Presigned upload failed with status {response.status_code}: {response.text}")
