...
COMMAND = 'command'
spl_char = ':'
string_delete_one = 'DeleteOne'
string_delete_many = 'DeleteMany'

logger = logging.getLogger(LOGGER_NAME[0] + '.probes.mongodb.mongodbdelete')

class MongodbDeleteProbe(Singleton):
    def __init__(self):
        pass
    ...
    def start(self, context):
        ...
        bulk_deleteone_flag = False
        bulk_deletemany_flag = False

        if string_delete_one in str(bulk_request):
          bulk_deleteone_flag = True
        else:
          bulk_deleteone_flag = False

        if string_delete_many in str(bulk_request):
          bulk_deletemany_flag = True
        else:
          bulk_deletemany_flag = False
                  
        bulk_operations = (bulk_deleteone_flag, bulk_deletemany_flag)
        bulk_write_flag = any(bulk_operations)
        
        if bulk_write_flag:
          for i in bulk_request:
            if string_delete_one in str(i):
               Command = "delete_one"
            ...
            if string_delete_many in str(i):
               Command = "delete_many"           
               portNum, dbname , collectionname = mongodbcontextinfo.get_context_info(request_string)
               context.set_params({DATABASE: "MongoDB", DATABASE_NAME: dbname, COLLECTION_NAME: collectionname, MONGODB_HOST: hostName, MONGODB_PORT: portNum,
                                   PYTHON_MODULE: "pymongo", COMMAND_TYPE: CommandType, COMMAND: Command, "ATTR_hostname": hostName, "ATTR_port": portNum,
