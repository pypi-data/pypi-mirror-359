...
from ca_apm_agent.utils import containerid_helpers
...
class Command:
    def __init__(self, host, port, instid, app_name, com_msg_queue):
        self.host = host
        self.port = port
        if containerid_helpers.getPodIPAddress():
            self.instid = containerid_helpers.getPodIPAddress()
        else:
            self.instid = instid
        ...
        self.pod_name = containerid_helpers.getPodName()
        self.pod_namespace = containerid_helpers.getPodNamespace()
        self.pod_container_name = containerid_helpers.getContainerName()
        self.pod_ip_address = containerid_helpers.getPodIPAddress()
    ...
    def _create_conn_msg(self):
        ...
        if self.pod_name:
            from ca_apm_agent.connections.constants import POD_NAME
            if self.params is None:
                self.params = {}
            self.params[POD_NAME] = self.pod_name

        if self.pod_namespace:
            from ca_apm_agent.connections.constants import POD_NAMESPACE
            if self.params is None:
                self.params = {}
            self.params[POD_NAMESPACE] = self.pod_namespace

        if self.pod_container_name:
            from ca_apm_agent.connections.constants import POD_CONTAINER_NAME
            if self.params is None:
                self.params = {}
            self.params[POD_CONTAINER_NAME] = self.pod_container_name

        if self.pod_ip_address:
            from ca_apm_agent.connections.constants import POD_IP_ADDRESS
            if self.params is None:
                self.params = {}
            self.params[POD_IP_ADDRESS] = self.pod_ip_address
