...
class BaseConfig:
    ...
    def _initialize(self, bert_path_or_bertModel_name, train_data, dev_data, test_data, rel_data, batch_size,
        ...
        # 设置缓存目录
        self.cache_dir = cache_dir if cache_dir and os.path.isdir(cache_dir) else os.path.join(os.path.expanduser("~"),
                                                                                               ".cache", "huggingface",
                                                                                               "transformers")
        if not os.path.exists(self.cache_dir):
            os.makedirs(self.cache_dir, exist_ok=True)
            logger.info(f"创建缓存目录: {self.cache_dir}")
        # 初始化 bert_path_or_bertModel_name 为绝对路径
        if not bert_path_or_bertModel_name or not self._is_valid_bert_path(bert_path_or_bertModel_name):
            self.bert_path_or_bertModel_name = self._download_default_bert()
        else:
            self.bert_path_or_bertModel_name = os.path.abspath(bert_path_or_bertModel_name)  # 转换为绝对路径
        try:
            # 检查缓存是否存在
            cache_dir = self._get_cache_dir(self.bert_path_or_bertModel_name)
            if os.path.exists(cache_dir):
                logger.info(f"复用缓存的 BERT 模型，绝对路径: {cache_dir}")
                self.bert_path_or_bertModel_name = cache_dir  # 复用缓存路径
            else:
                logger.info(f"下载 BERT 模型，目标路径: {cache_dir}")
            self.tokenizer = BertTokenizer.from_pretrained(self.bert_path_or_bertModel_name,
                                                           cache_dir=self.cache_dir)  # 指定缓存目录
            self.bert_model = BertModel.from_pretrained(self.bert_path_or_bertModel_name,
                                                        cache_dir=self.cache_dir)  # 指定缓存目录
            # 确保使用实际缓存路径
            actual_cache_dir = self.tokenizer._cache_dir or self.bert_model._cache_dir
            if actual_cache_dir:
                self.bert_path_or_bertModel_name = os.path.abspath(actual_cache_dir)  # 更新为实际缓存路径
            self.bert_dim = self._get_bert_dim(self.bert_path_or_bertModel_name)  # 动态设置 bert_dim
    ...
    def _download_default_bert(self):
        ...
        try:
            ...
            cache_dir = self._get_cache_dir(default_model)
