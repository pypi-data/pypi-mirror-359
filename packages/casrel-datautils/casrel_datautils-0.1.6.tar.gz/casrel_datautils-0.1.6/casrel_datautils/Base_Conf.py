...
class BaseConfig:
    ...
    def _initialize(self, bert_path_or_bertModel_name, train_data, dev_data, test_data, rel_data, batch_size,
        ...
        可用性：若 cache_dir 无效或不可用，回退到默认缓存目录。
        """
        ...
        # 初始化 bert_path_or_bertModel_name 为有效路径
        ...
        try:
            # 加载模型，指定缓存目录
            logger.info(f"尝试加载 BERT 模型，初始路径: {self.bert_path_or_bertModel_name}")
            ...
            if actual_cache_dir and os.path.exists(actual_cache_dir):
                ...
                logger.info(f"复用或下载后的 BERT 模型，绝对路径: {self.bert_path_or_bertModel_name}")
            else:
                logger.info(f"使用指定路径加载 BERT 模型，绝对路径: {self.bert_path_or_bertModel_name}")
