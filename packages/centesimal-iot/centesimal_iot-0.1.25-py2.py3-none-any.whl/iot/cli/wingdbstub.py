...
print(f"2- WINGHOME used: {WINGHOME}")
print(f"2- hostname: {hostname}")
# ------------------------------------------------------------------------
...
if (
    ...
    try:
        ...
        if hostname in ("dev00",):
            initial_pause = 0
        else:
            initial_pause = 5
        _info(f"HACK fo debugging: waiting {initial_pause} secs")
        time.sleep(initial_pause)

        # inspect.signature(netserver.CNetworkServer)
        # <Signature (host, port, attachport, err=None, firststop=0, security_token=None, pwfile_path=(), pwfile_name='wingdebugpw', internal_modules=(), autoquit=1)>
        # hasn't timeout!

        def try_to_connect_debugger(
            host,
            port,
            attachport,
            err,
            ...
            """A custom function that prevent wingide to block the application
            where can't reach the host, but for an unknown reason don't rails
            i.e. in some K8 enviroments.

            As the CNetworkServer doesn't provide timeout support I use signal
            to create a timeout watcher within the same thread without using
            threads or other alternatives.
            """
            import signal
            ...
            try:
                ...
                _debug("Timeout connecting remote debugger, continue ...")
                raise
        ...
        if False:
            ...
            debugger = try_to_connect_debugger(
                host,
                port,
                attachport,
                err,
