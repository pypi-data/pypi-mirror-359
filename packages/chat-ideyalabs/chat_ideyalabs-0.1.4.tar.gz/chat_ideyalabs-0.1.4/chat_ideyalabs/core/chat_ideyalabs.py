...
class ChatIdeyalabs:
    ...
    async def _validate_user_api_key(self):
        """
        Validate the user's API key against the database.
        ...
        try:
            user_info = await api_key_manager.validate_api_key(self.user_api_key)
            if not user_info:
                raise ValueError(
                    "Invalid or inactive API key. Please check your API key or contact your admin."
                )
            
            # Store user info for potential logging
            if not self.user_id:  # Only set if not already provided
