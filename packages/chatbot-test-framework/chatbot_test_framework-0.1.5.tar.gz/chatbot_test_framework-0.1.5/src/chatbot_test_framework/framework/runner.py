...
class TestRunner:
    ...
    def run_phase1_send_questions(self):
        ...
        if recorder_config.get('type') == 'local_json' and 'filepath' in recorder_config.get('settings', {}):
            # os.path.abspath resolves the path based on the current working directory
            # where the `chatbot-tester` command is being run.
            absolute_path = os.path.abspath(recorder_config['settings']['filepath'])
        ...
        for index, row in dataset_df.iterrows():
            ...
            try:
                client.send(question, session_id)
