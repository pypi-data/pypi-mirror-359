...
class SmartTextEdit(QPlainTextEdit):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_Tab:
            ...
            # Only delete a full indentation chunk if we're strictly in leading whitespace
            if text_before and text_before == " " * cursor_pos:
                spaces_to_delete = min(cursor_pos, cursor_pos % 4 or 4)
...
class CheesyMamas(QMainWindow):
    ...
    def save_file(self):
        ...
        if not path:
            self.save_file_as()  # attempt Save As
            # re-fetch path in case Save As succeeded
            editor_data = self.editors.get(idx)
            if len(editor_data) == 5:
                _, _, path, _, _ = editor_data
            else:
                _, _, path, _ = editor_data
            if not path:  # still not saved
                return
