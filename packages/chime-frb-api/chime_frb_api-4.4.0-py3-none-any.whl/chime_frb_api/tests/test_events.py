...
def test_get_events():
    ...
    event_data = master.events.get_events([TEST_EVENT["id"], TEST_EVENT2["id"]])
    assert event_data[TEST_EVENT["id"]]["id"] == TEST_EVENT["id"]
    assert event_data[TEST_EVENT["id"]]["fpga_time"] == TEST_EVENT["fpga_time"]
    assert (
        event_data[TEST_EVENT["id"]]["event_type"] == TEST_EVENT["event_type"]
    )
    assert (
        event_data[TEST_EVENT["id"]]["beam_numbers"]
        == TEST_EVENT["beam_numbers"]
    )
    assert event_data[TEST_EVENT2["id"]]["id"] == TEST_EVENT2["id"]
    assert (
        event_data[TEST_EVENT2["id"]]["fpga_time"] == TEST_EVENT2["fpga_time"]
    )
    assert (
        event_data[TEST_EVENT2["id"]]["event_type"] == TEST_EVENT2["event_type"]
    )
    assert (
        event_data[TEST_EVENT2["id"]]["beam_numbers"]
        == TEST_EVENT2["beam_numbers"]
    )
