# coding: utf-8

# GTO-MO-1-1 - Removed partial_header
# flake8: noqa

from datetime import date, datetime
import decimal
import functools
import io
import re
import typing
import typing_extensions
import uuid

import frozendict

from chkp_harmony_endpoint_management_sdk.generated.cloud import schemas


class RuleSettingsModifications(
    schemas.DictSchema
):
    """NOTE: This class is auto generated by OpenAPI Generator.
    Ref: https://openapi-generator.tech

    Do not edit the class manually.

    Settings modifications made to a rule
    """

    class MetaOapg:
        required = {
            "modified",
        }
        
        class properties:
            modified = schemas.BoolSchema
            __annotations__ = {
                "modified": modified,
            }
        additional_properties = schemas.NotAnyTypeSchema
    
    modified: MetaOapg.properties.modified
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["modified"]) -> MetaOapg.properties.modified: ...
    
    def __getitem__(self, name: typing.Union[typing_extensions.Literal["modified"], ]):
        # dict_instance[name] accessor
        return super().__getitem__(name)
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["modified"]) -> MetaOapg.properties.modified: ...
    
    def get_item_oapg(self, name: typing.Union[typing_extensions.Literal["modified"], ]):
        return super().get_item_oapg(name)

    def __new__(
        cls,
        *_args: typing.Union[dict, frozendict.frozendict, ],
        modified: typing.Union[MetaOapg.properties.modified, bool, ],
        _configuration: typing.Optional[schemas.Configuration] = None,
    ) -> 'RuleSettingsModifications':
        return super().__new__(
            cls,
            *_args,
            modified=modified,
            _configuration=_configuration,
        )
