...
class CladClient:
  def __init__(self, api_key: str, threshold: int = 3, filteredKeywords=[], redis_client=None):
    """
    Initializes the CladClient with API key, threshold, optional filteredKeywords, and optional Redis client.
    ...
    #keywords to filter out specific ads from being displayed.
  ...
  async def get_processed_input(self, user_input: str, user_id: str, discrete: str = "false") -> dict:
    ...
    should_filter = any(keyword.lower() in user_input.lower() for keyword in self.filteredKeywords)

    if new_count < self.threshold or should_filter:
