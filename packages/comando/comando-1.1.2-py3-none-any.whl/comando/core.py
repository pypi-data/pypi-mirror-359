...
"""
    ...
    def make_design_variable(
        self, name, domain=REAL, bounds=(None, None), init_val=None
    ):
    ...
    def make_operational_variable(
        ...
        var = comando.VariableVector(self._prefix(name), domain, bounds, init_val)
...
"""
    ...
    def extend_connection(self, bus_id, alias=None):
        ...
        if bus_id not in self.connections:
            raise RuntimeError("Tried to extend nonexisting connection " f"'{bus_id}'!")
    ...
    def detach(self, bus_id, connectors=None):
        ...
        if connectors is None:  # detach the entire bus
            ...
            if invalid_connectors:
                raise KeyError(
                    f"The connectors {invalid_connectors} are not "
                    f"part of bus {bus}!"
                )
    ...
    def add(self, component):
        ...
        if not isinstance(component, Component):
            raise RuntimeError(
                "Tried to add something that is not a Component"
    ...
    def aggregate_component_expressions(self, id, aggregator=sum):
        ...
        """
        return aggregator(c.get_expression(id, comando.Zero) for c in self.components)
...
class Problem:
        ...
        if scenarios:  # update initial states
            ...
            for state, (iv, *_) in self.states.items():
                if iv.is_Variable:
                    ...
                    new_iv = comando.VariableVector(
                        iv.name, iv.domain, iv.bounds, iv.init_val
                    )
                    new_iv.instantiate(scenarios)
                    iv_updates[state] = (new_iv, *_)
                elif iv.is_Parameter:
    ...
    def weighted_sum(self, op_expr, symbolic=True):
        ...
        if symbolic:
            ...
            def func(idx):
                ...
                sym_map = {sym: sym[idx] for sym in op_expr.free_symbols if sym.indexed}
    ...
    def _set_initial_states(self, state_name, value):
        ...
        if isinstance(value, (str, int, float)):
            value = float(value)
            if value != value and i_state.is_Variable:  # Need to change to Parameter
                new_i_state = comando.Parameter(i_state.name)
    ...
    def subs(self, sym=None, rep=None, **reps):
        ...
        if reps:  # keys are symbol identifiers
            ...
            reps = sym
            reps = {
                self[sym] if not isinstance(sym, comando.Symbol) else sym: rep
        ...
        for state, (init_state, der, roc) in self.states.items():
            ...
            if init_state in reps:
                ...
                new_init_state_rep = comando.linsolve(  # TODO: Assumes linear reps
                    [state_rep - init_state], [new_state]
                )[0]
            if isinstance(new_init_state_rep, comando.Symbol):
                new_init_state = new_init_state_rep
            else:
                index = comando.utility.get_index(new_init_state_rep)
                if any(sym.is_Variable for sym in new_init_state_rep.free_symbols):
                    if index is None:
                        new_init_state = comando.Variable(
                            init_state.name + "__aux",
                            comando.REAL,
                            comando.utility.bounds(new_init_state_rep),
                            comando.utility.evaluate(new_init_state_rep),
                        )
                    else:  # One inital state per scenario
                        lb, ub = comando.utility.bounds(new_init_state_rep)
                        new_init_state = comando.VariableVector(
                            init_state.name + "__aux",
                            comando.REAL,
                            (lb.min(), ub.max()),
                        )
                        ...
                        new_init_state.value = comando.utility.evaluate(
                            new_init_state_rep
                        )
                else:
                    new_init_state = comando.Parameter(
                        init_state.name + "__aux",
                        comando.utility.evaluate(new_init_state_rep),
                    )
            ...
            if isinstance(der_rep, comando.VariableVector):
                der_aux = der_rep
            else:
                lb, ub = comando.utility.bounds(der_rep)
                # We create an auxiliary variable that represents the
                # scaled derivative expression...
                der_aux = comando.VariableVector(
                    der.name + "__aux",
                    der.domain,
                    (lb.min(), ub.max()),
                ...
                der_aux.value = comando.utility.evaluate(der_rep)
                # ... and add a new constraint ensuring equality
                aux_cons[der_aux.name] = comando.Eq(der_aux, der_rep)
