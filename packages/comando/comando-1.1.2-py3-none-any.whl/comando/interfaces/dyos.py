...
# adaptType_type
from dyospy import WAVELET, SWITCHING_FUNCTION
...
# SolverType_type
from dyospy import NLEQ1S, CMINPACK
...
from dyospy import ADAPTATION, STRUCTURE_DETECTION, ADAPT_STRUCTURE
...
from dyospy import MINIMIZE, MAXIMIZE
...
class ParameterGridInput(dp.ParameterGridInput):
    ...
    def __init__(
        ...
        super().__init__()
        if timePoints:
            assert (0.0 <= min(timePoints)) and (
                max(timePoints) <= 1.0
            ), "Time points must be in the range [0.0,1.0]"
            assert list(timePoints) == sorted(
                timePoints
...
class DyosProblem(dp.DyosPy):
    ...
    def solve(self):
        ...
        opars = [p.name for p in self.P.parameters if p.indexed]
        if self.P.scenarios:
