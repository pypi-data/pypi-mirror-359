...
from comando.utility import get_index, handle_state_equations, split, indexed
...
def _split_des_op_cons(mp):
    ...
    return map(
        ...
        split(mp.P.constraints, indexed),
    )
...
class MaingoProblem(maingopy.MAiNGOmodel):
    ...
    def set_initial_point(self, point):
        ...
        if lp == lip:
            ...
            raise ValueError(
                f"Length of given point is {lp}, however number "
                f"of variables is {lip}!"
            )
...
class MaingoTwoStageProblem(maingopy.TwoStageModel):
    ...
    def __init__(self, P, relaxation_only=None, squash=None, output=None, use_cse=True):
        ...
        constants, parameters = split(P.parameters, indexed)
