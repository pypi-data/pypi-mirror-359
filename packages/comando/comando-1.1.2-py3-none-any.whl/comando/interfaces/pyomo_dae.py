...
import sympy
import pyomo.core
from pyomo.core import ConcreteModel, Var, Objective, ConstraintList
...
from pyomo.environ import SolverManagerFactory, SolverFactory, TransformationFactory
...
def to_pyomo_dae(problem, **dynamic_options):
    ...
    if P.timesteps is None:
        raise RuntimeError(
            "Problem without timesteps cannot be handled via "
            "the Pyomo.DAE interface!"
        )
    ...
    for init_state, *_ in P.states.values():
        if init_state.is_Parameter:
            if init_state.indexed:
                pars.update(init_state)
            else:
                pars.add(init_state)
        elif init_state.is_Variable:
            if init_state.indexed:
    ...
    # is flexible
    IntegratedVariableObjective = comando.VariableVector("IntegratedVariableObjective")
    ...
    for state in P.states:
        ...
        if initial_state is not None:
            m.constraints.add(parse2pyomo(sympy.Eq(state, initial_state), t_0))
