...
baron_str_map = {
    ...
    "Equality": lambda ZERO, lhs_minus_rhs: f"{lhs_minus_rhs} == 0",  # BARON needs a == 0
...
def variables_section(var_map, prios=None):
    ...
    _, p_vars = split(
        all_vars - b_vars, lambda v: all(v.lb == 0) if v.indexed else v.lb == 0
    )
    ...
    for group, vars in variable_groups.items():  # sections for each group
        if vars:
            var_reps = (
                ", ".join(var_map[v].values()) if v.indexed else var_map[v]
                for v in vars
            )
...
def handle_tanh(expr, tanh_definitions, var_map):
    ...
    for atom in expr.atoms(comando.Function):
        if (comando.utility.get_type_name(atom)) == "tanh":
            ...
            index = get_index(arg)
            if index is None:
                tanh_var = comando.Variable(f"tanh{handle_tanh.n}", bounds=(-1, 1))
                var_map[tanh_var] = next(var_name())
            else:
                tanh_var = comando.VariableVector(
...
def write_bar_file(P, file_name, options=None, cse=False, reuse=False):
    ...
    con_map = {
        normalize(c): (
            dict(zip(P.index, con_name(n))) if indexed(c) else next(con_name())
        )
...
def solve(P, file_name=None, silent=False, cse=False, reuse=None, **options):
    ...
    from comando.utility import syscall, canonical_file_name, check_reuse_or_overwrite
