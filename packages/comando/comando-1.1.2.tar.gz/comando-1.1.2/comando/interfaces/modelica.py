...
def _variables_section(P, controls):
    ...
    if not controls:
        ...
        dummy_input = comando.VariableVector("dummy_input")
...
def _equations_section(P, ineqs, parse):
        ...
        (
            ...
            + "  ".join(
                (
                    ...
                    if P.scenarios and comando.utility.indexed(ineq)
    ...
    for c_id, c in P.constraints.items():
        ...
        if P.scenarios and comando.utility.indexed(c):
            for s in P.scenarios:
...
def write_mo_file(
    ...
    _controls = set()
    if isinstance(controls, (str, comando.Symbol)):  # single control
        controls = [controls]
