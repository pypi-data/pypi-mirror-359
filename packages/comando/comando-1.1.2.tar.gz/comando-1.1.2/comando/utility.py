...
def _implicit_euler_constraints(states, timesteps, s=slice(None)):
    for state, (initial_state, derivative, _) in states.items():
        if initial_state.is_Parameter:
            if initial_state.indexed:  # One parameter for each scenario
                ...
                if init_val != init_val:  # float("nan") => cyclic!
                    ...
                    prev = initial_state
        elif initial_state.is_Variable:
            if initial_state.indexed:  # One Variable for each scenario
                prev = initial_state[s]
...
def get_latest(path_glob):
    ...
    if matches:
        ...
        raise FileNotFoundError(
            f"No file in {os.getcwd()} matches the glob " f"'{path_glob}'!"
        )


def depth(expr):
    """Compute the depth of the given expression."""
    if isinstance(expr, (int, float, comando.Number, comando.Symbol)):
        return 0
    else:
...
# TODO: Rename to is_indexed for consistency
def indexed(expr):
    """Test whether the given expression is indexed."""
    return any(sym.indexed for sym in expr.free_symbols)
...
def get_vars(expr):
    ...
    return {sym for sym in expr.free_symbols if sym.is_Variable}


def get_pars(expr):
    """Get a set of all parameters in `expr`."""
    return {sym for sym in expr.free_symbols if sym.is_Parameter}
...
def get_index(expr):
    ...
    dummy = sum((sym.value[:] * 0 for sym in expr.free_symbols if sym.indexed), 0)
...
def evaluate(expr, idx=None):
    ...
    if idx is None:  # No particular index desired
        ...
        res = _parse(
            expr,
            {
                sym: sym.value.values if sym.indexed else sym.value
    ...
    return float(
        expr.subs(
            {
                sym: sym[idx].value if sym.indexed else sym.value
                for sym in expr.free_symbols
            }
        )
    )
    ...
    # ...
    # res = _idx_parse(expr, {sym: sym[idx].value if sym.indexed else sym.value
...
class StrParser:
    ...
    def __call__(self, expr, idx=None):
        ...
        if expr.is_Symbol:  # Variables or Parameters
            ...
            if idx is not None and expr.indexed:
...
def lambdify(expr, vars=None, eval_params=True, **kwargs):
    ...
    v_reps = OrderedDict((v, sympy.Symbol(f"VALUE_OF_{v.name}")) for v in vars)
    p_reps = {}
    if eval_params:
        ...
        if p_reps and get_index(sum(p_reps)) is not None:  # multiple expressions
            ...
            funcs = Series(
                (sympy.lambdify(v_reps.values(), ei, **kwargs) for ei in exprs),
                exprs.index,
                "O",
            )
            ...
            return func
    return sympy.lambdify(v_reps.values(), parse(expr, {**p_reps, **v_reps}), **kwargs)
...
def make_function(expr):
    ...
    pars = sorted(
        (sym for sym in syms if isinstance(sym, comando.Parameter)),
        key=lambda x: x.name,
    )
    vars = sorted(
        (sym for sym in syms if isinstance(sym, comando.Variable)), key=lambda x: x.name
    )
