...
async def run_repl():
    ...
    if not valid:
        console.print(f"🚫[bold red]{error}[/bold red]. To fix this, run [bold green][italic]configen init[/italic][/bold green].")
        return None
    ...
    console.print("ℹ️Enter [bold]/help[/bold] to view commands. Use [bold]/new[/bold] to restart. Exit with [bold]/exit[/bold] or [bold]Ctrl+C[/bold].")
    console.print("_" * 50)
...
def cli(
        ...
        version_flag: bool = typer.Option(
            ...
            "-v",
            is_eager=True,
            help="Show the Configen CLI version and exit",
        ),
):
    if version_flag:
...
@app.command()
def init():
    console.print("🛠️[bold green]Initializing Configen setup...[/bold green]")

    property.CONFIG_DIR = property.CONFIG_DIR.mkdir(exist_ok=True)
    ...
    console.print(f"✅Config saved to [bold]{property.ENV_FILE}[/bold]. Run [bold][italic]configen[/italic][/bold] to get started.")
