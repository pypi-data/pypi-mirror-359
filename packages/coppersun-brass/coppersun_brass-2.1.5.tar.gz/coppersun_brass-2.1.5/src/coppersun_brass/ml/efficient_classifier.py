...
class EfficientMLClassifier:
    ...
    def _load_quantized_codebert(self, model_dir: Path):
        """Load the 266KB pre-trained ONNX model architecture."""
        try:
            import onnxruntime as ort
            
            # Look for the actual 266KB pre-trained model
            onnx_path = model_dir / 'codebert_small_quantized.onnx'
            ...
            logger.info(f"Loading 266KB pre-trained ONNX model: {onnx_path}")
            
            # Load custom tokenizer (14KB)
            tokenizer_path = model_dir / 'code_tokenizer.json'
            if not tokenizer_path.exists():
                return False
            
            # Load the ultra-lightweight classifier for this specific model
            from ..ml.ultra_lightweight_classifier import UltraLightweightClassifier
            self.ultra_classifier = UltraLightweightClassifier(model_dir)
            
            if self.ultra_classifier.enabled:
                self.session = self.ultra_classifier.onnx_session
                self.tokenizer = self.ultra_classifier
                self.use_quantized = True
                self.enabled = True
                logger.info("✅ Loaded 266KB pre-trained model successfully!")
                return True
            else:
                return False
            
        except Exception as e:
            logger.debug(f"266KB pre-trained model not available: {e}")
            return False
    ...
    def classify(self, file_path: str, content: str) -> Tuple[str, float]:
        ...
        # Use the ultra-lightweight 266KB classifier if available
        if hasattr(self, 'ultra_classifier') and self.ultra_classifier.enabled:
            return self.ultra_classifier.classify(file_path, content)
