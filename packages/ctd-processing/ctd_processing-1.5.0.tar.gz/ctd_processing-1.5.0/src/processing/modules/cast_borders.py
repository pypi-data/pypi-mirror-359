...
class cast_borders(ArrayModule):
    """
    ...
    def transformation(self):
        ...
        if self.get_cast_borders() == -1:
            ...
            if not self.args["downcast_only"]:
                print(
                    "Downcast Start: ",
                    self.ind_dc_start,
                    "Downcast End: ",
        ...
        if self.args["cut"]:
            new_array = self.cut_cast_borders()
        else:
            self.mark_cast_borders(self.CB_list)

        self.parameters["Date"] = datetime.datetime.now().strftime(
            "%m %d %Y  %H:%M:%S"
        )

        self.parameters["Cast Borders"] = [self.ind_dc_start, self.ind_dc_end]
    ...
    def ini_cast_borders(self):
        ...
        misc_data_dict = self.cnv.data_table_misc
...
def main(args):
    ...
    if input.suffix == ".psa":
        with open(input) as toml:
            ...
            instance = cast_borders(args=doc["arguments"])
            instance(
                input=input,
                parameters={},
                name="castborders",
        ...
        args = vars(args)
        instance = cast_borders(args=args)
        instance(
            input=input,
            parameters={},
            name="castborders",
            output="cnv",
            output_name=r"C:\Projects\processing\src\processing\modules\output\test.cnv",
        )
    else:
