...
class CTkRadarChart(tk.Canvas, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    ...
    def __init__(self,
        ...
        if self.bg_color=="transparent":
            if isinstance(master, customtkinter.CTkFrame):
                self.bg_color = master.master.cget("fg_color")
                
        CTkAppearanceModeBaseClass.__init__(self)
        CTkScalingBaseClass.__init__(self, scaling_type="widget")

        self.size = size
        self.radius = self._apply_widget_scaling(size)
        ...
        if data:
            for key, value in data.items():
                if "color" in value:
                    self.add_data(key, value["data"], value["color"])
                else:
                    self.add_data(key, value["data"])
    ...
    def draw_dataset(self, dataset, color, fill):
        ...
        self.create_polygon(data_points, outline=outline_color, fill=fill_color, stipple='gray12',
                            width=self._apply_widget_scaling(self.border_width))
    ...
    def add_data(self, tag, data=[], color=None, fill=True):
        ...
        data = [int(x.strip()) if isinstance(x, str) and x.strip().isdigit() else x for x in data if x]

        data = [min(100, max(0, value)) for value in data]
    ...
    def configure(self, **kwargs):
        ...
        if "size" in kwargs:
            ...
            self.radius = self._apply_widget_scaling(self.size)
            super().config(width=self.radius, height=self.radius)
