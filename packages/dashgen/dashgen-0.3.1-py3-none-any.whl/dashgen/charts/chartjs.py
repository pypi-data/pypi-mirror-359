...
def generate_chartjs_block(title, data, chart_type="bar", options=None):
    ...
    allowed_types = {"bar", "line", "pie", "scatter", "doughnut", "radar"}
    ...
    legend_position = options.get("legend_position")
    ...
    legend_position_js = (
        f", position: '{legend_position}'" if legend_position else ""
    )
    x_title_js = (
        f", title: {{ display: true, text: '{x_axis_title}' }}" if x_axis_title else ""
    )
    y_title_js = (
        f", title: {{ display: true, text: '{y_axis_title}' }}" if y_axis_title else ""
    )
      ...
      <script>
        ...
        new Chart(ctx_{chart_id}, {{
          ...
          options: {{
            ...
            plugins: {{
              legend: {{ display: {str(show_legend).lower()}{legend_position_js} }},
              datalabels: {{
            ...
            scales: {{
              x: {{ display: {str(show_x_axis).lower()}{x_title_js} }},
              y: {{ display: {str(show_y_axis).lower()}{y_title_js} }}
            }}
          }},
