...
async def save(code: str, data: C103, client: AsyncIOMotorClient) -> dict:
    coll = get_collection(client, DB_NAME, COL_NAME)
    await coll.create_index([("날짜", DESCENDING), ("코드", ASCENDING)], unique=True)
    await coll.create_index("손익계산서q.항목")
    await coll.create_index("손익계산서y.항목")
    await coll.create_index("재무상태표q.항목")
    await coll.create_index("재무상태표y.항목")
    await coll.create_index("현금흐름표q.항목")
    await coll.create_index("현금흐름표y.항목")

    return await _c10346.save(COL_NAME, code, data, client)


async def save_many(many_data: dict[str, C103|None], client: AsyncIOMotorClient) -> list[dict]:
