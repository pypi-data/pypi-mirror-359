...
async def save(code: str, data: C104, client: AsyncIOMotorClient) -> dict:
    coll = get_collection(client, DB_NAME, COL_NAME)
    await coll.create_index([("날짜", DESCENDING), ("코드", ASCENDING)], unique=True)
    await coll.create_index("수익성y.항목")
    await coll.create_index("성장성y.항목")
    await coll.create_index("안정성y.항목")
    await coll.create_index("활동성y.항목")
    await coll.create_index("가치분석y.항목")
    await coll.create_index("수익성q.항목")
    await coll.create_index("성장성q.항목")
    await coll.create_index("안정성q.항목")
    await coll.create_index("활동성q.항목")
    await coll.create_index("가치분석q.항목")
    return await _c10346.save(COL_NAME, code, data, client)


async def save_many(many_data: dict[str, C104|None], client: AsyncIOMotorClient) -> list[dict]:
