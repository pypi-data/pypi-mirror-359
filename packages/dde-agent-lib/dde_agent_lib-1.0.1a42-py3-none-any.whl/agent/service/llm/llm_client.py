...
from agent.utils.nacos_val import getVal
...
class LLMClient:
    ...
    def get_base_common_model(model_name: str = "qwen72b"):
        ...
        logger.info(f"debug get_the_new_nacos_config , {nacos_group}, {nacos_namespace}, {nacos_server_address},  {service_name}")
        system_config = get_system_config_from_nacos()
        logger.info(f"debug system_config: {system_config}")
    ...
    def llm_invoke(self, request: LLMRequest):
        ...
        if not "all" in log_mode and len(message_str) > 60:
            message_str = message_str[:30] + "[...]" + message_str[-30:]
        for url in all_urls:
            model = ""
            filtered_params = {}
            try:
                ...
                if response.choices:
                    ...
                    if is_valid:
                        ...
                        statis_log("normal", "common_api", "default", "llm_client", "invoke_" + url, "fail", "response.choices response invalid", url, rid, model, filtered_params, message_str, f"[{message_len}]{message_str}")
                else:
                    statis_log("normal", "common_api", "default", "llm_client", "invoke_" + url, "fail", "response.choices empty", url, rid, model, filtered_params, message_str, f"[{message_len}]{message_str}")
            except Exception as e:
                statis_log("normal", "common_api", "default", "llm_client", "invoke_" + url, "exception", e, url, rid, model, filtered_params, message_str, f"[{message_len}]{message_str}")
