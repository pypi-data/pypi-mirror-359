...
class RelationshipMapper:
    ...
    def __init__(self):
        ...
        self.metadata_collector = create_metadata_collector()
    ...
    def build_relationship_map(self,
        ...
        ENHANCED: Added removal of existing invalid relationship targets using in-memory data.
        ...
        # STEP 6: Detect all types of relationships (detector will filter for queryable entities)
        logger.info("=== DETECTING NEW RELATIONSHIPS ===")
        all_relationships = []
    ...
    def _is_valid_target_from_entities_map(self, target_name: str, entities_map: Dict[str, Dict]) -> bool:
        """
        ...
        for file_path, file_metadata in schema_metadata.items():
            ...
            for entity in entities:
                if isinstance(entity, dict) and entity.get('kind') == 'Relationship':
                    signature = self.relationship_detector._extract_relationship_signature(entity)
                    if signature:
    ...
    def _build_entities_map(self, schema_metadata: Dict[str, Dict], input_dir: str) -> Dict[str, Dict]:
        ...
        for file_path, file_metadata in schema_metadata.items():
            ...
            logger.info(f"Resolving Command fields for file: {file_path}")
                        ...
                        if command_definition:
                            ...
                            if root_field_kind == 'Query':
                                ...
                                if not command_object_type:
                                    ...
                                    command_object_type = self.metadata_collector._parse_graphql_output_type(
                                        output_type)

                                if command_object_type:
                                    query_commands_for_object_types.add(command_object_type)
                                    logger.info(f"Found Query Command for ObjectType: {command_object_type}")
                            else:
                                logger.debug(
            ...
            logger.info(f"Queryable ObjectTypes found: {queryable_object_types}")
            logger.info(f"  - Via Models: {models_for_object_types}")
            logger.info(f"  - Via Query Commands: {query_commands_for_object_types}")
            ...
            logger.info(f"  - Model only: {model_only}")
            logger.info(f"  - Command only: {command_only}")
            logger.info(f"  - Both Model and Command: {both_model_and_command}")
