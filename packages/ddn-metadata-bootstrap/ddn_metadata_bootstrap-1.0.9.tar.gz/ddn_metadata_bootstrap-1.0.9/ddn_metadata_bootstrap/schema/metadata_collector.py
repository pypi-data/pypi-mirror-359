...
class MetadataCollector:
    ...
    def resolve_command_fields(self, metadata: Dict) -> None:
        ...
        logger.info(f"Resolved fields for {commands_resolved} Commands from their referenced ObjectTypes")
    ...
    def _parse_command_output_type(self, command_entity: Dict) -> Optional[str]:
        ...
        parsed_type = self._parse_graphql_output_type(output_type)

        logger.debug(f"Command {command_entity.get('name')} outputType '{output_type}' -> ObjectType '{parsed_type}'")
        return parsed_type

    @staticmethod
    def _parse_graphql_output_type(output_type: str) -> Optional[str]:
        """
    ...
    def _extract_entity_info(self, data: Dict, entity_name: str, entity_kind: str,
        ...
        if 'definition' in data and isinstance(data['definition'], dict):
            ...
            # For Commands, extract arguments as fields (these are input parameters)
            # Note: The actual output fields will be resolved later from the referenced ObjectType
            # if entity_kind == "Command" and 'arguments' in definition and isinstance(definition['arguments'], list):
            #     for arg in definition['arguments']:
            #         if isinstance(arg, dict) and 'name' in arg:
            #             arg_info = self._extract_field_info(arg)
            #             arg_info['field_role'] = 'command_argument'  # Mark as argument, not output field
            #             fields_collected.append(arg_info)
