...
from deeporigin_data import DeeporiginData, AsyncDeeporiginData, DefaultAioHttpClient
from deeporigin_data._utils import is_dict
...
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncDeeporiginData]:
    ...
    async with AsyncDeeporiginData(
        base_url=base_url, token=token, org_id=org_id, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
