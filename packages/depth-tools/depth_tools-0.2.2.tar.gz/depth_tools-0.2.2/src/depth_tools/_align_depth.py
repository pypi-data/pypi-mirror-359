...
def align_shift_scale(
    *,
    ...
    if len(pred_values) == 0:
        ...
        return np.full(pred_values.shape, np.nan), math.nan, math.nan

    A = np.stack([pred_values, np.ones_like(pred_values)], axis=1)
    b = gt_values

    scale, shift = np.linalg.lstsq(A, b)[0]

    scaled_pred = pred_map.copy()
    scaled_pred[mask] = pred_map[mask] * scale + shift
    return scaled_pred, float(shift), float(scale)


def _verify_align_shift_and_scale_map_args(
    pred_map: np.ndarray, gt_map: np.ndarray, mask: np.ndarray, control_mask: np.ndarray
) -> None:
    """
    ...
    gt_depth
        The ground truth depth. It should have the same width and height as the predicted scalar map. Format: ``Im_Scalar``
    mask
        The mask that selects the valid pixels. It should have the same width and height as the predicted scalar map. Format: ``Im_Mask``.
    algorithm
        The algorithm to use.
    ...
    if not np.issubdtype(pred_map.dtype, np.floating):
        raise ValueError(
    ...
    if not np.issubdtype(gt_map.dtype, np.floating):
        raise ValueError(
    ...
    if not np.issubdtype(control_mask.dtype, np.bool_):
        raise ValueError(
