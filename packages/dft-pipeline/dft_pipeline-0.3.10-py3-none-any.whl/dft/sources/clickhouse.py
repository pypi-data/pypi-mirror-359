        ...
        try:
            ...
            if columns:
                ...
                if data:
                    ...
                    if data_list:
                        # Debug logging
                        self.logger.info(f"Creating table from {len(data_list)} rows with columns: {column_names}")
                        if data_list:
                            self.logger.info(f"First row sample: {data_list[0]}")
                        try:
                            table = pa.table(data_list)
                        except Exception as e:
                            self.logger.error(f"PyArrow table creation failed. Data sample: {data_list[:2] if len(data_list) >= 2 else data_list}")
                            # Fallback: create table with proper schema
                            empty_data = {col_name: [] for col_name in column_names}
                            table = pa.table(empty_data)
                    else:
                        # Data was present but all rows were filtered out somehow
                        empty_data = {col_name: [] for col_name in column_names}
                        table = pa.table(empty_data)
                else:
