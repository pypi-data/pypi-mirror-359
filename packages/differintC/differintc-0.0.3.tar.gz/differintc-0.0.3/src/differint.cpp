    ...
    std::size_t num_points)
{
    ...
    // Zero padding is not needed (vectors are size N)
    ...
    for (int i = 0; i <= N / 2; ++i) {
        // out_prod = out1 * out2 (complex multiply)
    ...
    // Scale and store result (FFTW outputs unnormalized iFFT)
