...
from transformers import AutoModelForTokenClassification, DataCollatorWithPadding, Trainer
...
def get_loss(labels, logits):
    """Get the loss for multi-task classification."""
    #print(labels)
...
class MultiTaskTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        #clf_loss = get_loss(labels[:, 0, :], logits[:, 0, :])
        #span_loss = get_loss(labels[:, 1:, :], logits[:, 1:, :])
        #loss = clf_loss + span_loss
        #print(clf_loss, span_loss)
