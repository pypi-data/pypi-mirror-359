...
def list(name, storage):
    """List available experiments"""
    storage_backend = get_storage(storage)
    ...
    # Check if .doteval exists and fetch ephemeral experiments from it
    if ".doteval" in experiment_names:
        from pathlib import Path

        storage_path = (
            storage.replace("json://", "") if storage.startswith("json://") else storage
        )
        doteval_path = Path(storage_path) / ".doteval"
        if doteval_path.exists():
            doteval_storage = get_storage(f"json://{doteval_path}")
            for exp in doteval_storage.list_experiments():
                ephemeral_experiments.append(f".doteval/{exp}")

    for exp_name in experiment_names:
        # Skip the .doteval directory itself
        if exp_name == ".doteval":
            continue
        elif exp_name.startswith(".doteval/"):
...
def show(experiment_name, storage, evaluation, full, errors):
...
@click.argument("experiment_name")
@click.option("--storage", default="json://.doteval", help="Storage backend path")
def delete(experiment_name, storage):
    ...
    if experiment_name not in experiments:
        console.print(
            f"[red]Experiment '{experiment_name}' not found. Run 'doteval list' to list the available experiments.[/red]"
        )
        return
