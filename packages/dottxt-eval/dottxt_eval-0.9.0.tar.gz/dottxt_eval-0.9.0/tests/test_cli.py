"""Consolidated tests for CLI functionality."""
...
# ============================================================================
# Basic CLI Commands Tests (from test_cli.py)
# ============================================================================
...
# ============================================================================
# CLI Edge Cases and Coverage Tests (from test_cli_coverage.py)
# ============================================================================
...
# ============================================================================
# CLI Retry Options Tests (from test_cli_retry_options.py)
# ============================================================================


def test_cli_retry_options_help():
    """Test that retry options appear in pytest help."""
    ...
    assert "Maximum number of retry attempts" in result.stdout
    assert "Initial delay between retries" in result.stdout


def test_cli_custom_retry_options():
    """Test passing custom retry options via CLI."""
    # Create a test file that uses retry logic
    test_content = """
import asyncio
...
@foreach("question,answer", [("Q1", "A1")])
def eval_with_retries(question, answer):
    global attempt_count
    attempt_count += 1

    # Write attempt count to a file for verification
    with open("attempt_count.txt", "w") as f:
        f.write(str(attempt_count))

    if attempt_count < 2:
        raise ConnectionError("Simulated connection error")

    return Result(prompt=f"Q: {question}", scores=[Score("test", True, [accuracy()])])
"""

    with tempfile.TemporaryDirectory() as temp_dir:
        test_file = Path(temp_dir) / "eval_test_retry.py"
        test_file.write_text(test_content)

        # Run with custom retry options
        result = subprocess.run(
            ["pytest", str(test_file), "-s", "--max-retries=2", "--retry-delay=0.1"],
            capture_output=True,
            text=True,
            cwd=temp_dir,
        )

        # Check that the test passed (meaning retry worked)
        assert "1 passed" in result.stdout

        # Verify retry happened by checking attempt count
        attempt_file = Path(temp_dir) / "attempt_count.txt"
        if attempt_file.exists():
            attempt_count = int(attempt_file.read_text())
            assert attempt_count == 2  # Initial attempt + 1 retry


def test_cli_disable_retry():
    """Test disabling retries via CLI."""
    test_content = """
from doteval import foreach
from doteval.models import Result, Score
from doteval.metrics import accuracy
...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        test_file = Path(temp_dir) / "eval_test_no_retry.py"
        test_file.write_text(test_content)

        # Run with retries disabled
        result = subprocess.run(
            ["pytest", str(test_file), "-s", "--max-retries=0"],
            capture_output=True,
            text=True,
            cwd=temp_dir,
        )

        # The test should complete (with an error recorded)
        assert "1 passed" in result.stdout
        # Should see error handling message
        assert (
            "1 error encountered" in result.stdout or "error" in result.stdout.lower()
        )


def test_cli_async_with_retry_options():
    """Test async evaluation with CLI retry options."""
    test_content = """
import asyncio
from doteval import foreach
from doteval.models import Result, Score
...
@foreach("question,answer", [("Q1", "A1")])
async def eval_async_with_retries(question, answer):
    global attempt_count
    attempt_count += 1

    with open("async_attempt_count.txt", "w") as f:
        f.write(str(attempt_count))

    if attempt_count < 2:
        raise ConnectionError("Simulated async connection error")
...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        test_file = Path(temp_dir) / "eval_async_retry.py"
        test_file.write_text(test_content)

        # Run with custom retry options
        result = subprocess.run(
            ["pytest", str(test_file), "-s", "--max-retries=1", "--retry-delay=0.05"],
            capture_output=True,
            text=True,
            cwd=temp_dir,
        )
        ...
        # Verify async retry happened
        attempt_file = Path(temp_dir) / "async_attempt_count.txt"
        if attempt_file.exists():
            attempt_count = int(attempt_file.read_text())
            assert attempt_count == 2  # Initial attempt + 1 retry
