...
def test_errors_included_in_metrics():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_errors"
        )
        ...
        result = eval_with_errors(session_manager=session_manager)
...
def test_all_errors_zero_accuracy():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_all_errors"
        )
        ...
        result = eval_all_errors(session_manager=session_manager)
...
def test_no_errors_normal_accuracy():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_no_errors"
        )
        ...
        result = eval_no_errors(session_manager=session_manager)
...
def test_mixed_errors_and_successes():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_mixed"
        )
        ...
        result = eval_mixed(session_manager=session_manager)
...
def test_error_preserves_dataset_row():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_preserve_row"
        )
        ...
        result = eval_preserve_row(session_manager=session_manager)
