...
def test_session_interruption_and_resume_basic(temp_storage):
    ...
    session_manager1 = SessionManager(
        f"json://{temp_storage}", experiment_name="test_experiment"
    )
...
def test_session_resumption_with_real_evaluation(temp_storage):
    ...
    session_manager1 = SessionManager(
        f"json://{temp_storage}", experiment_name="large_test_experiment"
    )
...
def test_evaluation_status_during_interruption(temp_storage):
    ...
    session_manager = SessionManager(
        f"json://{temp_storage}", experiment_name="status_test"
    )
...
def test_evaluation_status_detection_for_interrupted_evaluations(temp_storage):
    ...
    session_manager = SessionManager(
        f"json://{temp_storage}", experiment_name="interrupt_test"
    )
...
def test_multiple_experiment_concurrent_access(temp_storage):
    ...
    session_manager1 = SessionManager(
        f"json://{temp_storage}", experiment_name="experiment_a"
    )

    session_manager2 = SessionManager(
        f"json://{temp_storage}", experiment_name="experiment_b"
    )
...
def test_session_resumption_with_errors(temp_storage):
    ...
    session_manager1 = SessionManager(
        f"json://{temp_storage}", experiment_name="error_experiment"
    )
...
def test_session_data_persistence_across_processes(temp_storage):
    ...
    session_manager1 = SessionManager(
        f"json://{temp_storage}", experiment_name="persistent_experiment"
    )
...
def test_session_resumption_with_different_sample_limits(temp_storage):
    ...
    session_manager1 = SessionManager(
        f"json://{temp_storage}", experiment_name="samples_experiment"
    )
    ...
    assert len(result2.results) == 15  # Should have all results
...
def test_evaluation_cleanup_after_completion(temp_storage):
    ...
    session_manager = SessionManager(
        f"json://{temp_storage}", experiment_name="cleanup_experiment"
    )
...
def test_errored_items_retried_on_resume_detailed(temp_storage):
    ...
    session_manager = SessionManager(
        f"json://{temp_storage}", experiment_name="retry_test"
    )

    # Run evaluation - should skip 0 and 2, retry 1, process 3 and 4
    result = eval_retry(session_manager=session_manager)
...
def test_persistent_errors_remain_on_resume(temp_storage):
    ...
    session_manager1 = SessionManager(
        f"json://{temp_storage}", experiment_name="persist_error_test"
    )
    result1 = eval_persist_error(session_manager=session_manager1)
    ...
    result2 = eval_persist_error(session_manager=session_manager2)
...
def test_error_retry_with_connection_retries_enabled(temp_storage):
    ...
    session_manager = SessionManager(
        f"json://{temp_storage}", experiment_name="retry_connection_test"
    )

    # Run with retries enabled
    result = eval_with_retries(session_manager=session_manager, max_retries=3)
