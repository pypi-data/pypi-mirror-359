...
def test_sync_retry_on_connection_error():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_retry"
        )
        ...
        result = eval_with_retry(session_manager=session_manager, max_retries=3)
...
def test_sync_retry_exhaustion():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_exhaustion"
        )
        ...
        result = eval_always_fails(session_manager=session_manager, max_retries=2)
...
def test_sync_no_retry_on_other_errors():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_no_retry"
        )
        ...
        result = eval_with_value_error(session_manager=session_manager, max_retries=3)
...
def test_sync_configurable_retry_parameters():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_custom_retry"
        )
        ...
        result = eval_with_custom_retry(
            session_manager=session_manager, max_retries=1, retry_delay=0.1
        )
...
def test_sync_disable_retry():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_disable_retry"
        )
        ...
        result = eval_no_retry(session_manager=session_manager, max_retries=0)
...
async def test_async_retry_on_connection_error():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_async_retry"
        )
        ...
        result = await async_eval_with_retry(
            session_manager=session_manager, max_retries=3
        )
...
async def test_async_retry_exhaustion():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_async_exhaustion"
        )
        ...
        result = await async_eval_always_fails(
            session_manager=session_manager, max_retries=2
        )
...
async def test_async_no_retry_on_other_errors():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_async_no_retry"
        )
...
async def test_async_concurrent_retries():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_async_concurrent"
        )
        ...
        result = await async_eval_concurrent(
            session_manager=session_manager, max_retries=3, max_concurrency=3
        )
...
def test_retry_on_timeout_error():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_timeout"
        )
        ...
        result = eval_with_timeout(session_manager=session_manager, max_retries=2)
...
def test_retry_on_os_error():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_os_error"
        )
        ...
        result = eval_with_os_error(session_manager=session_manager, max_retries=2)
...
def test_retry_preserves_original_error():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir)
        session_manager = SessionManager(
            f"json://{storage_path}", experiment_name="test_preserve_error"
        )
        ...
        result = eval_with_specific_error(
            session_manager=session_manager, max_retries=1
        )
