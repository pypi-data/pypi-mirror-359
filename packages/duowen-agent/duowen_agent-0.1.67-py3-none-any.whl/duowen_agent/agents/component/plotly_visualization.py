...
class VisualizationModel(BaseModel):
    ...
    class Config:
        arbitrary_types_allowed = True  # 允许非基础类型

    def to_image(self, format="png", width=720, height=330, **kwargs):
        if self.figure is None:
            raise ValueError("figure is None")
        pio.kaleido.scope.mathjax = None  # 禁用 mathjax
        return self.figure.to_image(
            format=format, width=width, height=height, engine="kaleido", **kwargs
        )
...
❗ 返回纯Python代码块，禁止任何解释文本"""
    ...
    def generate_plotly_code(
        self, df: DataFrame, question: str = None, sql: str = None, **kwargs
    ) -> str:
        ...
        _prompt = self._build_prompt().add_user(_user_prompt)
