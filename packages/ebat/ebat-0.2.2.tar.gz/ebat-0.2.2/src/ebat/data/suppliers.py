...
class ResGaitSupplier(Supplier):
    ...
    def _init_users(self):
        all_users = [x for x in range(1, 173)]
        self.config["users"] = random.sample(all_users, self.config["user_num"])
        self._init_adver()
...
class OpenSLRSupplier(Supplier):

    def __init__(self, config):
        super().__init__(config)

        if not (self.data_path / "openslr/").exists():
            self._check_if_downloaded()
    ...
    def _init_users(self):
        users = os.listdir(self.data_path / "openslr")
    ...
    def _load_dataset(self, partition):
        ...
        X, y = [], []
        for user in users:
            session = sorted((openslr_path / user).glob("*"))[
                self.config[partition]["session"]
            ]
