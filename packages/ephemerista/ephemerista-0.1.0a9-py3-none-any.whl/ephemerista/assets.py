...
class Asset(BaseModel):
    ...
    def track(self, asset_ids: UUID4 | list[UUID4] | None = None, constellation_ids: UUID4 | list[UUID4] | None = None):
        """Track one or more assets and/or constellations.

        Parameters
        ----------
        asset_ids : UUID4 or list of UUID4, optional
            Asset IDs to track
        constellation_ids : UUID4 or list of UUID4, optional
            Constellation IDs to track
        """
        if asset_ids is not None:
            if isinstance(asset_ids, uuid.UUID):
                asset_ids = [asset_ids]
