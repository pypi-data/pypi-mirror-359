    ...
    def define_satellites(self) -> dict[str, Keplerian]:
        ...
        for p in range(self.nplanes):
            ...
            for j in range(self._sats_per_plane):
                ...
                satellites[f"Sat_P{p + 1}_{j}"] = Keplerian.from_elements(
                    time=self.time,
                    semi_major_axis=self.semi_major_axis,
                    eccentricity=self.eccentricity,
                    inclination=self.inclination,
    ...
    def define_satellites(self) -> dict[str, Keplerian]:
        ...
        for p in range(self.nplanes):
            for k in range(self._sats_per_plane):
                ...
                satellites[f"Sat_P{p + 1}_{k}"] = Keplerian.from_elements(
                    time=self.time,
                    semi_major_axis=self.semi_major_axis,
                    eccentricity=self.eccentricity,
                    inclination=self.inclination,
...
class Flower(AbstractConstellation):
    ...
    def define_satellites(self) -> dict[str, Keplerian]:
        ...
        for i_sat in range(self.nsats):
            ...
            satellites[f"Sat_{i_sat}"] = Keplerian.from_elements(
                time=self.time,
...
class Constellation(BaseModel):
    ...
    constellation_id: UUID4 = Field(alias="id", default_factory=uuid.uuid4, description="Constellation UUID4")
    ...
    def assets(self) -> list[Asset]:
        ...
        return [
            ...
            for name, k in self.model.satellites.items()
        ]
