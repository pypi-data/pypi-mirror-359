...
// e.g. Nsubjettiness_VISIBILITY static bool SomeClass::verbosity = true; // in SomeClass.cc
        ...
        #define softkiller_VISIBILITY __declspec(dllexport) // Export when building the DLL
    #else
        #define softkiller_VISIBILITY __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define softkiller_VISIBILITY
#endif
