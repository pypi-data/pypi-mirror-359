...
// e.g. Nsubjettiness_VISIBILITY static bool SomeClass::verbosity = true; // in SomeClass.cc
        ...
        #define signalfreebackgroundestimator_VISIBILITY __declspec(dllexport) // Export when building the DLL
    #else
        #define signalfreebackgroundestimator_VISIBILITY __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define signalfreebackgroundestimator_VISIBILITY
#endif
