...
// e.g. Nsubjettiness_VISIBILITY static bool SomeClass::verbosity = true; // in SomeClass.cc
        ...
        #define subjetcounting_VISIBILITY __declspec(dllexport) // Export when building the DLL
    #else
        #define subjetcounting_VISIBILITY __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define subjetcounting_VISIBILITY
#endif
