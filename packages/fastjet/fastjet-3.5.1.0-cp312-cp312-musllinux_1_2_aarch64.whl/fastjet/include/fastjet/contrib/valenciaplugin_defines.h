...
// e.g. Nsubjettiness_VISIBILITY static bool SomeClass::verbosity = true; // in SomeClass.cc
        ...
        #define valenciaplugin_VISIBILITY __declspec(dllexport) // Export when building the DLL
    #else
        #define valenciaplugin_VISIBILITY __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define valenciaplugin_VISIBILITY
#endif
