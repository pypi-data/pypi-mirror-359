...
// e.g. Nsubjettiness_VISIBILITY static bool SomeClass::verbosity = true; // in SomeClass.cc
        ...
        #define constituentsubtractor_VISIBILITY __declspec(dllexport) // Export when building the DLL
    #else
        #define constituentsubtractor_VISIBILITY __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define constituentsubtractor_VISIBILITY
#endif
