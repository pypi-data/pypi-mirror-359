...
class PageSchema(AmisNode):
    ...
    schemaApi: API = None  # If you want to pull through the base, please configure. The return path is
...
class App(Page):
    ...
    type: str = "app"
    api: API = None  # The page configuration base, if you want to pull the page configuration remotely,
...
class Service(AmisNode):
    ...
    className: str = None  # The class name of the outer Dom
    body: SchemaNode = None  # Content container
    api: API = None  # Initialize data domain base address
    ...
    schemaApi: API = None  # Used to get the remote schema base address
    ...
    # base, but it can be overridden here.
    # messages.fetchSuccess: str = None # Toast prompt text when the base request is successful
    # messages.fetchFailed: str = "Initialization failed" # Toast prompt text when base request fails
...
class Nav(AmisNode):
    ...
    source: API = None  # Navigation can be created dynamically via variable or API base
    deferApi: API = None  # The base used to delay loading option details. It can be left unconfigured,
    # and the public source base cannot be configured.
...
class FormItem(AmisNode):
    ...
    class AutoFill(BaseAmisModel):
        ...
        api: API = None  # Automatically populate the base/filter the CRUD request configuration with reference to entry
    ...
    validateApi: API = None  # Form validation base
...
class Form(AmisNode):
    ...
    # will keep polling and request this base until the returned finished attribute is true.
    initFetch: bool = None  # After initApi or initAsyncApi is set, the request will be sent by default, and if it is
    # set to false, the base will not be requested at the beginning
    ...
    asyncApi: API = None  # After setting this property, after the form is submitted and sent to save the base,
    # it will continue to poll and request the base, and it will not end until the returned finished property is
    # true.
...
class Checkboxes(FormItem):
    ...
    addApi: API = None  # Configure the new option base
    editable: bool = None  # False # edit options
    editControls: List[FormItem] = None  # Customize edit form items
    editApi: API = None  # Configure editing options base
    removable: bool = None  # False # remove option
    deleteApi: API = None  # Configure delete option base
...
class ConditionBuilder(FormItem):
    ...
    class Select(Field):
        ...
        # the base will be called, and the update options will be returned according to the base.
...
class GridNav(AmisNode):
    """Grid navigation
    menu navigation, does not support the configuration of the initialization base to initialize the data field,
    so you need to work with similar to Service, Form or CRUD, with the configuration of the base to initialize
...
class InputFile(FormItem):
    ...
    type: str = "input-file"
    receiver: API = None  # Upload file base
    ...
    nameField: str = None  # "name" # Which field is returned by the base to identify the file name
    ...
    autoFill: Dict[str, str] = None  # After the upload is successful, the value returned by the upload base can
...
class Transfer(FormItem):
    ...
    searchApi: API = None  # If you want to retrieve through the base, you can set an api.
...
class CRUD(AmisNode):
    ...
    # when filtering
    source: str = None  # The data mapping base returns the value of a field. If it is not set, the ${items} or
    # ${rows} returned by the base will be used by default. It can also be set to the content of the upper data
    # source.
...
class Chart(AmisNode):
    ...
    className: str = None  # The class name of the outer Dom
    body: SchemaNode = None  # Content container
    api: API = None  # Configuration item base address
    source: dict = None  # Get the variable value in the data chain as configuration through data mapping
    initFetch: bool = None  # whether to request the base when the component is initialized
    interval: int = None  # refresh time (minimum 1000)
...
class Wizard(AmisNode):
    ...
    class Step(AmisNode):
        ...
        api: API = None  # The current step saves the base, which can be left unconfigured.
        initApi: API = None  # The current step data initialization base.
        initFetch: bool = None  # whether the current step data initialization base is initially fetched.
        initFetchOn: Expression = None  # whether the current step data initialization base is initially fetched
    ...
    api: API = None  # The base saved in the last step.
    initApi: API = None  # Initialize data base
