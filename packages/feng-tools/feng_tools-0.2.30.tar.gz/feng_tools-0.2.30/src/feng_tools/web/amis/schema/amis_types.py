...
class AmisAPI(BaseAmisModel):
    url: str  # Current base API address
    ...
    responseData: Dict[str, Any] = None  # If the data structure returned by the base is not as expected, it can
    # be modified by configuring responseData, which also supports data mapping. The data that can be used for
    # mapping is the actual data of the base (the data part returned by the base), plus the api variable.
    # Among them, api.query is the query parameter sent by the base, and api.body is the original data of the
    # content body sent by the base.
    ...
    adaptor: str = None  # Receive adapter, if the base return does not meet the requirements, you can configure
    ...
    autoRefresh: bool = None  # Configure whether to automatically refresh the base.
