...
class FileIO:
    ...
    def load_system_config(self):
        if self.system_config is not None:
            if self.is_debug:
                print('Using provided system config: ')
    ...
    def load_config(self,config_path,ancestry=[]):
        ...
        #     default_config = self.load_config(default_config_path,ancestry+[config_path])
    ...
    def fetch_subconfig(self, config, subconfig_keys):
        ...
        for key in subconfig_keys:
            ...
            if isinstance(subconfig, str):
                ...
                subconfig = self._handle_defaults(subconfig)
        return subconfig
    ...
    def fetch_config_field(self,config, subconfig_keys = [],is_required=True,default_value=None):
    ...
    def save_data(self,datatype, data_args):
        ...
        # if self._can_print_debug_message(datatype):
        #     print(f'Saving data to {data_path} ({datatype.__name__})')
        # save_function(data,data_path)
        # if self._can_print_debug_message(datatype):
            
            
        self._handle_group_permissions_on_save(data_path,datatype=datatype)
    ...
    def create_directory(self,path):
        ...
        self._handle_group_permissions_on_save(dir_path)
        ...
        for i in range(len(path_components)):
            ...
            if os.path.islink(partial_path):
                ...
                if os.path.isabs(symlink_target):
                    ...
                    print('WARNING: see if this code can be consolidated with the other use case')
                else:
    ...
    def _build_relative_path_list(self,datatype,data_args):
        ...
        for entry in datatype.RELATIVE_PATH_TUPLE:
            if entry == FilePathEntries.BASE_DATA_DIRECTORY:
                ...
                if AbstractDatatype.TIMESTAMP_CONFIG_ENTRY_TIMESTAMP in data_args:
                    timestamp = data_args[AbstractDatatype.TIMESTAMP_CONFIG_ENTRY_TIMESTAMP]
    ...
    def is_file_present(self,datatype,data_args):
        ...
        is_file_present = os.path.exists(data_path)
    ...
    def _handle_group_permissions_on_save(self, data_path,datatype=None):
