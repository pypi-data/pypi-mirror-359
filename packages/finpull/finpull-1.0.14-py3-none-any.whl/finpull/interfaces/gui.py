...
class FinancialDataGUI:
    ...
    def setup_data_display(self, parent):
        ...
        columns = [
            ...
            "roe", "change_5y", "beta", "volume", "timestamp", "status"
        ]
    ...
    def add_ticker(self):
        ...
        for ticker in tickers:
            ...
            # Add temporary entry with loading indicator
            temp_values = [ticker, "Loading...", "", "", "", "", "", "", "", "", "", "", "", "", "", "🔄 Loading..."]
    ...
    def update_ticker_status(self, ticker: str, status: str):
        """Update the status column for a specific ticker"""
        for item in self.tree.get_children():
            item_data = self.tree.item(item)
            if item_data['values'][0] == ticker:
                # Update the status column (last column)
                values = list(item_data['values'])
                values[-1] = status  # Status is the last column
    ...
    def _refresh_error(self, error_msg):
        ...
        for item in self.tree.get_children():
            item_data = self.tree.item(item)
            values = list(item_data['values'])
            if values[-1] == "🔄 Loading...":  # Only update if it was loading
                values[-1] = "❌ Error"
            self.tree.item(item, values=values)
...
Built with Python and Tkinter
    ...
    def sort_treeview(self, column):
        ...
        def sort_key(item):
            ...
            if column in ['price', 'market_cap', 'pe_ratio', 'pb_ratio', 'eps_ttm', 
                         'dividend_yield', 'roa', 'roe', 'change_5y', 'beta', 'volume']:
