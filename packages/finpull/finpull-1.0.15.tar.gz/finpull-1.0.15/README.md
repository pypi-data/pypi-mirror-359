...
**Web-Only Installation (Minimal)**
For web applications and JavaScript integration, install with minimal dependencies:
```bash
pip install finpull[web]
```

**Full Installation (Explicit)**
For complete functionality including Excel export:
```bash
...
**Web-Only Installation** is perfect for:
- Web applications using Pyodide/WASM
- JavaScript integration projects
- Minimal server deployments
- When Excel export is not needed

**Full Installation** includes all features and is recommended for:
- Desktop applications
- Complete data analysis workflows
- When Excel export is required
...
### v1.0.15
- **Improved GUI Loading**: Loading indicators now appear directly in ticker column (🔄 AAPL) instead of separate status column
- **Fixed Ticker Column**: Ticker column remains visible during horizontal scrolling for better navigation
- **Cleaner Interface**: Removed separate status column for a more streamlined data view
- **Smart Sorting**: Ticker column sorting now properly handles status symbols
