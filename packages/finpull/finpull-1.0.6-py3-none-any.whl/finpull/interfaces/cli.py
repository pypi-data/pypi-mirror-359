...
class FinancialDataCLI:
    ...
    def __init__(self):
        ...
        print("You can use 'command params' or 'command' then enter params when prompted")
        print()
    ...
    def _handle_add(self, args: List[str]):
        ...
        if args:
            # Tickers provided as arguments
            tickers = [arg.strip() for arg in args if arg.strip()]
        else:
            # Prompt for tickers
            ticker_input = input("Enter ticker symbol(s) (space-separated for multiple): ").strip()
            if not ticker_input:
        ...
        # Validate tickers
        invalid_tickers = [t for t in tickers if not self.scraper.validate_ticker(t)]
        if invalid_tickers:
            print(f"Invalid ticker format(s): {', '.join(invalid_tickers)}")
            valid_tickers = [t for t in tickers if self.scraper.validate_ticker(t)]
            if not valid_tickers:
                return
            tickers = valid_tickers
        
        if len(tickers) == 1:
            # Single ticker - use original method
            ticker = tickers[0]
            try:
            ...
            # Multiple tickers
            try:
                results = self.scraper.add_multiple_tickers(tickers)
                
                if results['added']:
                    print(f"✓ Added: {', '.join(results['added'])}")
                
                if results['already_exists']:
                    print(f"! Already exist: {', '.join(results['already_exists'])}")
    ...
    def _handle_remove(self, args: List[str]):
        ...
        if confirm == 'y':
            try:
                ...
                print(f"✗ Error removing {ticker.upper()}: {e}")
        else:
            print("Cancelled")
    ...
    def _handle_show(self, args: List[str]):
        ...
        if args:
            # Specific ticker provided as argument
            ticker = args[0]
            if not self.scraper.has_ticker(ticker):
                print(f"'{ticker.upper()}' is not being tracked")
                return
    ...
    def _handle_refresh(self, args: List[str]):
    ...
    def _handle_export(self, args: List[str]):
        ...
        if args:
            # Format provided as argument
            if args[0].lower() in ['json', 'csv', 'xlsx']:
                format_type = args[0].lower()
                if len(args) > 1:
                    custom_filename = args[1]
    ...
    def _handle_help(self):
        ...
        print("  add [ticker(s)]  - Add ticker(s) (space-separated for multiple)")
        print("  remove [ticker]  - Remove a ticker")
        print("  show [ticker]    - Show ticker data in beautiful ASCII tables")
        print("                   - Without ticker: choose detailed or summary view")
        print("                   - With ticker: show detailed view for that ticker")
        print("  refresh [ticker] - Refresh data for ticker(s)")
        print("  export [format] [file] - Export data to file")
        ...
        print()
        print("Tips:")
        print("  - Use 'command params' or 'command' then enter params when prompted")
        print("  - Multiple tickers: 'add AAPL GOOGL MSFT' or 'add' then 'AAPL GOOGL MSFT'")
