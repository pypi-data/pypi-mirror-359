...
class FinancialDataScraper:
    ...
    def add_multiple_tickers(self, tickers: List[str]) -> dict:
        """
        ...
        Returns:
            dict: Results with 'added', 'already_exists', 'failed' lists
        """
        results = {
            'added': [],
            'already_exists': [],
        ...
        for ticker in tickers:
            ...
            if not self.validate_ticker(ticker):
                results['failed'].append({'ticker': ticker, 'error': 'Invalid ticker format'})
                continue
        ...
        # Add tickers to storage
        try:
            added_tickers = self.storage.add_multiple_tickers(valid_tickers)
        except Exception as e:
            # If storage fails, mark all as failed
            for ticker in valid_tickers:
                results['failed'].append({'ticker': ticker, 'error': f'Storage error: {e}'})
            return results
        
        # Fetch data for each added ticker
        for ticker in added_tickers:
            try:
                ...
                logger.error(f"Failed to fetch data for {ticker}: {e}")
                # Remove ticker if we can't fetch data
                try:
    ...
    def validate_ticker(self, ticker: str) -> bool:
        ...
        if len(ticker) > 10:  # Most tickers are 1-5 characters
            return False
        if not ticker.replace('.', '').replace('-', '').isalnum():  # Allow dots and hyphens
            return False
