...
def create_parser():
    ...
    remove_parser.add_argument('tickers', nargs='+', help='One or more ticker symbols to remove')
    
    # Show command
    show_parser = subparsers.add_parser('show', help='Show ticker data (auto-fetches if not present)')
    show_parser.add_argument('tickers', nargs='*', help='Tickers to show (will auto-add if not tracked)')
    ...
    export_parser.add_argument('path', nargs='?', help='Export file path (optional)')
    
    # Format options (mutually exclusive)
    format_group = export_parser.add_mutually_exclusive_group()
    format_group.add_argument('--json', '-j', action='store_true', help='Export as JSON')
    format_group.add_argument('--csv', '-c', action='store_true', help='Export as CSV') 
    format_group.add_argument('--xlsx', '-x', action='store_true', help='Export as Excel')
    
    # Legacy format argument for backwards compatibility
    export_parser.add_argument('--format', choices=['json', 'csv', 'xlsx'], help='Export format (legacy)')
    export_parser.add_argument('--output', '-o', help='Output file path (legacy)')
    ...
    try:
        if args.command == 'add':
            ...
            try:
                ...
                output_path = args.path or args.output
                if output_path:
                    import os
