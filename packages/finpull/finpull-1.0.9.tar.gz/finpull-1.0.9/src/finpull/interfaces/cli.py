...
class FinancialDataCLI:
    ...
    def _command_add(self, tickers: List[str]):
        ...
        for tk in tickers:
            if not self.scraper.validate_ticker(tk):
                print(f"'{tk}' is not a valid ticker symbol")
                continue
            try:
                ...
                print(f"✗ Error adding {tk.upper()}: {e}")
    ...
    def _command_show(self, args: List[str]):
        """Show tickers; if args provided, behave like non-interactive show"""
        if not args:
            self._handle_show()
            return
        for tk in args:
    ...
    def _handle_help(self):
        ...
        print("           - Format flags: --json/-j, --csv/-c, --xlsx/-x")
        print("           - Usage: export [path] [--json|--csv|--xlsx]")
        print("           - Example: export ~/Desktop/data.csv --csv")
