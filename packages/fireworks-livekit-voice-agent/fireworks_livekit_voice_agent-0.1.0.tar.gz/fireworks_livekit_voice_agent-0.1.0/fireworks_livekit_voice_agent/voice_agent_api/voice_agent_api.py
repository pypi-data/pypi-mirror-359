...
FIREWORKS_BASE_URL = "wss://audio-agent.link.fireworks.ai/v1/audio/agent"
...
class RealtimeSession(llm.RealtimeSession[Literal["agent_interrupted",]]):
    ...
    async def _create_ws_conn(self) -> aiohttp.ClientWebSocketResponse:
        ...
        url = process_base_url(self._realtime_model._opts.base_url)
        if lk_fw_debug:
    ...
    async def _handle_fireworks_event(self, event: AgentEgress):
        ...
        async with self._generation_lock:
            if isinstance(event, AgentOutputTranscript):
                ...
                logger.debug(f"Handling AgentOutputToolCall with {len(event.tool_calls)} tool(s).")
                if not self._current_generation:
                    logger.warning("Received tool call without an active generation, ignoring.")
                    return

                for tool_call in event.tool_calls:
                    self._emit_function_call(tool_call)
    ...
    async def _handle_fireworks_binary(self, data: bytes):
        async with self._generation_lock:
            ...
            if len(self._output_audio_buffer) > 0:
                padding_needed = TARGET_CHUNK_SIZE_BYTES - len(self._output_audio_buffer)
                if padding_needed > 0:
                    self._output_audio_buffer.extend(b"\x00" * padding_needed)
