...
class FunctionsObject:
    ...
    def youtube_playlist_download(self, url: str, regime: str = 'audio') -> Union[list[dict[str, Union[str, bytes]]], Exception]:
        """Функция для скачивания элементов из плейлиста с YouTube.\nurl: ссылка на плейлист.\nregime: что скачивать: аудио, или видео?\nВозвращает список, а точнее `list[dict[str, str | bytes]]`."""
        from pytubefix import Playlist
        from tqdm import tqdm
        
        playlist = Playlist(url, proxies=self.proxies)
        videos: list[dict[str, Union[str, bytes]]] = []
        ...
        if regime == 'video':
            for video in tqdm(playlist.videos, 'Скачиваем видео..', ncols=70):
                ...
                if video.age_restricted:
                    continue
                video.streams.get_lowest_resolution().stream_to_buffer(buffer)
                videos.append({"author":video.author, 'metadata':video.metadata, 'keywords':video.keywords, 'desc':video.description, 'title':video.title, 'length':video.length, 'video':buffer.getvalue()})
            return videos
        elif regime == 'audio':
            for audio in tqdm(playlist.videos, desc='Скачиваем аудио..', ncols=70):
                ...
                videos.append({"author":audio.author, 'metadata':audio.metadata, 'keywords':audio.keywords, 'desc':audio.description, 'title':audio.title, 'length':audio.length, 'audio':buffer.getvalue()})
            return videos
        else:
            raise Exception('Ты не правильный режим указал. ТОЛЬКО VIDEO И AUDIO!')
...
class AsyncFunctionsObject:
    ...
    async def deepseek_v3_0324(self, prompt: str, max_tokens: int = 4096, expression: str = 'auto', proxy: str = None):
        ...
        if expression == 'auto':
            ...
            return 'expression указан неверно! auto, toxic, либо non-toxic!'
    async def youtube_playlist_download(self, url: str, regime: str = 'audio') -> Union[list[dict[str, Union[str, bytes]]], Exception]:
        """Функция для скачивания элементов из плейлиста с YouTube.\nurl: ссылка на плейлист.\nregime: что скачивать: аудио, или видео?\nВозвращает список, а точнее `list[dict[str, str | bytes]]`."""
