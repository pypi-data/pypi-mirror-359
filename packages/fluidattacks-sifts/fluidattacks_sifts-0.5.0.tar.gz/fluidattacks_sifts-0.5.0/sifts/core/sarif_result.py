...
from sifts.io.db.dynamodb import get_snippet_by_hash
...
def render_snippet(snippet: SnippetFacet, focus_line: int) -> str:
    snippet_content: str = snippet.snippet_content
...
async def _get_context_region(
    ...
    snippet = await get_snippet_by_hash(
        group_name=vulnerability.group_name,
        root_id=vulnerability.root_id,
        file_path=vulnerability.file_path,
        snippet_hash=vulnerability.snippet_hash,
    )
    if snippet:
        region = sarif_om.Region(
            ...
            start_column=snippet.column_start,
            end_column=snippet.column_end,
                ...
                sarif_om.Location(
                    physical_location=sarif_om.PhysicalLocation(
                        artifact_location=sarif_om.ArtifactLocation(
                            uri=vulnerability.file_path,
                        ),
                        region=sarif_om.Region(
                            start_line=x,
                            source_language=snippet.language,
                        ),
                        context_region=await _get_context_region(vulnerability, x),
                    ),
                )
