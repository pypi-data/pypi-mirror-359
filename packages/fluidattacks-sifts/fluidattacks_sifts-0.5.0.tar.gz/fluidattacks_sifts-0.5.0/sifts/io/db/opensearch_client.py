...
async def search_similar_vulnerabilities_by_knn_code(
    *,
    ...
    result = await retry_on_exceptions(
        ...
        index="vulnerabilities_candidates_v2",
        body=body,
    )
    return cast(
        "list[SnippetHit]",
...
async def setup_opensearch_client(*, new_index: bool = False) -> AsyncOpenSearch:
    session = boto3.Session()
    return AsyncOpenSearch(
        transport_class=AsyncTransport,
        connection_class=AsyncHttpConnection,
        hosts=[
            FI_AWS_OPENSEARCH_HOST_SIFTS if new_index else FI_AWS_OPENSEARCH_HOST,
        ],
