...
def check_for_duplicate_combinations(df: pd.DataFrame, unique_cols: t.List[str], df_name: str = "DataFrame") -> None:
    ...
    if not duplicate_combinations.empty:
        ...
        # Get first 10 rows that have duplicate combinations
        duplicate_rows = df[df.set_index(unique_cols).index.isin(duplicate_combinations.index)].head(10)
