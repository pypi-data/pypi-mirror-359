...
from frogml.sdk.model_version.utils.validations import _validate_load_model
...
def _log_model(
    ...
    project_key: str = jml_customer_client.get_project_key(repository)
    ...
    properties = merge_properties_parameters_metrics(metrics, parameters, properties)
    dependencies = _dependency_files_handler(
        dependencies=dependencies, target_dir=target_dir
    )
    metadata = _get_model_metadata(
        model_flavor, framework_version, serialization_format
    )
    zipped_code_path = build_zip_code_path(
