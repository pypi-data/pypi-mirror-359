...
def load_espnet_asr_model(
        asr_base_path: str,
        asr_train_config: str,
        asr_model_file: str = None,
        device: str = 'cpu'):
    """Load an ESPnet ASR model from a config file and optionally a model file.

    Args:
        asr_base_path (str): Base path to the ASR model directory.
        asr_train_config (str): Path to the ASR training configuration file.
        asr_model_file (str, optional): Path to the ASR model file. Defaults to None.
        device (str, optional): Device to load the model on ('cpu' or 'cuda'). Defaults to 'cpu'.

    Returns:
        Tuple[torch.nn.Module, argparse.Namespace]: The loaded ASR model and its arguments.
    """
    from pathlib import Path
    from os import path
    import yaml
    ...
    config_file = Path(asr_train_config)
    with config_file.open("r", encoding="utf-8") as f:
        args = yaml.safe_load(f)
    args = argparse.Namespace(**args)
    # args.normalize_conf['stats_file'] = path.join(asr_base_path, args.normalize_conf['stats_file'])
    model = ASRTask.build_model(args)
    model.to(device)
    if asr_model_file is not None:
        model.load_state_dict(torch.load(asr_model_file, map_location=device))
...
class Speech2Text:
    ...
    def __init__(
        self,
        asr_base_path: Union[Path, str] = None,
        ...
        # )
        asr_model, asr_train_args = load_espnet_asr_model(
