...
class FURSBaseAPI(object):
    ...
    def _sign(self, content, algorithm=hashes.SHA256()):
        # if not hasattr(self.connector, 'p12') or self.connector.p12 is None or not hasattr(self.connector.p12, 'key') or self.connector.p12.key is None:
        #     raise ValueError("Connector's p12 or p12.key is not initialized. Cannot sign content.")
        print(str(type(self.connector.p12)))
