...
def create_tree_for_trace(records: Sequence[BaseStep]) -> Sequence[Trace]:
    ...
    for record in records:
        ...
        if record.type is StepType.trace:
            trace_id_to_trace[record.id] = Trace(**record.__dict__)
        elif record.type is not StepType.session:
            span_id_to_span[record.id] = cast(Span, record)
            if record.parent_id is not None:  # Only add value to the parent-child relationship if there is a parent
...
def create_tree_for_session(records: Sequence[BaseStep]) -> List[Session]:
    ...
    for record in records:
        ...
        if record.type is StepType.session:
            session_id_to_session[record.id] = Session(**record.__dict__)
        else:
            if record.session_id is not None:  # Only add value to the Session tree if part of a session
