...
def identify_software_version(banner):
    ...
    for regex, (vendor, product) in patterns.items():
        ...
        if m:
            version = m.group(1)
            # If vendor is not specified, use product as vendor
            if not vendor:
                vendor = product = m.group(1).lower()
...
def vuln_scanner():
    ...
    path_net_scan_report = path_cwd + "/reports/gh_network_scan_report.json"

    with open(path_net_scan_report, "r") as nsr:
        report = json.load(nsr)
        # console.print(f"Report: {report}, Report type: {type(report)}\n")

    for target in track(report, description="Analyzing vulnerabilities..."):
        # console.print(f"Target: {target}\n")
        # console.print(f"Target ip: {target['target']}\n")
        for port in target["ports"]:
            # console.print(f"Port number: {port['port_number']}\n")
    ...
    path_vuln_scan_report = path_cwd + "/reports/gh_vuln_scan_report.json"

    # if not os.path.exists(path_vuln_scan_report):
    #     os.makedirs(path_vuln_scan_report)
