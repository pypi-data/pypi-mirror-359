...
def group_endpoints_by_operation(paths: Dict[str, Dict]) -> Dict[str, List[Dict]]:
    """Group API endpoints by operation group."""
    grouped_operations = {}
    for path, methods in paths.items():
        for method, details in methods.items():
            operation_group = details.get('x-operation-group')
            if operation_group in SUPPORTED_OPERATIONS:
                if operation_group not in grouped_operations:
                    grouped_operations[operation_group] = []
                grouped_operations[operation_group].append(
...
def extract_parameters(endpoints: list[dict]) -> tuple[dict[str, dict], set, set]:
    """Extract parameters from all endpoints and determine which are required."""
    all_parameters = {}
    path_parameters = set()
    param_to_endpoints = {}  # Track which endpoints use each parameter

    # Collect parameters from all endpoints
    for i, endpoint in enumerate(endpoints):
        ...
        for param in path_params:
            ...
            all_parameters[param] = {'type': 'string', 'title': param.title()}
            if param not in param_to_endpoints:
                param_to_endpoints[param] = set()
            param_to_endpoints[param].add(i)
        ...
        for param in parameters:
            if 'name' in param:
                ...
                if param_name not in param_to_endpoints:
                    param_to_endpoints[param_name] = set()
                param_to_endpoints[param_name].add(i)

    required_parameters = set()
    op_group = endpoints[0]['details'].get('x-operation-group', '')

    # Check if we need to add body parameter
    ...
    async def tool_func(params: BaseModel) -> list[TextContent]:
        try:
            ...
            try:
                ...
                # Handle common OpenSearch connection errors
                error_msg = str(transport_error)
                
                # Check for common HTML response errors
                if 'unknown mimetype' in error_msg.lower() or 'text/html' in error_msg.lower():
                    return [TextContent(
                        type='text', 
                        text=f'OpenSearch authentication error: Server returned HTML instead of JSON. '
                             f'This usually indicates:\n'
                             f'1. Invalid credentials (check OPENSEARCH_USERNAME/PASSWORD)\n'
                             f'2. Server requires different authentication method\n'
                             f'3. URL redirects to login page\n'
                             f'4. Server is not accessible or misconfigured\n\n'
                ...
                if 'connection' in error_msg.lower() or 'timeout' in error_msg.lower():
                    return [TextContent(
                        type='text',
                        text=f'OpenSearch connection error: Unable to connect to server. '
                             f'Please check:\n'
                             f'1. OPENSEARCH_URL is correct and accessible\n'
                             f'2. Network connectivity\n'
                             f'3. SSL/TLS settings (try OPENSEARCH_SSL_VERIFY=false for testing)\n\n'
                             f'Original error: {error_msg}'
                    )]
                ...
                if 'ssl' in error_msg.lower() or 'certificate' in error_msg.lower():
                    return [TextContent(
                        type='text',
                        text=f'OpenSearch SSL/TLS error: Certificate verification failed. '
                             f'Try setting OPENSEARCH_SSL_VERIFY=false for testing or '
                             f'ensure proper SSL certificates are configured.\n\n'
                             f'Original error: {error_msg}'
                    )]
                
                # For other errors, return the original message with context
                return [TextContent(
                    type='text',
                    text=f'OpenSearch API error: {error_msg}\n\n'
                         f'Request details:\n'
                         f'- Method: {method}\n'
                         f'- Path: {api_path}\n'
