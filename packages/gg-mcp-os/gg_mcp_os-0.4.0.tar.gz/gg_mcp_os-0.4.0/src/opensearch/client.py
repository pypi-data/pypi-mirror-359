...
def initialize_client_with_cluster(cluster_info: ClusterInfo = None) -> OpenSearch:
    ...
    # Diagnose connection before attempting to create client
    logger.info(f"Diagnosing connection to {opensearch_url}...")
    success, diagnosis_msg = diagnose_connection(opensearch_url, opensearch_username, opensearch_password)
    
    if not success:
        logger.error(f"Connection diagnosis failed: {diagnosis_msg}")
        raise RuntimeError(f"OpenSearch connection failed: {diagnosis_msg}")
    else:
