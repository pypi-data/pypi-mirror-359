...
class ApiYamlBase(RequestBase):
    ...
    def __init__(self, service=None, account_key="admin", extra_headers: dict = None, **kwargs):
        ...
        self.kwargs = kwargs
        super().__init__(base_url=self.config["HOST"], expect_code="0000")
    ...
    def _request(self, method, url, **kwargs):
        ...
        if (response.status_code == settings.UNAUTHORIZED and self.kwargs.get('is_retry_login', True)
                and self.kwargs.get('need_login',True)):
            # token过期或未认证，默认需要重新登陆，特殊场景不需要，在用例层传入is_retry_login=False，代表期望过期
            # 也需要判断是否需要登录，不需要登录的也不走重新登录，如果nedd_login=True也要走重新认证
