...
class Chrome(Windows, Terminal):
    ...
    def cerrar_navegador(self) -> None:
        ...
        # Cierra los procesos de Chrome y Chromedriver
        # Esto es necesario para evitar que queden procesos huérfanos
        import subprocess
        subprocess.run(["taskkill", "/f", "/im", "chrome.exe"], capture_output=True, shell=True)
        subprocess.run(["taskkill", "/f", "/im", "msedge.exe"], capture_output=True, shell=True)
        subprocess.run(["taskkill", "/f", "/im", "chromedriver.exe"], capture_output=True, shell=True)
        subprocess.run(["taskkill", "/f", "/im", "msedgedriver.exe"], capture_output=True, shell=True)
    ...
    def _obtener_PID_activo(self):
        ...
        chrome_pid = self._driver.service.process.pid if hasattr(self._driver, "service") else None
        if not chrome_pid:
    ...
    def buscar_elemento_en_iframes(self, metodo_busqueda:By|str, texto_busqueda:str) -> tuple[WebElement | None, str | None]:
        ...
        iframes = self.encontrar_elementos(By.TAG_NAME, "iframe")
        if iframes:
            for iframe in iframes:
                try:
                    self._driver.switch_to.frame(iframe)
                    elementos = self.encontrar_elementos(metodo_busqueda, texto_busqueda)
                    if elementos:
