...
def pytest_configure(config):
    ...
    loopback_ep = entry_points(name="nx_loopback", group="graphdict.backends")
    if not loopback_ep:
        warnings.warn(
            "\n\n             WARNING: Mixed NetworkX configuration! \n\n"
            "        This environment has mixed configuration for graphdict.\n"
    ...
    if backend:
        # We will update `graphdict.config.backend_priority` below in `*_modify_items`
