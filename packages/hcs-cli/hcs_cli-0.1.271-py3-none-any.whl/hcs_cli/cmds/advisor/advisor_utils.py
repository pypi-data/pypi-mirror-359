...
def calculate_hours_from_history(history_values: List[int], timeslot_ms: int) -> float:
    """
    Calculate total hours from a list of history values and timeslot in milliseconds.
...
def calculate_usage_metrics(template_info: dict) -> List[List[str]]:
    ...
    if not template_info or "meta" not in template_info:
        return [
            ...
            ["Utilized VMs Hours", "N/A", "Unknown"],
            ["Idle VMs Hours", "N/A", "Unknown"],
    ...
    # Calculate metrics
    allocated_hours = calculate_hours_from_history(history.get("maxCapacity", []), timeslot_ms)
    idle_hours = calculate_hours_from_history(history.get("minFree", []), timeslot_ms)

    # Placeholder values for now
    powered_on_hours = 750  # TODO: Calculate from real data
    utilized_hours = 250  # TODO: Calculate from real data
    ...
    metrics = [
        ...
        [
            "Utilized VMs Hours",
            f"{utilized_hours:,.0f}",
            determine_status("Utilized VMs Hours", utilized_hours, allocated_hours),
        ],
        ["Idle VMs Hours", f"{idle_hours:,.0f}", determine_status("Idle VMs Hours", idle_hours, allocated_hours)],
        [
