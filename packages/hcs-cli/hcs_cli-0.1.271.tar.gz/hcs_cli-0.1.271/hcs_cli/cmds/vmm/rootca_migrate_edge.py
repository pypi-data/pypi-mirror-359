...
def migrate(org: str, edge_ids: str, ca_label: str, file: str, run_id: str, verbose: bool):
    ...
    logger.info(f"Starting one-time edge migration:\n")
    logger.info(f"default timeout for API calls: {os.environ.get('HCS_TIMEOUT', 30)} seconds")
    if file:
        logger.info(f"input file specified. file input takes the precedence. processing the file now")
        f = open(file)
        data = None
        try:
            data = json.load(f)
...
def migrate_edges(target_org_id: str, edge_ids: str, ca_label: str, verbose: bool):
    ...
    if not org:
        ...
        return
    logger.info(
        f"Migrating edges: target_org_id = {target_org_id}, edge_ids = {edge_ids}, ca_label = {ca_label}, verbose = {verbose}"
    )
    if edge_ids:
    ...
    for edge in edges:
        ...
        logger.info(f"one-time edge migration on org_id: %s, edge_id: %s", target_org_id, edge_id)
        stats = dict()
        ...
        try:
            logger.info(f"performing one-time edge migration on org_id: %s, edge_id: %s", target_org_id, edge_id)
            ...
            if edge.get("fqdnAutoGenerated") and edge.get("fqdnAutoGenerated") == True:
                if ".omnissa." in edge_mqtt_url:
                    ...
                    try:
                        ...
                        if is_sync_complete:
                            ...
                            stats["info"] = (
                                "Edge FQDN is updated but edge templates' sync not completed on all edge templates. "
                            )
            ...
            regional_mqtt_url = data_util.deep_get_attr(
            ...
            if edge_calabel and edge_calabel.get("caLabel") and edge_calabel.get("caLabel") == ca_label:
                ...
                logger.info(f"sleep 7 minutes for cache refresh in PKI")
                time.sleep(60 * 7)
            ...
            logger.error(
                f"Failed one-time edge migration on org_id: %s, edge_id: %s due to {str(error)}", target_org_id, edge_id
            )
            stats["info"] = str(error)
            send_event(stats, HocStatus.FAILURE.value, verbose)
            return
    logger.info(f"Finished one-time edge migration on org_id: %s", target_org_id)
        ...
        if uags_dict and uags_dict["content"] and uags_dict["content"][0]["id"]:
            ...
            if res.results:
                for key, value in res.results.items():
                    ...
                    if (
                        value
                        and value.get("diagnosticData")
                        and value.get("diagnosticData").startswith("Enable succeeded")
                    ):
...
def send_event(stats: dict, status: str, verbose: bool):
    ...
    if (
        ...
        and (
            ...
            and (
                stats.get("edge_pki_ca_label_mig_status") == "MIGRATED"
                or stats.get("edge_pki_ca_label_mig_status") == "ALREADY_MIGRATED"
            )
        )
