...
def prepare_vm_utilization_chart_data(template_info: dict) -> dict:
    ...
    max_capacity = history.get("maxCapacity", [])

    if start_timestamp and timeslot_ms and max_capacity:
        # Get full data arrays first (no sampling)
        provisioned_vms = max_capacity
        powered_on_vms = history.get("poweredOnVMs", [])
        idle_vms = history.get("minFree", [])

        # Ensure all data arrays have the same length as max_capacity
        data_length = len(max_capacity)
        powered_on_vms = powered_on_vms[:data_length]
        idle_vms = idle_vms[:data_length]

        # Pad shorter arrays with zeros if needed
        while len(powered_on_vms) < data_length:
            powered_on_vms.append(0)
        while len(idle_vms) < data_length:
            idle_vms.append(0)
...
def _get_pool_html_template(
    ...
    if template_info and "meta" in template_info:
        ...
        if start_timestamp and "maxCapacity" in history:
    ...
    <script>
        window.addEventListener('DOMContentLoaded', function () {
            ...
            var option = {
                ...
                legend: { data: ['Allocated VMs', 'Idle VMs'] },
