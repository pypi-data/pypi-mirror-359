...
def _format_template_value(value):
    ...
    if isinstance(value, (str, int, float, bool)):
        ...
        if "history" in value:
            ...
            for key in ["maxCapacity", "minFree"]:
        ...
        if "prediction" in value:
            ...
            for key in ["maxCapacity", "minFree", "idealCapacity"]:
...
def _format_meta_data(meta_data):
    ...
    if "history" in meta_data:
        ...
        if "startTimestamp" in history:
            ...
            len_history = len(history["maxCapacity"])
...
def create_pool_pdf_report(
    org_id: str, resource_id: str, resource_type: str, template_info: dict = None, filename: str = None
):
    ...
    try:
        ...
        doc = SimpleDocTemplate(
            filename, pagesize=landscape(letter), rightMargin=36, leftMargin=36, topMargin=36, bottomMargin=36
        )
        ...
        content.append(
            Paragraph(f"Generated on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", normal_style)
        )
        ...
        if template_info and "meta" in template_info:
            ...
            if "history" in meta_data:
                history = meta_data["history"]
                if "startTimestamp" in history and "maxCapacity" in history and "minFree" in history:
                    start_time = datetime.datetime.fromtimestamp(history["startTimestamp"] / 1000)
                    len_history = len(history["maxCapacity"])
                    ...
                    end_time_full = start_time + datetime.timedelta(
                        seconds=len_history * meta_data["timeslotMs"] / 1000
                    )

                    # if poweredOnVMs is available in history, then show the demo usage usage in full view chart. if poweredOnVMs not available, then it is not demo usage
                    if "poweredOnVMs" in history:
                        half_day_hours = 12
                        time_slots_full = [
                            start_time + datetime.timedelta(minutes=30 * i) for i in range(half_day_hours * 2)
                        ]
                        end_time_full = start_time + datetime.timedelta(hours=half_day_hours)
                        provisioned_vms_full = history["maxCapacity"][: half_day_hours * 2]
                        powered_on_vms_full = history["poweredOnVMs"][: half_day_hours * 2]
                        idle_vms_full = history["minFree"][: half_day_hours * 2]
                        used_vms_full = [p - i for p, i in zip(powered_on_vms_full, idle_vms_full)]
                        plt.fill_between(
                            time_slots_full, provisioned_vms_full, color="blue", alpha=0.5, label="Allocated VMs"
                        )
                        plt.fill_between(
                            time_slots_full, powered_on_vms_full, color="red", alpha=0.5, label="Powered-on VMs"
                        )
                        plt.fill_between(time_slots_full, used_vms_full, color="green", alpha=0.5, label="Utilized VMs")
                    else:
                        provisioned_vms_full = history["maxCapacity"]
                        idle_vms_full = history["minFree"]
                        plt.fill_between(
                            time_slots_full, provisioned_vms_full, color="blue", alpha=0.5, label="Allocated VMs"
                        )
                        plt.fill_between(time_slots_full, idle_vms_full, color="red", alpha=0.5, label="Idle VMs")
                    plt.title("VM Utilization Trends - Full View (Every 30 Minutes Sampling)")
                    plt.xlabel("Time")
                    plt.ylabel("VM Count")
                    plt.grid(True)
                    ...
                    provisioned_vms_daily_avg = [
                        sum(history["maxCapacity"][i * 48 : (i + 1) * 48]) / 48 for i in range(num_days)
                    ]
                    idle_vms_daily_avg = [sum(history["minFree"][i * 48 : (i + 1) * 48]) / 48 for i in range(num_days)]
                    plt.fill_between(
                        time_slots_daily, provisioned_vms_daily_avg, color="blue", alpha=0.5, label="Allocated VMs"
                    )
                    plt.fill_between(time_slots_daily, idle_vms_daily_avg, color="red", alpha=0.5, label="Idle VMs")
...
def create_org_pdf_report(org_details: dict, all_recommendations: list, filename: str = None):
    ...
    table_content_style = ParagraphStyle(
        "TableContent", parent=styles["Normal"], fontSize=9, leading=11, wordWrap="CJK"
    )
