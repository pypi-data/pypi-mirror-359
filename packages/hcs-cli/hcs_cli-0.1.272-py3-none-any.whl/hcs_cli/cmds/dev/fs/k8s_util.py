...
def kubectl(command: str, ignore_error: bool = False, get_json: bool = False):
    if get_json and ignore_error:
        fail("get_json=True and ignore_error=True are mutually exclusive. Please set one of them to False.")
    if get_json:
        command.find("-ojson") == -1 and command.find("--output=json") == -1
        command += " -ojson"
    ret = exec(_kubectl_command + " " + command, logOnError=not ignore_error, raise_on_error=not ignore_error)
    if get_json:
        try:
            import json
...
def validate_kubectl_config(fs_name: str, try_default_config: bool = True, raise_on_error: bool = True):
    ...
    if output == fs_name:
        log.good("kubectl config context")
        good = True
    else:
