...
def check_omnissa_privatelink_dns_records(id: str, org: str, verbose: bool):
    ...
    for line in lines:
        if (
            "sg-uag-module" in line
            or "mqtt-server-0" in line
            or "infra-azure-module" in line
            or "token-generator" in line
        ):
            ...
            if status == "Running":
                ...
                continue
    if status != "Running":
        return f"Failed to find running telemetry-server or mqtt-server pods"
...
def edge_get_uag_mgmt_certificates(edge_id: str, org: str, verbose: bool):
    """Copy privatelink edge dns records"""
    org_id = cli.get_org_id(org)
    edge_id = recent.require(edge_id, "edge")
    ...
    if uags_dict and uags_dict["content"] and uags_dict["content"][0]["id"]:
        ...
        for uag_gateway in gateways:
            log.info(
                f"Get cert on all uag vms; org_id: {org_id}, edge_id: {edge_id}, uag_id: {uag_id}, uag_gateway: {uag_gateway}"
            )
            ...
            if res and res.get("diagnosticData") and res.get("diagnosticData").startswith("Enable succeeded"):
                ...
                if verbose:
                    log.info(
                        f"cert on org_id: {org_id}, edge_id: {edge_id}, uag_id: {uag_id}, uag_gateway: {uag_gateway}:\n{cert}"
                    )
