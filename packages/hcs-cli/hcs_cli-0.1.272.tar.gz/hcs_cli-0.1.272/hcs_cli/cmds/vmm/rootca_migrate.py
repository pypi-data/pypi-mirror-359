...
def migrate_template_vms(
    ...
    if len(prolonged_powering_on_vms) > 0:
        ...
        logger.info(
            f"as a final attempt, sleep for {SLEEP_TIME_ON_PROLONGED_POWERON_VMS} more minutes for Vms to powerOn"
        )
        time.sleep(60 * SLEEP_TIME_ON_PROLONGED_POWERON_VMS)
        try:
            # perform migration only on prolonged powerOn vms
            perform_migration(
                template_key_dict, prolonged_powering_on_vms.values(), target_org_id, template_id, page_num, verbose
            )
        except Exception as error:
...
def perform_migration(
    template_key_dict: dict, vms: dict, target_org_id: str, template_id: str, batch_num: int, verbose: bool
):
    ...
    for vm in vms:
        ...
        if (
            ...
            and (
                (
                    EDGE_FQDN_AUTO_GENERATED in template_key_dict
                    and template_key_dict.get(EDGE_FQDN_AUTO_GENERATED) == False
                )
                or (
    ...
    for vm in migrate_vms:
        ...
        if prolonged_powering_on_vms.get(vm["id"]) is not None:
            ...
            logger.info(
                f"vm {vm['id']}: has powered-on, and moved from prolonged_powering_on_vms list to migrate_vms list"
            )
