                        ...
                        for key, versions in itertools.groupby(sorted(dms_vers(), key=keyfunc), key=keyfunc):
                            ...
                            async with DesktopObjectActionLifecycle(request=request,
                                ...
                                if not archived or first:
                                    ...
                                    raise response.status_bad_request(f'{awsservicelib._activity_object_display_name(bucket_name,key)} cannot be restored because one or more of its versions is archived and there are more recent versions of the object.')
