...
class MongoSortOrder(Enum):
    ...
    def from_request_dict(request: Request, default_attr: str | None = None,
                          default_sort_order: Optional['MongoSortOrder'] = None) -> SortDict | None:
        """
        ...
        :param default_sort_order: the default sort order to use if no sort order is present. If omitted, the default
        sort order is ascending.
        :return: a dict of attribute names to 1 (ascending) or -1 (descending), or None if no sort order is specified.
        """
        return {attr_: order.value for attr_, order in MongoSortOrder.from_request(request, default_attr,
                                                                                   default_sort_order=default_sort_order)}


def extract_sort_dict(request: Request, sort_attribute: str,
                      default_sort_order = MongoSortOrder.ASCENDING) -> SortDict:
    """
    Extracts the sort order from the request query parameters, and returns a sort dict suitable to pass into
    MongoDB queries. Assumes at most one sort query parameter is present.

    :param request: the HTTP request. Required.
    :param sort_attribute: the attribute to sort on. Required.
    :param default_sort_order: the default sort order to use if no sort order is present. If omitted, the default
    sort order is ascending.
    :return: a dict of attribute names to 1 (ascending) or -1 (descending).
    """
    return MongoSortOrder.from_request_dict(request, default_attr=sort_attribute,
                                            default_sort_order=default_sort_order)
