            ...
            if get_factid_attachment:
                ...
                try:
                        ...
                        if self._is_cp4d and self._cp4d_version < "5.1.0" :
                            ...
                            if self._is_new_usecase() and not self._external_model:
                                attachment_url = self._set_model_attachments_url_mastercopy(name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name,html_rendering_hint=html_rendering_hint)
                            else:
                            ...
                            if self._is_new_usecase() and not self._external_model :
                                attachment_url = self._set_model_attachments_url_new_mastercopy(name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name,html_rendering_hint=html_rendering_hint)
                            else:
    ...
    def set_cell_attachment_fact(self,
        ...
        if get_factid_attachment:
            ...
            try:
                ...
                if self._is_cp4d and self._cp4d_version < "5.1.0":
                    if self._is_new_usecase() and not self._external_model:
                        attachment_url = self._set_model_attachments_url_mastercopy(name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name)
                    else:
                        attachment_url = self._set_model_attachments_url(asset_id=model_asset_id,container_id=model_container_id, container_type=model_container_type, name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name)
                    with open(file_to_upload, "rb") as file:
                        base64_encoded_string = base64.b64encode(file.read())
                    body = base64_encoded_string
                else:
                    if self._is_new_usecase() and not self._external_model:
                        attachment_url = self._set_model_attachments_url_new_mastercopy(name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name)
                    else:
                        attachment_url = self._set_model_attachments_url_new(asset_id=model_asset_id,container_id=model_container_id, container_type=model_container_type, name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name)
                    with open(file_to_upload, "rb") as file:
                        body=file.read()
