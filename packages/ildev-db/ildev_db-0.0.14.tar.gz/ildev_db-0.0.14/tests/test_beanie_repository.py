...
# ✅ Repository fixture
@pytest.fixture()
async def beanie_repository(init_beanie_db) -> BeanieRepository[TestBeanieModel, TestBeanieModel, TestBeanieModel, TestBeanieOut]:
    return BeanieRepository(TestBeanieModel, TestBeanieOut)
...
async def test_aggregate_unwind(beanie_repository_with_data):
    ...
    await beanie_repository_with_data.create(
        TestBeanieModel(name="Tagged", tags=["fruit", "fresh"], quantity=1, price=1.0)
    )
