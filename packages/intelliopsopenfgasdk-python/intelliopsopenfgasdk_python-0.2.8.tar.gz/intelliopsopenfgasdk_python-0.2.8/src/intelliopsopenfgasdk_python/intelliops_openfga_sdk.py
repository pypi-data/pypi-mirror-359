...
class IntelliOpsOpenFgaSDK:
    ...
    def __init_fga(self, create_fga_model: CreateFgaModel) -> None:
        ...
        try:
            ...
            create_l1_l2_objects_model_response = self.http_client.post(
                create_l1_l2_objects_endpoint,
                json=create_l1_l2_objects_model.model_dump(),
            )
    ...
    def check_access(self, check_access_model: CheckAccessModel) -> bool:
        ...
        try:
            response = self.http_client.post(
                ...
                json=check_access_model.model_dump(),
            )
            response.raise_for_status()
    ...
    def check_multi_access(
        self, check_multi_access_model: CheckMultipleAccessModel
    ) -> dict:
        """
        Checks access for the user and returns a Python dict (hashmap) mapping the last part of each l2object to its access boolean.
        ...
        try:
            response = self.http_client.post(
                ...
                json=check_multi_access_model.model_dump(),
            )
            response.raise_for_status()
            res = response.json()
            # Return a Python dict (hashmap) for all items, not just those with hasAccess True
            return {
                item["l2object"].split("_")[-1]: item["hasAccess"]
                for item in res["results"] if item["hasAccess"]
            }
        except httpx.HTTPStatusError as exc:
            self._handle_http_error(exc)
        except httpx.RequestError as exc:
    ...
    async def async_check_multi_access(
        ...
        """
        Asynchronously checks access for the user and returns a Python dict (hashmap) mapping the last part of each l2object to its access boolean.
        Example: {"123": True, "456": False, ...}
        """
        check_multi_access_endpoint = "/access/multi-check"
        try:
