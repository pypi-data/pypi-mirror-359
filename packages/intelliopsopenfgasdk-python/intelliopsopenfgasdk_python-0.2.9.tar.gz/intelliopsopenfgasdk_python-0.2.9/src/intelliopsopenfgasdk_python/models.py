...
class FgaObjectModel(BaseModel):
    intelliopsConnectorUId: str = Field(..., description="Connector UId")
    objectId: str = Field(..., description="L2 Object ID")


class CheckAccessModel(BaseModel):
    intelliopsUserUId: str = Field(..., description="User UId")
    fgaObject: FgaObjectModel = Field(..., description="L2 Object ID")


class FgaObjectsModel(BaseModel):
    intelliopsConnectorUId: str = Field(..., description="Connector UId")
    objectIds: List[str] = Field(..., description="L2 Object IDs")


class CheckMultipleAccessModel(BaseModel):
    intelliopsUserUId: str = Field(..., description="User UId")
    fgaObjects: List[FgaObjectsModel] = Field(
        ..., description="List of Data Object IDs"
    )
