...
class DummyHttpClient:
    ...
    def post(self, url, json=None):
        ...
        if url in ("confluence/create-groups", "/confluence/create-groups"):
            return DummyResponse(200)
        elif url in ("confluence/create-l1-l2-objects", "/confluence/create-l1-l2-objects"):
            return DummyResponse(200)
        elif url in ("access/check", "/access/check"):
            # Return False for a specific fgaObject.objectId to test negative case
            if json:
                # Support both old and new payloads for robustness
                object_id = json.get("objectId")
                if not object_id and "fgaObject" in json and isinstance(json["fgaObject"], dict):
                    object_id = json["fgaObject"].get("objectId")
                if object_id == "t417ce8a_Confluence_e4448be1-8022-43a2-b234-e467842aac60_19048045":
                    return DummyResponse(200, {"hasAccess": False})
            return DummyResponse(200, {"hasAccess": True})
        elif url in ("auth/sync", "/auth/sync"):
            return DummyResponse(200)
        elif url in ("access/multi-check", "/access/multi-check"):
            # This is handled in DummyHttpClientMulti
...
def test_init_fga_success(monkeypatch):
    ...
    assert sdk.http_client.last_post[0].lstrip("/") == 'confluence/create-l1-l2-objects'
...
def test_check_access_true():
    sdk = IntelliOpsOpenFgaSDK()
    sdk.http_client = DummyHttpClient()
    from intelliopsopenfgasdk_python.models import FgaObjectModel
    model = CheckAccessModel(
        ...
        fgaObject=FgaObjectModel(
            intelliopsConnectorUId="connector",
            objectId="t417ce8a_Confluence_e4448be1-8022-43a2-b234-e467842aac60_190480451"
        )
    )
...
def test_check_access_false():
    ...
    model = CheckAccessModel(
        ...
        fgaObject=FgaObjectModel(
            intelliopsConnectorUId="connector",
            objectId="t417ce8a_Confluence_e4448be1-8022-43a2-b234-e467842aac60_19048045"
        )
    )
...
def test_initialize_calls_both(monkeypatch):
    ...
    calls = [c.lstrip("/") for c in sdk.http_client.calls]
    assert "auth/sync" in calls
    assert "confluence/create-groups" in calls
    assert "confluence/create-l1-l2-objects" in calls
...
def test_check_multi_access():
    class DummyHttpClientMulti(DummyHttpClient):
        def post(self, url, json=None):
            if url in ("access/multi-check", "/access/multi-check"):
                # Return the correct structure expected by the SDK: {"results": [ ... ]}
                return DummyResponse(200, {
                    "results": [
    ...
    from intelliopsopenfgasdk_python.models import FgaObjectsModel
    # According to SDK, all objectIds in all FgaObjectsModel are mapped in order to the response
    model = CheckMultipleAccessModel(
        intelliopsUserUId="t417ce8a_Confluence_e4448be1-8022-43a2-b234-e467842aac60_5b70c8b80fd0ac05d389f5e9",
        fgaObjects=[
            FgaObjectsModel(intelliopsConnectorUId="connector", objectIds=[
    ...
    # Only the last occurrence for each suffix is present in the result dict
    assert result["550568177"] is True
    assert result["190480451"] is True
    # '669876453' is not present because it is overwritten if suffixes are not unique
    assert "669876453" not in result
