...
def get_max_secret_level_threshold() -> int:
    ...
    try:
        return int(threshold) if threshold and threshold.isdigit() else 2
...
async def get_km_doc(doc_id: str, doc_title: str = "", doc_link: str = "", convert_to_md: bool = True) -> str:
    ...
    try:
        ...
        data = doc_json.get("data") if doc_json else {}
        secret_level = data.get("secretLevel", 0) if data else 0
        # 确保 secret_level 是数字类型
        if secret_level is None:
            secret_level = 0
