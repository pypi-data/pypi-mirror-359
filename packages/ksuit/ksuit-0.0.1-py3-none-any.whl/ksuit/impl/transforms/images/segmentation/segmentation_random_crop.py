...
class SegmentationRandomCrop(nn.Module):
    ...
    def __call__(self, sample: dict[str, Any]) -> dict[str, Any]:
        ...
        top, left, crop_height, crop_width = self._get_params(height=height, width=width)
    ...
    def _get_params(self, height: int, width: int) -> tuple[int, int, int, int]:
        ...
        top = int(rng.integers(max(0, height - self.size[0]) + 1, size=(1,)))
        left = int(rng.integers(max(0, width - self.size[1]) + 1, size=(1,)))
