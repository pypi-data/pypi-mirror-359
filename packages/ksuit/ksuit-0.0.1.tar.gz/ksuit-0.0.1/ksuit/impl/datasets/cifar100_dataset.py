...
class Cifar100Dataset(Dataset):
    ...
    def get_testrun_dataset(cls, config: dict[str, Any]) -> Self:
        return DummyDataset(
            data_generators=dict(
                ...
                y=data_generation_utils.UniformCategoricalGenerator(num_classes=100),
            ),
            attributes=dict(num_classes=100),
            size=11,
        )
    ...
    def __init__(self, split: Literal["train", "test"]):
        ...
        self.split = split
        self.dataset = CIFAR100(root=root, train=split == "train", download=True)
