...
class LinearWarmupCosineDecay(Schedule):
    """Schedule with linear warmup for `warmup_steps` steps or `warmup_ratio * 100`% of the `total_steps` followed
    by cosine decay. If instantiated via `ScheduleFactory`, `warmup_steps` can also be specified via `warmup_epochs`,
    `warmup_updates` or `warmup_samples`.
    """
    ...
    def _evaluate(self, step: int, total_steps: int | None) -> float:
        ...
        # cosine
        cosine_progress = (step - warmup_steps) / max(1, total_steps - 1 - warmup_steps)
        cosine_value = (1 + math.cos(math.pi * cosine_progress)) / 2
        cosine_delta = self.max_value - self.end_value
        return self.end_value + cosine_value * cosine_delta
