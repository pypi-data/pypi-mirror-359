...
class LazyLinOp(LinearOperator):
        ...
        if is_array_api_obj(op) or issparse(op) or _iscxsparse(op):
            ...
            if op.ndim == 1:
                ...
                res = xp.ravel(self.lambdas['@'](
                    xp.reshape(op,(array_api_compat.size(op), 1))))
            elif op.ndim > 2:
                from itertools import product
                # op.ndim > 2
                dtype = _binary_dtype(self.dtype, op.dtype)
                res = xp.empty((*op.shape[:-2], self.shape[0], op.shape[-1]),
                               dtype=dtype)
    ...
    def _vstack_mul_lambda(self, op, o):
        ...
        # def mul_mat(o):
        #     xp = array_api_compat.array_namespace(o)
    ...
    def check(self, array_namespace=np, dtype: str = 'float64',
        ...
        **Notations**:
        ...
        for p in xps:
            ...
            self_u = self @ u
            self_v = self.H @ v
            if array_api_compat.device(
                    self_u) != array_api_compat.device(u):
                raise Exception(
