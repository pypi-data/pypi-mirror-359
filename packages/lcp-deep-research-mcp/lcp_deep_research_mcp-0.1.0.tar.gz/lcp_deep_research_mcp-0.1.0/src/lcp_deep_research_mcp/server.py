#!/usr/bin/env python3
"""
一個 MCP 伺服器，提供統一的網路和學術搜尋研究工具，
會追蹤相關連結，並將全面的資訊返回給 Claude。

此伺服器整合了 DuckDuckGo 進行網路搜尋，以及 Semantic Scholar 進行學術內容搜尋。
"""
...
MAX_CONTENT_SIZE = 8000  # 最終回應的最大字元數
MAX_RESULTS = 3         # 要處理的最大結果數量
...
async def deep_research(query: str, sources: str = "both", num_results: int = 2) -> str:
    ...
    Args:
        query: 研究問題或主題
        sources: 要使用的來源："web" 代表一般資訊，"academic" 代表學術來源，"both" 代表所有來源
        num_results: 要檢查的來源數量 (預設為 2，最多 3)
