...
class Llava15ChatHandler:
    ...
    def initialize_mtmd_context(self, llama_model: llama.Llama):
        ...
        with suppress_stdout_stderr(disable=self.verbose):
            ...
            params.verbosity = 1 if self.verbose else 0
            self.clip_ctx = self._mtmd_cpp.mtmd_init_from_file(
            ...
            if self.clip_ctx is None:
                raise ValueError(f"Failed to load clip model: {self.clip_model_path}")

            def mtmd_free():
                with suppress_stdout_stderr(disable=self.verbose):
                    self._mtmd_cpp.mtmd_free(self.clip_ctx)
