...
class AudioProcessor(Model):
    ...
    def normalize_audio(
        ...
        return self.istft_norm(results)
    ...
    def load_audio(
        ...
        *,
        ...
        if istft_norm:
            results = self.istft_norm(results.squeeze().unsqueeze(0).to(self.device))
        return results.float().unsqueeze(0).to(self.device)
