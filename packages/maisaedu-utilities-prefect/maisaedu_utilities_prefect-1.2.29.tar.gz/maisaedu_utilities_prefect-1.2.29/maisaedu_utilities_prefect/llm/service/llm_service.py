...
class LLMService(BaseModel):
        ...
        results = select(
            ...
            """
                select
                    ...
                    temperature,
                    max_tokens
                from meta.llm_flows_configs where flow_name = %s
                and is_active = true
            """,
            (data.get("flow_name"),)
        )
    ...
    def __save_history(self, config_id, request, response):
        ...
        insert_sql = """
            INSERT INTO meta.llm_flows_runs_history (llm_flow_config_id, model, request, response)
            VALUES (%s, %s, %s, %s)
            RETURNING id;
        """
        ...
        for config in self.configs:
            ...
            try:
                response = requests.post(
                    OPEN_ROUTER_API_URL,
                    headers={
                        "Authorization": self.open_router_api_key,
