...
from mangotools.assertion._excel_assertion import ExcelAssertion
...
class TestExcelTaskSheet(unittest.TestCase):
    def test_excel_headers(self):
        ...
        ExcelAssertion.assert_headers(actual, expect)
    ...
    def test_excel_all_row_data(self):
        ...
        for index, row_data in enumerate(all_rows):
            ...
            ExcelAssertion.assert_row_data(actual, expect)

    def test_excel_cell_value(self):
        actual = {'文件路径': FILE_PATH, '工作表': SHEET_NAME}
        expect = {'单元格': 'B2', '预期值': '小红书关键词'}
        ExcelAssertion.assert_cell_value(actual, expect)
    ...
    def test_excel_row_count(self):
        ...
        ExcelAssertion.assert_row_count(actual, expect)

    def test_excel_column_count(self):
        actual = {'文件路径': FILE_PATH, '工作表': SHEET_NAME}
        expect = {'预期列数': 7}
        ExcelAssertion.assert_column_count(actual, expect)

    def test_excel_sheet_names(self):
        actual = {'文件路径': FILE_PATH}
        expect = {'工作表列表': [SHEET_NAME, 'Sheet2', 'Sheet3']}
        ExcelAssertion.assert_sheet_names(actual, expect)
    ...
    def test_excel_column_values(self):
        ...
        ExcelAssertion.assert_column_values(actual, expect)
