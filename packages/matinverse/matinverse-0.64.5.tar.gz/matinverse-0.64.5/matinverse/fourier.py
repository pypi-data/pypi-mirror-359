...
def Fourier( geo:                           Union[Geometry2D,Geometry3D],\
        ...
        r"""Differentiable Fourier solver based on the Finite-volume method.
         ...
         :param saveat: A jnp.array containing the time steps at which the solution will be saved. If not provided, it saves only the final solution.
        ...
        if mode =='nonlinear':
         ...
         #T = jnp.ones_like(X0)
          ...
          #TB = X[:,:,N:]
          T = X
          TB = jnp.zeros((NT,batch_size,N_flux_sides)) #TO DO: Add the boundary conditions for the transient case
        ...
        T = T.reshape((NT,batch_size,-1))
        TB = TB.reshape((NT,batch_size,-1))
        ...
        cond = jax.vmap(conductance, in_axes=(0, 0))(T, jnp.arange(NT)) 

        J,kappa_effective,P_boundary,P_internal,T_boundary = jax.vmap(get_flux)(T,TB,kappa,cond,jnp.arange(NT))
