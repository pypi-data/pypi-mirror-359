...
from matinverse.projection import smoothed_projection
import numpy as np
...
def lengthscale(geo,Ls,eta=0.5,epsilon=1e-8):
      ...
      projection_final = smoothed_projection(geo,eta=eta)
      ...
      def func(x,beta):
        ...
        gradient_filtered_field = jnp.gradient(filtered_field.reshape(geo.grid))
        grad_mag = ((gradient_filtered_field[0]/dx) ** 2 + (gradient_filtered_field[1]/dy)**2) #1/m^2
        common = jnp.exp(-c0*grad_mag).flatten()

        projected_field = projection_final(filtered_field,beta=beta)
