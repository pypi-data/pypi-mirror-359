...
class FireSmokeUseCase(BaseProcessor):
    ...
    def process(
        ...
        try:
            ...
            self.logger.info(
                f"Processing fire and smoke detection with format: {input_format.value}"
            )
            ...
            events = self._generate_events(fire_smoke_summary, alerts, config)
            tracking_stats = self._generate_tracking_stats(fire_smoke_summary, insights, summary_text, config)

            # Set processing time before marking context completed
    ...
    def _generate_events(
        ...
        """Generate structured events for fire and smoke detection output."""
        ...
        if total > 0:
            # Calculate bbox area for intensity
            detections = summary.get("detections", [])
            total_area = 0.0
            for det in detections:
                bbox = det.get("bounding_box") or det.get("bbox")
            ...
            human_lines = [
                "Event: Fire and Smoke Detection",
                f"Fire Regions: {total_fire} detected",
                f"Smoke Clouds: {total_smoke} detected",
                f"Intensity: {intensity * 10:.1f}%",
            ]
            if total_fire > 0:
            ...
            events.append(fire_smoke_event)

        # Add alert events separately
        for alert in alerts:
            alert_event = {
                ...
                "application_version": "1.0",
                "location_info": None,
                "human_text": "Fire and smoke alert triggered",
            }
    ...
    def _generate_tracking_stats(
        ...
        """Generate structured tracking stats for fire and smoke detection."""
        tracking_stats = []
        ...
        #  Alert messages will be generated inside _generate_human_text_for_tracking
        ...
        #  Alerts like "Alert: Fire detected" will appear in this human_text
        tracking_stat = {
    ...
    def _generate_human_text_for_tracking(
        ...
        """Generate human-readable tracking summary with alerts and insights."""
        text_parts = [
            f"Fire Regions Detected: {total_fire}",
            f"Smoke Clouds Detected: {total_smoke}",
            f"Total Intensity: {intensity_pct:.1f}%",
        ]

        #  Add alert info
        if total_fire > 0:
            text_parts.append("Alert: Fire detected")
        if total_smoke > 0:
            text_parts.append("Alert: Smoke detected")
    ...
    def _count_unique_tracks(self, summary: Dict) -> int | None:
