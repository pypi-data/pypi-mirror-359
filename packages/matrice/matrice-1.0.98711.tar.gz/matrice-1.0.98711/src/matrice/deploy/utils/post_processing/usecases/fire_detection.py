...
class FireSmokeUseCase(BaseProcessor):
    ...
    def process(
        ...
        Args:
            ...
            config: Fire and smoke detection configuration
            context: Processing context
            stream_info: Stream information containing frame details

        Returns:
            ProcessingResult: Structured result with fire/smoke analytics
        """
    ...
    def _generate_events(
        ...
        if total > 0:
            ...
            # Build minimal human-readable summary
            human_lines = [
            ...
            if total_fire > 0 and total_smoke > 0:
                human_lines.append("Alert: Fire and Smoke detected")
            elif total_fire > 0:
        ...
        # Add alert events with custom text
        for alert in alerts:
            detected_fire = alert.get("detected_fire", 0)
            detected_smoke = alert.get("detected_smoke", 0)

            if detected_fire > 0 and detected_smoke > 0:
                alert_text = "Fire and Smoke alert triggered"
            elif detected_fire > 0:
                alert_text = "Fire alert triggered"
            elif detected_smoke > 0:
                alert_text = "Smoke alert triggered"
            else:
                alert_text = "Hazard alert triggered"
            ...
            alert_event = {
                ...
                "application_version": "1.0",
                "location_info": None,
                "human_text": f"{datetime.now(timezone.utc).strftime('%Y-%m-%d-%H:%M:%S UTC')} : {alert_text}"
            }
