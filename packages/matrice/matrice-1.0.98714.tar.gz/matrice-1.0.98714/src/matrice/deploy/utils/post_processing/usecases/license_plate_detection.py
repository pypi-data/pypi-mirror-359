...
class LicensePlateUseCase(BaseProcessor):
    ...
    def process(
        ...
        try:
            ...
            # Extract frame information from stream_infoAdd commentMore actions
            frame_number = None
            if stream_info:
                start_frame = stream_info.get("start_frame")
                end_frame = stream_info.get("end_frame")
    ...
    def _generate_events(self, license_summary: Dict, vehicle_summary: Dict, alerts: List,
                         config: LicensePlateConfig,frame_number: Optional[int] = None) -> List[Dict]:
