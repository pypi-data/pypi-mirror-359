...
class PPEComplianceUseCase(BaseProcessor):
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None, stream_info: Optional[Dict[str, Any]] = None) -> ProcessingResult:
        ...
        for det in processed_data:
            ...
            if bbox:
                tracker_input.append({
                    ...
                    'class': det.get('category'),
                    'confidence': det.get('confidence', 1.0),
                    'features': det.get('features', None)
                })
        ...
        try:
            ...
            tracked = self.tracker.update(tracker_input)
            # Convert back to output format, merging tracker info
            processed_data = []
            for t in tracked:
                bbox = t.get('bbox')
                if bbox is not None:
                    processed_data.append({
                        'category': t.get('class'),
        ...
        processed_data = self._deduplicate_violations(processed_data, iou_thresh=0.85)
        ...
        # Add total unique violation counts after tracking
