...
class PPEComplianceUseCase(BaseProcessor):
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None, stream_info: Optional[Dict[str, Any]] = None) -> ProcessingResult:
        ...
        # Add total unique violation counts after tracking using AdvancedTracker's API
        if self.tracker is not None and hasattr(self.tracker, 'get_track_counts'):
            total_violation_counts = self.tracker.get_track_counts()
        else:
