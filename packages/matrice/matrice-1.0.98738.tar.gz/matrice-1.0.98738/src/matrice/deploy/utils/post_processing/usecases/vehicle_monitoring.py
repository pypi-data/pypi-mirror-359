...
class VehicleMonitoringUseCase(BaseProcessor):
    ...
    def _get_track_ids_info(self, detections: list) -> Dict[str, Any]:
        ...
        # Collect track_ids in this frame
        frame_track_ids = set()
        for det in detections:
            tid = det.get('track_id')
            if tid is not None:
                frame_track_ids.add(tid)
                self._vehicle_total_track_ids.add(tid)

        self._total_frame_counter += 1

        return {
            "total_count": len(self._vehicle_total_track_ids),
            "current_frame_count": len(frame_track_ids),
            "total_unique_track_ids": len(self._vehicle_total_track_ids),
    ...
    def _generate_insights(self, counting_summary: Dict, zone_analysis: Dict, config: VehicleMonitoringConfig) -> List[str]:
        """Generate human-readable insights, limited to EVENT."""
        insights = []
    ...
    def _generate_human_text_for_tracking(self, counting_summary: Dict, zone_analysis: Dict, insights: List[str], summary: str, config: VehicleMonitoringConfig) -> str:
        """Generate human-readable text for tracking stats with specific format."""
        category_counts = counting_summary.get("by_category", {})
        unique_count = self._count_unique_tracks(counting_summary) or 0
        ...
        human_text = " and ".join(parts) if parts else "No vehicles detected"
        human_text += f"\nUnique vehicle count: {unique_count}"
        return human_text