...
class VehicleMonitoringUseCase(BaseProcessor):
    ...
    def __init__(self):
        ...
        self.vehicle_categories = ["car", "bicycle", "bus", "garbagevan", "truck", "motorbike", "van"]
    ...
    def _get_track_ids_info(self, detections: list) -> Dict[str, Any]:
        """Get detailed information about track IDs for vehicle detections (per frame)."""
        frame_track_ids = set()
    ...
    def _deduplicate_vehicles(detections, iou_thresh=0.7):
        """Suppress duplicate/overlapping vehicle detections with same label and high IoU."""
        filtered = []
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None, stream_info: Optional[Dict[str, Any]] = None) -> ProcessingResult:
        ...
        try:
            ...
            # Step 2.7: Apply advanced tracking
            try:
                ...
                self.logger.debug(f"Applied advanced tracking, assigned track IDs to {len(vehicle_processed_data)} detections")
                
            except Exception as e:
                self.logger.warning(f"AdvancedTracker failed: {e}")
            
            # Step 2.8: Deduplicate overlapping vehicle detections
            vehicle_processed_data = self._deduplicate_vehicles(vehicle_processed_data, iou_thresh=0.7)
            self.logger.debug(f"Deduplicated to {len(vehicle_processed_data)} vehicle detections")
            
            # Step 2.9: Update vehicle tracking state
    ...
    def _generate_insights(self, counting_summary: Dict, zone_analysis: Dict, config: VehicleMonitoringConfig) -> List[str]:
        ...
        if config.enable_unique_counting:
            unique_count = self.get_total_vehicle_counts()
            total_unique = sum(unique_count.values())
            if total_unique is not None:
                insights.append(f"Unique vehicle count: {total_unique}")
                if total_unique != total_vehicles:
                    insights.append(f"Detection efficiency: {total_unique}/{total_vehicles} unique tracks")
    ...
    def _calculate_metrics(self, counting_summary: Dict, zone_analysis: Dict, config: VehicleMonitoringConfig, context: ProcessingContext) -> Dict[str, Any]:
        ...
        if config.enable_unique_counting:
            ...
            total_unique = sum(unique_count.values())
            if total_unique is not None:
                metrics["unique_vehicles"] = total_unique
                metrics["tracking_efficiency"] = (total_unique / total_vehicles) * 100 if total_vehicles > 0 else 0
