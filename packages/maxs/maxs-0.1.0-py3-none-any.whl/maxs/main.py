...
from maxs.tools import bash, file_ops


def create_agent(model_id=None, host=None):
    """
    Create a Strands Agent with Ollama model.

    Args:
        model_id: Ollama model ID (default: qwen3:1.7b)
    ...
    # Default values
    if model_id is None:
        model_id = os.getenv("MODEL_ID", "qwen3:1.7b")
    if host is None:
    ...
    try:
        ...
        print(f"💡 Make sure Ollama is running at {host}")
        print(
            f"💡 Model '{model_id}' should be available (run: ollama pull {model_id})"
        )
        sys.exit(1)

    # Create the agent
    agent = Agent(model=model, tools=[bash, file_ops])
...
def main():
    ...
    # Create agent
    agent = create_agent(model_id, host)
