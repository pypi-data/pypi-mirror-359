...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> list[Content]:
    ...
    try:
        ...
        if full_tool_name in TOOL_PARAM_MAP:
            ...
            # Special handling for org_member_add to send full payload
            if full_tool_name == "mcp_cldkctl_cldkctl_org_member_add":
                arguments = arguments.copy()
                if "project_ids" in arguments:
                    arguments["projects"] = [{"project_id": pid} for pid in arguments.pop("project_ids")]
                data = make_authenticated_request(method, endpoint, data=arguments)
                return [TextContent(type="text", text=format_response(data))]

            # Special handling for org_role_add to reconstruct privileges
            if full_tool_name == "mcp_cldkctl_cldkctl_org_role_add":
                arguments = arguments.copy()
                if "privilege_names" in arguments:
                    arguments["privileges"] = [{"name": name} for name in arguments.pop("privilege_names")]
                data = make_authenticated_request(method, endpoint, data=arguments)
                return [TextContent(type="text", text=format_response(data))]

            # Special handling for org_edit to parse org_data_json
            if full_tool_name == "mcp_cldkctl_cldkctl_org_edit":
                arguments = arguments.copy()
                if "org_data_json" in arguments:
                    import json as _json
                    arguments["org_data"] = _json.loads(arguments.pop("org_data_json"))
                data = make_authenticated_request(method, endpoint, data=arguments)
                return [TextContent(type="text", text=format_response(data))]

            # Repeat similar logic for all other *_data, *_object, *_claim, *_role, *_pod, *_console, *_deployment, *_daemonset, *_statefulset, *_service, *_pv, *_pvc, *_datavolume, *_resource, *_crd, *_vm, *_registry, *_labels, *_tag, *_password fields as needed.
