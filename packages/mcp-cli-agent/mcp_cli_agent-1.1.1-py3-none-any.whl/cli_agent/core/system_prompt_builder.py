...
class SystemPromptBuilder:
    ...
    def _get_role_prompt(self, role: str) -> str:
        """Retrieve role-specific prompt from ~/.config/agent/roles/."""
        import os
        role_prompt_path = os.path.expanduser(f"~/.config/agent/roles/{role}.txt")
        if os.path.exists(role_prompt_path):
            with open(role_prompt_path, "r", encoding="utf-8") as f:
                return f.read().strip()
        return None

    def create_system_prompt(self, for_first_message: bool = False, role: str = None) -> str:
        ...
        if override_path:
            ...
            with open(override_path, "r", encoding="utf-8") as f:
                ...
                return self._process_override_content(override_content)
    ...
    def _process_override_content(self, content: str) -> str:
        """Process dynamic placeholders in override content."""
        if "{{TOOLS}}" in content:
            # Build tool list
            available_tools = []
            for tool_key, tool_info in self.agent.available_tools.items():
                ...
                available_tools.append(f"- **{tool_name}**: {description}")
            tools_section = "\n" + "\n".join(available_tools)
            content = content.replace("{{TOOLS}}", tools_section)

        if "{{LLM_INSTRUCTIONS}}" in content:
            llm_instructions = self.agent._get_llm_specific_instructions()
            content = content.replace("{{LLM_INSTRUCTIONS}}", llm_instructions or "")
