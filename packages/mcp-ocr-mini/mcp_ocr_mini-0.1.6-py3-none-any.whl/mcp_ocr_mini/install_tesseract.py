...
def get_package_manager() -> Optional[str]:
    ...
    if system == "darwin":
        ...
        try:
            subprocess.run(["which", "brew"], stdout=sys.stderr, stderr=sys.stderr, check=True)
            return "brew"
        ...
        try:
            subprocess.run(["winget", "--version"], stdout=sys.stderr, stderr=sys.stderr, check=True)
            return "winget"
        except (subprocess.CalledProcessError, FileNotFoundError):
            pass
...
def install_homebrew_if_needed():
    ...
    try:
        ...
        subprocess.run(install_cmd, stdout=sys.stderr, stderr=sys.stderr, check=True)
...
def install_tesseract():
    ...
    try:
        if system == "darwin":
            ...
            subprocess.run(["brew", "install", "tesseract"], stdout=sys.stderr, stderr=sys.stderr, check=True)
            
        elif system == "linux":
            if pkg_manager == "apt":
                subprocess.run(["sudo", "apt-get", "update"], stdout=sys.stderr, stderr=sys.stderr, check=True)
                subprocess.run(["sudo", "apt-get", "install", "-y", "tesseract-ocr"], stdout=sys.stderr, stderr=sys.stderr, check=True)
            elif pkg_manager == "dnf":
                subprocess.run(["sudo", "dnf", "install", "-y", "tesseract"], stdout=sys.stderr, stderr=sys.stderr, check=True)
            elif pkg_manager == "pacman":
                subprocess.run(["sudo", "pacman", "-S", "--noconfirm", "tesseract"], stdout=sys.stderr, stderr=sys.stderr, check=True)
