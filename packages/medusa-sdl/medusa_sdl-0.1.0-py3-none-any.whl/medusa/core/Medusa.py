...
from medusa.core.driver_loader import get_driver, get_classes_by_category, DEVICE_REGISTRY
...
class Medusa(object):
    ...
    """
    _PUMP_CLASS     = get_classes_by_category("Pump")
    _VALVE_CLASS    = get_classes_by_category("Valve")
    _HOTPLATE_CLASS = get_classes_by_category("Hotplate")
    _RELAY_CLASS    = get_classes_by_category("Relay")
    _SHAKER_CLASS   = get_classes_by_category("Shaker")
    _PUMP_VALVE_CLASS   = _PUMP_CLASS + _VALVE_CLASS
    ...
    def _set_transfer_valves(
        ...
        for i in range(len(path_nodes) - 1):
            ...
            if (u, v) in self._graph.edges and "source_port" in self._graph.edges[(u, v)]:
                ...
                if isinstance(device_u, (self._VALVE_CLASS, self._PUMP_CLASS)):
