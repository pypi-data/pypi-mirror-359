...
def add_event_classes_rules(event_classes, rules):
    ...
    if source_classes and target_classes:
        rules.append(
            '(["{}"] in _Mentat.EventClass??[] or ["{}"] in _Mentat.TargetClass??[])'.format(
                '","'.join(source_classes), '","'.join(target_classes)
            )
        )
    elif source_classes:
        rules.append('["{}"] in _Mentat.EventClass??[]'.format('","'.join(source_classes)))
    else:
        rules.append('["{}"] in _Mentat.TargetClass??[]'.format('","'.join(target_classes)))
