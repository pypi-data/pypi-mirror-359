...
class CompoundMessageVerification:
    ...
    def __init__(self, limit=100):
        self.message_crud = GenericCRUD(default_entity_name="message",default_schema_name="message")
        ...
        self.profile_crud = GenericCRUD(default_entity_name="profile", default_schema_name="profile")
        self.user_crud = GenericCRUD(default_entity_name="user", default_schema_name="user")
        self.limit = limit
    ...
    def __get_body_from_lang_and_text_block_id(
        ...
        for text_block_id, body in zip(text_block_ids, body_list):
            if body is None or body == {} or text_block_id is None:
                continue
            string += "message_template_text_block_id = " + (str(text_block_id)) + " OR "
        string = string[:-4]
    ...
    def __get_profile_ml_names_for_hebrew(self, profile_id_list: list) -> list:
        ...
        for profile_id in profile_id_list:
            string += "(profile_id = " + (str(profile_id)) + " AND lang_code = 'he')" + " OR "
        string = string[:-4]
    ...
    def __get_profile_table(self, profile_id_list) -> list:
        ...
        user_ids = self.profile_crud.select_multi_tuple_by_where(
            ...
            where=(
                " OR ".join(
                    ["profile_id = " + str(profile_id) for profile_id in profile_id_list]
                ).replace("OR ", "", 0)
            ),
            limit=self.limit,
        )
        ...
        user_first_names = self.user_crud.select_multi_tuple_by_where(
            ...
            where=" OR ".join(
                ["user_id = " + str(user_id) for user_id in extended_user_id_list]
            ).replace("OR ", "", 0),
            limit=self.limit,
        )
    ...
    def check_for_errors(self, data=None):
        ...
        for json_data, message_id in zip(data, message_id_list):
            if json_data == '{"data": {}, "json_version": "240416"}':
                empty_messages.append("EMPTY MESSAGE ERROR, MESSAGE_ID: " + str(message_id))
                continue
        ...
        for profile in all_profiles:
            for profile_dict in profile:
                ...
                preferred_langs.append(self.__get_preferred_language(profile_dict))
                question_per_language.append(self.__get_question_per_language(profile_dict))
        ...
        for (
            ...
            if result:
                errors.append(
                    ", ".join(
                        (
                            "Question_per_lang_errors: " "Message_id: " + str(message_id),
                            "Channel: " + str(channel),
            ...
            if result:
                body_errors.append(
                    ", ".join(
                        (
                            "Body_per_lang_errors: " "Message_id: " + str(message_id),
                            "Channel: " + str(channel),
                            ("Profile_id: " + str(profile_id)),
                            (
                                "First name of user: "
                            ...
                            *[f"Body in {lang}: {body}" for lang, body in body_per_lang.items()],
                            "ERRORS: " + ", ".join(error_list),
                        )
                    )
                )
    ...
    def __extract_replacement(
        ...
        pattern = re.compile(r".*?" + escaped_template, re.IGNORECASE | re.DOTALL)
    ...
    def contains_name(self, question: str, name: str) -> bool:
        ...
        pattern = rf"\b{name}\b"  # regex pattern to match the word "name" as a whole word
