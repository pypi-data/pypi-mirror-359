...
class Mamba(nn.Module):
    ...
    def _top_p_filter(self, logits: Tensor, top_p: float) -> Tensor:
        """
        Optimized top-p (nucleus) filtering - Average O(n) complexity
        """
        batch_size, vocab_size = logits.shape

        # Use torch.kthvalue to find the threshold without full sorting
        # Estimate the number of tokens to retain
        estimated_k = min(int(vocab_size * (1 - top_p) * 2), vocab_size - 1)

        if estimated_k <= 0:
            return logits

        # Find the k-th largest value as the initial threshold
        kth_values, _ = torch.kthvalue(logits, estimated_k, dim=-1, keepdim=True)

        # Create a mask to only apply softmax to candidate tokens
        candidate_mask = logits >= kth_values

        # Compute softmax and cumulative probabilities for candidates
        masked_logits = logits.masked_fill(~candidate_mask, float('-inf'))
        probs = F.softmax(masked_logits, dim=-1)

        # Use a more precise cumulative probability calculation
        sorted_probs, sorted_indices = torch.sort(probs, descending=True, dim=-1)
        cumulative_probs = torch.cumsum(sorted_probs, dim=-1)

        # Identify tokens that exceed the top_p threshold
