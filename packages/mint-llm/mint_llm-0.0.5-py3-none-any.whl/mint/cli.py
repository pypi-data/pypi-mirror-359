...
@app.command()
def extract(src: str, dest: str) -> None:
    ...
    if not cpu:
        ...
        for name in sdks:
            if name == "cuda" and torch.cuda.is_available():
                ...
                # TODO: implement Vulkan check
                pass
            elif name == "zluda":
                # TODO: implement ZLUDA check
                pass
...
def infuse(
    ...
    mdl = cast(PreTrainedModel, _attach_layer(str(mpath), similarity_dir, alpha))
...
def generate(
    ...
    mdl, tokenizer, layer = load_wrapped_model(model, similarity_dir, alpha, device)
    ...
    def run(p: str, on: bool) -> None:
        outputs = pipe(p, logits_processor=[proc]) if on else pipe(p)
        typer.echo(outputs[0]["generated_text"])

    if prompt is not None and not interactive:
        print("Generation without similarity transformation:\n")
        run(prompt, False)
