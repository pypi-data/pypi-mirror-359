...
from mitm_tooling.representation.sql import ColumnName, TableName, SchemaName, ShortTableIdentifier, \
    Queryable
...
class TableMetaInfo(TableMetaInfoBase):
    ...
    def from_sa_table(cls, t: Table, queryable_source: Queryable | None = None,
        ...
        fkcs = [ForeignKeyConstraint.from_sa_constraint(fkc, t.schema or default_schema) for fkc in
...
class DBMetaInfo(DBMetaInfoBase):
    ...
    @classmethod
    def from_sa_tables(cls, default_schema: SchemaName, *tables: tuple[Table,...]) -> Self:
        db_structure = {}
        meta = sa.MetaData()
        for t in tables:
