...
from ..data_models import SourceDBType, DBMetaInfo, VirtualDB
...
class ExecutableDBMapping(BaseModel):
    ...
    def to_exportable(self, filename: str | None = None) -> Exportable:
        ...
        return MappingExport(mitm=self.mitm,
                             concept_mappings=self.concept_mappings,
                             filename=filename).apply(self.db_metas)
...
class StandaloneDBMapping(DBMapping):
    ...
    def recreate_virtual_db(self,
                            remote_engine: sa.Engine,
                            queryable_verifier: Callable[[Queryable], bool] | None = None) -> tuple[
        VirtualDB, dict[SourceDBType, DBMetaInfo]]:
    ...
    def to_exportable(self, remote_engine: sa.Engine, filename: str | None = None) -> Exportable:
        ...
        return MappingExport(mitm=self.mitm,
                             concept_mappings=self.concept_mappings,
                             filename=filename).apply(db_metas)
