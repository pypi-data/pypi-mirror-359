...
from mitm_tooling.representation.intermediate import HeaderEntry, Header, StreamingConceptData, StreamingMITMData, \
    MITMData
from mitm_tooling.utilities.sql_utils import AnyDBBind
from .concept_mapping import ConceptMapping, DataProvider, ConceptMappingException, InstancesProvider, \
...
class Exportable(pydantic.BaseModel):
    ...
    def export_as_stream(self, bind: AnyDBBind, validate: bool = False) -> StreamingZippedExport:
        ...
        for main_concept, dps in self.generalized_data_providers.items():
            ...
            chunk_iterators = []
            for dp in dps:
                def local_iter(dp: DataProvider=dp, columns=tuple(concept_file_columns)) -> Iterator[
            ...
            data_sources[main_concept] = StreamingConceptData(structure_df=structure_df,
