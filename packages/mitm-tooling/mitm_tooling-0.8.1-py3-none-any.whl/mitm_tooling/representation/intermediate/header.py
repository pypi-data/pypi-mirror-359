...
class HeaderEntry(pydantic.BaseModel):
    ...
    def attr_name_map(self) -> dict[ColumnName, ColumnName]:
        return {a_anon: a for a_anon, a in zip(mk_attr_columns(self.attr_k), self.attributes)}


def mk_typed_df_columns(mitm: MITM, he: HeaderEntry) -> tuple[list[str], dict[str, MITMDataType]]:
...
class Header(pydantic.BaseModel):
    ...
    def from_df(cls, df: pd.DataFrame, mitm: MITM) -> Self:
        return Header(mitm=mitm, header_entries=frozenset(
    ...
    def typed_df_columns(self) -> dict[ConceptName, dict[TypeName, tuple[list[str], dict[str, MITMDataType]]]]:
        return {c: {tp: mk_typed_df_columns(self.mitm, he) for tp, he in tps.items()} for c, tps in
                self.as_dict.items()}
