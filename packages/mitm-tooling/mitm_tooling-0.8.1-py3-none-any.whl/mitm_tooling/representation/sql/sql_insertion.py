    ...
    if (meta_tables := sql_rep_schema.meta_tables) is not None:
        ...
        with use_nested_conn(bind) as conn:
            ...
            if header.header_entries:
                conn.execute(meta_tables.types.insert().values(
                    [{'kind': he.kind, 'type': he.type_name, 'concept': he.concept}
                     for
                     he
                     in header.header_entries]))

                conn.execute(meta_tables.type_attributes.insert().values([{
                    'kind': he.kind,
                    'type': he.type_name,
                    'attribute_order': i,
                    'attribute_name': a,
...
def _insert_type_df(conn: sa.Connection,
                    sql_rep_schema: SQLRepresentationSchema,
                    mitm_def: MITMDefinition,
                    concept: ConceptName,
                    type_name: TypeName,
                    type_df: pd.DataFrame,
                    artificial_id_offset: int | None = None) -> tuple[int, int]:
    ...
    if (t_concept := sql_rep_schema.get_concept_table(parent_concept)) is not None:
        ...
        artificial_ids = pd.RangeIndex(start=start_id, stop=start_id + no_instances,
...
def _insert_type_dfs(conn: sa.Connection,
                     sql_rep_schema: SQLRepresentationSchema,
                     mitm_def: MITMDefinition,
                     concept: ConceptName,
                     typed_dfs: Iterable[tuple[TypeName, HeaderEntry, Iterable[pd.DataFrame]]]) -> tuple[
    ...
    for type_name, he, type_dfs in typed_dfs:
        ...
        for type_df in type_dfs:
            inserted_instances, inserted_rows = _insert_type_df(conn,
                                                                sql_rep_schema,
                                                                mitm_def,
                                                                concept,
                                                                type_name,
                                                                type_df,
                                                                artificial_id_offset=offsets[type_name])
...
def insert_instances(bind: AnyDBBind,
                     sql_rep_schema: SQLRepresentationSchema,
    ...
    with use_nested_conn(bind) as conn:
        for concept, typed_dfs in instances:
            inserted_types, inserted_instances, inserted_rows = _insert_type_dfs(conn,
                                                                                 sql_rep_schema,
                                                                                 mitm_def,
                                                                                 concept,
                                                                                 typed_dfs)
        ...
        # conn.commit()
    return SQLRepInsertionResult(inserted_instances=total_inserted_instances,
                                 inserted_rows=total_inserted_rows,
                                 inserted_types=total_inserted_types)


def insert_data(bind: AnyDBBind,
                gen_sql_rep_schema: Callable[[], SQLRepresentationSchema],
                gen_instances: Callable[[], TypedMitMDataFrameStream],
                gen_override_header: Callable[[], Header] | None = None) -> SQLRepInsertionResult:
    ...
    insert_header_data(bind,
                       sql_rep_schema,
                       header)
    return insertion_result


def append_data(bind: AnyDBBind,
                gen_sql_rep_schema: Callable[[], SQLRepresentationSchema],
                gen_instances: Callable[[], TypedMitMDataFrameStream]) -> SQLRepInsertionResult:
    sql_rep_schema = gen_sql_rep_schema()
    insertion_result = insert_instances(bind, sql_rep_schema, gen_instances())
    update_header_data(bind,
                       sql_rep_schema,
