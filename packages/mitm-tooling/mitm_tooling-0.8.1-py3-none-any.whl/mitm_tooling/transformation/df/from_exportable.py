...
from mitm_tooling.definition import get_mitm_def, TypeName
from mitm_tooling.extraction.sql.mapping import Exportable
from mitm_tooling.representation import mk_header_file_columns
from mitm_tooling.representation.df import TypedMitMDataFrameStream, StreamingMITMDataFrames, MitMDataFrameStream
from mitm_tooling.representation.intermediate import HeaderEntry
from mitm_tooling.representation.intermediate.header import mk_typed_df_columns
...
def exportable_to_mitm_dataframes_stream(
    ...
    with use_db_bind(source) as source_conn:
        for c, dps in exportable.data_providers.items():
            def df_chunks_iter(c=c, dps=dps) -> Iterator[
                tuple[TypeName, Iterable[pd.DataFrame]]]:
                for dp in dps:
                    chunks = dp.instance_provider.apply_db_chunked(source_conn) if stream_data else [
                        dp.instance_provider.apply_db(source_conn)]
                    for df_chunk in chunks:
                        df_chunk = dp.instance_postprocessor.apply_df(df_chunk)
                        for type_name, type_idx in df_chunk.groupby(mitm_def.get_properties(c).typing_concept).groups.items():
    ...
    with use_db_bind(source) as source_conn:
        for c, dps in exportable.data_providers.items():
            def typed_df_chunks_iter(c=c, dps=dps) -> Iterator[
                tuple[TypeName, HeaderEntry, Iterable[pd.DataFrame]]]:
                for dp in dps:
                    chunks = dp.instance_provider.apply_db_chunked(source_conn) if stream_data else [
                        dp.instance_provider.apply_db(source_conn)]
                    for df_chunk in chunks:
