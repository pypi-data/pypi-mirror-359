...
def db_engine_into_db_meta(engine: sa.Engine) -> DBMetaInfo:
    sa_meta, more_meta = connect_and_reflect(engine)
    return DBMetaInfo.from_sa_meta(sa_meta, default_schema=more_meta.default_schema)


def sql_rep_schema_into_db_meta(sql_rep_schema: SQLRepresentationSchema,
                                default_schema: str = SQL_REPRESENTATION_DEFAULT_SCHEMA) -> DBMetaInfo:
