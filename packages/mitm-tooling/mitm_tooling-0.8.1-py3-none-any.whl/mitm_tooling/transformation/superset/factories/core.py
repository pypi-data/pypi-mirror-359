...
def mk_pivot_post_processing(index_col: ColumnName, cols: list[ColumnName], aggregations: dict[ColumnName, str],
                             renames: dict[ColumnName, ColumnName | None] | None = None) -> list[
    SupersetPostProcessing]:
    pp: list[SupersetPostProcessing] = [Pivot(options=PivotOptions(aggregates=[{c: PivotOperator(operator=m)} for c, m
...
def mk_count_pivot_post_processing(cols: list[ColumnName], agg_cols: list[ColumnName], time_col: str = 'time') -> list[
    SupersetPostProcessing]:
    return mk_pivot_post_processing(time_col, cols, aggregations={f'AVG({c})': 'mean' for c in agg_cols})


def mk_time_avg_post_processing(cols: list[ColumnName], agg_cols: list[ColumnName], time_col: str = 'time') -> list[
    SupersetPostProcessing]:
    return mk_pivot_post_processing(time_col, cols, aggregations={f'AVG({c})': 'mean' for c in agg_cols})
