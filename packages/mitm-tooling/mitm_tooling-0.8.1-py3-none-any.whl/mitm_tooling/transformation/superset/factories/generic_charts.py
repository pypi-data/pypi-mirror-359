...
from .core import mk_adhoc_metric, mk_empty_adhoc_time_filter, \
    mk_pivot_post_processing, mk_adhoc_column, mk_time_avg_post_processing
...
from ..definitions import DatasetIdentifier, SupersetChartDef, SupersetAggregate, \
    ...
    TimeSeriesLineParams, QueryObjectExtras, SupersetAdhocMetric, BigNumberChartParams, BigNumberAggregation, \
    BigNumberTotalChartParams, HorizonChartParams, TableChartParams
...
def mk_pie_chart(name: str, dataset_identifier: DatasetIdentifier, col: ColumnName, dt: MITMDataType,
                 ...
                 uuid: UUID | None = None) -> SupersetChartDef:
...
def mk_metric_time_series_chart(name: str,
...
def mk_big_number_chart(name: str,
    ...
    if with_line:
        params = BigNumberChartParams(datasource=dataset_identifier,
                                      metric=metric,
                                      x_axis=time_col,
                                      aggregation=agg,
                                      time_grain_sqla=time_grain)
    else:
        params = BigNumberTotalChartParams(datasource=dataset_identifier,
                                           metric=metric)
    adhoc_time_col = mk_adhoc_column(time_col, timeGrain=time_grain)
    qo = mk_query_object(columns=[adhoc_time_col], metrics=[metric],
                         filters=[QueryObjectFilterClause.from_adhoc_filter(af) for af in adhoc_filters],
                         extras=QueryObjectExtras(time_grain_sqla=time_grain))
    qc = mk_query_context(datasource=dataset_identifier, queries=[qo], form_data=params)
...
def mk_horizon_chart(name: str,
                     ...
                     filters: list[SupersetAdhocFilter] | None = None,
                     uuid: UUID | None = None):
    params = HorizonChartParams(datasource=dataset_identifier,
                                metrics=metrics,
                                groupby=groupby_cols,
...
def mk_raw_table_chart(name: str,
                       dataset_identifier: DatasetIdentifier,
    ...
    params = TableChartParams(datasource=dataset_identifier,
                              query_mode='raw',
                              all_columns=columns,
    ...
    qo = mk_query_object(columns=columns,
                         filters=[QueryObjectFilterClause.from_adhoc_filter(af) for af in adhoc_filters],
                         orderby=orderby, extras=QueryObjectExtras(time_grain_sqla=TimeGrain.DAY))
...
def mk_agg_table_chart(name: str,
                       dataset_identifier: DatasetIdentifier,
                       metrics: list[str | SupersetAdhocMetric],
    ...
    qo = mk_query_object(columns=groupby,
                         metrics=metrics,
                         filters=[QueryObjectFilterClause.from_adhoc_filter(af) for af in adhoc_filters],
                         orderby=orderby)
