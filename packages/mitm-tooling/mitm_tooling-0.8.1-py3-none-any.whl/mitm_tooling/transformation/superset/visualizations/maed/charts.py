...
from mitm_tooling.representation.sql import SQLRepresentationSchema, mk_sql_rep_schema, TableName
from mitm_tooling.utilities.identifiers import naive_pluralize
from ..abstract import ChartDefCollection, ChartCreator, ChartCollectionCreator
from ...asset_bundles import NamedChartIdentifierMap
from ...definitions import FilterOperator, DatasetIdentifier, SupersetChartDef, DatasetIdentifierMap, SupersetAggregate, \
    ChartIdentifier, ChartIdentifierMap, MitMDatasetIdentifier
from ...factories.core import mk_adhoc_filter, mk_adhoc_metrics, mk_adhoc_metric, mk_metric
...
from ...factories.generic_charts import mk_pie_chart, mk_time_series_bar_chart, mk_avg_count_time_series_chart, \
    mk_metric_time_series_chart, mk_big_number_chart, mk_horizon_chart, mk_agg_table_chart, \
    mk_raw_table_chart
...
class ConceptCountTS(ChartCreator):
    ...
    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        ...
        return mk_time_series_bar_chart(chart_identifier.slice_name,
                                        dataset_identifier,
...
class RelationPie(ChartCreator):
    ...
    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        return mk_pie_chart(chart_identifier.slice_name,
                            dataset_identifier,
                            col=self.relation,
...
class TypeCountBigNumber(ChartCreator):
    ...
    @property
    def slice_name(self) -> str:
        return f'{self.type_name.title()} Big Number'

    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        return mk_big_number_chart(chart_identifier.slice_name,
                                   mk_adhoc_metric(self.props.typing_concept, SupersetAggregate.COUNT),
                                   dataset_identifier,
                                   agg='sum',
...
class TypeCountsHorizon(ChartCreator):
    ...
    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        ...
        return mk_horizon_chart(chart_identifier.slice_name,
                                [count_metric],
                                dataset_identifier,
...
class NumericAttributesTS(ChartCreator):
    ...
    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        return mk_metric_time_series_chart(chart_identifier.slice_name,
                                           mk_adhoc_metrics([a for a, dt in zip(self.header_entry.attributes,
                                                                                self.header_entry.attribute_dtypes) if
                                                             dt == MITMDataType.Numeric]),
                                           dataset_identifier,
                                           groupby_cols=self.groupby_relations,
                                           time_col=self.time_relation,
                                           uuid=chart_identifier.uuid)
...
class ConceptTypeAvgCountTS(ChartCreator):
    ...
    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        return mk_avg_count_time_series_chart(chart_identifier.slice_name,
                                              dataset_identifier,
                                              groupby_cols=self.groupby_relations,
                                              time_col=self.time_relation,
                                              uuid=chart_identifier.uuid)
...
class TypeCountsTableChart(ChartCreator):
    ...
    @property
    def slice_name(self) -> str:
        return f'Header Type Counts'

    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        return mk_agg_table_chart(chart_identifier.slice_name,
                                  dataset_identifier,
...
class TypesTableChart(ChartCreator):
    ...
    @property
    def slice_name(self) -> str:
        return f'Header Types'

    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        return mk_raw_table_chart(chart_identifier.slice_name,
                                  dataset_identifier,
                                  ['kind', 'concept', 'type'],
...
class AverageAttributesTSCollection(ChartCollectionCreator):
    ...
    def __init__(self, concept: ConceptName, header: Header, sql_rep_schema: SQLRepresentationSchema):
        ...
        self.header = header
        self.sql_rep_schema = sql_rep_schema
        assert all((t in self.header.as_dict[self.concept] for t in
                    self.sql_rep_schema.type_tables.get(self.concept, {}).keys()))
...
class BigNumberCollection(ChartCollectionCreator):
    ...
    @property
    def chart_creators(self) -> dict[str, tuple[TableName, ChartCreator]]:
        return {f'{c}-{type_name}-big-number': (tbl.name, TypeCountBigNumber(c, type_name)) for c in self.concepts for
                type_name, tbl in self.sql_rep_schema.type_tables.get(c, {}).items()}
...
class HeaderMetaTables(ChartCollectionCreator):
    ...
    def chart_creators(self) -> dict[str, tuple[TableName, ChartCreator]]:
        ...
        ccs = {}
        ccs[f'header-types-table'] = (self.sql_rep_schema.meta_tables.types.name, TypesTableChart())
        ccs[f'header-counts-table'] = (self.sql_rep_schema.meta_tables.types.name, TypeCountsTableChart())
        for c, type_map in self.header.as_dict.items():
            # ccs[f'{c}-types-table'] = (tbl_name, ConceptTypesTableChart(c))
            for type_name, he in type_map.items():
...
class BaselineMAEDCharts(ChartCollectionCreator):
    ...
    def chart_creators(self) -> dict[str, tuple[TableName, ChartCreator]]:
        ...
        ccs['event-horizon'] = (self.sql_rep_schema.views['events_view'].name, TypeCountsHorizon('event'))
        return ccs
    ...
    def mk_chart_collection(self, ds_id_map: DatasetIdentifierMap, ch_id_map: ChartIdentifierMap) -> ChartDefCollection:
        ...
        charts.update(BigNumberCollection(['event', 'measurement'],
                                          self.sql_rep_schema).mk_chart_collection(ds_id_map, ch_id_map))

        charts.update(ConceptTypesAvgCountTSCollection('event',
                                                       self.sql_rep_schema).mk_chart_collection(ds_id_map, ch_id_map))
        charts.update(AverageAttributesTSCollection('measurement', self.header,
                                                    self.sql_rep_schema).mk_chart_collection(ds_id_map, ch_id_map))
        return charts
...
class MAEDCustomChart(ChartCreator):
    ...
    def build_chart(self, dataset_identifier: DatasetIdentifier, chart_identifier: ChartIdentifier) -> SupersetChartDef:
        return mk_maed_custom_chart(chart_identifier.slice_name,
                                    self.mdi,
                                    dataset_identifier, uuid=chart_identifier.uuid)
