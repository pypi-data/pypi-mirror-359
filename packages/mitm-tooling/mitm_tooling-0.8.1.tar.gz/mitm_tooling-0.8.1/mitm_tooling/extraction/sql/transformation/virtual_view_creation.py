...
from mitm_tooling.representation.sql import TableName, SchemaName
from .db_transformation import RawCompiled
from ..data_models import DBMetaInfo, Queryable, TableIdentifier, SourceDBType, VirtualDB, CompiledVirtualView, \
    CompiledVirtualDB
from ..data_models import VirtualView
from ..transformation.db_transformation import TableTransforms, TableCreations, InvalidQueryException, \
...
class VirtualViewCreation(pydantic.BaseModel):
    ...
    def apply(self, db_metas: dict[SourceDBType, DBMetaInfo], override_if_exists: bool = False,
              queryable_verifier: Callable[[Queryable], bool] | None = None) -> VirtualView:
        try:
            ...
            vv = VirtualView.from_from_clause(self.name, from_clause,
...
class VirtualDBCreation(pydantic.BaseModel):
    ...
    def from_compiled(cls, compiled_vdb: CompiledVirtualDB) -> Self:
        return cls(virtual_view_creations=[VirtualViewCreation.from_compiled(cvv) for cvv in
                                           compiled_vdb.compiled_virtual_views])
    ...
    def apply(self, original_db_meta: DBMetaInfo, override_if_exists: bool = False,
              queryable_verifier: Callable[[Queryable], bool] | None = None) -> VirtualDB:
        ...
        for vvc in self.virtual_view_creations:
            db_metas[
                SourceDBType.VirtualDB] = vdb.to_db_meta_info()  # inefficient but this allows views to depend on each other
