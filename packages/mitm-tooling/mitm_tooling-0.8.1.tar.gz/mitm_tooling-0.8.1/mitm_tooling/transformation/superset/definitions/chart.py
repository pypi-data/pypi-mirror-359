...
JsonQueryContext = Annotated[QueryContext, pydantic.PlainSerializer(
    lambda x: x.model_dump_json(by_alias=True, exclude_none=True, warnings=False) if isinstance(x,
                                                                                                pydantic.BaseModel) else x,
    return_type=pydantic.Json), pydantic.BeforeValidator(lambda x: QueryContext.model_validate(**x) if isinstance(x,
                                                                                                                  dict) else x)]
