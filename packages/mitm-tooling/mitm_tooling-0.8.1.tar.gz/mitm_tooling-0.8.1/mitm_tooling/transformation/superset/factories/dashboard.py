...
def mk_filter_config(name: str,
                     ...
                     scope=NativeFilterScope()) -> NativeFilterConfig:
    def target_ref(x: UUID | tuple[ColumnName, UUID]) -> ColumnOfDataset | DatasetReference:
        return ColumnOfDataset(column=ColName(name=x[0]), datasetUuid=x[1]) if isinstance(x,
                                                                                          tuple) else DatasetReference(
            datasetUuid=x)
...
def mk_dashboard_header(text: str,
                        header_size: HeaderSize = 'MEDIUM_HEADER',
                        background: BackgroundType = 'BACKGROUND_TRANSPARENT',
                        id: DashboardInternalID | None = None) -> DashboardHeader:
    return DashboardHeader(id=id or f'HEADER-{mk_short_uuid_str()}',
                           meta=HeaderMeta(text=text, background=background, headerSize=header_size))


def mk_dashboard_markdown(text: str | None = None, height: int = 50, width: int = 3,
                          id: DashboardInternalID | None = None) -> DashboardMarkdown:
    return DashboardMarkdown(id=id or f'MARKDOWN-{mk_short_uuid_str()}',
                             meta=MarkdownMeta(height=height,
                                               width=width,
                                               code=(text if text else None)))
...
def mk_dashboard_column(children_ids: list[DashboardInternalID],
                        width: int = 3,
                        id: DashboardInternalID | None = None) -> DashboardColumn:
    return DashboardColumn(id=id or f'COLUMN-{mk_short_uuid_str()}',
                           children=children_ids,
                           meta=ColumnMeta(width=width))


def mk_dashboard_divider(id: DashboardInternalID | None = None) -> DashboardDivider:
    return DashboardDivider(id=id or f'DIVIDER-{mk_short_uuid_str()}')


def mk_dashboard_tab(text: str,
                     children_ids: list[DashboardInternalID],
                     id: DashboardInternalID | None = None) -> DashboardTab:
    return DashboardTab(id=id or f'TAB-{mk_short_uuid_str()}', children=children_ids, meta=TabMeta(text=text))


def mk_dashboard_tabs(tab_ids: list[DashboardInternalID], id: DashboardInternalID | None = None) -> DashboardTabs:
    return DashboardTabs(id=id or f'TABS-{mk_short_uuid_str()}', children=tab_ids)
...
def mk_dashboard_position_data(header_text: str,
                               top_level_comps: list[DashboardComponent],
                               inner_components: list[DashboardComponent]) -> DashboardPositionData:
    all_comps = top_level_comps + inner_components
    base: DashboardPositionData = mk_dashboard_base(header_text,
                                                    top_level_component_ids=[c.id for c in top_level_comps])
...
def put_in_columns(components: Iterable[tuple[int, list[DashboardComponent]]]) -> tuple[
    list[DashboardColumn], list[DashboardComponent]]:
    cols, elements = [], []
...
def put_in_column_group(components: Iterable[tuple[int, list[DashboardComponent]]]) -> tuple[
    DashboardRow, list[DashboardComponent]]:
...
def put_in_tabs(tabbed_components: dict[str, list[DashboardComponent]]) -> tuple[
    DashboardTabs, DashboardPositionData]:
    tabs, elements = [], []
...
def mk_dashboard_def(title: str,
...
def extract_charts(dashboard_def: SupersetDashboardDef) -> set[UUID]:
    return {c.meta.uuid for c in dashboard_def.position.values() if isinstance(c, DashboardComponent) and c.type == DashboardComponentType.CHART}
