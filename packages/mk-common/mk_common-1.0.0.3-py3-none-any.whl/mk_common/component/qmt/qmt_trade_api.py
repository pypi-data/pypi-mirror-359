...
# 下单
def order_buy(symbol, buy_price, buy_volume, account_no, qmt_exe_path):
    logger.warning("委托买入代码:{},买入价格:{},买入数量:{}", symbol, buy_price, buy_volume)
    xt_trader = get_xt_trader(qmt_exe_path)
...
# 卖出
def order_sell(symbol, sell_price, sell_volume, qmt_exe_path, account_no):
...
def get_position(qmt_exe_path, account_no):
    xt_trader = get_xt_trader(qmt_exe_path)
    acc = get_trade_acc(account_no)
...
# 取消
def order_cancel(entrust_no, symbol, qmt_exe_path, account_no):
    xt_trader = get_xt_trader(qmt_exe_path)
    acc = get_trade_acc(account_no)
    if symbol[-2:] == 'SZ':
...
# 获取连接对象
@lru_cache(maxsize=None)
def get_xt_trader(qmt_exe_path):
    session_id = int(time.time())
    xt_trader = XtQuantTrader(qmt_exe_path, session_id)
    # 创建交易回调类对象，并声明接收回调
...
if __name__ == '__main__':
    order_buy('002828.SZ', 7.62, 5200,
              'your_account', 'your_qmt_exe_path')
