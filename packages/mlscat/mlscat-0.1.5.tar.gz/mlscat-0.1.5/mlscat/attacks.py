...
def template_attack(profiling_traces, profiling_labels, test_traces, test_labels, leakage_model="HW"):
    # 按照label进行分组, ID or HW 默认HW
    
    # 标准化标签
    if leakage_model == "ID":
        pass
    elif leakage_model == "HW":
        profiling_labels = hw(profiling_labels)
        test_labels = hw(test_labels)
    else:
        raise "Unknow leakage model!"
    ...
    p_df = pd.DataFrame(profiling_data)
    a_df = pd.DataFrame(attack_data)
    
    p_grouped = p_df.groupby("profiling_labels")
    a_grouped = p_df.groupby("attack_labels")
