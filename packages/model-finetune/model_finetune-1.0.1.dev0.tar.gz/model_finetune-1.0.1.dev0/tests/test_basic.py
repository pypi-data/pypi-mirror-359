...
class TestCoreImports:
    ...
    def test_basic_imports(self):
        ...
        assert InterfaceProcessor is not None
...
class TestInterfaceProcessor:
    """测试接口处理器"""
    
    def test_interface_processor_init(self):
        """测试接口处理器初始化"""
        from model_finetune import InterfaceProcessor
        
        processor = InterfaceProcessor()
        assert processor is not None
        assert processor.output_dir.exists()
    
    def test_interface_processor_with_custom_output(self):
        """测试自定义输出目录"""
        from model_finetune import InterfaceProcessor
        
        with tempfile.TemporaryDirectory() as temp_dir:
            processor = InterfaceProcessor(output_dir=temp_dir)
            assert str(processor.output_dir) == temp_dir
...
class TestProcessInterfaceConfig:
    ...
    @patch('model_finetune.interface_processor.ResourceDownloader')
