import { i as ke, a as Q, r as Te, w as W, g as Oe, b as je } from "./Index-CSMEhAPR.js";
...
function Ze(e, t, r) {
  ...
  t = oe(t) || 0, Q(r) && (I = !!r.leading, c = "maxWait" in r, n = c ? Ye(oe(r.maxWait) || 0, t) : n, _ = "trailing" in r ? !!r.trailing : _);
  function m(u) {
    var E = s, U = i;
...
const Dt = xt(({
  ...
  const x = e["showUploadList.downloadIcon"] || e["showUploadList.removeIcon"] || e["showUploadList.previewIcon"] || e["showUploadList.extra"] || typeof r == "object", w = Tt(r), d = L(w.showPreviewIcon), F = L(w.showRemoveIcon), u = L(w.showDownloadIcon), E = L(i), U = L(n), D = L(s == null ? void 0 : s.format), xe = L(o), Ee = L(l), Le = L(f), Se = L(g), Re = L(I), [Fe, N] = X(!1), [j, B] = X(y);
  he(() => {
    B(y);
  }, [y]);
  const $ = we(() => {
  ...
  return /* @__PURE__ */ R.jsx(We.Dragger, {
    ...
    onChange: async (k) => {
      try {
        const v = k.file, C = k.fileList, ee = $.findIndex((b) => b.uid === v.uid);
        if (ee !== -1) {
          if (q)
            return;
          m == null || m(v);
          ...
          if (E && !await E(v, C) || q)
            return;
          N(!0);
          let b = C.filter((h) => h.status !== "done");
          if (a === 1)
            b = b.slice(0, 1);
          else if (b.length === 0) {
            N(!1);
