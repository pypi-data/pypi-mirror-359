...
def initialisation(kernel: Kernel) -> list[set[TYPE_TOROID_POINT]]:
    ...
    >>> pprint.pprint(initialisation(Kernel([1, 1, 1], anchor=(0,))))
    [{((0,), (0,)), ((0,), (2,)), ((0,), (1,))},
     {((1,), (0,)), ((0,), (2,)), ((0,), (1,))},
     {((1,), (1,)), ((0,), (2,)), ((1,), (0,))}]
    >>>
...
def one_step_decomposition(
    ...
    >>> for decomp_lattice, merge in one_step_decomposition(lattice):
    ...     reconst_lattice = [set.union(*(decomp_lattice[m] for m in merge[i])) for i in range(len(lattice))]
    ...     assert lattice == reconst_lattice
    >>>
    """
...
def all_steps_decomposition(
    ...
    ----------
    lattice : list[set[TYPE_TOROID_POINT]]
        An ordered set of kernels to be totaly decomposed.
    ...
    [{((0, 0), (1, 0)), ((0, 0), (2, 1))},
     ...
     {((0, 0), (2, 1))},
     {((0, 0), (2, 2))},
     {((0, 0), (1, 1)), ((0, 0), (2, 2)), ((0, 1), (2, 0))},
     {((1, 0), (0, 1)), ((0, -1), (2, 2)), ((1, 0), (0, 0))},
     {((0, 0), (2, 0)), ((1, 0), (0, 2)), ((1, 0), (0, 1))},
     {((0, 0), (1, 1)),
      ((0, 0), (2, 1)),
      ((0, 0), (2, 2)),
      ((1, 0), (0, 2)),
    ...
    for dec_lattice, sub_merge in one_step_decomposition(lattice):
        has_sub_dec = True
        yield from all_steps_decomposition(
            dec_lattice,
            {i: {j for s, _ in d for j in sub_merge[s]} for i, d in _merge.items()},
        )
    if not has_sub_dec:
        yield lattice, _merge


def full_decomposition(kernel: Kernel):
    ...
    ------
    lattice : list[Kernel]
        All the order set of the kernels to be convolved.
    merge_table : list[dict[int, tuple[int, ...]]]
        The kernels to be merged at each step.
    """
    lattice = initialisation(kernel)
    yield from all_steps_decomposition(lattice)
