...
def setup_fastapi_app(html_file: str, html_dir: str, base_name: str) -> FastAPI:
    ...
    async def convert_js_to_json(request: Request):
        ...
        json_file_name = os.path.join(html_dir, f"{base_name}_data.json")
    ...
    async def create_video(request: Request):
        ...
        try:
            ...
            video_path = os.path.join(html_dir, f"{base_name}_animation.mp4")
            create_video_from_frames(frames, video_path)
            
            print(f"MoVer converter: Animation saved as video to {video_path}")
            return JSONResponse(content={"success": True, "path": video_path})
    ...
    app.mount("/", StaticFiles(directory=os.path.join(os.path.dirname(__file__), "assets")), name="assets")
