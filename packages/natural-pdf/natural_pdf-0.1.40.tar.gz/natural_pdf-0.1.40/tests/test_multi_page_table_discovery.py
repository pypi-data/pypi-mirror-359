...
class TestMultiPageTableDiscovery:
    ...
    def test_multi_page_table_registry_basic(self):
        ...
        assert hasattr(table_region, 'constituent_regions'), "Expected FlowRegion with constituent_regions"
        ...
        assert multi_page_table.metadata.get("is_multi_page") is True, "Expected table to be marked as multi-page"
        ...
        for page in constituent_pages:
            ...
            assert found_table is not None, f"Page {page.page_number} should be able to find the multi-page table"
            assert found_table == multi_page_table, f"Page {page.page_number} should find the same multi-page table"
            
            # Also test find_all
            found_tables = page.find_all('table')
            assert len(found_tables) > 0, f"Page {page.page_number} should find tables via find_all"
            assert multi_page_table in found_tables.elements, f"Page {page.page_number} should include multi-page table in find_all results"
    ...
    def test_multi_page_table_components_registry(self):
        ...
        for page in constituent_pages:
            ...
            if results["regions"]["rows"]:
                ...
                multi_page_rows = [r for r in results["regions"]["rows"] if hasattr(r, 'constituent_regions')]
                if multi_page_rows:
                    # At least one multi-page row should be discoverable
                    assert any(row in found_rows.elements for row in multi_page_rows), \
                        f"Page {page.page_number} should find multi-page rows"
            
            # Test table columns  
            if results["regions"]["columns"]:
                found_cols = page.find_all('table_column')
                multi_page_cols = [c for c in results["regions"]["columns"] if hasattr(c, 'constituent_regions')]
                if multi_page_cols:
                    assert any(col in found_cols.elements for col in multi_page_cols), \
                        f"Page {page.page_number} should find multi-page columns"
            
            # Test table cells
            if results["regions"]["cells"]:
                found_cells = page.find_all('table_cell')
                multi_page_cells = [c for c in results["regions"]["cells"] if hasattr(c, 'constituent_regions')]
                if multi_page_cells:
                    assert any(cell in found_cells.elements for cell in multi_page_cells), \
    ...
    def test_single_page_table_still_works(self):
        ...
        if results["counts"]["table"] > 0:
            ...
            assert not hasattr(found_table, 'constituent_regions'), "Single-page table should be regular Region"
    ...
    def test_multi_page_auto_detection(self):
        ...
        if results["counts"]["table"] > 0:
            ...
            if guides._spans_pages():
                assert isinstance(table, FlowRegion), "Auto-detection should create FlowRegion for multi-page guides"
                assert table.metadata.get("is_multi_page") is True, "Should mark table as multi-page"
            else:
                # If guides don't span pages, should create regular Region
                assert not isinstance(table, FlowRegion), "Auto-detection should create regular Region for single-page guides"
    ...
    def test_selective_table_removal(self):
        ...
        assert len(found_tables_after) >= 2, f"Should have at least 2 tables, found {len(found_tables_after)}"
    ...
    def test_multi_page_table_returns_table_result(self):
        ...
        assert isinstance(table_result, TableResult), f"Expected TableResult, got {type(table_result)}"
        ...
        print(f"✅ Multi-page table extract_table() returns TableResult with {len(table_result)} rows") 