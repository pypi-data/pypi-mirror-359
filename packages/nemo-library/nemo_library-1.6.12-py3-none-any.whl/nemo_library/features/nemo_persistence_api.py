...
def _generic_metadata_create_or_update(
    ...
    for obj in objects:
        ...
        with_parent = (
            "/with-parent" if hasattr(obj, "parentAttributeGroupInternalName") else ""
        )
        ...
        if len(existing_object) == 1:
            ...
            response = requests.put(
                f"{config.get_config_nemo_url()}/api/nemo-persistence/metadata/{endpoint}{with_parent}/{obj.id}",
                json=obj.to_dict(),
                headers=headers,
                params=params,
            )
            if response.status_code != 200:
                log_error(
                    f"PUT Request failed.\nURL: {f"{config.get_config_nemo_url()}/api/nemo-persistence/metadata/{endpoint}{with_parent}/{obj.id}"}\nobject: {json.dumps(obj.to_dict())}\nStatus: {response.status_code}, error: {response.text}"
                )

        else:
            # Create new object
            response = requests.post(
                f"{config.get_config_nemo_url()}/api/nemo-persistence/metadata/{endpoint}{with_parent}",
                json=obj.to_dict(),
                headers=headers,
                params=params,
            )
            if response.status_code != 201:
                log_error(
                    f"POST Request failed.\nURL: {f"{config.get_config_nemo_url()}/api/nemo-persistence/metadata/{endpoint}{with_parent}"}\nobject: {json.dumps(obj.to_dict())}\nStatus: {response.status_code}, error: {response.text}"
                )
