...
class Nexel:
    ...
    def remove(self,value):
        if type(value).__name__ not in ['str','int','float','list','tuple','set']:
            value=self.information(value.nodes)
        x=Nexel(self.information(self.remove_internal(value,self.nodes)))
        self.nodes = x.nodes
        self.levels=x.levels
        self.max_depth=x.max_depth
    ...
    def fetch(self,level,count):
        return self.levels[level][count]
        #here counts or level may be : too
    ...
    def make_numpy_tree(self):
        import numpy as np
        return np.array(self.make_matrix())
    def make_numpy_original(self):
        import numpy as np
        try:
            return np.array(self.information(self.nodes))
        except:
            raise ValueError("Please make it a np array in tree form as levels x counts")
    ...
    def apply_each_node_internal(self,fx,data):
        for j,i in enumerate(data):
            if type(i).__name__ in ['str','int','float']:
                ...
                data[j]=self.apply_each_node_internal(fx, data)
            return data
    def apply(self,fx):
    ...
    def index_node_internal(self,data,value):
        if type(value).__name__ in ['str','int','float']:
            return 0
        if value in data:
            return data.index(value)
        for i in data:
            return i,self.index_node_internal(i,value)
        raise ValueError('Data asked to be indexed not found')
    ...
    def index_node(self,value):
        ...
        if value in x:
            return x.index(value)
        for i in x:
            return i,self.index_node_internal(x,value)
        raise ValueError('Data asked to be indexed not found')
    @property
    def size(self):
        keys=0
    ...
    @property
    def shape(self):
        return self.max_depth,self.max_count
    @property
    def symmetric_size(self):
        return int(self.max_depth*self.max_count)
    @property
