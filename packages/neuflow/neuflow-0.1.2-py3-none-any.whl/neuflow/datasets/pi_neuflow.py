...
class PINeuFlowDataset(PIDatasetAPI):
    ...
    def setup(self, stage: str):
        with open(os.path.join(self.dataset_path, 'scene_info.yaml'), 'r') as f:
            scene_info = yaml.safe_load(f)
            self._background_color = scene_info['background_color']  # shape (3,)
    ...
    def dataset(self, dataset_type: typing.Literal['train', 'val', 'test']) -> PIDataset:
        dataset = PINeuFlow(dataset_path=self.dataset_path, dataset_type=dataset_type, downscale=self.downscale, use_fp16=self.use_fp16)
