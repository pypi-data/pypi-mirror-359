...
from .renderers.hyfluid import HyFluidRenderer


@dataclasses.dataclass
class HyFluidConfig:
    name: str = dataclasses.field(default='HyFluid', metadata={'help': 'name of the model'})
    lrate: float = dataclasses.field(default=1e-3, metadata={'help': 'learning rate for the optimizer'})

    # dataset parameters
    dataset: typing.Literal['scalar', 'torch', 'torch_avo', 'torch2', 'torch2_avo', 'sphere', 'game'] = dataclasses.field(default='scalar', metadata={'help': 'dataset name'})
    ...
    # renderer parameters
    num_sample_rays: int = dataclasses.field(default=1024, metadata={'help': 'number of rays to sample per batch'})
    ...
    chunk: int = dataclasses.field(default=1024, metadata={'help': 'chunk size for rendering'})
...
class HyFluidTrainer(lightning.LightningModule):
    def __init__(self, cfg: HyFluidConfig):
        super().__init__()
        self.save_hyperparameters()
        self.model = HyFluidModel(
            ...
            num_layers_vel_sigma=cfg.num_layers_vel_sigma,
            hidden_dim_vel_sigma=cfg.hidden_dim_vel_sigma,
            use_background=cfg.use_background,
        )
        self.renderer = HyFluidRenderer(
    ...
    def training_step(self, batch, batch_idx):
        result_maps, pixels, pixels_mask = self.renderer(
            ...
            focals=batch['focals'],
            images=batch['images'],
            images_masks=batch['images_masks'],
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        os.makedirs(os.path.join(self.logger.log_dir, 'test_images'), exist_ok=True)
        torchvision.utils.save_image(final_map.permute(2, 0, 1), os.path.join(self.logger.log_dir, 'test_images', f'{self.current_epoch}_{batch_idx}.png'))
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        torch.save({
            ...
            'num_layers_vel_sigma': self.cfg.num_layers_vel_sigma,
            'hidden_dim_vel_sigma': self.cfg.hidden_dim_vel_sigma,
    ...
    def generate_map(result_maps, pixels, pixels_mask):
        ...
        acc_map_column = torch.cat([acc for acc in result_maps['acc_map']], dim=1) if 'acc_map' in result_maps else torch.zeros_like(rgb_map_column[..., :1])
        depth_map_column = torch.cat([depth for depth in result_maps['depth_map']], dim=1) if 'depth_map' in result_maps else torch.zeros_like(rgb_map_column[..., :1])
        vel_map_column = torch.cat([vel for vel in result_maps['vel_map']], dim=1) if 'vel_map' in result_maps else torch.zeros_like(rgb_map_column)
        ...
        final_map = torch.cat([final_map, vel_map_column], dim=1)
