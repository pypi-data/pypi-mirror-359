...
# ----- plugins -----
from ..plugins.train_sapce import Plugin, TrainSpace, ImageMaskLoss
...
class RendererAPI(torch.nn.Module):
    def __init__(self,
        ...
        self.use_normalized_directions = use_normalized_directions
        self.chunk = chunk

        self._plugins = []
        self.tsm: TrainSpace = TrainSpace(resolution=64)
        self.register_plugin(self.tsm)
        self.register_plugin(plugin=ImageMaskLoss())

        self.width_user = 800
        self.height_user = 800
    ...
    def forward(self,
                ...
                pose_indices: torch.Tensor,  # shape: (N) or None
        ...
        for plugin in self.plugins:
            plugin.reset_plugin(dtype=poses.dtype, device=poses.device)

        N = poses.shape[0]
        W = images.shape[-2] if images is not None else self.width_user
        H = images.shape[-3] if images is not None else self.height_user
        ...
        rays_o, rays_d, pixels, pixels_mask = self.generate_rays(
            ...
            use_normalized_directions=self.use_normalized_directions,
        )

        rays_o_valid, rays_d_valid, nears_valid, fars_valid, hit_mask = self.ray_aabb_intersection(
            rays_o=rays_o,
            rays_d=rays_d,
            aabb=self.tsm.aabb_std,
            min_near=0.1,  # TODO: Adjust as needed
            max_far=100,  # TODO: Adjust as needed
        )
        ...
        if num_rays > 0:
            ...
            rendered_maps = {
                'rgb_map': torch.ones_like(rays_o) * self.tsm.background_color if model.training else torch.zeros((N, H, W, 3), dtype=poses.dtype, device=poses.device),
            }

        # ---------- PLUGINS ----------
        if self.training:
            if pixels_mask is not None:
                image_mask_losses = {v for v in self.plugins if isinstance(v, ImageMaskLoss)}
                for iml in image_mask_losses:
                    iml: ImageMaskLoss
                    iml.compute(rgb_map=rendered_maps['rgb_map'], acc_map=rendered_maps['acc_map'], pixels_mask=pixels_mask, background_color=self.tsm.background_color)
        # ---------- PLUGINS ----------
    ...
    def sample_points(rays_o: torch.Tensor, rays_d: torch.Tensor, nears: torch.Tensor, fars: torch.Tensor, num_depth_samples: int, use_perturb: bool):
        ...
        def sample_z_vals(nears: torch.Tensor, fars: torch.Tensor, num_depth_samples: int, use_perturb: bool, dtype: torch.dtype, device: torch.device):
            ...
            if use_perturb:
                mids = 0.5 * (z_vals[..., 1:] + z_vals[..., :-1])  # [n_rays, n_depth_samples-1]
        ...
        z_vals = sample_z_vals(
            ...
            dtype=dtype,
            device=device
        )  # shape: (n_rays, num_depth_samples,)
