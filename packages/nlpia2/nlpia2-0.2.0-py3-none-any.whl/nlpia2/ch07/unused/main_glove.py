...
def load_dataset(
        params=HYPERPARAMS, expand_glove_vocab=False, seq_len=35,
        vocab_size=2000, embedding_size=64, num_stopwords=0,
        **kwargs):
    ...
    while len(argv):
        ...
        if a.startswith('--'):
            if '=' in a:
                ...
                k = a.lstrip('-')
                v = argv.pop()
            if hasattr(HYPERPARAMS, k):
                pipeline_kwargs[k] = type(getattr(HYPERPARAMS, k))(v)
            else:
                pipeline_kwargs[k] = eval(v, globals={}, locals={})
        else:
