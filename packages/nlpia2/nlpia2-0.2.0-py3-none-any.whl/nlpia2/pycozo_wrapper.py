...
>>> db.create('ABCs', *list('ABCDEFG'))
  status
0     OK
>>> db.create('123', *'col1 col2 col3'.split())
>>> db.create('123', *('col1 col2 col3'.split()))
...
class Client(pycozo.client.Client):
    ...
    TODO:
      delete(name): delete a stored (persistent) relation (relational DB table)
      query(name): run arbitrary cozo "select * where " query strings
    """
    def create(self, name, *args, **kwargs):
        # tx = self.multi_transact(True)
