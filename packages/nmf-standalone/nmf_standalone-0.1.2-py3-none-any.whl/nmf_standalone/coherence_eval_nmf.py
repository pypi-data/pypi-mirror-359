...
def run_coherence_evaluation(options):
    """
    Run the coherence evaluation process with the given parameters.
    
    Args:
        options (dict): Dictionary containing evaluation parameters:
            - filepath (str): Path to input file
            - desired_columns (str): Column name to analyze  
            - min_topics (int): Minimum number of topics (default: 2)
            - max_topics (int): Maximum number of topics (default: 15)
            - step (int): Step size (default: 1)
            - output_dir (str): Output directory (default: "./Output")
            - N_WORDS (int): Number of words per topic (default: 15)
            - LEMMATIZE (bool): Whether to lemmatize (default: False)
            - LANGUAGE (str): Language code (default: "TR")
            - tokenizer_type (str): Tokenizer type (default: "bpe")
    
    Returns:
        list: Coherence evaluation results
    """
    ...
    min_topics = options.get('min_topics', 2)
    max_topics = options.get('max_topics', 15)
    step = options.get('step', 1)
    N_WORDS = options.get('N_WORDS', 15)
    LEMMATIZE = options.get('LEMMATIZE', False)
    LANGUAGE = options.get('LANGUAGE', 'TR')
    tokenizer_type = options.get('tokenizer_type', 'bpe')
    
    # Generate base name from filepath if not provided
    data_name = os.path.basename(filepath).split(".")[0]
    base_name = f"{data_name}_evaluation_{LANGUAGE}_{tokenizer_type}"
