...
class SpiderInfo:
    def __init__(self, name=None, group_name=None, domain=None, url=None, freq=None, metadata=None, user=None,
                 monitor=True, task_type=0, log=True, show_debug=False, monitor_endpoint=PY_MONITOR_ENDPOINT):
        self.__id = 0
        # 进程ID
        ...
        self.__log = log
        # 上报端点
        self.__monitor_endpoint = monitor_endpoint
        # 是否输出debug日志
    ...
    def report(self, filter_data=True):
        ...
        try:
            ...
            data = self.to_dict()
            logger.success(data)
            logger.success(self.__last_state)
            if filter_data:
                # 只上报发生变化的数据
