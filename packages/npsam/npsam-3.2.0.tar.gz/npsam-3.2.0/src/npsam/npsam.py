...
class NPSAMImage:
    ...
    def combine(
        ...
        other_segmentation = seg_params_to_str(
            other_segmentation.seg.metadata.Segmentation
        )
        segmentations = [own_segmentation] * len(own_masks) + [
            other_segmentation
        ] * len(other_masks)
        ...
        if not self.seg.metadata.has_item("Segmentation.combination"):
            self.seg.metadata.Segmentation.combination = "Combined segmentation"
...
class NPSAM(MutableSequence):
    ...
    def create_script(self) -> None:
        """
        Creates a Python script that can be used to reproduce the current state of the
        NPSAM object, including segmentation and characterization parameters.

        The script will include:
            - Importing necessary libraries.
            - Loading images.
            - Setting scaling and units.
            - Performing segmentation and characterization.
            - Applying filtering conditions.
            - Saving results.

        The script is saved in the current working directory with a filename based on the
        current timestamp.
        """
