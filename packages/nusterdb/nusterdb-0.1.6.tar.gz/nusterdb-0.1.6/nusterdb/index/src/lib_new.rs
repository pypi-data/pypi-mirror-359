        ...
        let mut distances: Vec<(usize, f32)> = self.vectors
            ...
            .map(|(id, vector)| {
                let distance = match self.config.distance_metric {
                    ...
                    DistanceMetric::Dot => dot_distance(query, vector),
                };
                (*id, distance)
            })
            .collect();
...
pub struct HnswIndex {
    config: IndexConfig,
    hnsw: UnsafeCell<Option<Hnsw<f32, DistL2>>>,
...
impl AnnIndex for HnswIndex {
    fn with_config(config: IndexConfig) -> Self {
        let hnsw_config = config.hnsw_config.as_ref().unwrap_or(&HnswConfig::default());
        
        let hnsw = Hnsw::<f32, DistL2>::new(
            hnsw_config.max_nb_connection,
            hnsw_config.max_nb_elements,
    ...
    fn insert(&mut self, id: usize, vector: Vector) {
        let hnsw = unsafe { &mut *self.hnsw.get() };
        let hnsw_ref = hnsw.as_mut().unwrap();

        if let Some(&internal_id) = self.id_to_index.get(&id) {
            // Update existing vector - not directly supported by HNSW
            // We'd need to rebuild, but for now just skip
            return;
        }
        ...
        let data_slice = vector.as_slice();
        hnsw_ref.insert((internal_id, data_slice));
        
        self.id_to_index.insert(id, internal_id);
        self.index_to_id.insert(internal_id, id);
    }

    fn search(&self, query: &Vector, k: usize) -> Vec<(usize, f32)> {
        let hnsw = unsafe { &*self.hnsw.get() };
        let hnsw_ref = hnsw.as_ref().unwrap();
        
        let ef_search = self.config.hnsw_config.as_ref()
            .and_then(|c| c.ef_search)
            .unwrap_or_else(|| k.max(50));

        hnsw_ref.set_ef(ef_search);
        
        let query_slice = query.as_slice();
        let results = hnsw_ref.search(query_slice, k, ef_search);
        
        results.into_iter()
            .map(|neighbor| {
                let internal_id = neighbor.d_id;
                let distance = neighbor.distance;
                let external_id = *self.index_to_id.get(&internal_id).unwrap_or(&internal_id);
    ...
    fn stats(&self) -> IndexStats {
        IndexStats {
            vector_count: self.id_to_index.len(),
            dimension: self.config.dim,
            index_type: "HNSW".to_string(),
            distance_metric: self.config.distance_metric,
            memory_usage: self.id_to_index.len() * (std::mem::size_of::<usize>() * 2 + self.config.dim * std::mem::size_of::<f32>()),
            build_time_ms: None,
            last_search_time_ms: None,
            total_searches: 0,
            total_inserts: self.id_to_index.len() as u64,
            additional_stats: HashMap::new(),
        }
    }
...
impl SnapshotIndex for HnswIndex {
    fn dump(&self) -> Vec<u8> {
        // HNSW doesn't have direct serialization support
...
fn dot_distance(a: &Vector, b: &Vector) -> f32 {
    let a_slice = a.as_slice();
    let b_slice = b.as_slice();
