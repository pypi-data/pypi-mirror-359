...
with actual_target_check as (
    SELECT 
        ...
        Table_Name,
        MAX(CASE 
            WHEN (LOWER(Column_Name) LIKE '%revenue%' OR LOWER(Column_Name) LIKE '%actual%')
              AND LOWER(Column_Name) NOT LIKE '%revenuetarget%' THEN 1 ELSE 0 END) AS has_actuals,
        MAX(CASE 
    ...
    WHERE LOWER(Data_Type) IN ('int64', 'double', 'decimal')
      AND NOT (LOWER(Column_Name) LIKE 'is%' OR LOWER(Column_Name) LIKE 'has%')
...
LEFT JOIN PII_check pc ON c.Dataset_ID = pc.Dataset_ID AND c.Table_Name = pc.Table_Name
left JOIN factoct f 
