    ...
    def process_data(self, vcf_lines,input_format='json',data_type='p'):
        ...
        for q_list in q_lists:
            if self.data_type == 'p':
                ...
                variant_str, dc = generate_variant_str_from_data_in_json(vcf_lines,q_list)
            else:
                variant_str, dc = generate_variant_str_from_data_in_json(vcf_lines, q_list)

            try:
                if variant_str != '':
                    variant_str = urllib.parse.quote(variant_str)
                                    ...
                                    if res != "Error":
                                        try:
                                            ...
                                            if self.data_type == "p":
                                                ...
                                                # annotate with gene data
                                                annotated_data[dc[qid]]["UTA_Adapter_gene"] = res
