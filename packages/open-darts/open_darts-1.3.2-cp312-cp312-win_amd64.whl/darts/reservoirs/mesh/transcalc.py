...
class TransCalculations:
    ...
    def compute_area(coord_nodes_to_face):
        ...
        if len(coord_nodes_to_face) == 3:
            ...
            area = 0.5 * (
                np.linalg.norm(np.cross(vec_edge_1, vec_edge_2))
    ...
    def calc_trans_mat_mat(connection, mat_cell_info_dict, coord_nodes_to_face):
        ...
        n = np.cross(
            vec_edge_1, vec_edge_2
        )  # Cross product gives perpendicular vector to plane
        ...
        projection = TransCalculations.projection_con(
            centroid_i, centroid_j, n_unit, centroid_int
        )

        # Calc Ti
        f_i = (
            centroid_i - projection
        )  # result is vector between centroid_i and projection (note: length of f_i is D_i)
        f_i_unit = f_i / np.linalg.norm(f_i)  # normalize resulting vector

        perm_i = mat_cell_info_dict[
            connection[0]
        ].permeability  # get permeability from cell info

        # Calculate (average) based on f permeability
        k_dir = perm_i * f_i_unit  # Directionally scaled permeability
        con_perm = np.linalg.norm(
            np.array(k_dir)
        )  # Interface scaled permeability value

        # Calculate transmissibility
        trans_i = (
            conn_area * con_perm * abs(np.dot(n_unit, f_i_unit)) / np.linalg.norm(f_i)
        )
        ...
        trans_j = (
            conn_area * con_perm * abs(np.dot(n_unit, f_j_unit)) / np.linalg.norm(f_j)
        )

        # Harmonic average
        trans_i_j = (
            trans_i * trans_j / (trans_i + trans_j) * TransCalculations.darcy_constant
        )
        thermal_i_j = conn_area / (np.linalg.norm(f_i) + np.linalg.norm(f_j))
        return trans_i_j, thermal_i_j

    @staticmethod
    def calc_trans_mat_frac(
        mat_element_id,
        frac_element_id,
        mat_cell_info_dict,
        frac_cell_info_dict,
        ...
        if np.linalg.norm(
            np.abs(
                n_unit * frac_cell_info_dict[frac_element_id].frac_aperture
                + centroid_int
            )
            ...
            centroid_frac = (
                np.array(frac_cell_info_dict[frac_element_id].centroid)
        ...
        # Projection to get the point on the face when the two centroid's are connected to each other.
        projection = TransCalculations.projection_con(
            centroid_mat, centroid_frac, n_unit, centroid_int
        )
        ...
        trans_mat = (
            conn_area
            * con_perm_mat
            * abs(np.dot(n_unit, f_mat_unit))
            / np.linalg.norm(f_mat)
        )
        ...
        trans_frac = (
            conn_area
            * con_perm_frac
            * abs(np.dot(n_unit, f_frac_unit))
            / np.linalg.norm(f_frac)
        )
        ...
        trans_mat_frac = (
            trans_mat
            * trans_frac
            / (trans_mat + trans_frac)
            * TransCalculations.darcy_constant
        )
        thermal_mat_frac = conn_area / (np.linalg.norm(f_mat) + np.linalg.norm(f_frac))
        return trans_mat_frac, thermal_mat_frac

    @staticmethod
    def calc_trans_frac_frac(
        connect_array, temp_frac_elem, frac_cell_info_dict, coord_frac_nodes
    ):
        """
        ...
        conn_area = (
            length_frac_intersection * frac_cell_info_dict[frac_elem_i].frac_aperture
        )
        ...
        # First for ithFrac:
        proj_frac_i_to_int = np.dot(
            frac_cell_info_dict[frac_elem_i].centroid - coord_frac_nodes[0],
            dist_vec_fracs,
        ) / np.dot(dist_vec_fracs, dist_vec_fracs)
        new_proj_cen_rac_i = coord_frac_nodes[0] + proj_frac_i_to_int * dist_vec_fracs

        # Then for jthFrac:
        proj_frac_j_to_int = np.dot(
            frac_cell_info_dict[frac_elem_j].centroid - coord_frac_nodes[0],
            dist_vec_fracs,
        ) / np.dot(dist_vec_fracs, dist_vec_fracs)
        ...
        if rem_frac_elem.size:
            ...
            for ii, ith_rem_frac in enumerate(rem_frac_elem):
                ...
                proj_frac_rem_to_int[ii] = np.dot(
                    frac_cell_info_dict[ith_rem_frac].centroid - coord_frac_nodes[0],
                    dist_vec_fracs,
                ) / np.dot(dist_vec_fracs, dist_vec_fracs)
                new_proj_cen_rac_rem[:, ii] = (
                    coord_frac_nodes[0] + proj_frac_rem_to_int[ii] * dist_vec_fracs
                )
        ...
        centroid_int = (
            new_proj_cen_rac_i
            + new_proj_cen_rac_j
            + np.sum(new_proj_cen_rac_rem, axis=1)
        ) / (2 + len(rem_frac_elem))
        ...
        dist_frac_i = np.linalg.norm(
            frac_cell_info_dict[frac_elem_i].centroid - centroid_int
        )
        alpha_frac_i = (
            conn_area * frac_cell_info_dict[frac_elem_i].permeability / dist_frac_i
        )
        ...
        dist_frac_j = np.linalg.norm(
            frac_cell_info_dict[frac_elem_j].centroid - centroid_int
        )
        alpha_frac_j = (
            conn_area * frac_cell_info_dict[frac_elem_j].permeability / dist_frac_j
        )
        ...
        if rem_frac_elem.size:
            ...
            for ii, ith_rem_frac in enumerate(rem_frac_elem):
                ...
                dist_frac_rem[ii] = np.linalg.norm(
                    frac_cell_info_dict[ith_rem_frac].centroid - centroid_int
                )
