(
  ...
  let steps = args.recordingData?.processed_steps || args.recordingData?.processedSteps || []
  // 执行检测并存储结果
  ...
  function detectFramework() {
    ...
    function detectReact() {
      ...
      if (window.React) {
        ...
        results.details.push('Found window.React')
      }

      // 方法2: 检查 React DevTools
      if (window.__REACT_DEVTOOLS_GLOBAL_HOOK__) {
        results.react = true
        results.details.push('Found React DevTools Hook')
      }

      // 方法3: 检查 DOM 元素的 React 属性
      const reactElements = document.querySelectorAll('[data-reactroot], [data-reactid]')
      if (reactElements.length > 0) {
        results.react = true
        results.details.push('Found React DOM attributes')
      }
      ...
      if (rootElement && rootElement._reactRootContainer) {
        results.react = true
        results.details.push('Found React Root Container')
      }

      // 方法5: 检查元素的 React 内部属性
      const allElements = document.querySelectorAll('*')
      for (let element of allElements) {
        const keys = Object.keys(element)
        if (keys.some(key => key.startsWith('__reactInternalInstance') || key.startsWith('__reactFiber'))) {
          results.react = true
          results.details.push('Found React internal properties')
          break
        }
      }
    }
    ...
    function detectVue() {
      ...
      if (window.Vue) {
        results.vue = true
        results.vueVersion = window.Vue.version || 'Unknown'
        results.details.push('Found window.Vue')
      }

      // 方法2: 检查 Vue DevTools
      if (window.__VUE__) {
        results.vue = true
        results.details.push('Found Vue DevTools')
      }

      // 方法3: 检查 Vue 3 的全局属性
      if (window.__VUE_DEVTOOLS_GLOBAL_HOOK__) {
        results.vue = true
        results.details.push('Found Vue DevTools Hook')
      }

      // 方法4: 检查 DOM 元素的 Vue 属性
      const vueElements = document.querySelectorAll('[data-v-], [v-cloak]')
      if (vueElements.length > 0) {
        results.vue = true
        results.details.push('Found Vue directives in DOM')
      }
      ...
      for (let element of allElements) {
        if (element.__vue__ || element._vnode || element.__vueParentComponent) {
          results.vue = true
          results.details.push('Found Vue instance properties')
          break
        }
      }
      ...
      if (window.__VUE_PROD_DEVTOOLS__) {
        results.vue = true
        results.details.push('Found Vue 3 production devtools')
      }
    }
  ...
  function isInteractiveElement(element) {
    ...
    const steps = recordingData?.processed_steps || recordingData?.processedSteps || []
  ...
  function isElementDistinctInteraction(element) {
    if (frameworkInfo.react) {
      const reactHandlesKey = Object.keys(element).find(key =>
        key.startsWith('__reactEventHandlers') || key.startsWith('__reactProps')
      )
      if (reactHandlesKey) {
        const reactHandles = element[reactHandlesKey]
        const targetEvent = ["onClick", "onMouseDown", "onMouseUp", "onKeyDown", "onKeyUp", "onKeyPress", "onInput", "onFocus", "onBlur"]
        const hasEvent = targetEvent.some(event => reactHandles[event])
        if (hasEvent) {
          return true
        }
      }
  ...
  function getReactFiber(node) {
    ...
    // if (reactKey) {
    //   function findDatePickerComponent(fiber) {
    //     if (!!fiber.memoizedProps.onChange && !!fiber.memoizedProps.onSubmit && fiber.memoizedProps.prefixCls === 'ant-picker') {
    //       return fiber
    //     }
    //     return findDatePickerComponent(fiber.return)
    //   }
    //   return findDatePickerComponent(node[reactKey])
    // }
    if (reactKey) {
  ...
  function findAntdDatePickerProps(node) {
    const reactKey = Object.keys(node).find(key =>
      key.startsWith('__reactInternalInstance') ||
      key.startsWith('__reactFiber')
    )
    if (reactKey) {
      function findDatePickerComponent(fiber) {
        if (!!fiber.memoizedProps.onChange && !!fiber.memoizedProps.onSubmit) {
          return fiber
        }
        return findDatePickerComponent(fiber.return)
      }
  ...
  function simulateUserInput(input, component, value) {
    // 1. 聚焦
    input.focus()
    ...
    // 3. 模拟输入事
    component.pendingProps.onFocus()
    const inputEvent = new Event('input', { bubbles: true, cancelable: true })
    input.value = value
    input.dispatchEvent(inputEvent)

    // 4. 触发组件的 onChange
    if (component.pendingProps?.onChange) {
      component.pendingProps.onChange({
        target: input
      })
    }
  }

  function parseDatePickerElement(node) {
    if (frameworkInfo.react) {
      if (node.classList.contains("ant-picker")) {
        const component = findAntdDatePickerProps(node)
        const isRange = node.classList.contains('ant-picker-range')
        const result = {
          componentType: 'DatePicker',
          componentProvider: 'antd',
          componentProps: {
            isRange,
            showTime: component?.memoizedProps?.showTime ?? false,
            format: component?.memoizedProps?.format?.[0] ?? 'YYYY-MM-DD',
            // action: (value) => {
            //   if (isRange) {
            //     const startInput = node.querySelector('input[date-range="start"]')
            //     const endInput = node.querySelector('input[date-range="end"]')
            //     const start = getReactFiber(startInput)
            //     const end = getReactFiber(endInput)
            //     simulateUserInput(startInput, start, value[0])
            //     setTimeout(() => {
            ...
            //       simulateUserInput(endInput, end, value[1])
            //       end.pendingProps?.onBlur()
            //     }, 100)
            //   } else {
            //     const input = node.querySelector('input')
            //     simulateUserInput(input, getReactFiber(input), value)
        ...
        return result
      } else if (node.classList.contains("arco-picker")) {
        const fiber = getReactFiber(node)
        const component = fiber?.return
        const isRange = node.classList.contains('arco-picker-range')
        const format = component?.memoizedProps?.format ?? 'YYYY-MM-DD'
        const result = {
          componentType: 'DatePicker',
          componentProvider: 'arco',
          componentProps: {
            isRange,
            showTime: format.includes('HH:mm:ss') ?? false,
            format: format,
          }
        }
    ...
    } else if (frameworkInfo.vue) {
      if (node.classList.contains("el-date-editor") && !node.querySelector('input[readonly]')) {

        const isRange = node.classList.contains('el-range-editor')
        const classList = node.classList
        const showTime = [...classList].some(item => item.includes('time'))
        const result = {
          componentType: 'DatePicker',
          componentProvider: 'element-ui',
          componentProps: {
            isRange,
            showTime,
