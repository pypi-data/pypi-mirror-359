...
class Terminal:
    ...
    ENV_KEYS = ["NPM_CONFIG_REGISTRY", "UV_DEFAULT_INDEX", "PIP_INDEX_URL"]

    def __init__(self, name: str, default_wd: str):
        self.name = name
        self.default_wd = default_wd

    async def init(self, wd: str = None, env_keys: list[str] = None):
        self.history = []
        ...
        logger.info(f'Initializing terminal {self.name} with command: {command}, working directory: {wd or self.default_wd}, environment: {json.dumps(env)}')
        ...
        setup_commands = [
            ...
            f'export {key}="{os.environ[key]}"' for key in env_keys if key in os.environ
        ]
