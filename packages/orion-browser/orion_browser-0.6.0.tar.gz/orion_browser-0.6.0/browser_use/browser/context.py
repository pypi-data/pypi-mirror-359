...
from typing import TYPE_CHECKING, Any, Callable, List, Literal, Optional, TypedDict, cast
...
class BrowserContext:
    ...
    @time_execution_async('--do_date_picker')
    async def do_date_picker(self, element_node: DOMElementNode, date: str, date_range: List[str]):
        """
        Do date picker
        """
        # 获取组件属性
        is_range = element_node.component_props.get('isRange', False)
        date_format = element_node.component_props.get('format', 'YYYY-MM-DD')
        
        # 根据 isRange 属性选择使用的参数
        if is_range:
            # 使用日期范围参数
            if not date_range or len(date_range) < 2:
                raise ValueError("当 isRange 为 True 时，date_range 参数必须包含至少两个日期")
            ...
            for date_str in date_range:
                ...
                # 将 dayjs 格式转换为 Python datetime 格式
                python_format = self._convert_dayjs_to_python_format(date_format)
                formatted_date = parsed_date.strftime(python_format)
                formatted_dates.append(formatted_date)
                
            target_dates = formatted_dates
        else:
            # 使用单个日期参数
            if not date:
                raise ValueError("当 isRange 为 False 时，date 参数不能为空")
        ...
        try:
            ...
            if not element_handle:
                raise ValueError("无法定位到日期选择器元素")
            if is_range:
                # 处理日期范围选择
                if len(target_dates) < 2:
                    raise ValueError("日期范围必须包含开始和结束日期")
                
                # 查找所有输入框
                input_elements = await element_handle.query_selector_all('input')
                if len(input_elements) < 2:
                    raise ValueError(f"日期范围选择器需要至少2个输入框，但只找到了{len(input_elements)}个")
                
                # 获取开始和结束日期输入框
                start_input = input_elements[0]
                end_input = input_elements[1]
                
                # 输入开始日期
                await start_input.click()
                await start_input.fill(target_dates[0])
                
                # 模拟Tab键切换到结束日期输入框
                await start_input.press('Tab')
                await asyncio.sleep(0.1)  # 等待100ms，对应setTimeout
                
                # 输入结束日期
                await end_input.fill(target_dates[1])
                
                # 按下回车键确认输入
                await end_input.press('Enter')
                
                # 触发blur事件
                await end_input.evaluate('element => element.blur()')
                
            else:
                # 处理单个日期选择
                # 查找输入框
                input_element = await element_handle.query_selector('input')
                if not input_element:
                    raise ValueError("无法找到日期输入框")
                
                # 输入日期
                await input_element.click()
                await input_element.fill(target_dates[0])
            ...
            logger.error(f"日期选择器操作失败: {str(e)}")
            raise
    
    def _convert_dayjs_to_python_format(self, dayjs_format: str) -> str:
        """
        将 dayjs 格式转换为 Python datetime 格式
        """
        # dayjs 到 Python datetime 的格式映射
        format_mapping = {
            'YYYY': '%Y',  # 四位年份
            'YY': '%y',    # 两位年份
            'MM': '%m',    # 月份 (01-12)
            ...
            'mm': '%M',    # 分钟 (00-59)
            'ss': '%S',    # 秒钟 (00-59)
            'SSS': '%f',   # 毫秒 (000-999) - 在Python中是微秒
        }
        
        python_format = dayjs_format
        for dayjs_token, python_token in format_mapping.items():
            python_format = python_format.replace(dayjs_token, python_token)
        ...
        try:
            ...
            if 'class' in element.attributes and element.attributes['class'] and include_dynamic_attributes:
                ...
                for class_name in classes:
                    ...
                    # Escape special characters in class names (Tailwind CSS syntax)
                    escaped_class_name = (class_name
                                         .replace(':', r'\:')      # Pseudo-classes like hover:
                                         .replace('!', r'\!')      # Important modifier like !w-full
                                         .replace('[', r'\[')      # Arbitrary values like w-[100px]
                                         .replace(']', r'\]')      # Arbitrary values like w-[100px]
                                         .replace('/', r'\/')      # Fractions like w-1/2
                                         .replace('%', r'\%')      # Percentages like w-50%
                                         .replace('#', r'\#')      # Hash in colors like bg-[#ff0000]
                                         .replace('(', r'\(')      # Parentheses
                                         .replace(')', r'\)')      # Parentheses
                                         .replace('.', r'\.')      # Dots in decimal values
                                         .replace('&', r'\&')      # Ampersand in [&_svg] syntax
                                         .replace('>', r'\>')      # Greater than in has-[>svg]
                                         .replace('<', r'\<')      # Less than symbol
                                         .replace('*', r'\*')      # Asterisk in [class*='size-']
                                         .replace("'", r"\'")      # Single quotes
                                         .replace('"', r'\"')      # Double quotes
                                         .replace('=', r'\=')      # Equals sign in attribute selectors
                                         .replace('^', r'\^')      # Caret in attribute selectors
                                         .replace('$', r'\$')      # Dollar sign in attribute selectors
                                         .replace('|', r'\|')      # Pipe symbol
                                         .replace('~', r'\~')      # Tilde in attribute selectors
                                         .replace(' ', r'\ ')      # Spaces (though they shouldn't be in class names)
                                         )
                    
                    # Append the escaped class name to the CSS selector
                    css_selector += f'.{escaped_class_name}'
    ...
    async def get_locate_element(self, element: DOMElementNode) -> Optional[ElementHandle]:
        ...
        logger.info('css_selector', css_selector)
