...
class QueryBuilder:
    ...
    @staticmethod
    def sample_query(table_name: str, limit: int, dialect: Optional[str] = None) -> Tuple[str, dict]:
        """
        ...
        Args:
            ...
            dialect: Database dialect name (e.g., 'mssql', 'postgresql', 'mysql')
            
        Returns:
            Tuple of (query, parameters)
        """
        if dialect == 'mssql':
            return f"SELECT TOP (:limit) * FROM {table_name}", {"limit": limit}
        else:
