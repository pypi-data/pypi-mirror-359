...
class Route:
    ...
    def from_json(
        ...
        """
        # Extract the query SMILES from the 'overview' section
    ...
    def _extract_reactants_from_reaction_smiles(
        ...
        if reactants_string:
            raise ValueError(
                f"Unknown compound(s) '{reactants_string}' left in LHS of reaction smiles '{reaction_smiles}'."
            )
...
def _smiles_to_svg(smiles: str) -> tuple[str, float, float]:
    """
    Generates an SVG representation of a molecule based on a provided SMILES string.
    The function uses RDKit to parse the SMILES string into a molecule, generate the
    SVG for rendering, and extracts the tightly cropped dimensions of the generated
    SVG. A default placeholder SVG is provided for invalid SMILES strings.

    Args:
        smiles: A string representing a molecule in SMILES format.

    Returns:
        A tuple containing the SVG content as a string, the width of the SVG with
        padding, and the height of the SVG with padding.
    """
    ...
    # Extract the full width and height of the SVG canvas (including whitespace).
    width_match: Optional[Match[str]] = re.search(r"width=['\"]([\d.]+)px['\"]", svg)
    height_match: Optional[Match[str]] = re.search(r"height=['\"]([\d.]+)px['\"]", svg)

    full_width: float = float(width_match.group(1)) if width_match else 150
    full_height: float = float(height_match.group(1)) if height_match else 100

    # Define a relative factor for cropping. 0.85 means the final node
    # will be 85% of the original SVG canvas size. This value can be
    # tuned to achieve the desired aesthetic.
    crop_factor: float = 0.80

    # Calculate the final node dimensions as a percentage of the full SVG size.
    node_width: float = full_width * crop_factor
    node_height: float = full_height * crop_factor

    # Return the full SVG, but the smaller, cropped dimensions for the node.
    return svg, node_width, node_height
...
def _copy_static_directory(directory_name: str, output_dir: Path):
    ...
    try:
        ...
        logger.error(f"{error_message} - Original error: {exception}")
        raise HtmlGenerationError(error_message) from exception
...
def generate_html_report(output_dir: Path, *json_results: Dict[str, Any]) -> None:
    ...
    # re-raise any exception we expect downstream as HtmlGenerationError
        ...
        error_message: str = "Failed to generate HTML report"
        logger.error(f"{error_message} - Original error: {exception}")
        raise HtmlGenerationError(f"{error_message}: {exception}") from exception


# --- Helper method for functionality implemented in __main__ ---
