...
def stitch_crop(patch_size, overlap_size, original_image_path, seg_path, stitch_path):
    ...
    PredNameList = [
        os.path.join(seg_dir, file).replace("\\", "/")
        for file in os.listdir(seg_dir)
        if (
            file.startswith(seg_basename)
            and (file.endswith(".PNG") or file.endswith(".png"))
        )
...
def stitch_crop_folder(
    ...
    """
    imgs = [
        os.path.relpath(os.path.join(root, file), original_image_folder).replace(
            "\\", "/"
        )
