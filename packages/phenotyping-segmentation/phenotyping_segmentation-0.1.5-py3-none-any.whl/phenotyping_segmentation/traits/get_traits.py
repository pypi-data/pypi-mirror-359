...
def get_traits_cylinder(
    ...
    imgs = [
        file for file in os.listdir(seg_path) if file.lower().endswith(valid_extensions)
    ]
    ...
    for img_name in imgs:
        ...
        if area > 0:
            ...
            x_centroid = y_centroid = angle_avg = shallow = medium = steep = (
                total_length
            ) = primary_root_length = lateral_root_length = lateral_root_number = (
                average_lateral_root_length
            ) = np.nan
            angle_whole = angle_top = angle_bottom = angle_left_most = (
                angle_right_most
            ) = angle_left_top = angle_right_top = angle_left_bottom = (
                angle_right_bottom
            ) = np.nan
