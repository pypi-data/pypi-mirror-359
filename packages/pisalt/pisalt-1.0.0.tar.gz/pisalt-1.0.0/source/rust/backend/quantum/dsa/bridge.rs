...
// MAIN
fn dsaalgorithm(name: &str) -> Result<Sig, pyo3::PyErr> {
...
#[pyo3::pyfunction]
pub fn dsakeygen(name: &str) -> pyo3::PyResult<(Vec<u8>, Vec<u8>)> {
...
#[pyo3::pyfunction]
pub fn dsasign(name: &str, secretkey: &[u8], message: &[u8]) -> pyo3::PyResult<Vec<u8>> {
    let algorithm = dsaalgorithm(name)?;
...
#[pyo3::pyfunction]
pub fn dsaverify(name: &str, publickey: &[u8], signature: &[u8], message: &[u8]) -> pyo3::PyResult<bool> {
    let algorithm = dsaalgorithm(name)?;
