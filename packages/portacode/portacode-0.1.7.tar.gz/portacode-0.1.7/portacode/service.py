...
class _WindowsTask:
    ...
    def install(self) -> None:
        ...
        # Ensure log directory exists and create wrapper cmd script
        self._script_path.parent.mkdir(parents=True, exist_ok=True)
        if use_pyw:
            script = f"""@echo off
cd /d %USERPROFILE%
"{pyw}" -m portacode connect --non-interactive >> "%USERPROFILE%\.local\share\portacode\connect.log" 2>>&1
"""
        else:
            script = f"""@echo off
cd /d %USERPROFILE%
...
"""
        ...
        # Auto-start immediately so user doesn't need to log off/on
        self.start()
