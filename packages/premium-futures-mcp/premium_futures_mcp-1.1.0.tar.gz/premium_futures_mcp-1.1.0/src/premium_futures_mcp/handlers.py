        ...
        async with BinanceClient(self.config) as client:
            ...
            if name == "get_account_info":
                ...
                if "assets" in account_data:
                    ...
                    account_data["non_zero_assets_count"] = len(non_zero_assets)
                    account_data["original_assets_count"] = original_asset_count
                    account_data["note"] = "Optimized response - showing only non-zero assets"
                    
                    # Add debug info if no non-zero assets found
                    if len(non_zero_assets) == 0:
                        account_data["debug_note"] = f"No non-zero assets found out of {original_asset_count} total assets"
                ...
                optimized_response = ResponseOptimizer.optimize_balance(balance_data)
                
                # Check if there was an error in parsing
                if "error" in optimized_response:
                    return {
                        "parsing_error": "Failed to parse balance data",
                        "error_details": optimized_response,
                        "raw_data_sample": balance_data[:3] if isinstance(balance_data, list) else balance_data,
                        "note": "There was an issue parsing the balance data. Check error_details for more information."
                    }
                
                # Add debug information to help troubleshoot balance issues
                debug_info = {
                    "raw_response_count": len(balance_data) if isinstance(balance_data, list) else 0,
                    "raw_sample": balance_data[:2] if isinstance(balance_data, list) and len(balance_data) > 0 else balance_data,
                    "optimized_response": optimized_response
                }
                
                # If no non-zero balances found, return debug info
                if optimized_response.get("asset_count", 0) == 0:
                    return {
                        "message": "No non-zero balances found",
                        "debug": debug_info,
                        "parsing_debug": optimized_response.get("debug", {}),
                        "note": "If you expect to see balances, check the raw_sample and parsing_debug data above"
                    }
                
                return optimized_response
            elif name == "get_balance_debug":
                balance_data = await client._make_request("GET", "/fapi/v3/balance", security_type="USER_DATA")
                return {
                    "raw_balance_data": balance_data,
                    "data_type": type(balance_data).__name__,
                    "count": len(balance_data) if isinstance(balance_data, list) else "N/A",
                    "note": "This is the unfiltered response from Binance API"
                }
            elif name == "get_account_info_debug":
                account_data = await client._make_request("GET", "/fapi/v3/account", security_type="USER_DATA")
                return {
                    "raw_account_data": account_data,
                    "data_type": type(account_data).__name__,
                    "assets_count": len(account_data.get("assets", [])) if isinstance(account_data, dict) else "N/A",
                    "note": "This is the unfiltered response from Binance API /fapi/v3/account endpoint"
                }
            elif name == "test_api_connection":
                try:
                    # Test server time (public endpoint)
                    server_time = await client._make_request("GET", "/fapi/v1/time")
                    
                    # Test account access (authenticated endpoint)
                    try:
                        account_status = await client._make_request("GET", "/fapi/v3/account", security_type="USER_DATA")
                        auth_test = {
                            "status": "success",
                            "account_total_assets": len(account_status.get("assets", [])) if isinstance(account_status, dict) else 0,
                            "account_keys_present": list(account_status.keys()) if isinstance(account_status, dict) else []
                        }
                    ...
                    return {
                        ...
                        "connection_status": "API connection working"
                    }
