...
class ProxyFleet:
    ...
    async def load_proxies(self, proxy_data: Union[str, List[Dict[str, Any]], List[ProxyServer]]):
        """
        ...
        async def check_single_proxy(proxy: ProxyServer):
            async with health_semaphore:
                try:
                    ...
                    if is_healthy:
                        logger.debug(f"Proxy {proxy.host}:{proxy.port} is healthy (response_time: {response_time:.2f}s)")
                    else:
                        logger.warning(f"Proxy {proxy.host}:{proxy.port} is unhealthy: {error}")
        ...
        for attempt in range(task.max_retries + 1):
            try:
                ...
                if use_direct_connection:
                    ...
                    status_code, headers, data, response_time = await self._make_direct_request(
                        url=task.url,
                    ...
                    status_code, headers, data, response_time = await make_request_with_proxy(
                        proxy=proxy,
                        url=task.url,
                        method=task.method.value,
                        headers=task.headers,
                ...
                logger.debug(f"Task {task.task_id} completed successfully via {result.proxy_used}")
                break
                ...
                logger.warning(f"Task {task.task_id} attempt {attempt + 1} failed via {result.proxy_used}: {error_msg}")
    ...
    async def execute_tasks(self, tasks: List[HttpTask]) -> List[TaskResult]:
        ...
        logger.info(f"Completed {len(tasks)} tasks: {successful} successful, {len(tasks) - successful} failed")
