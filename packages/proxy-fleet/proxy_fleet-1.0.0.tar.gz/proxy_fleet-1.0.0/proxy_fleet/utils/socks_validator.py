...
class ValidationResult:
    ...
    def __init__(self, is_valid: bool, version: Optional[SocksVersion] = None, 
                 ip_info: Optional[Dict[str, Any]] = None, error: Optional[str] = None):
...
class SocksValidator:
    ...
    async def check_ip_info_via_proxy(self, host: str, port: int, protocol: str = "socks5") -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get('https://ipinfo.io/json') as response:
                    if response.status == 200:
                        ...
                        logger.debug(f"IP info via {host}:{port}: {data.get('ip')} ({data.get('country')})")
                        return data
                    else:
            ...
            logger.warning("aiohttp-socks package required for IP info check: pip install aiohttp-socks")
            return None
        except Exception as e:
            logger.debug(f"Failed to get IP info via {host}:{port}: {e}")
            return None

    def validate_socks4(self, host: str, port: int, target_host: str = "8.8.8.8", target_port: int = 80) -> ValidationResult:
        """
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"SOCKS4 {host}:{port} - Cannot resolve target host: {target_host}")
                return ValidationResult(is_valid=False, error="Cannot resolve target host")
            ...
            if response[0] != int("0x00", 16):  # First byte should be 0x00 for SOCKS4 response
            ...
            if response[1] != int("0x5A", 16):  # 0x5A = request granted  
                logger.debug(f"SOCKS4 {host}:{port} - Server returned error (code: {response[1]})")
                return ValidationResult(is_valid=False, error=f"Server returned error (code: {response[1]})")
            
            logger.debug(f"SOCKS4 {host}:{port} - Handshake successful")
            
            # Note: IP info check moved to async methods to avoid event loop issues
            return ValidationResult(is_valid=True, version=SocksVersion.SOCKS4, ip_info=None)
    ...
    def validate_socks5(self, host: str, port: int) -> ValidationResult:
        ...
        try:
            ...
            if len(auth_response) < 2:
                ...
                return ValidationResult(is_valid=False, error="Null authentication response")
            ...
            # Note: IP info check moved to async methods to avoid event loop issues
            return ValidationResult(is_valid=True, version=SocksVersion.SOCKS5, ip_info=None)
    ...
    def _test_socks4_on_socket(self, sock: socket.socket, host: str, port: int, 
                              target_host: str = "8.8.8.8", target_port: int = 80) -> bool:
    ...
    def validate_http(self, host: str, port: int, test_url: str = "http://httpbin.org/ip") -> ValidationResult:
        """
        驗證 HTTP 代理伺服器
        ...
        try:
            ...
            with opener.open(request, timeout=self.timeout) as response:
                if response.getcode() == 200:
                    ...
                    return ValidationResult(is_valid=False, error=f"HTTP {response.getcode()}")
    ...
    async def async_validate_http(self, host: str, port: int, test_url: str = "http://httpbin.org/ip") -> ValidationResult:
        """Async wrapper for HTTP proxy validation with IP info check"""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, self.validate_http, host, port, test_url)
    ...
    async def async_validate_socks4(self, host: str, port: int, target_host: str = "8.8.8.8", target_port: int = 80) -> ValidationResult:
        """Async wrapper for SOCKS4 validation with IP info check"""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, self.validate_socks4, host, port, target_host, target_port)
...
class ProxyDownloader:
    ...
    async def download_proxy_list(self, proxy_type: str, limit: Optional[int] = None) -> list:
        """
...
async def test_socks_validation():
    ...
    try:
        ...
        print(f"🔍 Testing {len(socks4_proxies)} SOCKS4 proxies with raw socket handshake...")
    ...
    try:
        ...
        print(f"🔍 Testing {len(socks5_proxies)} SOCKS5 proxies with raw socket handshake...")
