...
class FleetConfig(BaseModel):
    ...
    # File paths
    proxy_file: str = Field(default="proxies.json", description="Path to proxy configuration file")
    output_dir: str = Field(default="output", description="Directory for storing request results")
    ...
    max_concurrent_requests: int = Field(default=20, ge=1, description="Maximum concurrent requests")
    max_concurrent_health_checks: int = Field(default=10, ge=1, description="Maximum concurrent health checks")
    
    # Failure handling
    max_recent_failures: int = Field(default=5, ge=1, description="Max failures before marking proxy unhealthy")
    failure_window_minutes: int = Field(default=1, ge=1, description="Time window for recent failure tracking")
    unhealthy_retry_interval: int = Field(default=600, ge=60, description="Retry interval for unhealthy proxies in seconds")
    ...
    # Output settings
    save_response_data: bool = Field(default=True, description="Whether to save response data to files")
    max_response_size: int = Field(default=10485760, ge=1024, description="Maximum response size to save (bytes)")
    compress_output: bool = Field(default=False, description="Whether to compress output files")
