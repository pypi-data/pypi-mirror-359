...
class OutputManager:
    ...
    def save_response_data(
        self, result: TaskResult, max_size: int = 10485760
    ) -> Optional[str]:
        """
        ...
        """
        if (
            not result.response_data
            or result.response_size
            and result.response_size > max_size
        ):
            return None
        ...
        try:
            ...
            if self.compress:
                with gzip.open(
                    file_path, mode, encoding="utf-8" if "t" in mode else None
                ) as f:
                    f.write(data)
            else:
                with open(
                    file_path, mode, encoding="utf-8" if "b" not in mode else None
                ) as f:
                    f.write(data)
