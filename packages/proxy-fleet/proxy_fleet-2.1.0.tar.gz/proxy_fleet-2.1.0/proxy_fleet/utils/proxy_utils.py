...
async def test_proxy_health(
    proxy: ProxyServer, test_urls: List[str], timeout: int = 10
...
async def make_request_with_proxy(
    ...
    try:
        ...
        if proxy.protocol.value in ["http", "https"]:
            ...
            try:
                ...
                connector = ProxyConnector(
                    ...
                    port=proxy.port,
                    username=(
                        proxy.username if proxy.protocol.value == "socks5" else None
                    ),
                    password=(
                        proxy.password if proxy.protocol.value == "socks5" else None
                    ),
                )
                proxy_url = None
                ...
                raise Exception(
                    "SOCKS proxy support requires aiohttp-socks: pip install aiohttp-socks"
                )
        else:
        ...
        async with aiohttp.ClientSession(
            ...
            request_kwargs = {"headers": request_headers, "data": data}
        ...
        response_time = (datetime.now() - start_time).total_seconds()
        raise Exception(
            f"Request failed through proxy {proxy.host}:{proxy.port}: {str(e)}"
        )
