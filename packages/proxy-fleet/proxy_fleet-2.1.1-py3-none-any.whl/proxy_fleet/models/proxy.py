...
class ProxyServer(BaseModel):
    ...
    status: ProxyStatus = Field(
        default=ProxyStatus.UNKNOWN, description="Current proxy status"
    )
    ...
    recent_failures: int = Field(
        default=0, ge=0, description="Failures in recent time window"
    )
    average_response_time: Optional[float] = Field(
        default=None, ge=0, description="Average response time in seconds"
    )

    # Configuration
    max_recent_failures: int = Field(
        default=5, ge=1, description="Max failures before marking unhealthy"
    )
