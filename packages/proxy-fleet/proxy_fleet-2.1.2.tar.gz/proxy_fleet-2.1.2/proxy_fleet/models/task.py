...
class HttpTask(BaseModel):
    ...
    params: Optional[Dict[str, str]] = Field(
        default_factory=dict, description="URL parameters"
    )
...
class TaskResult(BaseModel):
    ...
    proxy_used: Optional[str] = Field(
        default=None, description="Proxy server used (host:port)"
    )
    ...
    response_time: Optional[float] = Field(
        default=None, ge=0, description="Response time in seconds"
    )
    ...
    response_data: Optional[Union[str, bytes]] = Field(
        default=None, description="Response body"
    )
    response_size: Optional[int] = Field(
        default=None, ge=0, description="Response size in bytes"
    )
    ...
    @classmethod
    def from_task(
        cls, task: HttpTask, status: TaskStatus = TaskStatus.PENDING
    ) -> "TaskResult":
