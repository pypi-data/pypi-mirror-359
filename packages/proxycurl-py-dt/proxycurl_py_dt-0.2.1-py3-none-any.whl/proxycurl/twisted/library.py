...
class _LinkedinPerson:
    ...
    def search(
        ...
        This API endpoint is powered by [LinkDB](https://enrichlayer.com/proxycurl/linkdb), our exhaustive dataset of people and company profiles.
        ...
        :param current_role_before: Filter people who started their current role **before** this date. You'll be looking for profiles on [LinkDB](https://enrichlayer.com/proxycurl/linkdb) that show a person's current job. However, keep in mind that some of these profiles may not be up-to-date, which means you might sometimes see a person's old job instead of their current job on LinkedIn.

            This parameter takes a ISO8601 date. Default value of this parameter is `null`.
        :type current_role_before: str
        :param current_role_after: Filter people who started their current role **after** this date. You'll be looking for profiles on [LinkDB](https://enrichlayer.com/proxycurl/linkdb) that show a person's current job. However, keep in mind that some of these profiles may not be up-to-date, which means you might sometimes see a person's old job instead of their current job on LinkedIn.
    ...
    def resolve(
        ...
        :param enrich_profile: Enrich the result with a cached profile of the lookup result.
            ...
            * `enrich`: enriches the result with cached profile data
            ...
            If you require [fresh profile data](https://enrichlayer.com/blog/how-fresh-are-profiles-returned-by-proxycurl-api/),
            please chain this API call with the [People Profile Endpoint](https://enrichlayer.com/proxycurl/docs#people-api-person-profile-endpoint) with the `use_cache=if-recent` parameter.
    ...
    def resolve_by_email(
        ...
        :param enrich_profile: Enrich the result with a cached LinkedIn profile of the LinkedIn Profile URL result (if any).
            ...
            * `enrich`: enriches the result with cached profile data.
            ...
            If you require [fresh profile data](https://enrichlayer.com/blog/how-fresh-are-profiles-returned-by-proxycurl-api/),  please chain this API call with the `linkedin_profile_url` result with the [Person Profile Endpoint](https://enrichlayer.com/proxycurl/docs#people-api-person-profile-endpoint) with the `use_cache=if-recent` parameter.
        :type enrich_profile: str
        :return: An object of Deferred or **None** if there is an error.
        :rtype: Deferred
        :raise ProxycurlException: Every error will raise a :class:`proxycurl.twisted.ProxycurlException`
    ...
    def profile_picture(
        ...
        Profile pictures are served from cached people profiles found within [LinkDB](https://enrichlayer.com/proxycurl/linkdb).
        If the profile does not exist within [LinkDB](https://enrichlayer.com/proxycurl/linkdb), then the API will return a `404` status code.
...
class _LinkedinCompany:
    ...
    def get(
        ...
        :param resolve_numeric_id: Enable support for Company Profile URLs with numerical IDs that you most frequently fetch from Sales Navigator.
            We achieve this by resolving numerical IDs into vanity IDs with cached company profiles from [LinkDB](https://enrichlayer.com/proxycurl/linkdb).
    ...
    def search(
        ...
        Search for companies that meet a set of criteria within
            ...
            This API endpoint is powered by [LinkDB](https://enrichlayer.com/proxycurl/linkdb), our exhaustive dataset of company profiles.
    ...
    def resolve(
        ...
        :param enrich_profile: Enrich the result with a cached profile of the lookup result.
            ...
            * `enrich`: enriches the result with cached profile data
            ...
            If you require [fresh profile data](https://enrichlayer.com/blog/how-fresh-are-profiles-returned-by-proxycurl-api/),
            please chain this API call with the [Company Profile Endpoint](https://enrichlayer.com/proxycurl/docs#company-api-company-profile-endpoint) with the `use_cache=if-recent` parameter.
        :type enrich_profile: str
        :return: An object of Deferred or **None** if there is an error.
        :rtype: Deferred
        :raise ProxycurlException: Every error will raise a :class:`proxycurl.twisted.ProxycurlException`
    ...
    def find_job(
        ...
        :param geo_id: The `geo_id` of the location to search for.
            ...
            See [this article](https://enrichlayer.com/blog/how-to-fetch-geo_id-parameter-for-the-job-api/?utm_source=blog&utm_medium=web&utm_campaign=docs-redirect-to-geo_id-article) as to how you may be able to match regions to `geo_id` input values.
        :type geo_id: str
    ...
    def employee_list(
        ...
        This API endpoint is powered by [LinkDB](https://enrichlayer.com/proxycurl/linkdb), our comprehensive dataset of people and company profiles.
    ...
    def employee_search(
        ...
        This API endpoint is powered by [LinkDB](https://enrichlayer.com/proxycurl/linkdb), our comprehensive dataset of people
    ...
    def profile_picture(
        ...
        Profile pictures are served from cached company profiles found within [LinkDB](https://enrichlayer.com/proxycurl/linkdb).
        If the profile does not exist within [LinkDB](https://enrichlayer.com/proxycurl/linkdb), then the API will return a `404` status code.
