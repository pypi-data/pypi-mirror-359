...
class AgentBase(ABC):
    ...
    async def run(
        self, prompt: str, postprocess_fn: Callable[[str], Any] | None = None, **kwargs: Any,
    ) -> AgentRunner:
        """Run with retries"""
        agent = self.create_agent()
        try:
            await self.runner.run(agent, prompt, postprocess_fn=postprocess_fn, **kwargs)
        except Exception:
            await self._switch_credential()
            return await self.run(prompt, postprocess_fn=postprocess_fn, **kwargs)
        return self.runner
    ...
    async def run_stream(
        ...
        **kwargs: Any,
        ...
        agent = self.create_agent()
        try:
            await self.runner.run_stream(agent, prompt, postprocess_fn=postprocess_fn, **kwargs)
        except Exception:
            await self._switch_credential()
            await self.run_stream(prompt, postprocess_fn=postprocess_fn, **kwargs)
        return self.runner
