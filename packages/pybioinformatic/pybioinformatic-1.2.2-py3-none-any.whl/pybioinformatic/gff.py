...
# Rename method=========================================================================================================
    ...
    def __rename_mRNA(grouped_df: DataFrame):
        ...
        if mRNAs.Strand.unique()[0] == '+':
            mRNAs.sort_values(by=['Chromosome', 'Start'], ascending=True, inplace=True)
        else:
            mRNAs.sort_values(by=['Chromosome', 'Start'], ascending=False, inplace=True)
        mRNAs['Number'] = range(1, len(mRNAs) + 1)
        mRNAs['New_ID'] = [f'{loci}.{i}' for i in mRNAs.Number]
        grouped_df['New_ID'].fillna(mRNAs.New_ID, inplace=True)
