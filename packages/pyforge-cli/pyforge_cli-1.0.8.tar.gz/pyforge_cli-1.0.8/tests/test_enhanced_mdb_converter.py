...
from pyforge_cli.readers.dual_backend_mdb_reader import DualBackendMDBReader
...
class TestEnhancedMDBConverter:
    ...
    def test_list_tables_with_space_names(self):
        ...
        mock_reader.list_tables.return_value = [
            'Customers', 'Employee Privileges', 'Order Details', 
            'Purchase Orders', 'Sales Reports'
        ]
        mock_reader.get_active_backend.return_value = 'UCanAccess'
    ...
    def test_get_table_info_success(self):
        ...
        # Check column info
        name_column = next(col for col in table_info['columns'] if col['name'] == 'Name')
        assert name_column['nullable'] == True  # Has null value
    ...
    def test_validate_input_valid_mdb(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.mdb') as tmp_file:
            ...
            with patch('pyforge_cli.converters.enhanced_mdb_converter.DatabaseFileDetector') as mock_detector_class:
                mock_detector = Mock()
...
class TestEnhancedMDBConverterIntegration:
    ...
    def test_backend_selection_ucanaccess_preferred(self, mock_pyodbc_class, mock_ucanaccess_class, mock_detect):
        ...
        with patch('pyforge_cli.converters.enhanced_mdb_converter.DualBackendMDBReader') as mock_reader_class:
            mock_reader = Mock()
            mock_reader.connect.return_value = True
            mock_reader.get_active_backend.return_value = 'UCanAccess'
            mock_reader.get_connection_info.return_value = {'backend': 'UCanAccess'}
    ...
    def test_convert_with_progress_structure(self, mock_console_class):
        ...
        with tempfile.NamedTemporaryFile(suffix='.accdb') as tmp_file:
            ...
            with patch.object(converter, '_connect_to_database') as mock_connect, \
                 patch.object(converter, '_list_tables') as mock_list, \
                 patch.object(converter, '_get_table_info') as mock_info, \
                 patch.object(converter, '_convert_tables_to_parquet') as mock_convert, \
                 patch.object(converter, '_close_connection') as mock_close, \
                 patch('pyforge_cli.converters.enhanced_mdb_converter.detect_database_file') as mock_detect:
