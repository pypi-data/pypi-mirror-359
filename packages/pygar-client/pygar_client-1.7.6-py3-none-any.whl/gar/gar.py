...
def walk_decimal(obj: Any) -> Any:
    """ " Convert all floats to decimal."""
    if isinstance(obj, list):
        return [walk_decimal(v) for v in obj]
    if isinstance(obj, dict):
        return {k: walk_decimal(v) for k, v in obj.items()}
        ...
        while self.connected and self.running:
            try:
                if self.websocket:
                    ...
                    raw_msg = self.decoder.decode(message)
                    msg = walk_decimal(raw_msg)
