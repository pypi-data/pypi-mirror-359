...
def get_max_id(client: MilvusClient, collection: str) -> int:
    ...
    try:
        # Get collection stats to check if it has data
        stats = client.get_collection_stats(collection_name=collection)
        row_count = stats.get("row_count", 0)
        
        if row_count == 0:
            logger.info("Collection is empty, starting from ID 0")
            return 0
            
        # For now, use a safe starting ID based on timestamp
        # This ensures uniqueness even if we can't query the exact max ID
        safe_start_id = int(time.time()) * 10000
        logger.info(f"Using safe starting ID: {safe_start_id} (collection has {row_count} rows)")
...
def create_collection(client: MilvusClient, name: str, drop_if_exists: bool = True):
...
def ingest(threads, compare_interval, duration, uri, token, pg_conn, collection, drop_existing):
...
def validate(uri, pg_conn, collection, full_scan):
