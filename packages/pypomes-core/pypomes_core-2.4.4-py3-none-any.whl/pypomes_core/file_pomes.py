...
def file_get_mimetype(file_data: Path | str | bytes) -> Mimetype | str:
    ...
    if not mimetype and isinstance(file_data, Path):
        mimetype, _encoding = mimetypes.guess_file_type(file_data)
...
def file_is_pdf(file_data: Path | str | bytes) -> bool:
    ...
    The heuristics used, as heuristics go, provide an educated guess, not an accurate result.
    In the present case, the file is flagged as *PDF* if its first 4 characters is the string *%PDF*.
