...
def db_count(errors: list[str] | None,
    ...
    If not a positive integer, *offset_count* is ignored. The targer database engine, specified or default,
    must have been previously configured. The parameter *committable* is relevant only if *connection*
    is provided, and is otherwise ignored. A rollback is always attempted, if an error occurs.
...
def db_exists(errors: list[str] | None,
    ...
    If not positive integers, *min_count*, *max_count*, and *offset-count* are ignored. If both *min_count*
    and *max_count* are specified with equal values, then exactly that number of tuples must exist.
    If neither one is  specified, than at least one tuple is expected to exist.
