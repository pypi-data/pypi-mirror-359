...
@pytest.fixture(scope="module")
def _grpc_aio_server(request, grpc_addr, grpc_interceptors):
    max_workers = request.config.getoption("grpc-max-workers")
    try:
        max_workers = max(request.module.grpc_max_workers, max_workers)
    except AttributeError:
...
async def grpc_aio_server(
    _grpc_aio_server, grpc_addr, grpc_add_to_server, grpc_servicer
):
    grpc_add_to_server(grpc_servicer, _grpc_aio_server)
    _grpc_aio_server.add_insecure_port(grpc_addr)
    await _grpc_aio_server.start()
    yield _grpc_aio_server
    await _grpc_aio_server.stop(grace=None)


@pytest.fixture(scope="module")
def grpc_aio_create_channel(request, grpc_addr, grpc_aio_server):
    def _create_channel(credentials=None, options=None):
        if request.config.getoption("grpc-fake"):
            return FakeAIOChannel(grpc_server, credentials)
        if credentials is not None:
...
def grpc_aio_stub(grpc_stub_cls, grpc_aio_channel):
    return grpc_stub_cls(grpc_aio_channel)
