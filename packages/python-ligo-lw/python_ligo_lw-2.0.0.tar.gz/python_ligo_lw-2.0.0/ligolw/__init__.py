...
class Param(Element):
	...
	def from_pyvalue(cls, name, value, **kwargs):
		...
		<Param Name="example" Type="lstring">test</Param>
		>>> # short string (non-empty data = not NULL)
		>>> Param.from_pyvalue("example", "").write(sys.stdout)
		<Param Name="example" Type="lstring"> </Param>
...
class Stream(Element):
	...
	Encoding = attributeproxy("Encoding", enc = (lambda x: ",".join(sorted(x))), dec = (lambda s: set(map(str.strip, s.split(",")) if s else ())), default = set())
	Name = attributeproxy("Name")
