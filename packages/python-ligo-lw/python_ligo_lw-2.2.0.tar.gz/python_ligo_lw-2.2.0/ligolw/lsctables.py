...
class segmentproperty(object):
	"""
	Descriptor used internally to expose a start and stop pair of
	values as a segment-valued property.  A segment may be set to None,
	which is encoded by setting both start and stop values to None.
	Likewise if both start and stop are None then the value reported by
	this descriptor is None, not (None, None).

	For start and or stop values that are GPS-valued, see the
	documentation for gpsproperty for more information on the encodings
	it uses for special values and the limitations they create.
	"""
