/*
 * Copyright (C) 2006-2009,2011,2014-2017,2019,2021--2022,2024--2025  Kipp C. Cannon
 *
 ...
 * characters.
 *
 * NOTE:  the documentation states that only " is accepted as a quote
 * character.  The fact that ' is also accepted is an undocumented feature
 * to improve compatibility with non-compliant implementations.  There is a
 * performance penalty for supporting multiple quote characters, and this
 * flexibility might be removed in the future.
 *
 * FIXME:  because we don't warn people that a token starting with ' will
 * be interpreted as a quoted string instead of an unquoted string whose
 * first character is ', there is a risk that this flexibility will corrupt
 * documents.  normally all strings in a Stream are quoted unconditionally
 * because I/O performance is valued and it's too costly for writing codes
 * to test each string to determine if quoting is necessary.  therefore,
 * the real risk is nearly 0.  nevertheless, there is this additional
 * reason for removing support for multiple quote characters in the future.
 */
...
{
	PyObject *buffer_str = PyUnicode_FromWideChar(buffer, buffer_length);
	PyObject *pos_str = PyUnicode_FromWideChar(pos, 1);
 ...
 * the difference between a zero-length token and an absent token.  The
 * return value is the Python type to which the text should be converted,
 * or NULL on error.  On error, the values of start and end are undefined.
 * Raises StopIteration if the end of the tokenizer's internal buffer is
 * reached, or ValueError if a parse error occurs.
 *
...
static PyObject *next_token(ligolw_Tokenizer *tokenizer, wchar_t **start, wchar_t **end)
{
	...
	/*
	 * Here, a complete, delimiter-terminated, token has been found.
	 */

	/*
	 * Advance tokenizer->pos to the first character after the
	 * delimiter that terminated this token (the first character to be
	 * processed for the next token).
	 */

	tokenizer->pos = ++pos;

	/*
	 * Advance the type pointer to the next token's type
	 */
...
static PyObject *append(PyObject *self, PyObject *data)
{
	...
	/* FIXME:  came to be required in 3.3, deprecated in 3.10 (sigh,
	 * because reasons), became a no-op in 3.12.  remove this call
...
static struct PyMethodDef methods[] = {
	{"append", append, METH_O, "Append a unicode string object to the tokenizer's internal buffer.  Returns self."},
...
"be removed before conversion to the target type.  Unquoted leading and\n" \
"trailing whitespace is removed from each token before conversion to the target\n" \
"type.  To include a '\"' character in a quoted token, escape it with a '\\'\n" \
"character, '\\\"'.  Do the same to include a '\\' character in a quoted token.\n" \
"An empty token (two delimiters with nothing between them, other than\n" \
"whitespace) is returned as None regardless of the requested type.  To prevent\n" \
"a zero-length string token from being interpreted as None, place it in quotes.",
