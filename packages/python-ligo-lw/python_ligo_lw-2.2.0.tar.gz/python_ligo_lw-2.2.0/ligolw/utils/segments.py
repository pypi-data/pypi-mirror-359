# Copyright (C) 2008-2010,2012-2022,2025  Kipp Cannon
#
...
strong ternary logic.  When the desired manipulations are complete, the
...
try:
	...
	# see if Ducan and Leo's forked version is available
...
class LigolwSegmentList(object):
	...
	LIGO Light-Weight XML segments are three-state objects.  A segment
	can be on, off, or undefined.  Two sequences of segments are used
	for this:  the "valid" list defines the intervals when the state is
	known, and the "active" list defines the intervals when the state
	is on.  It is not an error for the state to be active, or on,
	during times when the state is said to be unknown, this code does
	not impose any policy in that regard.
	...
	def sort(self, *args, **kwargs):
		...
		kwargs are passed to the .sort() method of the segmentlist
		objects.  This can be used to control the sort order by
	...
	def coalesce(self):
		"""
		Coalesce the internal segmentlist objects.  Returns self.
		"""
		self.valid.coalesce()
	...
	def __ior__(self, other):
		...
		result is false, otherwise the result is unknown.  This and
		other's .instruments sets must be equal or the operation is
		forbidden.
		"""
	...
	def __iand__(self, other):
		...
		the result is true, otherwise the result is unknown.  This
		and other's .instruments sets must be equal or the
...
class LigolwSegments(set):
	"""
	An interface shim between code that makes use of segments
	algebraically in the form of segmentlist objects, and LIGO
	Light-Weight XML I/O code.

	An instance of this class is attached (see the examples below) to
	an XML Document object, at which time it populates its contents
	from the contents of the segment tables from the document, deleting
	the contents of those tables (preventing a second LigolwSegments
	object from being meaningfully attached to the same document).
	This provides an interface to the contents of the segments tables
	which allows them to be manipulated algebraically rather than by
	operating on the tabular data directly.  When the application is
	finished manipulating the segment lists, they can be inserted into
	the XML document at which time the contents of the LigolwSegments
	object are cleared (preventing any further manipulations).
	...
	segment lists in the original XML document, as defined by the
	segment_definer table.

	Although it is not necessary to do so, the class is intended to be
	used as a context manager to automate the replacement of segments
	back into the XML document, including in the event of an untrapped
	exception.  When used as a context manager, the process parameter
	of the .__init__() method is not optional.
	...
	>>> process = xmldoc.register_process("program_name", {})
	>>> # clear some metadata so test suite accepts the result
	>>> process.node = process.unix_procid = process.start = process.username = None
	...
	<LIGO_LW>
		...
		<Table Name="process_params:table">
			...
			<Stream Name="process_params:table">
			</Stream>
		</Table>
		<Table Name="segment_definer:table">
			...
			<Column Name="comment" Type="lstring"/>
			<Stream Name="segment_definer:table">
				0,0,"H1","test",,,
				0,1,"L1","test",,,
			</Stream>
		</Table>
		<Table Name="segment_summary:table">
			...
			<Column Name="comment" Type="lstring"/>
			<Stream Name="segment_summary:table">
	...
	def __init__(self, xmldoc, process = None):
		...
		self.segment_def_table = ligolw.lsctables.SegmentDefTable.ensure_exists(xmldoc, columns = LigolwSegmentList.segment_def_columns)
		self.segment_sum_table = ligolw.lsctables.SegmentSumTable.ensure_exists(xmldoc, columns = LigolwSegmentList.segment_sum_columns)
		self.segment_table = ligolw.lsctables.SegmentTable.ensure_exists(xmldoc, columns = LigolwSegmentList.segment_columns)
	...
	def insert_from_segwizard(self, fileobj, instruments, name, version = None, comment = None):
		...
		itself.
		"""
		active = segmentsUtils.fromsegwizard(fileobj, coltype = ligolw.lsctables.LIGOTimeGPS)
	...
	def insert_from_segmentlistdict(self, seglists, name, version = None, comment = None):
		...
		"""
		# construct the segmentlist defining the interval of
		# validity.  if there are no segments .extent_all() raises
		# ValueError, which must be trapped.
		try:
			valid = [seglists.extent_all()]
		except ValueError:
			valid = []
		valid = segments.segmentlist(valid)

		for instrument, active in seglists.items():
			self.add(LigolwSegmentList(active = active, valid = valid, instruments = set([instrument]), name = name, version = version, comment = comment))
	...
	def finalize(self, process_row = None):
		...
		def row_generator(segs, target_table, process_id, segment_def_id):
			...
			if isinstance(target_table, ligolw.lsctables.SegmentSumTable):
...
def has_segment_tables(xmldoc, name = None):
	...
	"""
	try:
		names = ligolw.lsctables.SegmentDefTable.get_table(xmldoc).getColumnByName("name")
		ligolw.lsctables.SegmentTable.get_table(xmldoc)
		ligolw.lsctables.SegmentSumTable.get_table(xmldoc)
...
def segmenttable_get_by_name(xmldoc, name):
	...
	def_table = ligolw.lsctables.SegmentDefTable.get_table(xmldoc)
	seg_table = ligolw.lsctables.SegmentTable.get_table(xmldoc)
