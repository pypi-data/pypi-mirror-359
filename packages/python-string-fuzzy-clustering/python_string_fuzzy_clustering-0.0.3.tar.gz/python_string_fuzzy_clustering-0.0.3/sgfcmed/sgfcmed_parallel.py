...
def _parallel_total_distance(candidate, S):
        """
        Wrapper for multiprocessing: returns candidate and its total distance to S.
        """
        return candidate, sum(_lev(candidate, s) for s in S)
...
class SGFCMedParallel:
    ...
    def _update_prototype(self, s, S, alphabet, cluster_idx=None):
        ...
        for i in range(len(s)):
            ...
            func = partial(_parallel_total_distance, S=S) # def func(candidate): return parallel_total_distance(candidate, S=S)
    ...
    def _get_debug_candidates(self, cluster_idx=None):
        """
