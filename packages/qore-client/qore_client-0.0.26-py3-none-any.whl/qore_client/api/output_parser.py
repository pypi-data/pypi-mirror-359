...
def extract_workflow_output(result: Dict[str, Any]) -> Any:
    """
    ...
    if not output_data:
        raise ValueError("Workflow execution output is empty.")

    # Get the inner output
    inner_output = output_data["output"]

    # Check if it's a pickled object
    if isinstance(inner_output, dict) and inner_output.get("_pickled") is True:
        try:
            # Decode the base64 pickled data
            pickled_bytes = base64.b64decode(inner_output["pickled_data"])
            # Unpickle the data
    ...
    if isinstance(inner_output, str):
        try:
            ...
            parsed_data = json.loads(inner_output)
            return parsed_data
