...
async def step4a_make_classifier_prompt(dataset_uuid: uuid.UUID) -> uuid.UUID:
    with Halo(text="💠 Creating classifier prompts...\n", spinner=SPINNER):
        q_input = qr.ContextClassifierPromptInput(
            ...
            classifier_ids=["0197a903-2e05-7c30-a4ed-333437bb6a00","0197a904-d1b6-7773-b8a8-c7dd84a45ab1",
                            "0197ad5b-5789-75f1-a317-a5b93cb60138","0197ad5c-0351-7df3-8b30-8d9c8dab9336"]
        )
...
async def main():
    ...
    try:
        ...
        #segments = await step3_chunk_text(
        #   await step2_extract_text(
        #        await step1_load_raw()
        #    )
        #)
        # Fork the flow to run classification and extraction prompts in parallel
        await step6a_parse_classifier_llm("0197c273-5037-7f20-b509-f5176d34730c"
            #await step5a_classifier_inference(
            #    await step4a_make_classifier_prompt(segments)
            #)
        )

        #await step6b_parse_extractor_llm(
        #    await step5b_extractor_inference(
        #        await step4b_make_extractor_prompt(segments)
        #    )
        #)
