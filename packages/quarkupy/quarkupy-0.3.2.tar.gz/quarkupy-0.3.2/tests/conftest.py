...
from quarkupy import Quarkupy, AsyncQuarkupy, DefaultAioHttpClient
...
def client(request: FixtureRequest) -> Iterator[Quarkupy]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Quarkupy(base_url=base_url, api_key=api_key, cookie=cookie, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncQuarkupy]:
