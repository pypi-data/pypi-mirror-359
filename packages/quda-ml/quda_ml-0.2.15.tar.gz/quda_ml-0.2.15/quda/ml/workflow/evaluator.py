    ...
    with mlflow.start_run() as top_run:
        ...
        for i, train_batch in enumerate(train_batch_list):
            ...
            with mlflow.start_run(nested=True, ) as partial_run:
                ...
                # 加载训练数据
                train_dl = workflow.data.fetch(train_batch, batch_size=-1, show_progress=False, eager=True)

                train_data = next(train_dl)
                if workflow.preprocessor:
                    train_data = workflow.preprocessor.partial_fit_transform(train_data)
                if valid_size > 0:
                    train_data = train_data.sample(train_data.height, shuffle=shuffle, seed=random_state)
                    valid_data = train_data.tail(int(train_data.height * valid_size))
                    train_data = train_data.head(int(train_data.height * (1 - valid_size)))
                else:
                    valid_data = None
