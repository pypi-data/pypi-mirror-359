...
class PredictionResult(NamedTuple):
    """
    Detailed prediction result from Quill API.
    
    Attributes:
        value: The predicted numeric value
        citation_href: URL linking to the source document section
    """
    value: Optional[Union[int, float]]
    citation_href: str


class QuillResponse(NamedTuple):
    """
    Complete response from Quill API with predictions and metadata.
    
    Attributes:
        dataframe: DataFrame with original data plus predicted values
        predictions: List of detailed prediction results
        next_period: Name of the predicted period (e.g., "3Q23")
...
def fillDfModelDetailed(
    ...
    """
    Fill a financial model DataFrame using Quill's AI-powered data extraction with detailed metadata.
    
    This function returns comprehensive prediction data including citations and source links.
    
    Args:
        df: DataFrame with line items as index and periods as columns
        company: Company ticker symbol or name (e.g., 'AAPL', 'BOKF')
        base_url: Optional base URL for the API
        token: Optional API token
    
    Returns:
        QuillResponse containing DataFrame with predictions and detailed metadata
    """
    # Make API call
    api_response = _call_quill_api(df, company, base_url, token)
    
    # Create detailed response with metadata
    return _create_detailed_response(df, api_response)
...
def fillDfModel(
    ...
    """
    # Use the detailed function and extract just the DataFrame
    detailed_response = fillDfModelDetailed(df, company, base_url, token)
    return detailed_response.dataframe
...
def _call_quill_api(
    ...
    """
    Make API call to Quill Express API.
    
    Args:
        df: DataFrame with line items as index and periods as columns
        company: Company ticker symbol or name
        base_url: Optional base URL for the API
        token: Optional API token
    ...
    try:
        response = requests.post(
            ...
            headers={
                ...
                'User-Agent': 'quillai-python/0.1.1'
            },
            timeout=60
        )
...
def _create_detailed_response(df: pd.DataFrame, api_response: Dict[str, Any]) -> QuillResponse:
    """
    Create detailed QuillResponse from API response.
    ...
    Returns:
        QuillResponse with DataFrame and detailed prediction metadata
    """
    # Parse periods from original DataFrame
    periods = _parse_periods(df.columns.tolist())
    next_period = _get_next_period(periods[-1])
    
    # Extract detailed predictions
    predictions = _extract_detailed_predictions(api_response)
    
    # Create updated DataFrame with predicted values
    updated_df = df.copy()
    predicted_values = [pred.value for pred in predictions]
    
    # Pad with None if we have fewer predictions than rows
    while len(predicted_values) < len(df):
        predicted_values.append(None)
    
    updated_df[next_period] = predicted_values[:len(df)]
    
    return QuillResponse(
...
def _extract_detailed_predictions(api_response: Dict[str, Any]) -> List[PredictionResult]:
    """
    Extract detailed prediction results from API response.
    
    Args:
        api_response: Raw API response containing results
        
    Returns:
        List of PredictionResult objects with detailed metadata
    """
    predictions = []
    results = api_response.get('results', [])
    
    for result in results:
        formula = result.get('text', '')
        href = result.get('href', '')
        
        # Parse the formula to extract numeric value
        value = _parse_formula_value(formula)
