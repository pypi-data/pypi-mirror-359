...
from .execution_strategy import  PromptTaskExecutionStrategy, TaskExecutionStrategy
# 任务类
class Task:
...
class Task_new:
    def __init__(self, name: str, belong: str,execution_strategy: TaskExecutionStrategy):
        self.name = name
        self._state = TodoState() # Initial state
        self.execution_strategy = execution_strategy
        self.belong = belong
...
class TaskManager:
    ...
    def add_task_new(self, task_info: dict):
        ...
        strategy = PromptTaskExecutionStrategy()
        new_task = Task_new(name=task_info.get('content'),belong = task_info.get('belong'), execution_strategy=strategy)

        if new_task in self._tasks_order:
            return f"任务 '{task_info.get('content')}' 已存在。"

        self._tasks_order.append(new_task) # Add to ordered list
        # self._tasks[task_info.get('content')] = new_task   # Add to dictionary
        print(self._tasks_order)
        print(len(self._tasks_order))
        return f"任务 '{task_info.get('content')}' (prompt) 已添加。"
    ...
    def get_current_sequential_task_new(self,id:str) -> Task | None:
        """获取当前按顺序应执行的任务 (第一个未完成/失败的任务)"""
        # Consider FailedState as needing attention, so it's also a potential "current" task
        for task in self._tasks_order:
            print('task__get',task)
            print('task__get',task.belong)
    ...
    def complete_current_task_new(self,id:str):
        """完成当前按顺序应执行的任务"""
        current_task = self.get_current_sequential_task_new(id)
        if current_task:
            # Calls the task's complete_task method, which calls the state's complete method
            # If the state is TodoState and strategy is Automatic, the script executes here.
            return current_task.complete_task()
    ...
    def get_task_status_new(self, task: object):
        """获取指定任务的状态"""
        if task:
            return f"任务 '{task.name}' 的状态是：{task.get_status()}"
        else:
            return f"任务 '{task.name}' 不存在。"
    def get_task_list(self):
        """获取所有任务及其状态"""
        return [(name, self.get_task_status(name)) for name in self._tasks_order]
    
    def get_task_list_new(self,id:str):
        """获取所有任务及其状态"""
        return [(task.name, self.get_task_status_new(task)) for task in self._tasks_order if task.belong == id]