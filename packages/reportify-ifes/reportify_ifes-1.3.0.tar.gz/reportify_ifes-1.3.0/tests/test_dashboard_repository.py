...
def test_repository_env_variables(monkeypatch):
    # Simula variáveis de ambiente
    monkeypatch.setenv("GITHUB_TOKEN", "fake_token")
    monkeypatch.setenv("GITHUB_REPOSITORY", "fake/repo")

    dev = GitHubIssueStats(save_func=lambda x, y: None, token=os.getenv("GITHUB_TOKEN"), repo=os.getenv("GITHUB_REPOSITORY"))
