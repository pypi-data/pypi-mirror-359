...
class LWDETR(nn.Module):
    ...
    def forward_export(self, tensors):
        ...
        # Static topk with explicit self.num_queries
        topk_values, topk_indexes = torch.topk(prob.view(outputs_class.shape[0], -1), self.num_queries, dim=1)
        scores = topk_values
        b_size = prob.shape[0]
        # Fix type conversion - apply to the result, not the divisor
        topk_boxes = (topk_indexes // outputs_class.shape[2])
        labels = (topk_indexes % outputs_class.shape[2]).view(b_size, self.num_queries).to(torch.int32)
        ...
        # Create scale factor with explicit reshape to ensure static shape
        img_h, img_w = tensors.shape[2].unsqueeze(dim=0).expand(1, self.num_queries), tensors.shape[3].unsqueeze(dim=0).expand(1, self.num_queries)
        # Ensure static shapes with reshape operations
        scale_fct = torch.cat([img_w, img_h, img_w, img_h], dim=1).view(b_size, self.num_queries, 4)
        # Ensure boxes has static shape by forcing dimension
        boxes = boxes * scale_fct
        boxes = boxes.reshape(b_size, self.num_queries, 4)
        scores = scores.reshape(b_size, self.num_queries)

        # Create num_dets with static shape
        num_dets = torch.full((b_size, 1), self.num_queries, dtype=torch.int32, device=boxes.device).view((b_size, 1))

        # Return with explicit static shapes
