...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/mqtt_return/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/mqtt_return/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/mqtt_return/*"
            )
            ...
            #    "--include=src/saltext/mqtt_return/*,tests/*"
            # )
        finally:
