...
class SARMClient:
    ...
    def _init_apis(self):
        """初始化所有API模块"""
        from .apis import (
            OrganizationAPI, CarrierAPI, SecurityCapabilityAPI, 
            CarrierDataImportAPI, VulnerabilityAPI, SecurityIssueAPI,
            ComponentAPI, BusinessSystemAPI, ApplicationAPI, OrganizeUserAPI
        )
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        if not (200 <= response.status_code < 300):
            error_msg = data.get('msg_zh') or data.get('message') or f"API错误 (HTTP {response.status_code})"
            raise SARMAPIError(error_msg, status_code=response.status_code, response_data=data)
    ...
    def get_api_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'endpoints': {
                ...
                'carrier_data_import': '/api/insert/insert_carrier_issue'
            }
        }
