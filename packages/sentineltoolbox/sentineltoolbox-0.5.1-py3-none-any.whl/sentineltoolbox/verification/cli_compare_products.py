    ...
    **kwargs: Any,
    ...
    for ref_node in dt_ref.subtree:
        ...
        for field, ref_obj, new_obj in get_fields_for_comparisons(ref_ds, new_ds, **kwargs):
            path_str = f"[{ref_path}][{field}]"
            diff = deepdiff_compare_objects(ref_obj, new_obj, prefix=path_str + ": ")
            if diff:
                failed_logger.info(f"{path_str} fields are not identical")
                for msg in diff:
        ...
        for ref_var in ref_ds.values():
            ...
            ref_path = f"{ref_path}/{ref_var.name}"
            ...
            for field, ref_obj, new_obj in get_fields_for_comparisons(ref_var, new_var, **kwargs):
                ...
                if diff:
                    ...
                    failed_logger.info(
                        f"{path_str} fields are not identical{skip_data_comparison_message}:",
                    )
                    for msg in diff:
                        failed_logger.info(msg)
