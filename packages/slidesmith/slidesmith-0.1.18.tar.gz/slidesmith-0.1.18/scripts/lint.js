...
async function lintDeck() {
  ...
  try {
    ...
    // Launch browser
    console.log('Launching browser for linting...');
    ...
    const slideContainer = await page.$('.slide-container');
    if (slideContainer) {
    ...
    for (const element of textWithBg) {
      ...
      if (styles.color && styles.background && 
        ...
        console.log('Contrast check needed for:', styles);
      }
    }
    ...
    console.log(`Issues: ${issues.length} (${summary.error} errors, ${summary.warning} warnings, ${summary.info} info)`);
