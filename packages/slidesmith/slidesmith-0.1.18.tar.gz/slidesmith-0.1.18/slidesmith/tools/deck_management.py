...
def register_deck_management_tools(mcp: "FastMCP") -> None:
    ...
    def init_deck(params: Union[InitDeckIn, str, dict]) -> DeckInfo:
        ...
        # Copy template files
        # First check user home (where setup installs them)
        user_template_dir = Path.home() / ".slidesmith" / "templates" / params.template.value
        # Then check package directory
        package_template_dir = Path(__file__).parent.parent / "templates" / params.template.value
        # Finally check development directory
        dev_template_dir = Path(__file__).parent.parent.parent / "templates" / params.template.value
        
        template_dir = user_template_dir
        if not template_dir.exists() and package_template_dir.exists():
            template_dir = package_template_dir
        elif not template_dir.exists() and dev_template_dir.exists():
