...
# File extensions to include (Python and config files)
INCLUDE_EXTENSIONS = {
    '.py', '.pyi', '.toml', '.yaml', '.yml', '.json',
    '.txt', '.cfg', '.ini', '.js', '.jsx', '.tsx', '.mdx'
}
...
def should_include_file(file_path: Path) -> bool:
    ...
    if file_path.name in EXCLUDE_FILES or any(file_path.name.endswith(pattern) for pattern in ['.pyc', '.pyo']):
        return False
    ...
    # Skip test files unless they're part of the main structure
