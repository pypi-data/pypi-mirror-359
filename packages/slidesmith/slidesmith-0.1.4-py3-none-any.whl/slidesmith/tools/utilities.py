"""
Utility tools: get_component_doc, svg_icon.
"""
...
def register_utility_tools(mcp: "FastMCP") -> None:
    ...
    @mcp.tool()
    def get_component_doc(params: ComponentDocIn) -> MarkdownOut:
        ...
        component_file = templates_dir / "components" / f"{params.component}.jsx"
    ...
    @mcp.tool()
    def svg_icon(params: SvgIconIn) -> PathOut:
        """Get SVG icon from offline collection."""
        # Load icon map and SVG paths
        templates_dir = Path(__file__).parent.parent.parent / "templates" / "markov-pro"
        icon_map_path = templates_dir / "assets" / "icons" / "icon-map.json"
        svg_paths_file = templates_dir / "assets" / "icons" / "svg-paths.json"
