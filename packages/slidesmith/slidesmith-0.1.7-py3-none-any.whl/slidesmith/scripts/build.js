...
async function buildDeck() {
  ...
  try {
    ...
    // Import modules from resolved paths
    const { build } = await import(esbuildPath);
    const mdxDefault = await import(mdxPath);
    ...
    const result = await build({
      ...
      external: ['react', 'react-dom', '@mdx-js/react', 'react/jsx-runtime'],
      define: {
...
<body>
    ...
    <script type="module">
      ...
      for (const key in window) {
        if (key.includes('title') && typeof window[key] === 'function') {
