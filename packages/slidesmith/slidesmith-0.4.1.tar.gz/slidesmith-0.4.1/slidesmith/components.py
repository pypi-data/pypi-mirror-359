...
class CardComponent(Component):
    ...
    def render(self, props: Dict[str, Any]) -> str:
        ...
        template = '''
        <div class="bg-white border-2 {{ colors.border }} rounded-2xl p-10 shadow-lg hover:shadow-xl transition-shadow {{ className }}">
            {% if icon or title %}
            <div class="flex items-center mb-6">
                {% if icon %}
                <div class="h-16 w-16 rounded-2xl {{ colors.bg }} flex items-center justify-center mr-6 shadow-md">
                    <i class="fas fa-{{ icon }} text-3xl {{ colors.icon }}"></i>
                </div>
                {% endif %}
                {% if title %}
    ...
    def render(self, props: Dict[str, Any]) -> str:
        ...
        <div class="grid grid-cols-{{ cols }} gap-12 {{ className }}">
            ...
            <div class="relative group">
                <div class="absolute inset-0 bg-gradient-to-r {{ colors[loop.index0 % colors|length] }} rounded-2xl opacity-10 group-hover:opacity-20 transition-opacity"></div>
                <div class="relative bg-white rounded-2xl shadow-lg p-10 text-center border border-gray-100">
                    <div class="text-6xl font-bold bg-gradient-to-r {{ colors[loop.index0 % colors|length] }} bg-clip-text text-transparent mb-4">
                        {{ item.value }}
                    </div>
                    <div class="text-2xl text-gray-700 font-semibold mb-2">{{ item.label }}</div>
                    {% if item.trend %}
                    <div class="text-lg text-gray-500 font-medium">{{ item.trend }}</div>
                    {% endif %}
                </div>
            </div>
            {% endfor %}
    ...
    def render(self, props: Dict[str, Any]) -> str:
        ...
        <div class="relative {{ className }}">
            ...
            <div class="flex items-start mb-12 {% if not loop.last %}pb-12{% endif %}">
                ...
                <div class="relative z-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl w-16 h-16 flex items-center justify-center shadow-lg">
                    <span class="text-white font-bold text-xl">{{ loop.index }}</span>
                </div>
                <div class="ml-8 flex-1">
                    <div class="bg-white rounded-2xl shadow-md p-8 border border-gray-100">
                        <div class="text-lg text-blue-600 font-semibold mb-2">{{ item.date }}</div>
                        <h4 class="text-2xl font-bold text-gray-800 mb-3">{{ item.title }}</h4>
                        {% if item.description %}
                        <p class="text-lg text-gray-600 leading-relaxed">{{ item.description }}</p>
                        {% endif %}
                    </div>
                </div>
            </div>
