...
def register_export_tools(mcp: "FastMCP") -> None:
    ...
    def pdf_export(params: Union[PdfExportIn, str, dict]) -> PathOut:
        ...
        if not deck_root.exists():
            raise ValueError(
                f"Deck {params.deck_id} not found in {workspaces_dir}\n"
                "Tip: Use list_templates to see available decks, or init_deck to create a new one"
            )
        ...
        if not build_dir.exists() or not (build_dir / "deck.html").exists():
            raise ValueError(
                f"Deck not built - run html_preview first\n"
                "Tip: Use html_preview to generate the HTML before exporting to PDF"
            )
        ...
        try:
            ...
            # Create enhanced response with metadata info
            response = PathOut(
                path=pdf_data['path'],
                size_bytes=pdf_data['size_bytes']
            )
            
            # Add metadata information to response if available
            if 'metadata' in pdf_data and 'bookmarks' in pdf_data:
                metadata = pdf_data['metadata']
                bookmarks = pdf_data['bookmarks']
                
                # Add summary to response
                setattr(response, 'pages', pdf_data.get('pages', 0))
                setattr(response, 'title', metadata.get('title', ''))
                setattr(response, 'author', metadata.get('author', ''))
                setattr(response, 'bookmarks_count', len(bookmarks))
                setattr(response, 'has_metadata', True)
            else:
                setattr(response, 'has_metadata', False)
