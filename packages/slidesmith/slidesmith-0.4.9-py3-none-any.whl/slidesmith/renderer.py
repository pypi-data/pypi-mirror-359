...
<head>
    ...
    <script>
        ...
        function updatePresenterView() {{
            ...
            // Update presenter slide preview
            const presenterSlide = document.querySelector('.presenter-slide');
            const currentSlideElement = document.querySelector(`.slide:nth-child(${{currentSlide}})`);
            if (presenterSlide && currentSlideElement) {{
                presenterSlide.innerHTML = currentSlideElement.outerHTML;
                const previewSlide = presenterSlide.querySelector('.slide');
                if (previewSlide) {{
            ...
            const currentNotes = currentSlideElement?.querySelector('[data-speaker-notes]');
            if (notesContainer) {{
                if (currentNotes) {{
                    notesContainer.innerHTML = currentNotes.innerHTML;
        ...
        function togglePresentationMode() {{
            ...
            const presMode = document.querySelector('.presentation-mode');
            ...
            if (presentationMode) {{
                ...
                // Try to open audience window if possible
                try {{
                    ...
                    console.log('Could not open audience window - using split screen mode');
                }}
            }} else {{
        ...
        function updateTimer() {{
            ...
            if (timerElement) {{
                ...
                // Warning color if over target time
        ...
        function setTargetTime() {{
            const minutes = prompt('Set target presentation time (minutes):', '20');
            if (minutes && !isNaN(minutes)) {{
                targetDuration = parseInt(minutes) * 60000;
                const btn = document.querySelector('.control-btn[onclick="setTargetTime()"]');
                if (btn) {{
                    btn.textContent = `Target: ${{minutes}}m`;
                }}
            }}
        }}
        ...
        document.addEventListener('keydown', (e) => {{
            if (e.key === 'ArrowRight' || e.key === ' ') {{
                ...
                // Toggle slide animation (only if not in presentation mode)
                if (!presentationMode) {{
                ...
                // Reset timer in presentation mode
                if (presentationMode) {{
                    stopPresentationTimer();
                    startPresentationTimer();
                }}
    ...
    <div class="presentation-mode">
        <div class="presenter-screen">
            <div class="presenter-slide">
                <!-- Current slide preview will be inserted here -->
            </div>
            ...
            <div class="presentation-controls">
                ...
                <div class="control-buttons">
                    <button class="control-btn" onclick="setTargetTime()">Set Target</button>
                    <button class="control-btn" onclick="stopPresentationTimer(); startPresentationTimer()">Reset Timer</button>
                    <button class="control-btn" onclick="togglePresentationMode()">Exit (Esc)</button>
                </div>
            </div>
        </div>
        
        <div class="audience-screen">
            <!-- This shows the current slide for the audience -->
            <div id="audience-slide" style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-size: 24px;">
                Press 'P' to start presentation mode
            </div>
        </div>
    </div>
</body>
