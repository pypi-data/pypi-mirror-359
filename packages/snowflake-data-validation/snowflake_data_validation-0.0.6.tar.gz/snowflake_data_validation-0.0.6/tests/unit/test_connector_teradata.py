...
class TestConnectorTeradata:
    ...
    def test_execute_query_fails(self, mock_import_teradata):
        ...
        assert FAILED_TO_EXECUTE_QUERY.format(query="SELECT * FROM test_table") in str(
            exc_info.value
        )
    ...
    def test_execute_query_without_return_fails(self, mock_import_teradata):
        ...
        assert str(exc_info.value) == FAILED_TO_EXECUTE_QUERY.format(
            query="CREATE TABLE TEST_TABLE (INDEX_ INTEGER)"
        )
