        ...
        <script>
            ...
            function setupEventListeners() {{
                ...
                document.getElementById('save-docx').onclick = () => {{
                    const contentEl = document.querySelector('.document');
                    const paragraphs = Array.from(contentEl.querySelectorAll('p, h1, h2, h3, li'));
                    const doc = new docx.Document();
                    const children = paragraphs.map(el => new docx.Paragraph(el.innerText));
