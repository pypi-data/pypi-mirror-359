...
class ImageGenerateAgent(BaseChatAgent):
    ...
    async def _generate_image_prompt(self, metadata: dict) -> Optional[str]:
        """Generate an optimized image prompt."""
        try:
            logger.info(f"Generating image with metadata: {metadata}")
            serialized_metadata = json.dumps(metadata, ensure_ascii=False)
            response = await self.text_model_client.create([
                UserMessage(
                    content=[self.system_message, serialized_metadata],
                    source="user",
                ),
            ])
