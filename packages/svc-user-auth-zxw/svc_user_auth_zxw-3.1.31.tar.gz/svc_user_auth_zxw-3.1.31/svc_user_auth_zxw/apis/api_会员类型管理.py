...
async def _check_admin_permission(user: User, db: AsyncSession) -> bool:
    """检查用户是否有管理员权限"""
    logger.info(f"检查用户权限 - 用户ID: {user.id}, 用户名: {user.username}")
    ...
    logger.info(f"权限检查结果 - 用户ID: {user.id}, 是否有管理员权限: {has_admin_role}")
    return has_admin_role


async def _require_admin_permission(user: User, db: AsyncSession):
    """要求管理员权限，如果没有则抛出异常"""
    logger.info(f"验证管理员权限 - 用户ID: {user.id}")

    if not await _check_admin_permission(user, db):
        logger.warning(f"权限不足 - 用户ID: {user.id} 尝试访问管理员功能但无权限")
    ...
    logger.info(f"管理员权限验证通过 - 用户ID: {user.id}")
...
async def internal_get_membership_types(
    ...
    Returns:
        会员类型响应列表
    """
    logger.info(f"获取会员类型列表 - skip: {skip}, limit: {limit}, include_disabled: {include_disabled}")
    ...
    try:
        ...
        logger.info(f"成功获取会员类型列表 - 返回 {len(membership_types)} 条记录")
        return [会员类型响应.model_validate(mt) for mt in membership_types]

    except Exception as e:
        logger.error(f"获取会员类型列表失败 - skip: {skip}, limit: {limit}, 错误: {str(e)}")
        raise HTTPException_AppToolsSZXW(
...
async def internal_get_membership_type_by_id(
    ...
    Returns:
        会员类型响应对象
    """
    logger.info(f"获取会员类型详情 - ID: {membership_type_id}, include_disabled: {include_disabled}")
    ...
    try:
        ...
        if not membership_type:
            ...
            logger.warning(f"会员类型未找到 - ID: {membership_type_id}, {error_msg}")
            raise HTTPException_AppToolsSZXW(
        ...
        logger.info(f"成功获取会员类型详情 - ID: {membership_type_id}, 名称: {membership_type.name}")
        return 会员类型响应.model_validate(membership_type)

    except Exception as e:
        if isinstance(e, HTTPException_AppToolsSZXW):
            raise
        logger.error(f"获取会员类型详情失败 - ID: {membership_type_id}, 错误: {str(e)}")
        raise HTTPException_AppToolsSZXW(
            error_code=ErrorCode.系统异常,
...
async def internal_create_membership_type(
    ...
    Returns:
        会员类型响应对象
    """
    logger.info(f"创建会员类型 - 名称: {request.name}, 价格: {request.price}, 天数: {request.duration_days}")

    try:
        # 检查会员类型名称是否已存在
        logger.debug(f"检查会员类型名称是否重复 - 名称: {request.name}")
        result = await db.execute(
            select(MembershipType).where(MembershipType.name == request.name)
        )
        if result.scalar_one_or_none():
            logger.warning(f"会员类型名称已存在 - 名称: {request.name}")
            raise HTTPException_AppToolsSZXW(
                error_code=ErrorCode.参数错误,
                detail="会员类型名称已存在",
        ...
        # 创建会员类型
        logger.debug(f"创建新的会员类型对象 - 名称: {request.name}")
        ...
        if request.role_names and request.app_name:
            logger.info(f"处理角色关联 - 应用: {request.app_name}, 角色: {request.role_names}")
            ...
            if not app:
                logger.info(f"创建新应用 - 名称: {request.app_name}")
            ...
            for role_name in request.role_names:
                logger.debug(f"处理角色 - 名称: {role_name}, 应用ID: {app.id}")
                ...
                if not role:
                    logger.info(f"创建新角色 - 名称: {role_name}, 应用ID: {app.id}")
        ...
        logger.info(f"会员类型创建成功 - ID: {membership_type.id}, 名称: {membership_type.name}")
        return 会员类型响应.model_validate(membership_type)

    except Exception as e:
        await db.rollback()
        logger.error(f"会员类型创建失败 - 名称: {request.name}, 错误: {str(e)}")
        if isinstance(e, HTTPException_AppToolsSZXW):
            raise
        raise HTTPException_AppToolsSZXW(
            error_code=ErrorCode.系统异常,
...
async def internal_update_membership_type(
    ...
    Returns:
        会员类型响应对象
    """
    logger.info(f"更新会员类型 - ID: {membership_type_id}")
    ...
    try:
        ...
        if not membership_type:
            logger.warning(f"会员类型不存在 - ID: {membership_type_id}")
            raise HTTPException_AppToolsSZXW(
                error_code=ErrorCode.参数错误,
                detail="会员类型不存在",
                http_status_code=404
        ...
        logger.debug(f"找到会员类型 - ID: {membership_type_id}, 当前名称: {membership_type.name}")

        # 更新基本信息
        if request.name is not None:
            logger.debug(f"更新名称 - 从 '{membership_type.name}' 到 '{request.name}'")
            # 检查名称是否重复
            ...
            if name_check.scalar_one_or_none():
                logger.warning(f"会员类型名称冲突 - 新名称: {request.name}")
                raise HTTPException_AppToolsSZXW(
                    error_code=ErrorCode.参数错误,
                    detail="会员类型名称已存在",
                    http_status_code=400
        ...
        if request.description is not None:
            logger.debug(f"更新描述 - ID: {membership_type_id}")
            membership_type.description = request.description
        if request.duration_days is not None:
            logger.debug(f"更新时长 - ID: {membership_type_id}, 新时长: {request.duration_days}天")
            membership_type.duration_days = request.duration_days
        if request.price is not None:
            logger.debug(f"更新价格 - ID: {membership_type_id}, 新价格: {request.price}")
            membership_type.price = request.price
        if request.is_active is not None:
            logger.debug(f"更新状态 - ID: {membership_type_id}, 新状态: {request.is_active}")
            membership_type.is_active = request.is_active

        # 更新角色关联
        if request.role_names is not None and request.app_name:
            logger.info(
                f"更新角色关联 - ID: {membership_type_id}, 应用: {request.app_name}, 角色: {request.role_names}")
            # 清除现有角色关联
        ...
        logger.info(f"会员类型更新成功 - ID: {membership_type_id}, 名称: {membership_type.name}")
        return 会员类型响应.model_validate(membership_type)

    except Exception as e:
        await db.rollback()
        logger.error(f"会员类型更新失败 - ID: {membership_type_id}, 错误: {str(e)}")
        if isinstance(e, HTTPException_AppToolsSZXW):
            raise
        raise HTTPException_AppToolsSZXW(
            error_code=ErrorCode.系统异常,
...
async def internal_delete_membership_type(
    ...
    Returns:
        删除成功返回True
    """
    logger.info(f"删除会员类型 - ID: {membership_type_id}")
    ...
    try:
        ...
        logger.debug(f"找到会员类型 - ID: {membership_type_id}, 名称: {membership_type.name}")

        # 检查是否有用户使用此会员类型
        if membership_type.memberships:
            logger.warning(
                f"会员类型有关联用户，无法删除 - ID: {membership_type_id}, 关联用户数: {len(membership_type.memberships)}")
            raise HTTPException_AppToolsSZXW(
                error_code=ErrorCode.参数错误,
        ...
        logger.info(f"会员类型删除成功 - ID: {membership_type_id}, 名称: {membership_type.name}")
        return True

    except Exception as e:
        await db.rollback()
        logger.error(f"会员类型删除失败 - ID: {membership_type_id}, 错误: {str(e)}")
        if isinstance(e, HTTPException_AppToolsSZXW):
            raise
        raise HTTPException_AppToolsSZXW(
            error_code=ErrorCode.参数错误,
...
async def 获取会员类型列表(
    ...
    """获取会员类型列表（只返回启用的类型）"""
    logger.info(f"API调用: 获取会员类型列表 - skip: {skip}, limit: {limit}")
    ...
    logger.info(f"API响应: 获取会员类型列表成功 - 返回 {len(membership_types)} 条记录")
    return 通用响应(
...
async def 获取会员类型详情(
    ...
    """获取会员类型详情（只返回启用的类型）"""
    logger.info(f"API调用: 获取会员类型详情 - ID: {membership_type_id}")
    ...
    logger.info(f"API响应: 获取会员类型详情成功 - ID: {membership_type_id}")
    return 通用响应(
...
async def 管理员创建会员类型(
    ...
    logger.info(f"API调用: 管理员创建会员类型 - 用户ID: {current_user.id}, 会员类型名称: {request.name}")
    ...
    logger.info(f"API响应: 管理员创建会员类型成功 - 用户ID: {current_user.id}, 新会员类型ID: {membership_type.id}")
    return 通用响应(
        data=membership_type,
        message="会员类型创建成功"
    )
...
async def 管理员获取所有会员类型(
    ...
    logger.info(
        f"API调用: 管理员获取所有会员类型 - 用户ID: {current_user.id}, skip: {skip}, limit: {limit}, include_disabled: {include_disabled}")
    ...
    logger.info(f"API响应: 管理员获取所有会员类型成功 - 用户ID: {current_user.id}, 返回 {len(membership_types)} 条记录")
    return 通用响应(
        data=membership_types,
        message="获取成功"
    )
...
async def 管理员获取会员类型详情(
    ...
    """管理员获取会员类型详情（包括已停用的）"""
    logger.info(f"API调用: 管理员获取会员类型详情 - 用户ID: {current_user.id}, 会员类型ID: {membership_type_id}")
    ...
    logger.info(f"API响应: 管理员获取会员类型详情成功 - 用户ID: {current_user.id}, 会员类型ID: {membership_type_id}")
    return 通用响应(
        data=membership_type,
        message="获取成功"
    )
...
async def 管理员更新会员类型(
    ...
    logger.info(f"API调用: 管理员更新会员类型 - 用户ID: {current_user.id}, 会员类型ID: {membership_type_id}")
    ...
    logger.info(f"API响应: 管理员更新会员类型成功 - 用户ID: {current_user.id}, 会员类型ID: {membership_type_id}")
    return 通用响应(
        data=membership_type,
        message="会员类型更新成功"
    )
...
async def 管理员删除会员类型(
    ...
    logger.info(f"API调用: 管理员删除会员类型 - 用户ID: {current_user.id}, 会员类型ID: {membership_type_id}")
    ...
    logger.info(f"API响应: 管理员删除会员类型成功 - 用户ID: {current_user.id}, 会员类型ID: {membership_type_id}")
    return 通用响应(
