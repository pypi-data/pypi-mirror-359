...
from swarms_client import BaseModel, SwarmsClient, AsyncSwarmsClient
from swarms_client._response import (
    APIResponse,
    BaseAPIResponse,
    AsyncAPIResponse,
    BinaryAPIResponse,
...
from swarms_client._streaming import Stream
from swarms_client._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'swarms_client._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: SwarmsClient) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from swarms_client import BaseModel`",
    ):
...
def test_response_parse_custom_stream(client: SwarmsClient) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=b"foo"),
        client=client,
        stream=True,
...
def test_response_parse_custom_model(client: SwarmsClient) -> None:
    response = APIResponse(
...
def test_response_parse_annotated_type(client: SwarmsClient) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=client,
        stream=False,
