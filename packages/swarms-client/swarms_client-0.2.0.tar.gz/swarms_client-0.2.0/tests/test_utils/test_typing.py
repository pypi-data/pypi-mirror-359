...
from swarms_client._utils import extract_type_var_from_base
...
class SubclassDifferentOrderGenericMultipleTypeArgs(
    BaseGenericMultipleTypeArgs[_T2, _T, _T3]
): ...
...
def test_extract_type_var_multiple() -> None:
    ...
    assert extract_type_var_from_base(
        typ, index=2, generic_bases=generic_bases
    ) == type(None)
