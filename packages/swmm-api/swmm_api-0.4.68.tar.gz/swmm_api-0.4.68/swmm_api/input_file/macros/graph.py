...
def conduit_iter_over_inp(inp, start, end):
    ...
    if start and end:
        for shortest_path_nodes in nx.all_simple_paths(g, start, end):
            for i in itertools.pairwise(shortest_path_nodes):
                edges = g.get_edge_data(*i)['label']
                if isinstance(edges, (set, list, tuple)):
                    yield edges[0]  # use just the first link in container
                else:
                    yield edges
            break
    # else:
