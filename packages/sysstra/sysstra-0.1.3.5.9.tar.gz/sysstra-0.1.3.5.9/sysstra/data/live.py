...
def fetch_live_candle(redis_cursor, market, symbol):
    ...
    try:
        ...
        if market == "IN":
            ...
            live_candle = underlying_dict[symbol]

            live_candle["close"] = live_candle["last_price"]
            live_candle["timestamp"] = live_candle["exchange_timestamp"]
            ...
            key_symbol = f"us_underlying:{symbol}"
            ...
            underlying_dict = redis_cursor.get("crypto_underlying")
            underlying_dict = json.loads(underlying_dict)
            live_candle = underlying_dict[symbol]
            live_candle["close"] = live_candle["last_price"]
            live_candle["timestamp"] = datetime.datetime.strptime(str(live_candle["timestamp"]), '%Y-%m-%d %H:%M:%S')
            live_candle["date"] = datetime.datetime.strptime(str(live_candle["timestamp"].date()), '%Y-%m-%d')
...
def fetch_recent_candle(redis_cursor, symbol):
    ...
    try:
        ...
        recent_candle = redis_cursor.lindex(symbol, -1)
