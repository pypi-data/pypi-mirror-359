...
def add_order_to_redis(redis_cursor, request_id, order_dict, mode):
    ...
    try:
        ...
        redis_cursor.rpush(str(order_dict["user_id"]) + "_{}".format(mode) + "_orders", json.dumps(order_dict, default=str))
        ...
        if orders_list:
            ...
            for order in orders_list:
                ...
                if order["trade_type"] == "ENTRY":
                    ...
                    if "t1_price" in order:
                        quantity_dict[trade_symbol]["t1_price"] = order["t1_price"]

                    if "sl_price" in order:
                        quantity_dict[trade_symbol]["sl_price"] = order["sl_price"]

                    if "hka_option_order_price" in order:
                        quantity_dict[trade_symbol]["hka_option_order_price"] = order["hka_option_order_price"]

                    if "option_order_price" in order:
                        quantity_dict[trade_symbol]["option_order_price"] = order["option_order_price"]
        ...
        for order in orders_list:
            if order["trade_action"] == "ENTRY":
                ...
                if market_type == "options":
                    trade_dict["option_type"] = order["option_type"]
                    trade_dict["expiry"] = order["expiry"]

                if "liquidation_price" in order:
                    trade_dict["liquidation_price"] = order["liquidation_price"]

            else:
                if market_type == "equity" or market_type == "futures":
                ...
                if order["exit_type"] in order_exit_levels:
                    ...
                    brokerage, net_pnl = calculate_brokerage(buy_price=trade_dict["entry_price"], sell_price=trade_dict["exit_price"],
                                                             ...
                                                             lot_size=trade_dict["lot_size"], order_type=order["order_type"], trade_type=order["position_type"])
