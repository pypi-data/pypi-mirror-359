...
def generate_peak_classifier(
    ...
    with busy_indicator("generating classifier ..."):
        classifier = create_classifier(path, max_depth, score_col, area_col)
    msg = f"saving classifier to {path_to_folder} ..."
    print(msg)
    save_classifier(
        classifier, classifier_name, path_to_folder=path_to_folder, overwrite=overwrite
    )
