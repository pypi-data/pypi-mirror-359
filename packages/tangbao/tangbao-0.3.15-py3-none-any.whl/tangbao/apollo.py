...
class Apollo:
    ...
    def get_embeddings(self, texts, model="openai-text-embedding-3-large", dimensions=512, **kwargs):
        if self._base_url.endswith("/apollo"): # Apollo v2
            raise NotImplementedError("get_embeddings() is not available yet with Apollo v2")
        else:
            return self.get_embeddings_v1(texts, model, dimensions, **kwargs)
    
    @retry(stop_max_attempt_number=10, wait_exponential_multiplier=1000, wait_exponential_max=10000)
    def get_embeddings_v1(self, texts, model="openai-text-embedding-3-large", dimensions=512, **kwargs):
        """
    ...
    def get_model_info(self):
        try:
            api_url = f'{self._base_url}/llm-api/model/info' if self._base_url.endswith("/apollo") else f'{self._base_url}/model/info'
