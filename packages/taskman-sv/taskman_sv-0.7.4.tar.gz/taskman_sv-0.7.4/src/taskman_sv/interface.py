...
def calculate_daily_stats():
    ...
    for task in tasks:
        ...
        if "Completed" in task['status']:
            ...
            # Add any worked time from pending tasks (from previous sessions)
            if 'actual_time_worked' in task and task['actual_time_worked'] > 0:
                total_worked += task['actual_time_worked']
            ...
            if task.get('mode') == 'pomodoro':
                ...
                # If no actual_time_worked, add completed Pomodoros time (fallback)
...
def display_tasks():
    ...
    if tasks:
        ...
        for i, task in enumerate(tasks, 1):
            ...
            # Get worked time for display
            worked_time = task.get('actual_time_worked', 0)
            worked_display = format_time_duration(worked_time) if worked_time > 0 else "0m"
            
            # Display task with badge, number, name, duration, worked time, status and carryforward info
            if task['mode'] == 'custom':
                planned_display = f"{hrs}h {mins}m"
                print(f"{badge} {i}. {carryforward_info}{task['name']} [Planned: {planned_display} | Worked: {worked_display}] - {status}")
            else:
                ...
                print(f"{badge} {i}. {carryforward_info}{task['name']} [{completed}/{total} Pomodoros | Worked: {worked_display}] - {status}")
