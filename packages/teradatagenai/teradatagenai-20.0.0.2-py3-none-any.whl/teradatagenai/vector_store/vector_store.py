...
def _ProcessDataFrameObjects(obj):
    ...
    if obj is not None:
        ...
        return list(map(lambda obj_temp: obj_temp._table_name.replace("\"", "")
        if _Validators._check_isinstance(obj_temp, DataFrame) else obj_temp, obj))
...
class VectorStore:
    def __init__(self,
        ...
        # set the variable to check if vector store entry exists or not in list.
        vs_exists = False
