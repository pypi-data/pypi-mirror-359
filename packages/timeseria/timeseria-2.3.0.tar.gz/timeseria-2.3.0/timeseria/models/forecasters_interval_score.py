    ...
    def evaluate(self, series, steps=1, error_metrics=['RMSE', 'MAE'], return_evaluation_series=False, plot_evaluation_series=False,
        ...
        for data_label in evaluate_data_labels:
            ...
            if confidence_metrics:
                total = 0
                correct = 0
                interval_score_total = 0  # new accumulator
                ...
                for i in range(len(actual_values[data_label])):
                    ...
                    # Empirical coverage
                    if pred_value_lower <= actual_value <= pred_value_upper:
                        correct += 1

                    total += 1

                    # Interval score
                    interval_width = pred_value_upper - pred_value_lower
                    penalty = 0
                    alpha = confidence_interval[0] + (1 - confidence_interval[1])

                    if actual_value < pred_value_lower:
                        penalty = (2 / alpha) * (pred_value_lower - actual_value)
                    elif actual_value > pred_value_upper:
                        penalty = (2 / alpha) * (actual_value - pred_value_upper)

                    interval_score = interval_width + penalty
                    interval_score_total += interval_score

                if total > 0:
                    alpha = confidence_interval[0] + (1 - confidence_interval[1])

                    if 'EC' in confidence_metrics:
                        results['{}_EC'.format(data_label)] = correct / total
                    ...
                    if 'IS' in confidence_metrics:
                        results['{}_IS'.format(data_label)] = interval_score_total / total
