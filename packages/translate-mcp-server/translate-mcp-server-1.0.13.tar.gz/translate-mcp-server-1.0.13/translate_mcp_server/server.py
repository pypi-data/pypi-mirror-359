...
logging.basicConfig(level=logging.INFO, 
                   ...
                   filename='translate_server.log',
                   filemode='a')
logger = logging.getLogger('translate_mcp_server')
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            try:
                ...
                logger.info(f"读取文件出错 {file_path}: {e}")
...
async def translate_chinese_to_english(
    ...
    logger.info(f"开始翻译中文到英文，输入文件: {input_file}, 批量大小: {batch_size}")
    logger.info(f"访问密钥ID: {access_key_id[:4]}*** (隐藏部分)")
    """将中文翻译为英文
    ...
    if access_key_id is None:
        ...
        logger.info(f"translate_chinese_to_english: 从环境变量获取 ALIYUN_ACCESS_KEY_ID: {access_key_id[:4] if access_key_id else None}***")
    if access_key_secret is None:
        access_key_secret = os.environ.get('ALIYUN_ACCESS_KEY_SECRET')
        logger.info(f"translate_chinese_to_english: 从环境变量获取 ALIYUN_ACCESS_KEY_SECRET: {access_key_secret[:4] if access_key_secret else None}***")
    ...
    logger.info(f"总共需要翻译 {total_items} 条文本，批量大小: {batch_size}")
    
    # 如果文本数量过多，给出警告
    if total_items > 500:
        logger.warning(f"警告: 文本数量较多 ({total_items} 条)，可能需要较长时间处理，建议减小批量大小或分批处理")
    ...
    for i in range(0, total_items, batch_size):
        ...
        logger.info(f"正在处理第 {current_position} 到 {end_position} 条，共 {total_items} 条...")
        ...
        for attempt in range(retry_count):
            try:
                ...
                # 发送批量翻译请求
                logger.info(f"发送批量翻译请求，批次大小: {len(batch_to_translate)}")
                try:
                    ...
                    logger.error(f"发送批量翻译请求失败: {str(e)}")
                    raise
                logger.debug(f"翻译响应: {response_dict}")
                ...
                if response_dict and str(response_dict.get('Code')) == '200':
                    if 'TranslatedList' in response_dict and response_dict['TranslatedList'] is not None:
                        ...
                        logger.debug(f"TranslatedList类型: {type(translated_list)}, 内容: {translated_list}")
                        
                        # 确保translated_list是一个列表
                        if not isinstance(translated_list, list):
                            logger.warning(f"警告: TranslatedList不是列表类型，而是{type(translated_list)}")
                            if isinstance(translated_list, dict):
                                # 尝试从字典中提取翻译结果
                                logger.info(f"尝试从字典中提取翻译结果: {translated_list}")
                        ...
                        for item in translated_list:
                            try:
                                if 'index' in item and 'translated' in item:
                                    ...
                                    logger.warning(f"警告: 翻译项缺少index或translated字段: {item}")
                            except Exception as e:
                                logger.error(f"处理翻译项时出错: {str(e)}, 项: {item}")
                        
                        logger.info(f"批量翻译成功: 处理了{len(translated_list)}条文本，成功{success_count}条")
                        break
                    else:
                        error_message = {"status": "error", "attempt": attempt+1, "message": "翻译响应中缺少TranslatedList或为None"}
                        logger.error(f"批量翻译失败 (第{attempt+1}次尝试): 响应中缺少TranslatedList或为None")
                        # 如果是最后一次尝试，保留原值
                        if attempt == retry_count - 1:
                            logger.error(f"批量翻译失败: 已达到最大重试次数，保留原值")
                    ...
                    logger.error(f"批量翻译失败 (第{attempt+1}次尝试): {response_dict}")
            except Exception as e:
                error_message = {"status": "error", "attempt": attempt+1, "message": str(e)}
                logger.error(f"批量翻译失败 (第{attempt+1}次尝试): {str(e)}")
                if attempt == retry_count - 1:
                    logger.error(f"批量翻译失败: 已达到最大重试次数，保留原值")
                    # 保留原值
                    for key, value in batch_to_translate.items():
...
async def _complete_i18n_workflow(
    ...
    if access_key_id is None:
        access_key_id = os.environ.get('ALIYUN_ACCESS_KEY_ID')
        logger.info(f"_complete_i18n_workflow: 从环境变量获取 ALIYUN_ACCESS_KEY_ID: {access_key_id[:4] if access_key_id else None}***")
    if access_key_secret is None:
        access_key_secret = os.environ.get('ALIYUN_ACCESS_KEY_SECRET')
        logger.info(f"_complete_i18n_workflow: 从环境变量获取 ALIYUN_ACCESS_KEY_SECRET: {access_key_secret[:4] if access_key_secret else None}***")
    ...
    try:
        ...
        # 步骤1: 提取中文
        logger.info(f"[任务 {task_id}] 步骤1: 提取中文...")
        ...
        # 步骤2: 翻译中文
        logger.info(f"[任务 {task_id}] 步骤2: 翻译中文...")
        ...
        # 步骤3: 替换文本
        logger.info(f"[任务 {task_id}] 步骤3: 替换文本...")
...
async def start_i18n_workflow(
    ...
    if access_key_id is None:
        access_key_id = os.environ.get('ALIYUN_ACCESS_KEY_ID')
        logger.info(f"从环境变量获取 ALIYUN_ACCESS_KEY_ID: {access_key_id[:4] if access_key_id else None}***")
    if access_key_secret is None:
        access_key_secret = os.environ.get('ALIYUN_ACCESS_KEY_SECRET')
        logger.info(f"从环境变量获取 ALIYUN_ACCESS_KEY_SECRET: {access_key_secret[:4] if access_key_secret else None}***")
...
def run_server(transport='stdio', host='127.0.0.1', port=8000):
    ...
    logger.info(f"环境变量 ALIYUN_ACCESS_KEY_ID: {aliyun_key_id[:4] if aliyun_key_id else 'Not Set'}***")
    logger.info(f"环境变量 ALIYUN_ACCESS_KEY_SECRET: {aliyun_key_secret[:4] if aliyun_key_secret else 'Not Set'}***")
    
    # 启动清理任务
    loop = asyncio.get_event_loop()
    loop.create_task(cleanup_expired_tasks())
    logger.info("已启动任务清理服务")
