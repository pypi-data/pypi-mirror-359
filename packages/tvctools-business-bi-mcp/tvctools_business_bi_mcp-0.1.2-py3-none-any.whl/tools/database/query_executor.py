    ...
    ecommerce_tables = {
        ...
        "products": {
            ...
            "columns": {
                ...
                "category": {"type": "VARCHAR(50)", "description": "商品类别"},
                ...
                "weight": {"type": "DECIMAL(6,2)", "description": "重量"},
        ...
        "customers": {
            ...
            "columns": {
                ...
                "email": {"type": "VARCHAR(100)", "description": "邮箱地址"},
                "first_name": {"type": "VARCHAR(50)", "description": "名字"},
                "last_name": {"type": "VARCHAR(50)", "description": "姓氏"},
                "country": {"type": "VARCHAR(50)", "description": "国家"},
                "registration_date": {"type": "TIMESTAMP", "description": "注册日期"},
                ...
                "status": {"type": "VARCHAR(20)", "description": "客户状态"}
            } if include_columns else {}
        },
        "inventory": {
            "description": "库存管理表",
            "columns": {
                "inventory_id": {"type": "BIGINT", "description": "库存ID", "primary_key": True},
                "sku": {"type": "VARCHAR(100)", "description": "商品SKU"},
                "warehouse": {"type": "VARCHAR(50)", "description": "仓库"},
                "quantity": {"type": "INT", "description": "库存数量"},
                "reserved_quantity": {"type": "INT", "description": "预留数量"},
                "cost": {"type": "DECIMAL(8,2)", "description": "库存成本"},
        ...
        "marketing_campaigns": {
            "description": "营销活动表",
            "columns": {
                "campaign_id": {"type": "VARCHAR(50)", "description": "活动ID", "primary_key": True},
                "campaign_name": {"type": "VARCHAR(100)", "description": "活动名称"},
                "platform": {"type": "VARCHAR(20)", "description": "投放平台"},
                "campaign_type": {"type": "VARCHAR(30)", "description": "活动类型"},
                "start_date": {"type": "TIMESTAMP", "description": "开始日期"},
                "end_date": {"type": "TIMESTAMP", "description": "结束日期"},
                "budget": {"type": "DECIMAL(10,2)", "description": "预算"},
                "spent": {"type": "DECIMAL(10,2)", "description": "花费"},
                "impressions": {"type": "BIGINT", "description": "展示量"},
                "clicks": {"type": "INT", "description": "点击量"}
            } if include_columns else {}
        }
    }
...
def _generate_ecommerce_schema_suggestions(schema_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "orders" in table_names:
        suggestions["核心业务分析"].append("订单趋势分析：按时间、平台、市场维度分析订单量和金额")
        suggestions["常用查询模式"].append("SELECT platform, COUNT(*), SUM(total_amount) FROM orders GROUP BY platform")
    
    if "order_items" in table_names and "orders" in table_names:
        suggestions["表关联建议"].append("订单-商品明细关联：orders.order_id = order_items.order_id")
        suggestions["常用查询模式"].append("SELECT o.platform, SUM(oi.total_price) FROM orders o JOIN order_items oi ON o.order_id = oi.order_id")
    
    if "customers" in table_names:
        suggestions["核心业务分析"].append("客户分析：客户地域分布、消费行为、复购率")
        suggestions["常用查询模式"].append("SELECT country, COUNT(*), AVG(total_spent) FROM customers GROUP BY country")
    
    if "products" in table_names:
        suggestions["核心业务分析"].append("商品分析：热销商品、类别表现、价格分析")
        suggestions["常用查询模式"].append("SELECT category, COUNT(*), AVG(selling_price) FROM products GROUP BY category")
    
    if "inventory" in table_names:
        suggestions["核心业务分析"].append("库存分析：库存周转、缺货风险、成本分析")
        suggestions["性能优化提示"].append("定期清理历史库存记录，建立索引提升查询性能")
    
    if "marketing_campaigns" in table_names:
        suggestions["核心业务分析"].append("营销效果分析：ROI计算、渠道效果对比")
        suggestions["常用查询模式"].append("SELECT platform, SUM(spent), SUM(clicks) FROM marketing_campaigns GROUP BY platform")
...
def _generate_sql_hints(schema_info: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    hints.append({
        "类型": "基础统计",
        "SQL模板": "SELECT COUNT(*) as 总数, MAX(created_at) as 最新记录 FROM {表名}",
        "说明": "快速了解表的数据量和更新情况"
    })
    
    # 时间维度分析
    if any("date" in str(columns).lower() or "time" in str(columns).lower() 
           for columns in [table.get("columns", {}) for table in tables.values()]):
        hints.append({
            "类型": "时间趋势",
            "SQL模板": "SELECT DATE_TRUNC('month', {日期字段}) as 月份, COUNT(*) FROM {表名} GROUP BY 1 ORDER BY 1",
            "说明": "按月份统计趋势变化"
        })
    ...
    hints.append({
        "类型": "分组统计",
        "SQL模板": "SELECT {分组字段}, COUNT(*) as 数量, AVG({数值字段}) as 平均值 FROM {表名} GROUP BY 1 ORDER BY 2 DESC",
        "说明": "按维度分组分析分布情况"
    })
