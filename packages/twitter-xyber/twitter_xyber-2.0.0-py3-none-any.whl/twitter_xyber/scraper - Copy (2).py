    ...
    def timeline_tweet(self, username: str, tweet_count: int = 80) -> dict:
        ...
        with sync_playwright() as pw:
            ...
            while _scroll_count < _max_scrolls:
                ...
                for tr in tweet_result:
                    try:
                        if any("tweet" in s for s in tr['entryId'].split("-")):
                            ...
                            source_html = tr['content']['itemContent']['tweet_results']['result']['source']
                            if "advertiser-interface" not in source_html:
                                soup = BeautifulSoup(source_html, 'html.parser')
                                source = soup.get_text()

                                source_data.setdefault(source, {"count": 0, "percentage": 0})
                ...
                total_source = sum(data["count"] for data in source_data.values())
                ...
                for source, data in source_data.items():
                    data["percentage"] = (data["count"] / total_source) * 100 if total_source > 0 else 0
                ...
                source_result = [{"source": source, "count": data["count"], "percentage": data["percentage"]} for source, data in source_data.items()]

            context.close()
            browser.close()
            # return timeline
            return {"timelines": timeline, "hashtags": hashtags_result, "mentions": mentions_result, "source":source_result}

if __name__ == "__main__":
    # Create an instance of the TwitterScraper class with desired parameters
    twitter_bot = TwitterScraper(headless=False, username='Samboladobalado', password='sambo123!@#')
    # twitter_bot.login()

    # Call the login method to execute the login process
    # twitter_bot.login()
    result = twitter_bot.timeline_tweet("ganjarpranowo")
