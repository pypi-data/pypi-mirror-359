...
echo "Installing ubo-app in $VERSION_ENVIRONMENT..."
$VERSION_ENVIRONMENT/bin/python -m pip install${WHEELS_DIRECTORY:+ --pre --find-links="$WHEELS_DIRECTORY"} ubo-app --force-reinstall | tee >(grep -c '^Collecting ' >"$INSTALLATION_PATH/.packages-count")

echo "ubo-app installed successfully in $VERSION_ENVIRONMENT."
