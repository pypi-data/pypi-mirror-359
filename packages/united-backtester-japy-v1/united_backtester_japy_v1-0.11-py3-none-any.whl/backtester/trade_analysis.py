...
def analyze_trade_history(trade_history, df_ohlc, symbol, save_trades=True, ptc=0.0005, **strategy_params):
    '''
    ...
    **strategy_params: 전략 관련 파라미터들 (키워드 인자로 동적 전달)
    ...
    # 거래 내역 저장
    if save_trades and symbol == 'BTCUSDT':
        df.to_csv(f'trade_details_{symbol}.csv', index=False)
    ...
    # 익절 비율에 따라 익절 가격 계산
    df['profit_pct'] = df['profit_pct'] * abs(df['position_size'])
    df['net_return_pct'] = (df['profit_pct']) - (ptc * 200) * abs(df['position_size'])
    
    # 수수료 없는 수익률 계산
    initial_balance_no_fee = 10000
    ...
    for ret in df['profit_pct']:
        ...
        current_value_no_fee = current_value_no_fee * (1 + ret / 100)
    ...
    # mdd 계산
    full_portfolio_values = pd.Series(portfolio_values)
    ...
    # 승률 계산 (수수료 차감 후 기준)
    total_trades = len(df)
    profitable_trades = len(df[df['net_return_pct'] > 0])
    ...
    for month, return_value in merged_df.iterrows():
        result_dict[f'{month} 수익'] = f'{round(return_value["net_return_pct"], 2)}%'
    
    return result_dict

def analyze_trade_history_with_drawdown(trade_history, df_ohlc, symbol, ptc=0.0005, **strategy_params):
    """
    거래 내역과 OHLC 데이터를 이용하여 트레이딩 성과를 분석하고 drawdown 정보를 추가하는 함수
    ...
    **strategy_params: 전략 관련 파라미터들 (키워드 인자로 동적 전달)
    ...
    Returns:
    dict: drawdown 정보가 추가된 분석 결과 딕셔너리
    """

    result_dict = analyze_trade_history(trade_history, df_ohlc, symbol, ptc, **strategy_params)
    df = pd.DataFrame(trade_history)
    
    result_dict["거래당종가기준pct_change최소"] = f'{round(df["drawdown_pct"].min(),2)}%'
    result_dict["거래당종가기준pct_change평균"]=f'{round(df["drawdown_pct"].mean(),2)}%'
