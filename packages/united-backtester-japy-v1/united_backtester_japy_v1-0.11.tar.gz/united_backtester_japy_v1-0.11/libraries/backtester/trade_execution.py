...
def record_trade_with_drawdown(symbol, position,position_size ,entry_price, exit_price, close_type, entry_time, exit_time,drawdown_close,ptc= 0.0005):
    """
    거래 기록을 trade_history 리스트에 추가하는 함수
    ...
    return {
        ...
        'drawdown_pct':((drawdown_close - entry_price) / (entry_price))* 100 if position == 1
                     else ((entry_price - drawdown_close) / (entry_price))* 100,
        'entry_time': entry_time,
        'exit_time': exit_time
    }
