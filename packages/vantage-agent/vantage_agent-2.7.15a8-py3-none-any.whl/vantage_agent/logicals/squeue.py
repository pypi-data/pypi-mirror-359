...
def upload_squeue_queue():
    ...
    logger.debug(f"Current Slurm's queue: {current_config}")

    logger.debug("Loading cached Slurm's queue")
    cached_config = load_cached_dict("slurm_queue.json")
    logger.debug(f"Cached Slurm's queue: {cached_config}")
    ...
    logger.debug(f"Difference in Slurm's queue: {diff_queue}")
