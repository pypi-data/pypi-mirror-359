...
import asyncio  # added to support async functions
    ...
    if store_multi_values:
        if not isinstance(output, Iterable) or save_name or not auto_namer:
            raise TypeError(
                ...
                f"must be None and auto_namer must be defined. Got: {output}, {save_name}, {auto_namer}"
            )
...
def store_on_output(
    ...
    *,
    ...
    if store is None:
        store = dict()
    # Select appropriate version based on whether func is async.
