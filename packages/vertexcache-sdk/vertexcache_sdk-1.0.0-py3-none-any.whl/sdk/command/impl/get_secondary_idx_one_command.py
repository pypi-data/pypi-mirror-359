...
class GetSecondaryIdxOneCommand(CommandBase):
    ...
    Retrieves the value for a given secondary index key (idx1) from the cache.
    ...
    def __init__(self, key: str):
        ...
        if key is None or key.strip() == "":
            raise VertexCacheSdkException("GET By Secondary Index (idx1) command requires a non-empty key")
        self.key = key
        self.value = None

    def build_command(self) -> str:
        return f"GETIDX1 {self.key}"
    ...
    def parse_response(self, response_body: str):
        if response_body.strip().lower() == "(nil)":
            ...
            self.set_failure(f"GETIDX1 failed: {response_body}")
        else:
