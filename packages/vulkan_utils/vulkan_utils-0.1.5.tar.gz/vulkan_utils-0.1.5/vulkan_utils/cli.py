...
@click.option(
    "--version", 
    default="latest", 
    help="SDK version to download (default: latest)"
)
@click.option(
    "--platform", 
...
@click.option(
    "--download-only",
    is_flag=True,
    help="Only download, don't extract/install"
)
def install_sdk(
    version: str,
    ...
    try:
        ...
        if download_only:
            ...
            installed_path = sdk_manager.install_sdk(version, install_path, target_platform)
            ...
            if target_platform == "mac":
                ...
                click.echo("• If system-wide installation succeeded, the SDK should be available automatically")
                click.echo("• If system-wide installation failed, you may need to set environment variables:")
                click.echo(f"  export VULKAN_SDK={installed_path}")
                click.echo(f"  export PATH=$VULKAN_SDK/bin:$PATH")
                click.echo(f"  export DYLD_LIBRARY_PATH=$VULKAN_SDK/lib:$DYLD_LIBRARY_PATH")
                click.echo(f"  export VK_LAYER_PATH=$VULKAN_SDK/share/vulkan/explicit_layer.d")
                click.echo("• You can add these to your ~/.zshrc or ~/.bash_profile file")
            elif target_platform == "linux":
                click.echo("\n📝 To use the SDK, you may need to set environment variables:")
                click.echo(f"export VULKAN_SDK={installed_path}")
                click.echo(f"export PATH=$VULKAN_SDK/bin:$PATH")
                click.echo(f"export LD_LIBRARY_PATH=$VULKAN_SDK/lib:$LD_LIBRARY_PATH")
                click.echo(f"export VK_LAYER_PATH=$VULKAN_SDK/share/vulkan/explicit_layer.d")
...
@click.option(
    "--version", 
    default="latest", 
    help="SDK version to get info for (default: latest)"
)
@click.option(
    "--platform", 
    type=click.Choice(["linux", "mac", "windows", "auto"]),
