...
class ClaudeInterface:
    ...
    def create_claude_prompt(self, bdd_summary: str, complexity_issues: List[str] = None, 
                           iteration: int = 1) -> str:
        """Create prompt for Claude based on BDD results and complexity issues"""
        prompt_parts = []
        
        # Header
        prompt_parts.append(f"BDD Development Assistant - Iteration {iteration}")
        ...
        if complexity_issues:
            prompt_parts.append("CODE COMPLEXITY ISSUES:")
        ...
        instructions = [
            "INSTRUCTIONS:",
            "1. Analyze the BDD test results above",
            "2. Fix any failed scenarios by implementing or correcting step definitions",
            "3. Implement any undefined steps with real, working code",
            "4. Ensure all implementations are authentic - NO MOCKS OR PLACEHOLDERS",
            "5. If this is a UI feature, ensure proper browser automation with Selenium",
            "6. Update dependency.md with any feature dependencies you discover",
            "7. Keep code simple and readable (complexity grade B or better)",
            "8. Take screenshots for UI tests to verify real functionality",
            "",
            "DEPENDENCY MANAGEMENT:",
            "- Update dependency.md if features depend on each other",
            "- Format: 'Feature A -> Feature B' (A must be completed before B)",
            "- Consider: user auth before profiles, database before API, etc.",
            "",
            "REALITY CHECK:",
            "- All step implementations must connect to real systems",
            "- Database connections should be real (use SQLite for simplicity)",
            "- API calls should hit real endpoints or mock servers",
            "- UI tests should interact with actual web elements",
            "- No 'pass' statements or placeholder implementations",
            "",
            "Focus on making the failing tests pass with real, working implementations."
        ]
    ...
    def run_claude_automated(self, prompt: str, timeout_minutes: int = 30) -> bool:
        ...
        try:
            # Prepare Claude command with full automation flags
            cmd = [
                "claude",
                "--yes",  # Auto-accept all prompts
                "--dangerous-direct-action",  # Skip safety prompts
                "--non-interactive",  # No interactive prompts
                "--file", prompt_file,  # Read prompt from file
                str(self.project_dir)  # Work in project directory
            ]
            ...
            try:
                self.current_process.wait(timeout=timeout_minutes * 60)
    ...
    def wait_for_completion(self, check_interval: float = 1.0) -> bool:
        ...
        try:
            while self.is_claude_running():
                ...
                for log in logs:
                    # Show important logs to console
                    if any(keyword in log.lower() for keyword in ['error', 'complete', 'finished', 'done']):
                        self.console.print(f"[dim]Claude: {log}[/dim]")
