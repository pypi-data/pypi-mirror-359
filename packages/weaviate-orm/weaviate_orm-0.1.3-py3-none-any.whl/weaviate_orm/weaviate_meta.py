...
class Weaviate_Meta(ABCMeta):
    ...
    def __new__(mcs, name, bases, dct):
        ...
        # build or inherit the per-class, model-scoped registry of callbacks ---
        merged: Dict[str, List[Callable]] = defaultdict(list)

        for base in bases:                          # MERGE *all* direct bases
            for ev, fns in getattr(base, "_callbacks", {}).items():
                for fn in fns:
                    if fn not in merged[ev]:        # de-duplicate
                        merged[ev].append(fn)

        # always create the attribute so static checkers are happy
        setattr(cls, "_callbacks", dict(merged))
        ...
        if name != "Base_Model":
            ...
            if '__init__' not in dct:
                # If no __init__ is defined, we create a dynamic one.
                # This will allow instantiation with properties and references as keyword arguments.
