...
class ParquetHandler:
    ...
    def read_parquet_from_gcs(self, file_name, columns=None):
        ...
        try:
            ...
            logging.warning(f"[GCS] Parquet 파일이 존재하지 않음: {file_name}")
            return None  # 또는 빈 DataFrame: pd.DataFrame()
        
        except Exception as e:
            logging.error(f"Error in read_parquet_from_gcs: {e}")
            raise   
    ...
    def save_parquet_to_gcs(self, file_name, df):
        ...
        try:
            ...
            logging.error(f"Error in save_parquet_to_gcs: {e}")
            raise   
        ...
        try:
            if blob.exists():  # 파일이 존재하면 읽기
                try: 
                    ...
                    logging.warning(f"[!] blob.exists()는 True지만 파일 읽기 실패 → 새로 생성: {e}")
                    combined_df = new_df
            else:
                # 파일이 없으면 새로 생성
                combined_df = new_df
