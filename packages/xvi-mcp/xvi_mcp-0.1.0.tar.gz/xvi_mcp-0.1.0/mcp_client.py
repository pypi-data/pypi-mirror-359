...
@app.tool()
async def index(string: str) -> str:
    return string


@app.tool()
async def get_vulns(keyword: str = "", cve: str = "", publishTimeStart: str = "", publishTimeEnd: str = "",
                    vulnType: str = "", riskLevels: list = []) -> str:
    """
    根据提供的关键词keyword进行模糊查询，该关键词可以是标题关键词也可以是正文关键词,cve可有可无，默认情况下是空值，publishTimeStart是开始时间，可有可无。
    ...
    logging.info(data)
    data['resultsPerPage'] = '1'
    response = requests.post(url, headers=headers, json=data)
    res: dict = response.json()
    # res.pop('relateFormal')
...
if __name__ == "__main__":
    ...
    # print(find_most_similar("跨站脚本", vulnTypeList))
    # print(vulnType_to_cweCode(find_most_similar("命令执行", vulnTypeList)))
