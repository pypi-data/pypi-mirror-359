...
class ParameterValidator:
    ...
    def validate_numpy_array(
        ...
        The 'numpy_alias' parameter is to check by
        using that alias name because we don't
        import the library to check, we only use
        the name.
        
        If you, in your code, use the line as 
        'import numpy as np', you should set the
        'numpy_alias = "np"'. If you only do
        'import numpy', you should set it as
        'numpy_alias = "numpy"'.
        """
        if (
            value is not None and
            not PythonValidator.is_numpy_array(value, numpy_alias)
        ):
    ...
    def validate_mandatory_numpy_array(
        ...
        """
        ParameterValidator.validate_mandatory(name, value)
        ParameterValidator.validate_numpy_array(name, value, numpy_alias)
