...
def load_config():
    ...
    # ==================== 代码修改开始 ====================
    # 核心修改：从环境变量 'ini' 读取配置文件路径。
    # 如果'ini'环境变量不存在，则退回到原始行为，查找当前目录下的 'config.ini'。
    # 这样既支持MCP配置，也方便本地直接运行测试。
    config_file_path = os.getenv('ini', 'config.ini')
    print(f"正在尝试从路径加载配置: {config_file_path}")
    # ==================== 代码修改结束 ====================

    if not os.path.exists(config_file_path):
        # 更新错误提示，让用户知道可以通过环境变量配置
        print(f"错误: 配置文件 '{config_file_path}' 未找到。请检查路径或 'ini' 环境变量的设置。")
        if not API_KEY:
            return
    else:
        # 增加 encoding='utf-8' 以支持包含中文字符的配置文件路径或内容
