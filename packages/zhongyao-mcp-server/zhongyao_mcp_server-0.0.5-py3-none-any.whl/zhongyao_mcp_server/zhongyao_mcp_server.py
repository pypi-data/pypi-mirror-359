...
def load_config():
    ...
    # ==================== 代码修改开始 ====================
    # 步骤1: 优先从环境变量加载所有配置项
    # 遵循 `DOUBAO_API_KEY` 的模式，为每个配置项检查其对应的环境变量
    ...
    env_base_url = os.getenv("DOUBAO_BASE_URL")
    if env_base_url:
        BASE_URL = env_base_url
        print("base_url 已从环境变量 DOUBAO_BASE_URL 加载。")

    env_chat_model = os.getenv("DOUBAO_CHAT_MODEL")
    if env_chat_model:
        DEFAULT_CHAT_MODEL = env_chat_model
        print("chat_model 已从环境变量 DOUBAO_CHAT_MODEL 加载。")

    env_image_model = os.getenv("DOUBAO_IMAGE_MODEL")
    if env_image_model:
        DEFAULT_IMAGE_MODEL = env_image_model
        print("image_model 已从环境变量 DOUBAO_IMAGE_MODEL 加载。")
        
    env_video_model = os.getenv("DOUBAO_VIDEO_MODEL")
    if env_video_model:
        DEFAULT_VIDEO_MODEL = env_video_model
        print("video_model 已从环境变量 DOUBAO_VIDEO_MODEL 加载。")

    env_tts_api_key = os.getenv("TTS_API_KEY")
    if env_tts_api_key:
        TTS_API_KEY = env_tts_api_key
        print("tts_api_key 已从环境变量 TTS_API_KEY 加载。")
        
    env_tts_base_url = os.getenv("TTS_BASE_URL")
    if env_tts_base_url:
        TTS_BASE_URL = env_tts_base_url
        print("tts_base_url 已从环境变量 TTS_BASE_URL 加载。")

    env_cos_region = os.getenv("COS_REGION")
    if env_cos_region:
        COS_REGION = env_cos_region
        print("cos_region 已从环境变量 COS_REGION 加载。")

    env_cos_secret_id = os.getenv("COS_SECRET_ID")
    if env_cos_secret_id:
        COS_SECRET_ID = env_cos_secret_id
        print("cos_secret_id 已从环境变量 COS_SECRET_ID 加载。")

    env_cos_secret_key = os.getenv("COS_SECRET_KEY")
    if env_cos_secret_key:
        COS_SECRET_KEY = env_cos_secret_key
        print("cos_secret_key 已从环境变量 COS_SECRET_KEY 加载。")

    env_cos_bucket = os.getenv("COS_BUCKET")
    if env_cos_bucket:
        COS_BUCKET = env_cos_bucket
        print("cos_bucket 已从环境变量 COS_BUCKET 加载。")
    # ==================== 代码修改结束 ====================
    
    config = configparser.ConfigParser()
    config_file_path = os.getenv('ini', 'config.ini')
    print(f"正在尝试从路径加载配置: {config_file_path}")

    if not os.path.exists(config_file_path):
        print(f"警告: 配置文件 '{config_file_path}' 未找到。将仅依赖环境变量或默认值。")
    else:
        config.read(config_file_path, encoding='utf-8')

    try:
        # 步骤2: 从配置文件加载，但仅在相应全局变量尚未被环境变量设置时生效
        if VIDEO_GENERATION_TIMEOUT is None:
        ...
        if not COS_SECRET_ID or 'YOUR_COS_SECRET_ID' in COS_SECRET_ID:
            print("警告: [common] COS配置未正确设置 (通过环境变量或config.ini)。TTS和视频合成功能将不可用。")
...
def generate_audio_from_text(text: str, voice: str = "zh-CN-XiaoxiaoNeural", speed: float = 1.0) -> Dict[str, Any]:
    ...
    try:
        tts_client = OpenAI(api_key=TTS_API_KEY, base_url=TTS_BASE_URL,timeout=60.0)
