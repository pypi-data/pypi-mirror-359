...
impl ZScoreE {
    ...
    fn detect_outliers(&self, data: Vec<f64>) -> PyResult<PyZScoreEResult> {
        let result = self.params.detect_outliers(&data)?;
...
/// Convenience function for ZScoreE detection with default parameters
#[pyfunction]
#[pyo3(signature = (data, window_percent=0.1))]
fn detect_outliers_e(data: Vec<f64>, window_percent: f64) -> PyResult<PyZScoreEResult> {
    let result = zscore_e::detect_outliers_e_with_window(&data, window_percent)?;
    Ok(PyZScoreEResult { inner: result })
}
...
fn mad(data: Vec<f64>, center: Option<f64>) -> PyResult<f64> {
    let result = stats::mad(&data, center)?;
    Ok(result)
}
...
fn biweight_location(data: Vec<f64>, c: Option<f64>) -> PyResult<f64> {
    let result = stats::biweight_location(&data, c)?;
    Ok(result)
}
...
fn ewma(data: Vec<f64>, span: usize, adjust: bool) -> PyResult<Vec<f64>> {
    let result = stats::ewma(&data, span, adjust)?;
    Ok(result)
}
